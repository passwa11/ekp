...
/**
 * @author wangwh
 * @description:业务基础数据工具类
 * @date 2021/5/7
 */
public class EopBasedataUtil{
	private static Map<String, IBaseService> serviceMap = new ConcurrentHashMap<String, IBaseService>();

	public static <S extends IBaseService> S getService(Class<S> serviceClz, String beanName) {
		S rtn = null;
		try {
			String beanNameToUse = beanName;
			if (StringUtil.isNull(beanNameToUse)) {
				beanNameToUse = resolveBeanName(serviceClz);
			}
			if (serviceMap.containsKey(beanNameToUse)) {
				rtn = (S) serviceMap.get(beanNameToUse);
			} else {
				rtn = (S) SpringBeanUtil.getBean(beanNameToUse);
				serviceMap.put(beanNameToUse, rtn);
			}
		} catch (Exception e) {
		}
		return rtn;
	...
	private static <S extends IBaseService> String resolveBeanName(Class<S> serviceClz) {
		String rtnName = "";
		// String[] beanNamesArr =
		// SpringBeanUtil.getApplicationContext().getBeanNamesForType(serviceClz);//很慢
		String clzName = serviceClz.getSimpleName();
		if (serviceClz.isInterface()) {
			// 接口
			rtnName = ("" + clzName.charAt(1)).toLowerCase() + clzName.substring(2);
		} else {
			// 具体的service类
			rtnName = ("" + clzName.charAt(0)).toLowerCase() + clzName.substring(1, clzName.length() - 3);
		}
		return rtnName;
	}
	
	private static Logger logger = LoggerFactory.getLogger(EopBasedataUtil.class);
	
	static final String IOPUT_EXTENSION_POINT_ID = "com.landray.kmss.eop.basedata.io.put";
	
	static final String IOPUT_EXTENSION_NAME = "svr";
	
	static final String IOPUT_PARAM_NAME_FUNCTION = "function";
	
	static final String IOPUT_PARAM_NAME_BEAN = "bean";
	...
	/**
	 * 获取推拉数据服务
	 * @param function    EOP功能点（modelName)
	 * @return
	 */
	public static List<IEopBasedataPullAndPushService> getPullAndPushService(String function) {
		
		IExtension[] extensions = Plugin.getExtensions(IOPUT_EXTENSION_POINT_ID, "*", IOPUT_EXTENSION_NAME);
		if(extensions == null || extensions.length == 0) {
			logger.info("未找到任何扩展点,不作任何处理...");
			return null;
		}
		
		
		List<IEopBasedataPullAndPushService> eopBasedataIOPutServiceList = new ArrayList<>();
		
		for(IExtension extension : extensions){
			
			String func = Plugin.getParamValueString(extension, IOPUT_PARAM_NAME_FUNCTION);
			if(func.equals(function)) {
				IEopBasedataPullAndPushService<?,?> importService = Plugin.getParamValue(extension, IOPUT_PARAM_NAME_BEAN);
				eopBasedataIOPutServiceList.add(importService);
			}
		}
		
		return eopBasedataIOPutServiceList;
	}
}
