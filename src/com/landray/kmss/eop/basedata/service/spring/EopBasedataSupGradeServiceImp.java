...
/**
 * @author wangwh
 * @description:供应商等级业务类实现类
 * @date 2021/5/7
 */
public class EopBasedataSupGradeServiceImp extends ExtendDataServiceImp implements IEopBasedataSupGradeService,IXMLDataBean {
    ...
    public IBaseModel convertBizFormToModel(IExtendForm form, IBaseModel model, ConvertorContext context)
		...
		if (model instanceof EopBasedataSupGrade) {
			EopBasedataSupGrade eopBasedataSupGrade = (EopBasedataSupGrade) model;
			eopBasedataSupGrade.setDocAlterTime(new Date());
			eopBasedataSupGrade.setDocAlteror(UserUtil.getUser());
		}
		return model;
	}
    ...
    public IBaseModel initBizModelSetting(RequestContext requestContext) throws Exception {
		EopBasedataSupGrade eopBasedataSupGrade = new EopBasedataSupGrade();
		eopBasedataSupGrade.setDocCreateTime(new Date());
		eopBasedataSupGrade.setDocAlterTime(new Date());
		eopBasedataSupGrade.setFdStatus(Integer.valueOf("0"));
		eopBasedataSupGrade.setDocCreator(UserUtil.getUser());
		eopBasedataSupGrade.setDocAlteror(UserUtil.getUser());
		EopBasedataUtil.initModelFromRequest(eopBasedataSupGrade, requestContext);
		return eopBasedataSupGrade;
	}
	...
	private boolean checkCodeOrNameExist(String fdCode, String fdName) throws Exception {
		...
		String whereString = "eopBasedataSupGrade.fdName=:fdName";
		hqlInfo.setParameter("fdName", fdName);
		if (StringUtil.isNotNull(fdCode)) {
			whereString += " or eopBasedataSupGrade.fdCode = :fdCode";
			hqlInfo.setParameter("fdCode", fdCode);
		}
		hqlInfo.setWhereBlock(whereString);
		List<EopBasedataSupGrade> results = findList(hqlInfo);
	...
	public String add(IBaseModel modelObj) throws Exception {
		EopBasedataSupGrade eopBasedataSupGrade = EopBasedataSupGrade.class.cast(modelObj);
		String fdCode = getSysNumberFlowService().generateFlowNumber(eopBasedataSupGrade);
		eopBasedataSupGrade.setFdCode(fdCode);
	...
	public boolean checkUnique(String fdId, String fdName) throws Exception {
		boolean flag = false;
		HQLInfo hqlInfo = new HQLInfo();
		String whereBlock = "eopBasedataSupGrade.fdName=:fdName";
		hqlInfo.setParameter("fdName", fdName);
		if (StringUtil.isNotNull(fdId)) {
			whereBlock += " and eopBasedataSupGrade.fdId!=:fdId";
