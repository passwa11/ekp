...
/**
 * @author wangwh
 * @description:供应商等级action
 * @date 2021/5/7
 */
public class EopBasedataSupGradeAction extends ExtendAction {
    ...
    public void changeFindPageHQLInfo(HttpServletRequest request, HQLInfo hqlInfo) throws Exception {
		HQLHelper.by(request).buildHQLInfo(hqlInfo, EopBasedataSupGrade.class);
		hqlInfo.setOrderBy(getFindPageOrderBy(request, hqlInfo.getOrderBy()));
		EopBasedataUtil.buildHqlInfoDate(hqlInfo, request, EopBasedataSupGrade.class);
    ...
    public ActionForm createNewForm(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		EopBasedataSupGradeForm eopBasedataSupGradeForm = (EopBasedataSupGradeForm) super.createNewForm(mapping, form,
				request, response);
		((IEopBasedataSupGradeService) getServiceImp(request)).initFormSetting((IExtendForm) form,
				new RequestContext(request));
		return eopBasedataSupGradeForm;
	}
	...
	public ActionForward deleteall(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		TimeCounter.logCurrentTime("EopBasedataSupGradeAction-deleteall", true, getClass());
		...
		TimeCounter.logCurrentTime("EopBasedataSupGradeAction-deleteall", false, getClass());
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		TimeCounter.logCurrentTime("EopBasedataSupGradeAction-delete", true, getClass());
		KmssMessages messages = new KmssMessages();
		try {
		...
		TimeCounter.logCurrentTime("EopBasedataSupGradeAction-delete", false, getClass());
		if (messages.hasError()) {
			return getActionForward("failure", mapping, form, request, response);
		} else {
			return getActionForward("success", mapping, form, request, response);
	...
	public void checkUnique(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		boolean check = IEopBasedataSupGradeService.class.cast(getServiceImp(request)).checkUnique(fdId, fdName);
