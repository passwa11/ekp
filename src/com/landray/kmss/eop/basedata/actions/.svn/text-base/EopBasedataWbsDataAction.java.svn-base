...
public class EopBasedataWbsDataAction extends EopBasedataBusinessAction {
	...
	public ActionForward fdParent(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if (StringUtil.isNotNull(keyWord)) {
				String where = "";
				where += "(eopBasedataWbs.fdName like :fdName";
				hqlInfo.setParameter("fdName", "%" + keyWord + "%");
				where += " or eopBasedataWbs.fdCode like :fdCode";
			...
			if(StringUtil.isNotNull(fdProjectId)){
				hqlInfo.setJoinBlock(StringUtil.linkString(hqlInfo.getJoinBlock(), " "," left join eopBasedataWbs.fdProject project "));
				hqlInfo.setWhereBlock(StringUtil.linkString(hqlInfo.getWhereBlock(), " and ",
						"( project.fdId=:fdProjectId or project is null )"));
			...
			if(StringUtil.isNotNull(fdNotId)){
				hqlInfo.setWhereBlock(
						StringUtil.linkString(hqlInfo.getWhereBlock(), " and ", " eopBasedataWbs.fdId<>:fdNotId "));
			...
			if(StringUtil.isNotNull(fdCompanyId)){
				hqlInfo.setJoinBlock(StringUtil.linkString(hqlInfo.getJoinBlock(), " "," left join eopBasedataWbs.fdCompanyList company"));
			...
			hqlInfo.setWhereBlock(
					StringUtil.linkString(hqlInfo.getWhereBlock(), " and ", " eopBasedataWbs.fdIsAvailable=:fdIsAvailable "));
	...
	public ActionForward fdWbs(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
