...
/**
 * @author wangwh
 * @description:物料单位action
 * @date 2021/5/7
 */
public class EopBasedataMateUnitAction extends ExtendAction {
    ...
    public void changeFindPageHQLInfo(HttpServletRequest request, HQLInfo hqlInfo) throws Exception {
        HQLHelper.by(request).buildHQLInfo(hqlInfo, EopBasedataMateUnit.class);
        hqlInfo.setOrderBy(getFindPageOrderBy(request, hqlInfo.getOrderBy()));
        EopBasedataUtil.buildHqlInfoDate(hqlInfo, request, EopBasedataMateUnit.class);
        EopBasedataUtil.buildHqlInfoModel(hqlInfo, request);
    }

    @Override
    public ActionForm createNewForm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        EopBasedataMateUnitForm eopBasedataMateUnitForm = (EopBasedataMateUnitForm) super.createNewForm(mapping, form, request, response);
        ((IEopBasedataMateUnitService) getServiceImp(request)).initFormSetting((IExtendForm) form, new RequestContext(request));
        return eopBasedataMateUnitForm;
    }
    ...
    public ActionForward deleteall(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        TimeCounter.logCurrentTime("EopBasedataMateUnitAction-deleteall", true, getClass());
        ...
        TimeCounter.logCurrentTime("EopBasedataMateUnitAction-deleteall", false, getClass());
    ...
    public ActionForward delete(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        TimeCounter.logCurrentTime("EopBasedataMateUnitAction-delete", true, getClass());
        KmssMessages messages = new KmssMessages();
        try {
        ...
        TimeCounter.logCurrentTime("EopBasedataMateUnitAction-delete", false, getClass());
        if (messages.hasError()) {
            return getActionForward("failure", mapping, form, request, response);
        } else {
            return getActionForward("success", mapping, form, request, response);
