...
public class EopBasedataPayBankDataAction extends BaseAction {
    ...
    public ActionForward fdBank(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        TimeCounter.logCurrentTime("fdBank", true, getClass());
        ...
        try {
            ...
            if (StringUtil.isNotNull(keyWord)) {
                String where = "";
                where += "(eopBasedataPayBank.fdBankAccount like :keyWord";
                hqlInfo.setParameter("keyWord", "%" + keyWord + "%");
                where += " or eopBasedataPayBank.fdBankName like :keyWord";
                where += ")";
                hqlInfo.setWhereBlock(where);
            }
            hqlInfo.setWhereBlock(
					StringUtil.linkString(hqlInfo.getWhereBlock(), " and ", " eopBasedataPayBank.fdIsAvailable=:fdIsAvailable "));
            String fdCompanyId = request.getParameter("fdCompanyId");
            if(StringUtil.isNotNull(fdCompanyId)) {
            	hqlInfo.setJoinBlock(" left join eopBasedataPayBank.fdCompanyList comp");
            ...
            HQLHelper.by(request).buildHQLInfo(hqlInfo, EopBasedataPayBank.class);
            Page page = getServiceImp(request).findPage(hqlInfo);
            request.setAttribute("queryPage", page);
        } catch (Exception e) {
            messages.addError(e);
        }
        TimeCounter.logCurrentTime("fdBank", false, getClass());
        if (messages.hasError()) {
            KmssReturnPage.getInstance(request).addMessages(messages).addButton(KmssReturnPage.BUTTON_CLOSE).save(request);
            return mapping.findForward("failure");
        } else {
            return mapping.findForward("fdBank");
        }
    }
}
