...
public class EopBasedataInnerOrderDataAction extends EopBasedataBusinessAction {
	...
	public ActionForward fdParent(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if (StringUtil.isNotNull(keyWord)) {
				String where = "";
				where += "(eopBasedataInnerOrder.fdName like :fdName";
				hqlInfo.setParameter("fdName", "%" + keyWord + "%");
				where += " or eopBasedataInnerOrder.fdCode like :fdCode";
			...
			if(StringUtil.isNotNull(fdNotId)){
				hqlInfo.setWhereBlock(
						StringUtil.linkString(hqlInfo.getWhereBlock(), " and ", " eopBasedataInnerOrder.fdId<>:fdNotId "));
				hqlInfo.setParameter("fdNotId", fdNotId);
			}
			hqlInfo.setWhereBlock(
					StringUtil.linkString(hqlInfo.getWhereBlock(), " and ", " eopBasedataInnerOrder.fdIsAvailable=:fdIsAvailable "));
			...
			if(StringUtil.isNotNull(fdCompanyId)){
				hqlInfo.setJoinBlock("left join eopBasedataInnerOrder.fdCompanyList comp");
	...
	public ActionForward fdInnerOrder(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
