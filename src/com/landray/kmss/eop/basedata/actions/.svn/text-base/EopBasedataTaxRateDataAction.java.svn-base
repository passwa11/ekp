...
public class EopBasedataTaxRateDataAction extends EopBasedataBusinessAction  {
	private IEopBasedataTaxRateService eopBasedataTaxRateService;

	@Override
    public IEopBasedataTaxRateService getServiceImp(HttpServletRequest request) {
	...
	public ActionForward getTaxRate(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		TimeCounter.logCurrentTime("getTaxRate", true, getClass());
		...
		try {
			...
			if (StringUtil.isNotNull(keyWord)) {
				String where = "";
				where += "(eopBasedataTaxRate.fdAccount.fdName like :fdName";
				hqlInfo.setParameter("fdName", "%" + keyWord + "%");
				where += " or cast(eopBasedataTaxRate.fdRate,string) like :fdCode";
			...
			if(StringUtil.isNotNull(fdNotId)){
				hqlInfo.setWhereBlock(
						StringUtil.linkString(hqlInfo.getWhereBlock(), " and ", " eopBasedataTaxRate.fdId<>:fdNotId "));
				hqlInfo.setParameter("fdNotId", fdNotId);
			}
			hqlInfo.setWhereBlock(StringUtil.linkString(hqlInfo.getWhereBlock(), " and ",
					"eopBasedataTaxRate.fdCompany.fdId=:fdCompanyId"));
			hqlInfo.setParameter("fdCompanyId", request.getParameter("fdCompanyId"));
			hqlInfo.setWhereBlock(
					StringUtil.linkString(hqlInfo.getWhereBlock(), " and ", " eopBasedataTaxRate.fdIsAvailable=:fdIsAvailable "));
			...
			hqlInfo.setOrderBy("eopBasedataTaxRate.fdOrder");
		...
		TimeCounter.logCurrentTime("getTaxRate", false, getClass());
		if (messages.hasError()) {
			KmssReturnPage.getInstance(request).addMessages(messages).addButton(KmssReturnPage.BUTTON_CLOSE)
					.save(request);
			return mapping.findForward("failure");
		} else {
			return mapping.findForward("getTaxRate");
		}
    }
}
