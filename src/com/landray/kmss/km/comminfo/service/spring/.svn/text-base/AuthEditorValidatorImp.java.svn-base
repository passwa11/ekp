...
public class AuthEditorValidatorImp implements IAuthenticationValidator {
	...
	public boolean validate(ValidatorRequestContext validatorContext)
		...
		// 用于文档或模板
		if (recModel != null && recModel instanceof KmComminfoMain) {
			if (UserUtil.checkUserId(
					((KmComminfoMain) recModel).getDocCreator().getFdId())) {
				// 当前用户是创建者，直接通过
				return true;
			}
		}
	...
	private boolean validateCategory(String fdId) throws Exception {
		...
		boolean flag = UserUtil.checkUserModels(categoryModel
				.getAuthEditors());
		return flag;
	}
