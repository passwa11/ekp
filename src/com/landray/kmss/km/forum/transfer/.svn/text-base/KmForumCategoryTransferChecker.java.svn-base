...
public class KmForumCategoryTransferChecker
	...
	public SysAdminTransferCheckResult
		...
		try {
			...
			String selectSql = "select count(*) from km_forum_category";
			List supportList = service.getBaseDao().getHibernateSession().createNativeQuery(selectSql).list();
