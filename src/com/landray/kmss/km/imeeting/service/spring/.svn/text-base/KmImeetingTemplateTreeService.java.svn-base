 ...
 * 模板树菜单
 */
public class KmImeetingTemplateTreeService extends BaseTemplateTreeService {
	...
	public List getDataList(RequestContext requestInfo) throws Exception {
		...
		if (StringUtil.isNotNull(categoryId)) {
			...
			whereString += "(" + tableName + ".fdIsAvailable is null or "
					+ tableName + ".fdIsAvailable = true)";
			info.setWhereBlock(whereString);
			info.setOrderBy(tableName + ".fdOrder");

			SysCategoryAuthPlugin.getFilter(KmImeetingTemplate.class.getName())
					.hqlFilter(requestInfo, info);
