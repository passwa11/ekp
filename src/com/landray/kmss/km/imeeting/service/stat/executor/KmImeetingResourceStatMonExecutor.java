 ...
 * 会议室资源使用环比执行器
 */
public class KmImeetingResourceStatMonExecutor extends
		AbstractKmImeetingStatExecutor {

	private static Logger logger = org.slf4j.LoggerFactory.getLogger(KmImeetingResourceStatMonExecutor.class);
	...
	protected List<Map<String, ?>> getQueryChartData(
		...
		if (resIds != null && !resIds.isEmpty()) {
			for (int i = 0; i < resIds.size(); i++) {
				...
				valueMap.put("resId", resIds.get(i));
				valueMap.put("resName", resNames.get(i));
				...
				if (periods != null && !periods.isEmpty()) {
					for (Object tmpObj : periods) {
						...
						if (period != null) {
							...
							tmpMap.put("resId", resIds.get(i));
							tmpMap.put("resName", resNames.get(i));
							int counts = 0;
							Double timeDate = 0.0;

							// 获取普通会议信息
							StatHqlInfo hqlInfo = getNormalMainHQL(period,
									resIds.get(i), parameters);
							...
							String templateId = (String) parameters
									.get("fdTemplateId");
							if (StringUtil.isNull(templateId)) {
								// 获取普通预约信息
								hqlInfo = getNormalBookHQL(period,
										resIds.get(i), parameters);
								...
								for (int j = 0; j < q.list().size(); j++) {
									Object[] result = (Object[]) q.list()
											.get(j);
									counts = counts + Integer
											.valueOf(result[0].toString());
									if (Integer.valueOf(
											result[0].toString()) > 0) {
										if (result[1] == null) {
											result[1] = 0;
								...
								Map<String, Object> rangeResult = getStatInRangeBook(
										period, resIds.get(i), parameters);
								int rangeCounts = (Integer) rangeResult
										.get("counts");
								if (rangeCounts > 0) {
									counts += rangeCounts;
									timeDate += (Double) rangeResult
											.get("timeDate");
								}
							}
							...
							if (timeDate > 0L) {
								tmpMap.put("sum",
										NumberUtil.roundDecimal(timeDate,
												ResourceUtil.getMessage(
														"{km-imeeting:decimal.format}")));
							} else {
								tmpMap.put("sum", 0);
							}
	...
	private Map<String, Object> getStatInRangeMain(PeriodModel period,
			String resId, Map<String, Object> parameters) throws Exception {
		...
		for (int j = 0; j < q.list().size(); j++) {
			...
			if (StringUtil.isNotNull(recurrenceStr)) {
				List<Date> dates = RecurrenceUtil.getExcuteDateList(
						recurrenceStr, (Timestamp) result[3],
						period.getFdStart(),
						period.getFdEnd());
				for (Date date : dates) {
	...
	// 获取周期性会议预定的统计数据
	private Map<String, Object> getStatInRangeBook(PeriodModel period,
			String resId,
	...
	private StatHqlInfo getNormalMainHQL(PeriodModel period, String resId,
			Map<String, Object> parameters) throws Exception {
		StatHqlInfo hqlInfo = new StatHqlInfo();
		StringBuffer sb = new StringBuffer();
	...
	private StatHqlInfo getNormalBookHQL(PeriodModel period, String resId,
			Map<String, Object> parameters) throws Exception {
		StatHqlInfo hqlInfo = new StatHqlInfo();
		StringBuffer sb = new StringBuffer();
		String templateId = (String) parameters.get("fdTemplateId");
	...
	private StatHqlInfo getListDetailSqlBlock(Map<String, Object> parameters)
		...
		StatHqlInfo tmpHqlInfo = getTimeScopeSqlInfo("m", period.getFdStart(),
				period.getFdEnd());
		if (StringUtil.isNotNull(tmpHqlInfo.getHqlBlock())) {
			sb.append(" and " + tmpHqlInfo.getHqlBlock() + " ");
			hqlInfo.setParameter(tmpHqlInfo.getParameterList());
		...
		if (StringUtil.isNull(templateId)) {
			...
			// 组装时间条件
			periodId = (String) parameters.get("periodId");
			period = null;
			try {
				period = PeriodTypeModel.getSinglePeriod(periodId);
			} catch (Exception e) {
			...
			tmpHqlInfo = getTimeScopeSqlInfo("b", period.getFdStart(),
					period.getFdEnd());
			if (StringUtil.isNotNull(tmpHqlInfo.getHqlBlock())) {
				sb.append(" and " + tmpHqlInfo.getHqlBlock() + " ");
				hqlInfo.setParameter(tmpHqlInfo.getParameterList());
