...
public class KmReviewTemplateMobileChecker implements ISysAdminTransferChecker {
	...
	public SysAdminTransferCheckResult check(SysAdminTransferCheckContext sysAdminTransferCheckContext) {
		try {
			...
			List<Long> list = kmReviewTemplateService.getBaseDao().findValue("count(*)", "kmReviewTemplate.fdIsMobileCreate is null or kmReviewTemplate.fdIsMobileApprove is null", null);
			if (list.get(0) > 0) {
