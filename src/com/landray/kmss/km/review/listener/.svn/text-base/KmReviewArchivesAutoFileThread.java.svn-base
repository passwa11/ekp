 ...
 *
 */
public class KmReviewArchivesAutoFileThread implements Runnable {
	
	private static Logger logger = org.slf4j.LoggerFactory.getLogger(KmReviewArchivesAutoFileThread.class);
	...
	public KmReviewArchivesAutoFileThread(String modelName, String modelId, Thread preThread, long joinTime){
	...
	public void doRun() throws Exception{
		KmReviewMain kmReviewMain = null;
    	...
    	if (StringUtil.isNotNull(modelName)) {
			...
			modelClassName = firstChar + modelClassName.substring(1);
    	...
    	if(serviceName != null && modelId != null){ 
    		IBaseService service = (IBaseService) SpringBeanUtil.getBean(serviceName);
				try {
					kmReviewMain = (KmReviewMain) service.findByPrimaryKey(modelId);
				} catch (Exception e) {
		...
		try {
			...
			kmReviewMain.setFdIsFiling(false);
