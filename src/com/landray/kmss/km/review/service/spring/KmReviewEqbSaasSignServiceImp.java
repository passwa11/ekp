...
import com.landray.kmss.elec.device.client.ElecIdentityTypeEnum;
...
public class KmReviewEqbSaasSignServiceImp implements IElecChannelAnsyService, IKmReviewEqbSignService {
	...
	public static final String outsign_type_eqb = "eqbs";

	private static final String MODEL_ALIAS_NAME = "km-Review";

	private Logger logger = LoggerFactory.getLogger(KmReviewEqbSaasSignServiceImp.class);
	...
	public Object execute(JSONObject json) throws Exception {
		logger.info("com.landray.kmss.km.review.service.spring.KmReviewEqbSaasSignServiceImp.execute方法:{}", json.toString());
	...
	private Object doExecute(JSONObject json) throws Exception{
		logger.info("E签宝SAAS签署回调通知:{}", json.toString());
		...
		if(!"SIGN_FLOW_FINISH".equalsIgnoreCase(action)){
			return null;
		}
		//E签宝签署流程结束状态
	...
	private void updateSignFile(String signId, KmReviewMain kmReviewMain) throws Exception {
		...
		try {
			...
			if (CollectionUtils.isNotEmpty(fileVOs)) {
				logger.debug("删除旧文件...");
				getSysAttMainService().delete(kmReviewMain,  EqbXFormFieldEnum.FD_EQB_SIGN_AFTER_FILES.getFieldId());
				logger.debug("删除完毕...");
				for (IElecChannelFileVO fileVO : fileVOs) {
					...
					logger.debug("开始写入文件...");
					getSysAttMainService().addAttachment(signId, KmReviewMain.class.getName(),  EqbXFormFieldEnum.FD_EQB_SIGN_AFTER_FILES.getFieldId(), bytes, fileVO.getFileName(), "attachment");
...
//		elecChannelContractSignInfo.setReturnUrl(contentPath+"/km/review/km_review_main/kmReviewMain.do?method=view&fdId="+kmReviewMain.getFdId()+"&_referer="+contentPath+"/km/review");
		...
		for(JSONObject singer:signers) {
			...
			ParticipantInfo pInfo = elecChannelContractSignInfo.new ParticipantInfo()
					...
					.setIdentNo(singer.getString("signUserCard"))
					.setIdentType(ElecIdentityTypeEnum.IDENTITY_CARD)
			...
			if("10".equals(singer.getString("signType"))) {
				...
				if(org!=null){
					pInfo.setEnterpriseName(org.getFdName())
						 .setEnterpriseIdentNO(singer.getString("signOrgCard"))
						 .setEnterpriseIdentType(ElecIdentityTypeEnum.ENTERPRISE_CREDIT_CODE);
				}
			}		
