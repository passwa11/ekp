...
public class KmArchivesJspGenerator {

	private static Logger logger = org.slf4j.LoggerFactory.getLogger(KmArchivesJspGenerator.class);
	...
	public String execGenerate(HttpServletRequest request, File formFile) throws Exception {
		...
		try {
			...
			return createFile(jsp.toString(), formFile);
		} finally {
	...
	public StringBuilder execute(HttpServletRequest request, InputStream is) throws Exception {
		String jspContent = IOUtils.toString(is);
	...
	private void exRelationChoose(HttpServletRequest request,StringBuilder jsp) {
		...
		Pattern p = Pattern.compile("<xformflag (?:(?!</xformflag>)[\\s\\S])* flagtype='xform_relation_choose' _xform_type='relationChoose'>");
		Matcher m = p.matcher(jsp.toString());
	...
	private static void exAttchment(StringBuilder jsp) {
		replace(jsp, "/sys/attachment/sys_att_main/sysAttMain_", "/km/archives/import/archive_sysAttMain_");
	}
	...
	public static String createFile(String jspContent, File formFile) {
		...
		try {
			...
			//覆盖
			is = IOUtils.toInputStream(jspContent);
