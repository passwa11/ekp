 ...
 */
public class HrAttendLactationMonitor {
	...
	private static final Logger log = org.slf4j.LoggerFactory.getLogger(HrAttendLactationMonitor.class);
...
//		List<String> orgNameList = new ArrayList<String>();
		// JSONArray buru_Array = new JSONArray();
		// JSONArray chanqian_Array = new JSONArray();
		...
		if (person_list.size() > 0) {
			...
			for (Map<String, Object> map : person_list) {
				...
				if (list_sys_attend_stat.size() == 0) {
					context.logMessage(person_name + " 在 " + start_time + " 与 " + end_time + "期间无迟到早退记录");
					continue;
				}
...
//					if (!orgNameList.contains(person_name)) {
...
//					}
					...
					if (is_ok.length() > 0) { // 说明系统已经做了这个人关于两个假期的请假处理，真的迟到早退了
						log.warn(statDetail.getFdDate() + is_ok + "，无符合条件的");
						continue;
					} else {
						log.warn(statDetail.getFdDate() + is_ok + "，开始计算");
						...
						if (tmpList.size() > 0) {
							for (SysAttendBusiness bus : tmpList) {
								...
								// 1.新建请假明细
								SysTimeLeaveDetail leaveDetail = addLeaveDetail(bus, statDetail);
								// 3.保存流程表单数据
								...
								// docCreatorObj.put("Id", person_id);
								// obj1.put("docCreator", person_id);
								// obj1.put("createTime",
								// bus.getDocCreateTime());
								//
								// if (fd_jiaqi_type == fd_leave_type_buru) {
								// buru_Array.add(obj1);
								// }
								//
								// if (fd_jiaqi_type == fd_leave_type_chanqian)
								// {
								// chanqian_Array.add(obj1);
								// }

							}
						} else {
							log.warn(statDetail.getFdDate() + ",未找到未处理的迟到早退:");
						}
		...
		if (mainList2 != null && !mainList2.isEmpty()) {
			for (SysAttendMain main : mainList2) {
				...
				if (main.getFdWorkType() == AttendConstant.SysAttendMain.FD_WORK_TYPE[0]) {
					log.warn("标准签卡时间" + sdfymdhms.format(work_time) + ";上班，");
					// 上班签到记录（计算迟到）
					...
					if (fdLeaveType == fd_leave_type_buru) {// 哺乳假每天两次
						...
						if (cishu == 0) {// 可以休半小时或1小时
							...
							log.warn("第一次休哺乳假期时长" + fdTotalTime + "分钟");
						} else if (cishu == 1 && lei_ji_day_fenzhong <= 30) {// 可以再休半小时
							fdTotalTime = 30;
							lei_ji_day_fenzhong = lei_ji_day_fenzhong + 30;
							ca.set(Calendar.MINUTE, ca.get(Calendar.MINUTE) + 30);// 时间+半小时

							log.warn("第二次休哺乳假期时长" + fdTotalTime + "分钟");
						} else {
							continue;
						}
						...
						if (cishu == 0 && lei_ji_day_fenzhong == 0) {
							...
							log.warn("产前工间假期时长" + fdTotalTime + "分钟");
						} else {
							continue;
						}
					} else {
					...
					log.warn("标准签卡时间" + sdfymdhms.format(work_time) + ";下班，");
					Calendar ca = Calendar.getInstance();// 得到一个Calendar的实例
					ca.setTime(work_time);
	 ...
	 * @throws Exception
	 */
	protected SysTimeLeaveDetail addLeaveDetail(SysAttendBusiness business, SysAttendStat statDetail) throws Exception {
		...
		log.warn(statDetail.getFdStandWorkTime()+ "没有排班，产前工间或哺乳假不设置请假天数");
		
		
		if(statDetail.getFdStandWorkTime() == null){//非数字
			log.warn(leaveDetail.getFdLeaveName() + statDetail.getDocCreator().getFdName() + business.getFdBusStartTime() + "至" + business.getFdBusEndTime()+ "没有排班，产前工间或哺乳假不设置请假天数");
		} else {
			if(statDetail.getFdStandWorkTime() > 0) {
				String leaveTime = String.format("%.3f", business.getFdCountHour() /statDetail.getFdStandWorkTime());
				leaveDetail.setFdLeaveTime(Float.valueOf(leaveTime));// 请假/加班天数
			} else {
				log.warn(leaveDetail.getFdLeaveName() + statDetail.getDocCreator().getFdName() + business.getFdBusStartTime() + "至" + business.getFdBusEndTime()+ "排班标准工作时长0，产前工间或哺乳假不设置请假天数");
			}
		}
