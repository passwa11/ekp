 ...
 * @see MPS类员工关怀流程-入职第一周
 * 
 */
public class HrEmployeeCareRemind {

	private static final Log log = LogFactory.getLog(HrEmployeeCareRemind.class);
	...
	public void createForm(SysQuartzJobContext context) throws Exception {
		...
		if (personlist.size() > 0) {
			for (Map<String, Object> map : personlist) {
				...
				// 文档模板id
				form.setFdTemplateId("184174361f3886d8f8c131845ba91b28");

				// 文档标题Employee care
				form.setDocSubject(
						map.get("fd_name") + "(" + map.get("fd_staff_no") + ")" + sdf.format(new Date()) + "入职第一周員工关怀提醒");

				// 流程发起人（固定人员：彭舒燕）
				form.setDocCreator(HrCurrencyParams.getStaffQuartzCreator("HrEmployeeCareRemind"));

				// 文档关键字
				form.setFdKeyword("[\"人事\", \"福利\"]");
				...
				if (map.get("fd_org_person_id") != null) {
					jo.put("fd_3b37da09cebd68", map.get("fd_org_person_id"));// 报告人员
				} else {
...
//				jo.put("fd_3b256386a344c0", map.get("rank_name"));
				...
				// fd_3b37cdafd151c6 入职1个月的时间
				jo.put("fd_3b37cdafd151c6", map.get("fd_entry_time_1") + "");
				// fd_3b37cddaa51e52 入职2个月的时间
				jo.put("fd_3b37cddaa51e52", map.get("fd_entry_time_2") + "");
				// fd_3b37e1eb2e5a40  试用期
				jo.put("fd_3b37e1eb2e5a40", map.get("fd_probation_period") + "");
				
				// fd_3b376dacd58ed4 入职6个月的时间
//				jo.put("fd_3b376dacd58ed4", map.get("fd_entry_time_5") + "");
	...
	private List<Map<String, Object>> getBecomeRegularWorkerPerson() throws Exception {
		...
		sb.append(" DATE_ADD(date(fd_entry_time), INTERVAL 1 MONTH) as fd_entry_time_1, ");
		sb.append(" DATE_ADD(date(fd_entry_time), INTERVAL 2 MONTH) as fd_entry_time_2 ");
		...
		sb.append(" and (r.fd_name like 'M%' or r.fd_name like 'P%' or r.fd_name like 'S%')");
