 ...
 * 更新家庭信息
 */
public class HrStaffWelfareRobotServiceImp extends AbstractRobotNodeServiceImp {
    private Logger logger = LoggerFactory.getLogger(HrStaffWelfareRobotServiceImp.class);

    //家庭信息
    private IHrStaffSecurityService hrStaffSecurityService;

    public void setHrStaffSecurityService(IHrStaffSecurityService hrStaffSecurityService) {
        this.hrStaffSecurityService = hrStaffSecurityService;
    }
    private IHrStaffAccumulationFundService hrStaffAccumulationFundService;

    public void setHrStaffAccumulationFundService(IHrStaffAccumulationFundService hrStaffAccumulationFundService) {
        this.hrStaffAccumulationFundService = hrStaffAccumulationFundService;
    }
...
//        String fieldValue = (String) parameters.get("fdName");
        ...
        hrStaffPersonInfoService.getBaseDao().getHibernateSession().flush();
...
//                for (Map<String, Object> map : fileList) {
                    //校验人员是否存在于人事档案中
                    //校验成功，保存家庭信息
                	HrStaffSecurity detailed = new HrStaffSecurity();
                	HrStaffAccumulationFund detailed1 = new HrStaffAccumulationFund();
                    detailed.setFdPersonInfo(personInfo);
                    detailed1.setFdPersonInfo(personInfo);
                    // 家庭关系
                    BeanUtils.setProperty(detailed, "fdRelated", modelData
                            .get(parameters.get("fdRelated")));
                    BeanUtils.setProperty(detailed1, "fdRelated1", modelData
                            .get(parameters.get("fdRelated1")));
//                    // 姓名
...
//                    detailed.setFdRelatedProcess(getUrl(model));
                    hrStaffSecurityService.add(detailed);
                    hrStaffAccumulationFundService.add(detailed1);
//                }
//            }
//        }
        return null;
