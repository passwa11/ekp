 ...
 * 
 */
public class HrStaffPersonUtil {

	private static final Logger logger = org.slf4j.LoggerFactory.getLogger(HrStaffPersonUtil.class);
	/**
	 * 调休
	 */
	...
	private static IHrStaffPersonInfoSettingNewService hrStaffPersonInfoSettingNewService;

	public static IHrStaffPersonInfoSettingNewService
			getHrStaffPersonInfoSettingService() {
	 ...
	 * @throws Exception
	 */
	public static String buildPersonInfoSettingHtml(String fieldName,
		...
		List<HrStaffPersonInfoSettingNew> settings = getHrStaffPersonInfoSettingService()
				.getByType(fieldName);

		if (request.getAttribute("mobileField") != null) {
			StringBuffer settingArr = new StringBuffer();
			for(int i = 0;i<settings.size();i++){
				settingArr.append(settings.get(i).getFdName());
	...
	public static String buildEducationHtml(String fieldName,
			HttpServletRequest request) throws Exception {
		HrStaffPersonExperienceEducationForm form = (HrStaffPersonExperienceEducationForm) request
				.getAttribute("hrStaffPersonExperienceEducationForm");
		String defValue = null;
		try {
			defValue = BeanUtils.getProperty(form, fieldName);
		} catch (Exception e) {
	...
	public static String buildDegreeHtml(String fieldName,
			HttpServletRequest request) throws Exception {
		HrStaffPersonExperienceEducationForm form = (HrStaffPersonExperienceEducationForm) request
				.getAttribute("hrStaffPersonExperienceEducationForm");
		String defValue = null;
		...
		List<HrStaffPersonInfoSettingNew> settings = getHrStaffPersonInfoSettingService()
				.getByType("fdHighestDegree");
		return buildHtml(settings, fieldName, defValue);
	}

	public static String buildBonusMalusTypeHtml(String fieldName,
			HttpServletRequest request) throws Exception {
		HrStaffPersonExperienceBonusMalusForm form = (HrStaffPersonExperienceBonusMalusForm) request
				.getAttribute("hrStaffPersonExperienceBonusMalusForm");
		String defValue = null;
		try {
			defValue = BeanUtils.getProperty(form, fieldName);
		} catch (Exception e) {
	...
	private static String buildHtml(List<HrStaffPersonInfoSettingNew> settings,
			String fieldName, String defValue) throws Exception {
		StringBuffer buf = new StringBuffer();
		buf.append("<select class='hr_select' name='").append(fieldName)
				.append("'>");
		...
		if (settings != null) {
			for (HrStaffPersonInfoSettingNew setting : settings) {
				buf.append("<option value='")
					.append(setting.getFdName())
						.append("'")
						.append(
							setting.getFdName().equals(defValue)
										? " selected"
										: "")
						.append(">").append(
							setting.getFdName())
	...
	public static String getImgUrl(HrStaffPersonInfo staffInfo,
			HttpServletRequest request) throws Exception {
		String fdOrgId = staffInfo.getFdOrgPerson()==null?"":staffInfo.getFdOrgPerson().getFdId();//非组织架构新建的员工没有组织架构人员ID
	 ...
	 * @return
	 */
	public static String getDetailKey(String key, Map<String, Object> modelData, Map<String, Object> detailData) {
		if (key.contains(".")) {
	 ...
	 * @throws Exception
	 */
	public static Object processData(String id, String constant, Map<String, Object> modelData, Map<String, Object> detailData, Object detailed) throws Exception{
	...
	public static HrStaffPersonInfo getPersonInfo(String fieldValue,
											Map<String, Object> modelData, Map<String, Object> detailData, IHrStaffPersonInfoService service) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static HrStaffPersonInfo getPersonInfoByIdCard(String fdIdCard) throws Exception {
		...
		if(StringUtil.isNull(fdIdCard)){
			return null;
		}
		HQLInfo info = new HQLInfo();
		info.setWhereBlock("fdIdCard=:fdIdCard");
		info.setParameter("fdIdCard",fdIdCard);
		List<HrStaffPersonInfo> personInfos = hrStaffPersonInfoService.findList(info);
		return !ArrayUtil.isEmpty(personInfos)?personInfos.get(0):null;
	}
}
