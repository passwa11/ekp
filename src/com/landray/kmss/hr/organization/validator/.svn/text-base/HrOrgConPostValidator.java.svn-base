...
public class HrOrgConPostValidator implements IAuthenticationValidator {
	...
	public boolean validate(ValidatorRequestContext validatorContext) throws Exception {
		if (!UserUtil.checkRole("ROLE_HRORGANIZATION_TRACK_RECORD_SCOPE")) {
			return false;
		}
		return true;
	}

	public static boolean validateConPostRole(String deptId) throws Exception {
		StringBuffer whereBlock = new StringBuffer();
		HQLInfo hqlInfo = new HQLInfo();

		Boolean isAdmin = UserUtil.getKMSSUser().isAdmin();
		if (isAdmin || UserUtil.checkRole("ROLE_HRORGANIZATION_ORG_ADMIN")) {
			return true;
		}
		...
		if (UserUtil.checkRole("ROLE_HRORGANIZATION_TRACK_RECORD")) {
			return true;
		}
