...
public class HrOrganizationRankServiceImp extends ExtendDataServiceImp implements IHrOrganizationRankService {
	...
	public IBaseModel convertBizFormToModel(IExtendForm form, IBaseModel model, ConvertorContext context)
        ...
        if (model instanceof HrOrganizationRank) {
            HrOrganizationRank hrOrganizationRank = (HrOrganizationRank) model;
            hrOrganizationRank.setDocAlterTime(new Date());
            hrOrganizationRank.setDocAlteror(UserUtil.getUser());
        }
        return model;
    }
	...
	public IBaseModel initBizModelSetting(RequestContext requestContext) throws Exception {
        HrOrganizationRank hrOrganizationRank = new HrOrganizationRank();
        hrOrganizationRank.setDocCreateTime(new Date());
        hrOrganizationRank.setDocAlterTime(new Date());
        hrOrganizationRank.setDocCreator(UserUtil.getUser());
        hrOrganizationRank.setDocAlteror(UserUtil.getUser());
        HrOrganizationUtil.initModelFromRequest(hrOrganizationRank, requestContext);
        return hrOrganizationRank;
    }
    ...
    @Override
	public List<HrOrganizationRank> findByFdGrade(HrOrganizationGrade fdGrade) throws Exception {
        HQLInfo hqlInfo = new HQLInfo();
        hqlInfo.setWhereBlock("hrOrganizationRank.fdGrade.fdId=:fdId");
        hqlInfo.setParameter("fdId", fdGrade.getFdId());
		...
		try {
			...
			if (sheet.getLastRowNum() < 1 || sheet.getRow(0).getLastCellNum() < columnSize - 1) {
				...
				for (int i = 1; i <= sheet.getLastRowNum(); i++) {
					...
					//所属职等
					String gradeNameValue = HrOrgUtil.getCellValue(row.getCell(1));
					//权重
					String fdWeight = HrOrgUtil.getCellValue(row.getCell(2));

					rank = new HrOrganizationRank();
					if (StringUtil.isNotNull(nameValue)) {
						boolean flag = checkNameUnique(nameValue, null, null);
						if (flag) {
							rank.setFdName(nameValue);
						} else {
							HrOrgUtil.addRowError(errorRow, rowIndex, 0,
									HrOrgUtil.getStr("hrOrganizationRank.fdName.Unique"));
						}
					} else {
						HrOrgUtil.addRowError(errorRow, rowIndex, 0,
								HrOrgUtil.getStr("hrOrganizationRank.fdName.notnull"));
					}

					if (StringUtil.isNotNull(gradeNameValue)) {
						HrOrganizationGrade grade = hrOrganizationGradeService.getHrOrgGradeByName(gradeNameValue);
						if (null != grade) {
							rank.setFdGrade(grade);
						} else {
							HrOrgUtil.addRowError(errorRow, rowIndex, 1,
									HrOrgUtil.getStr("hrOrganizationRank.fdGrade.notnone"));
						}
					} else {
						HrOrgUtil.addRowError(errorRow, rowIndex, 1,
								HrOrgUtil.getStr("hrOrganizationRank.fdGrade.notnull"));
					}

					if (StringUtil.isNotNull(fdWeight)) {
						rank.setFdWeight(Integer.valueOf(fdWeight));
					} else {
						HrOrgUtil.addRowError(errorRow, rowIndex, 2, HrOrgUtil.getStr("hrOrganizationRank.fdWeight"));
					}
				...
				if (hasError == 1) { // 有错误
					...
					titles.add(HrOrgUtil.getStr("hrOrganizationRank.fdName"));
					titles.add(HrOrgUtil.getStr("hrOrganizationRank.fdGrade"));
					titles.add(HrOrgUtil.getStr("hrOrganizationGrade.fdWeight"));
	...
	public HrOrganizationRank findRankByName(String fdName) throws Exception {
		HQLInfo hqlInfo = new HQLInfo();
		hqlInfo.setWhereBlock("hrOrganizationRank.fdName = :fdName");
		hqlInfo.setParameter("fdName", fdName);
		List<HrOrganizationRank> list = this.findList(hqlInfo);
	...
	public WorkBook buildTemplateWorkbook(HttpServletRequest request) {
		String[] baseColumns = new String[] { HrOrgUtil.getStr("hrOrganizationRank.fdName"),
				HrOrgUtil.getStr("hrOrganizationRank.fdGrade"), HrOrgUtil.getStr("hrOrganizationGrade.fdWeight"), };
		// 必填的列的下标
		Integer[] notNullArr = new Integer[] { 0, 1, 2 };
		List notNullList = Arrays.asList(notNullArr);
		String filename = HrOrgUtil.getStr("hrOrganizationRank.import.templateFile");
	...
	public List<HrOrganizationRank> findByGradeId(String gradeId) throws Exception {
		HQLInfo hqlInfo = new HQLInfo();
		hqlInfo.setWhereBlock("hrOrganizationRank.fdGrade.fdId = :gradeId");
		hqlInfo.setParameter("gradeId", gradeId);
		List<HrOrganizationRank> list = this.findList(hqlInfo);
		return ArrayUtil.isEmpty(list) ? null : list;
	}

	@Override
	public boolean checkNameUnique(String fdName, String fdId, String fdGradeId)
			throws Exception {
		HQLInfo hqlInfo = new HQLInfo();
		...
		sbf.append(" and fdGrade.fdId!=:gradeId");
		hqlInfo.setParameter("gradeId", fdGradeId);
		hqlInfo.setParameter("fdName", fdName);
