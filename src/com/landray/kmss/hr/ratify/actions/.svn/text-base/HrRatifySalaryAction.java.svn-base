...
public class HrRatifySalaryAction extends HrRatifyMainAction {
    ...
    public void changeFindPageHQLInfo(HttpServletRequest request, HQLInfo hqlInfo) throws Exception {
        HQLHelper.by(request).buildHQLInfo(hqlInfo, HrRatifySalary.class);
        hqlInfo.setOrderBy(getFindPageOrderBy(request, hqlInfo.getOrderBy()));
        com.landray.kmss.hr.ratify.util.HrRatifyUtil.buildHqlInfoDate(hqlInfo, request, com.landray.kmss.hr.ratify.model.HrRatifySalary.class);
    ...
    public ActionForm createNewForm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		((IHrRatifySalaryService) getServiceImp(request)).initFormSetting(
				(IExtendForm) form, new RequestContext(request));
        HrRatifySalaryForm hrRatifySalaryForm = (HrRatifySalaryForm) super.createNewForm(mapping, form, request, response);
		hrRatifySalaryForm.setDocTemplateName(
				getTemplatePath(hrRatifySalaryForm.getDocTemplateId()));
		if (null != orgElement) {
			hrRatifySalaryForm.setFdSalaryStaffId(orgElement.getFdId());
			hrRatifySalaryForm.setFdSalaryStaffName(orgElement.getFdName());
			if (null != orgElement.getFdParent()) {
				hrRatifySalaryForm.setFdSalaryDeptId(orgElement.getFdParent().getFdId());
				hrRatifySalaryForm.setFdSalaryDeptName(orgElement.getFdParent().getFdName());
			}
		}
        return hrRatifySalaryForm;
    }
    ...
    public ActionForward print(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			// 引入打印机制
			HrRatifySalaryForm ratifyForm = (HrRatifySalaryForm) form;
			...
			HrRatifySalary main = (HrRatifySalary) getServiceImp(
					null)
