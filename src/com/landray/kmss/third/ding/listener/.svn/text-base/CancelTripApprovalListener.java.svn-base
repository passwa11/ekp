...
public class CancelTripApprovalListener implements IEventListener {
	private static final Logger logger = org.slf4j.LoggerFactory.getLogger(CancelTripApprovalListener.class);
			...
			if (StringUtil.isNotNull(fdCancelType)) {
				...
				if (StringUtil.isNotNull(fdStartDate)) {
					...
					if (sDate != null && end_Date != null) {
						logger.debug(
								"根据fdStartDate和fdEndDate判断该销假需要重新提交表单："
										+ fdStartDate);
						resubmit = 1;
					} else {
						logger.warn(
								"根据fdStartDate和fdEndDate至少有一个为null判断该销假不需要重新提交表单！");
					}
