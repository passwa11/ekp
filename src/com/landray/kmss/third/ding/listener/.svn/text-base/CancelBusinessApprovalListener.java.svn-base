...
public class CancelBusinessApprovalListener implements IEventListener {
	private static final Logger logger = org.slf4j.LoggerFactory.getLogger(CancelBusinessApprovalListener.class);
	...
	public void handleEvent(EventExecutionContext execution, String parameter)
		...
		try {
			...
			// "fdCancelType");
			String fdCancelType = DING_BUSINESS;
			...
			if (StringUtil.isNotNull(fdCancelType)) {
				...
				if (StringUtil.isNotNull(fdStartDate)) {
					...
					if (sDate != null && end_Date != null) {
						logger.debug(
								"根据fdStartDate和fdEndDate判断该销外出需要重新提交表单："
										+ fdStartDate);
						resubmit = 1;
					} else {
						logger.warn(
								"根据fdStartDate和fdEndDate至少有一个为null判断销外出不需要重新提交表单！");
					}
