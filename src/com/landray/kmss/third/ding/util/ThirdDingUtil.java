...
public class ThirdDingUtil{
	...
	public static org.json.simple.JSONArray importData(FormFile file,
			org.json.simple.JSONArray jsArray)
			throws Exception {
		Workbook wb = null;
		Sheet sheet = null;
		try {
	...
	public static String geFdEventTypeTipt(String fdEventType) {
		//事件集合
		Map<String, String> eventmap  = new HashMap<>();
		eventmap.put("user_add_org", "通讯录用户增加");
		eventmap.put("calendar_event_change", "用户日程发生变更事件");
		eventmap.put("user_modify_org", "通讯录用户更改");
		eventmap.put("user_dept_change", "通讯录用户部门变更"); 
		eventmap.put("user_role_change", "通讯录用户角色变更"); 
		eventmap.put("user_active_org", "加入企业后用户激活"); 
		eventmap.put("user_leave_org", "通讯录用户离职"); 
		eventmap.put("org_dept_create", "通讯录企业部门创建"); 
		eventmap.put("org_dept_modify", "通讯录企业部门修改"); 
		eventmap.put("org_dept_remove", "通讯录企业部门删除"); 
		eventmap.put("bpms_instance_change", "审批实例开始，结束"); 
		eventmap.put("bpms_task_change", "审批任务开始，结束，转交"); 
		String fdEventTypeTipt = eventmap.get(fdEventType);
		return fdEventTypeTipt;
	}
