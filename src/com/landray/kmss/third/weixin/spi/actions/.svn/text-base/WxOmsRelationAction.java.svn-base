...
import com.landray.kmss.third.weixin.spi.forms.WxOmsRelationModelForm;
 ...
 */
public class WxOmsRelationAction extends ExtendAction {
	protected IWxOmsRelationService wxOmsRelationService;

	@Override
    protected IBaseService getServiceImp(HttpServletRequest request) {
		if (wxOmsRelationService == null) {
			wxOmsRelationService = (IWxOmsRelationService) getBean(
					"wxOmsRelationService");
		}
	...
	public ActionForward loadExcel(ActionMapping mapping, ActionForm form,
		...
		WxOmsRelationModelForm mainForm = (WxOmsRelationModelForm) form;
		FormFile file = mainForm.getFile();
		JSONArray jsonArr = null;
		try {
			jsonArr = ((IWxOmsRelationService) getServiceImp(request))
					.addExcel(file);
		} catch (Exception e) {
			e.printStackTrace();
			messages.addError(e);
	...
	public ActionForward addOrUpdateCheck(ActionMapping mapping,
		...
		try {
			...
			if (StringUtil.isNull(fdAppPkId) || StringUtil.isNull(type)
				...
				Map<String, String> map = ((IWxOmsRelationService) getServiceImp(
			...
			if (StringUtil.isNotNull(fdEkpId)) {
				...
				if (person != null) {
					if (UserOperHelper.allowLogOper("checkEKP",
							getServiceImp(request).getModelName())) {
						UserOperContentHelper.putFind(person);
					}
					json.put("status", 1);
	...
	public ActionForward checkThird(ActionMapping mapping, ActionForm form,
		...
		try {
			String fdId = request.getParameter("fdId");
			String fdAppPkId = request.getParameter("fdAppPkId");
			boolean flag = ((IWxOmsRelationService) getServiceImp(request))
    ...
    protected void loadActionForm(ActionMapping mapping, ActionForm form,
		...
		if (!StringUtil.isNull(id)) {
			...
			if (model != null) {
				...
				WxOmsRelationModelForm modelForm = (WxOmsRelationModelForm) rtnForm;
    ...
    public ActionForward list(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			Page page = ((IWxOmsRelationService) getServiceImp(request))
