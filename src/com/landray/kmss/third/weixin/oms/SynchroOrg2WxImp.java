package com.landray.kmss.third.weixin.oms;
...
public class SynchroOrg2WxImp
	...
	private WxOmsConfig wxOmsConfig = null;
	...
	public void triggerSynchro(SysQuartzJobContext context) {
		...
		if (!"true".equals(WeixinConfig.newInstance().getWxEnabled())) {
			logger.info("微信企业号集成已经关闭，故不同步数据");
			context.logMessage("微信企业号集成已经关闭，故不同步数据");
			return;
		}
		if (!"true".equals(WeixinConfig.newInstance().getWxOmsOutEnabled())) {
			logger.info("微信企业号组织架构接出已经关闭，故不同步数据");
			context.logMessage("微信企业号组织架构接出已经关闭，故不同步数据");
			return;
		}
		locked = true;
		try {
			...
			// 0不处理，1禁用，2删除
			String personHandle = WeixinConfig.newInstance()
	...
	private String updateDept(SysOrgElement element) throws Exception {
		...
		if (element.getFdOrder() != null) {
			dept.setOrder(element.getFdOrder().longValue());
		}
		...
		if (flag) {
			String logInfo = "";
			try {
				JSONObject result = wxApiService.departUpdate(dept);
					...
					if (dept.getFdIsAvailable()) {
						if (!relationMap.keySet().contains(dept.getFdId())) {
							...
							if (dept.getFdOrder() != null) {
								wxdept.setOrder(dept.getFdOrder().longValue());
							}
							n++;
				...
				if (dept.getFdOrgType() == ORG_TYPE_ORG
					...
					if (!dept.getFdIsAvailable()) {
						...
						if (!relationMap.keySet().contains(dept.getFdId())) {
							...
							wxOmsRelationService.deleteByKey(dept.getFdId(),
									getAppKey());
	...
	private WxUser getUser(SysOrgPerson element) throws Exception {
		...
		if (posts != null && !posts.isEmpty()) {
			String position = ArrayUtil.joinProperty(posts, "fdName", ";")[0];
...
//								}
	...
	private void addPerson(SysOrgPerson element) throws Exception {
		...
		try {
			JSONObject result = wxApiService.userCreate(getUser(element));
			logInfo += " ,created";
			String wxln = WeixinConfig.newInstance().getWxLoginName();
			if ("id".equalsIgnoreCase(wxln)) {
	...
	private void updatePerson(SysOrgPerson element) throws Exception {
		String logInfo = "更新个人到微信 " + element.getFdName() + ",loginName:"
				...
				+ ",对应父微信ID:" + (element.getFdParent() != null
						? relationMap.get(element.getFdParent().getFdId())
						: "");
		logInfo += " ,retmsg:";
		String errorCode = "";
		WxUser wxuser = getUser(element);
		try {
			if (relationMap.get(element.getFdId()) != null) {
                wxuser.setUserId(relationMap.get(element.getFdId()));
            }
			JSONObject result = wxApiService.userUpdate(wxuser);
			logInfo += " updated";
		...
		if (StringUtil.isNotNull(errorCode)) {
			...
			if ("60121".equals(errorCode) || "60111".equals(errorCode)) {
				...
				try {
					JSONObject result = wxApiService.userCreate(wxuser);
					logInfo += " created";
	...
	private void addNewAnddelOld(SysOrgPerson element) throws Exception {
		...
		try {
			JSONObject result = wxApiService.userDelete(oldLoginName);
			logInfo += " ,deleted";
		...
		logInfo += "\n";
		wxOmsRelationService.deleteByKey(element.getFdId(), getAppKey());
	...
	private void deletePerson(SysOrgPerson element) throws Exception {
		...
		try {
			...
			logInfo += " ,deleted";
			wxOmsRelationService.deleteByKey(element.getFdId(),
					getAppKey());
			relationMap.remove(element.getFdId());
	...
	private void addRelation(SysOrgElement element, String appPkId)
			throws Exception {
		WxOmsRelationModel model = new WxOmsRelationModel();
	...
	private boolean isOmsRootOrg(String rootId) {
		if ("true".equals(WeixinConfig.newInstance().getWxOmsRootFlag())) {
			return true;
		} else if (WeixinConfig.newInstance().getWxOrgId()
				.indexOf(rootId) != -1) {
			return false;
		}
		return true;
	...
	private List<SysOrgElement> getAllOrgByRootOrg() throws Exception {
		...
		for (int i = 0; i < allOrgChildren.size(); i++) {
			...
			if (StringUtil.isNotNull(WeixinConfig.newInstance().getWxOrgId())) {
				SysOrgElement parent = org.getFdParent();
				while (parent != null) {
					if (WeixinConfig.newInstance().getWxOrgId()
	...
	private void init() throws Exception {
		...
		if(StringUtil.isNotNull(WeixinConfig.newInstance().getWxRootId())) {
            WX_ROOT_DEPT_ID = Long.parseLong(WeixinConfig.newInstance().getWxRootId());
        }
		String WX_OMS_ROOT_ORG_ID = WeixinConfig.newInstance().getWxOrgId();
		...
		List list = wxOmsRelationService
				.findList("fdAppKey='" + getAppKey() + "'", null);
		for (int i = 0; i < list.size(); i++) {
			WxOmsRelationModel model = (WxOmsRelationModel) list.get(i);
			relationMap.put(model.getFdEkpId(), model.getFdAppPkId());
		}
		...
		if (deletePersons != null && !deletePersons.isEmpty()) {
			for (int i = 0; i < deletePersons.size(); i++) {
				if (personFlag) {
					...
					WxUser wxuser = getUser(deletePersons.get(i));
					try {
						//禁用人员TODO
						if (relationMap
		...
		if (deleteDepts != null && !deleteDepts.isEmpty()&&personFlag) {
			...
			for (int i = 0; i < deleteDepts.size(); i++) {
				try {
					...
					wxOmsRelationService.deleteByKey(
	...
	private String getWxSyncTime() throws Exception {
		String time = null;
		String sql = "select fd_value from sys_app_config where fd_key='com.landray.kmss.third.weixin.oms.WxOmsConfig' and fd_field='lastUpdateTime'";
