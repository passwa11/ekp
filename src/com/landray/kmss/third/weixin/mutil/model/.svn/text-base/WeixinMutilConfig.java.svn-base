 ...
 * 多企业微信集成配置
 */
public class WeixinMutilConfig extends ThirdWxBaseConfig {

	private static final Logger logger = org.slf4j.LoggerFactory.getLogger(WeixinMutilConfig.class);


	public static WeixinMutilConfig newInstance(String key) {
		WeixinMutilConfig config = null;
		try {
			config = new WeixinMutilConfig(key);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("", e);
		}
	...
	public static WeixinMutilConfig newInstance() {
		WeixinMutilConfig config = null;
		try {
			config = new WeixinMutilConfig(null);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("", e);
		}
	...
	public WeixinMutilConfig(String key) throws Exception {
		super(key);
	...
	// EKP中根机构name
	public String getWxOrgName() {
		return getValue("wxOrgName");
	}
