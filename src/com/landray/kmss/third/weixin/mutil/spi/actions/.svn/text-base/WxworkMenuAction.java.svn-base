...
public class WxworkMenuAction extends ExtendAction {
	...
	public ActionForward publish(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			String fdWxKey = wxForm.getFdWxKey();
			init(fdWxKey);

			wxmutilApiService
					.menuDelete(WeixinMutilConfig.newInstance(fdWxKey).getWxAgentid());
			...
			for (int i = 0; i < jsa.size(); i++) {
				...
				for (int j = 0; j < jas.size(); j++) {
					...
					String domainName = WeixinMutilConfig.newInstance(fdWxKey)
							.getWxDomain();
			...
			wxmutilApiService.menuCreate(
					WeixinMutilConfig.newInstance(fdWxKey).getWxAgentid(), wxMenu);
