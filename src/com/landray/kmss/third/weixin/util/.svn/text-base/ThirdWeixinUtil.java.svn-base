...
public class ThirdWeixinUtil{

	private final static Logger logger = LoggerFactory
			.getLogger(ThirdWeixinUtil.class);
	...
	private static HSSFCellStyle getCellStyle(final HSSFWorkbook wb,
		...
		if (isCenter) {
			style.setBorderRight(
					org.apache.poi.ss.usermodel.BorderStyle.valueOf((short) 2));
			style.setBorderRight(
					org.apache.poi.ss.usermodel.BorderStyle.valueOf((short) 1));
		}
	...
	public static String getAttUrl(String attUrl, String attId)
			throws Exception {
		...
		if (!permission) {
			...
			String picUrl = urlPrefix
					+ "/resource/third/ding/attachment.do?method=downloadFile&fdId="
					+ attId + "&Signature="
					+ genSignature(Expires + "", attId)
					+ "&Expires=" + Expires + "&open=true";

			return picUrl;
		}
	}

	public static String genSignature(String expires, String fdId)
			throws Exception {
		...
		SysAttMain sysWsMain = (SysAttMain) sysAttMainService
				.findByPrimaryKey(fdId);
		if (sysWsMain == null) {
			return null;
		}
		String calcSign = SignUtil
				.getHMAC(signStr,
						sysWsMain.getFdModelId());
		return calcSign;

	}

	public static boolean validateSignature(String sign, String expires,
			String fdId)
			throws Exception {
		boolean auth = false;
