...
public class ThirdWxworkOmsInitListService implements IXMLDataBean, WxConstant, SysOrgConstant {
	...
	/*public List getDataList(RequestContext requestInfo) throws Exception {
		...
		List<WxworkOmsRelationModel> relationlist = wxworkOmsRelationService
		...
		if ("0".equals(type)) {
			where = "fdOrgType=8 and fdLoginName not in ('admin','everyone','anonymous')";
		}
		if (StringUtil.isNotNull(search)) {
			where += " and fdName like '%" + search + "%'";
		}
		...
		if ((orgType & ORG_TYPE_PERSON) == ORG_TYPE_PERSON
			...
			for (SysOrgElement post : postList) {
				...
				ss = sysOrganizationStaffingLevelService
						.getStaffingLevelFilterResult((List<SysOrgElement>) ss);
				for (int i = 0; i < ss.size(); i++) {
					SysOrgElement soe = ThirdWxUtil
							.cloneBean((SysOrgElement) ss.get(i));
					if (soe.getFdIsAvailable() && soe.getFdIsBusiness()
							&& elemList != null
							&& !elemList.contains(soe)) {
						soe.setFdOrder(post.getFdOrder());
