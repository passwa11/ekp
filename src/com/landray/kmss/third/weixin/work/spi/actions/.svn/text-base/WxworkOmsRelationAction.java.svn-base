...
public class WxworkOmsRelationAction extends ExtendAction {
	...
	public ActionForward addOrUpdateCheck(ActionMapping mapping,
		...
		try {
			...
			if (StringUtil.isNull(fdAppPkId) || StringUtil.isNull(type)
				...
				Map<String, String> map = ((IWxworkOmsRelationService) getServiceImp(request)).handle(fdId.trim(), fdAppPkId.trim(), type,fdEkpId);
				if (map != null) {
	...
	public ActionForward checkEKP(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (StringUtil.isNotNull(fdEkpId)) {
				if("0".equals(type)){
					SysOrgPerson person = (SysOrgPerson) getSysOrgPersonServiceImp(request).findByPrimaryKey(fdEkpId, null, true);
					if (person != null) {
						//检查映射关系是否已存在
						boolean passFlag = ((IWxworkOmsRelationService) getServiceImp(request)).checkEKP(fdId, fdEkpId);
						if(passFlag){
							json.put("status", 1);
							json.put("msg", person.getFdLoginName());
						}else{
							json.put("status", 0);
							json.put("msg","映射关系已存在");
						}
					} else {
						json.put("status", 0);
						json.put("msg", ResourceUtil.getString(
	...
	public ActionForward checkThird(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			String fdAppPkId = request.getParameter("fdAppPkId");
			String type = request.getParameter("type");
			boolean flag = ((IWxworkOmsRelationService) getServiceImp(request)).checkThird(fdId, fdAppPkId, type);
			if (!flag) {
				json.put("status", 0);
				json.put("msg", ResourceUtil.getString(
	...
	public ActionForward update(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (messages.hasError()) {
			...
			KmssReturnPage.getInstance(request).addMessages(messages)
					.addButton(
							"button.back","/third/weixin/work/spi/wxwork_oms_relation/wxworkOmsRelation.do?method=view&fdId="+fdModelId+"&type="+type, false).save(request);
