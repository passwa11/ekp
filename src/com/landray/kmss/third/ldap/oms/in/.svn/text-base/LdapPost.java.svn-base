...
public class LdapPost extends DefaultOrgPost {
	public LdapPost(BaseLdapEntry entry) throws Exception {
		...
		if (entry.isPropertyMap("member")) {
			setPersons((String[]) entry.getImportInfoValues("member").toArray(
					new String[0]));
			/*
			 ...
			 * .getStringProperty("unid"); } setPersons(members);
			 */
			required.add("persons");
		}
	...
	public String toString(String split) {
		...
		if (this.getPersons() != null) {
			...
			for (int i = 0; i < this.getPersons().length; i++) {
				sb.append(this.getPersons()[i] + split);
			}
			sb.append("]" + split);
		}
