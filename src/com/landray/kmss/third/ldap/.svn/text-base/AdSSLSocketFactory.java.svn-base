...
//        return context.getSSLContext().getSocketFactory(); 
		...
		Map map;
		try {
			map = new LdapDetailConfig().getDataMap();
		} catch (Exception e) {
			// TODO 自动生成 catch 块
			e.printStackTrace();
			return null;
		}
		String trustStore = (String)map.get("kmss.ldap.type.auth.trustStore");
		String trustStorePassword = (String)map.get("kmss.ldap.type.auth.trustStorePassword");
		MyTrustManager trustManager = new MyTrustManager(trustStore, trustStorePassword.toCharArray());
        MySSLContext context = new MySSLContext("TLS", null, trustManager);
        return context.getSSLContext().getSocketFactory();
    }
