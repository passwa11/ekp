...
public class Oauth2AuthProcessingFilter implements Filter, InitializingBean {
	...
	private String oauth2buildAuthorizationUrl(String redirectUri,
		...
		String url = "https://login.welink.huaweicloud.com/sso/oauth2/authorize?";
		url += "client_id=" + config.getWelinkClientid()
				+ "&response_type=code&scope=backendlogin";
		try {
		...
		logger.debug("weilink sso redirect url = " + url);
		return url;
	}
    ...
    public void doFilter(ServletRequest req, ServletResponse res,
		...
		if ("true".equals(ssoEnable)
				...
				&& MobileUtil.THIRD_WELINK != MobileUtil.getClientType(request)
				&& MobileUtil.WELINK_PC != MobileUtil.getClientType(request)
			...
			if (StringUtil.isNull(code)) {
				...
				try {
					String accessToken = thirdWelinkService.getSsoAccessToken(
							code, getRedirectUri(request), null);
					userid = thirdWelinkService.getSsoUserId(accessToken);
				...
				if (StringUtil.isNull(userid)) {
					logger.debug("welink集成无法获取当前的操作人员(" + userid + ")");
				} else {
					try {
