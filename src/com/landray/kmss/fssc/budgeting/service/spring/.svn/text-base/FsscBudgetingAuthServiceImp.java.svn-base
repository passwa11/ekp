...
public class FsscBudgetingAuthServiceImp extends ExtendDataServiceImp implements IFsscBudgetingAuthService {
    ...
    public IBaseModel convertBizFormToModel(IExtendForm form, IBaseModel model, ConvertorContext context) throws Exception {
        ...
        if (model instanceof FsscBudgetingAuth) {
            FsscBudgetingAuth fsscBudgetingAuth = (FsscBudgetingAuth) model;
            fsscBudgetingAuth.setDocAlterTime(new Date());
            fsscBudgetingAuth.setDocAlteror(UserUtil.getUser());
        }
        return model;
    }
    ...
    public IBaseModel initBizModelSetting(RequestContext requestContext) throws Exception {
        FsscBudgetingAuth fsscBudgetingAuth = new FsscBudgetingAuth();
        fsscBudgetingAuth.setFdIsAvailable(Boolean.valueOf("true"));
        fsscBudgetingAuth.setDocCreateTime(new Date());
        fsscBudgetingAuth.setDocAlterTime(new Date());
        fsscBudgetingAuth.setDocCreator(UserUtil.getUser());
        fsscBudgetingAuth.setDocAlteror(UserUtil.getUser());
        FsscBudgetingUtil.initModelFromRequest(fsscBudgetingAuth, requestContext);
        return fsscBudgetingAuth;
    }
    ...
    @Override
    public List<FsscBudgetingAuth> findByFdCostCenterList(EopBasedataCostCenter fdCostCenterList) throws Exception {
        HQLInfo hqlInfo = new HQLInfo();
        hqlInfo.setWhereBlock("fsscBudgetingAuth.fdCostCenterList.fdId=:fdId");
        hqlInfo.setParameter("fdId", fdCostCenterList.getFdId());
        return this.findList(hqlInfo);
    }

    @Override
    public List<FsscBudgetingAuth> findByFdBudgetItemList(EopBasedataBudgetItem fdBudgetItemList) throws Exception {
        HQLInfo hqlInfo = new HQLInfo();
        hqlInfo.setWhereBlock("fsscBudgetingAuth.fdBudgetItemList.fdId=:fdId");
        hqlInfo.setParameter("fdId", fdBudgetItemList.getFdId());
        return this.findList(hqlInfo);
    }

    @Override
    public List<FsscBudgetingAuth> findByFdProjectList(EopBasedataProject fdProjectList) throws Exception {
        HQLInfo hqlInfo = new HQLInfo();
        hqlInfo.setWhereBlock("fsscBudgetingAuth.fdProjectList.fdId=:fdId");
	...
	@Override
	public void saveImport(FsscBudgetingAuthForm mainForm,HttpServletRequest request) throws Exception {
		Workbook wb=null;
		...
		try {
			...
			List<FsscBudgetingAuth> auhList=new ArrayList<FsscBudgetingAuth>();  //缓存excel数据对象
			String where="";
			// 取到工作表里面的数据
			for (int i = 1,shLen=sheet.getLastRowNum(); i <=shLen ; i++) {
				...
				FsscBudgetingAuth auth=new FsscBudgetingAuth();
			...
			if(ArrayUtil.isEmpty(validateInfoList)){//说明校验通过,保存所有的对象
				for (FsscBudgetingAuth auth : auhList) {
					this.add(auth);
				}
			}
		} catch (Exception e) {
