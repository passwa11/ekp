 ...
 *
 */
public class FsscExpenseAbandonEvent implements IEventListener{
	...
	public void handleEvent(EventExecutionContext execution, String parameter) throws Exception {
		...
		if(FsscCommonUtil.checkHasModule("/fssc/ledger/")){
			if(getFsscCommonLedgerService()!=null){
		...
		if(FsscCommonUtil.checkHasModule("/fssc/mobile/")){
			...
			getFsscExpenseMainService().getBaseDao().getHibernateSession().createQuery(hql).setParameter("fdId", main.getFdId()).executeUpdate();
		}
