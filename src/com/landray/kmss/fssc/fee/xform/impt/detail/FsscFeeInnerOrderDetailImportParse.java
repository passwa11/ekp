...
public class FsscFeeInnerOrderDetailImportParse implements ISysTransportImportPropertyParse{
	...
	public boolean parse(ImportInDetailsCellContext detailsCellContext) throws Exception {
		...
		String hql = "select ord from "+EopBasedataInnerOrder.class.getName()+" ord left join ord.fdCompanyList comp where ord.fdIsAvailable=:fdIsAvailable and ord.fdName=:code and (comp.fdId=:fdCompanyId or comp is null)";
		List<EopBasedataInnerOrder> list = getFsscFeeMainService().getBaseDao().getHibernateSession().createQuery(hql)
		...
		if(ArrayUtil.isEmpty(list)) {
			KmssMessage message = new KmssMessage(
					...
					ResourceUtil.getString("table.eopBasedataInnerOrder","eop-basedata")
			);
