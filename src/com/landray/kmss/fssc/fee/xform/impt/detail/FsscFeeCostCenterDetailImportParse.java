...
public class FsscFeeCostCenterDetailImportParse implements ISysTransportImportPropertyParse{
	...
	public boolean parse(ImportInDetailsCellContext detailsCellContext) throws Exception {
		...
		String hql = "select cost from "+EopBasedataCostCenter.class.getName()+" cost left join cost.fdCompanyList comp where cost.fdIsAvailable=:fdIsAvailable and cost.fdName=:code and (comp.fdId=:fdCompanyId or comp is null) and cost.fdIsGroup='1'";
		List<EopBasedataCostCenter> list = getFsscFeeMainService().getBaseDao().getHibernateSession().createQuery(hql)
		...
		if(ArrayUtil.isEmpty(list)) {
			KmssMessage message = new KmssMessage(
					...
					ResourceUtil.getString("table.eopBasedataCostCenter","eop-basedata")
			);
