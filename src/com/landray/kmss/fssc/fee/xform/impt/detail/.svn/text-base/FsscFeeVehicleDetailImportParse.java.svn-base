...
public class FsscFeeVehicleDetailImportParse implements ISysTransportImportPropertyParse{
	...
	public boolean parse(ImportInDetailsCellContext detailsCellContext) throws Exception {
		...
		String fdVehicleId = propertyId+"_vehiclee_id";
		String fdVehicleName = propertyId+"_vehiclee_name";
		...
		String hql = "select berth from "+EopBasedataBerth.class.getName()+" berth left join berth.fdCompanyList comp where berth.fdIsAvailable=:fdIsAvailable and berth.fdName=:code and (comp.fdId=:fdCompanyId or comp is null)";
		List<EopBasedataBerth> list = getFsscFeeMainService().getBaseDao().getHibernateSession().createQuery(hql)
		...
		if(ArrayUtil.isEmpty(list)) {
			KmssMessage message = new KmssMessage(
					...
					ResourceUtil.getString("table.eopBasedataVehicle","eop-basedata")
			);
			contentMessage.addError(message);
			temp.put(propertyId, "");
			temp.put(propertyName, "");
			temp.put(fdVehicleId, "");
			temp.put(fdVehicleName, "");
			return false;
		}
		...
		temp.put(fdVehicleId, list.get(0).getFdVehicle().getFdId());
		temp.put(fdVehicleName, list.get(0).getFdVehicle().getFdName());
		return true;
	}
