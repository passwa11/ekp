...
public class FsscFeeProjectDetailImportParse implements ISysTransportImportPropertyParse{
	...
	public boolean parse(ImportInDetailsCellContext detailsCellContext) throws Exception {
		...
		String hql = "select project from "+EopBasedataProject.class.getName()+" project left join project.fdCompanyList comp where project.fdIsAvailable=:fdIsAvailable and project.fdName=:code and (comp.fdId=:fdCompanyId or comp is null)";
		List<EopBasedataProject> list = getFsscFeeMainService().getBaseDao().getHibernateSession().createQuery(hql)
		...
		if(ArrayUtil.isEmpty(list)) {
			KmssMessage message = new KmssMessage(
					...
					ResourceUtil.getString("table.eopBasedataProject","eop-basedata")
			);
