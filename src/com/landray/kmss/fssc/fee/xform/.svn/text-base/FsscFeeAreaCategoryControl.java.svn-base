...
public class FsscFeeAreaCategoryControl implements ISysFormTemplateControl, FilterAction,ISysFormTemplateDetailsTableControl{
	...
	public boolean parse(Node node, Lexer lexer, StringBuilder jsp, List<ISysFormTemplateControl> controls)
		...
		if(node instanceof TagNode){
			...
			if(isType("areaCategory",tagNode)){
	...
	private void doParse(Node node, final StringBuilder jsp, String idPrefix)
		...
		if (node instanceof InputTag) {
			...
			String id = idPrefix+props.optString("id");
			String name = idPrefix+props.optString("id")+"_name";
			StringBuilder tempJsp = new StringBuilder();
			tempJsp.append(ENTER).append(getElementTab()).append("<xformflag flagtype=\"area\" flagid='"+id+"' _xform_type='area' property='extendDataFormInfo.value("+id+")' id='_xform_extendDataFormInfo.value("+id+")'>");
			...
			setTitleAndSubject(tempJsp, props.optString("subject"));
			setAttribute(tempJsp, "dialogJs", "Designer_DialogSelect(false,'eop_basedata_city_selectCity','extendDataFormInfo.value("+id+")','extendDataFormInfo.value("+name+")',null,{fdCompanyId:'"+props.optString("companyId")+"'});");
			tempJsp.append(">");
			...
			tempJsp.append("<script>").append(ENTER).append("window.Designer_Control_AreaCategory_List=window.Designer_Control_AreaCategory_List||[];").append(ENTER);
			tempJsp.append("window.Designer_Control_AreaCategory_List.push('"+id+"')").append(ENTER);
			tempJsp.append("window.Designer_Control_Company_Event_List['"+props.optString("companyId")+"'].push(Designer_Control_AreaCategory_Load);").append(ENTER).append("</script>");
			System.out.println("HTML:"+tempJsp.toString());
