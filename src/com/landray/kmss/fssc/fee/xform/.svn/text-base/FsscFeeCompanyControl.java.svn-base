...
public class FsscFeeCompanyControl implements ISysFormTemplateControl, FilterAction,ISysFormTemplateDetailsTableControl{
	...
	private void doParse(Node node, final StringBuilder jsp, String idPrefix)
		...
		if (node instanceof InputTag) {
			...
			jsp.append(ENTER).append(getElementTab()).append("<xformflag flagtype=\"company\" flagid=\""+id+"\" _xform_type='company' property='extendDataFormInfo.value("+id+")' id='_xform_extendDataFormInfo.value("+id+")'>");
			...
			if(props.containsKey("width")){
				String width = String.valueOf(props.get("width"));
				jsp.append("style=\"width:").append(width).append("\" ");
			}else{
			...
			setAttribute(jsp, "dialogJs", "Designer_DialogSelect(false,'eop_basedata_company_fdCompany','extendDataFormInfo.value("+id+")','extendDataFormInfo.value("+name+")',Designer_Control_Company_Changed);");
			jsp.append(">");
			jsp.append(ENTER).append(getElementTab()).append(getElementTab());
			jsp.append(ENTER).append(getElementTab()).append("</xform:dialog>")
					.append(ENTER);
			//引入对话框组件
			jsp.append("<script>Com_IncludeFile(\"dialog.js\",Com_Parameter.ContextPath+\"fssc/fee/fssc_fee_xform/extendJs/\",\"js\",true);</script>").append(ENTER);
			...
			jsp.append("<script>window.Designer_Control_Company_Event_List=window.Designer_Control_Company_Event_List||{};").append(ENTER)
			.append("window.Designer_Control_Company_Event_List['"+id+"']=[];").append(ENTER);
			//公司改变后执行事件
			jsp.append("function Designer_Control_Company_Changed(rtn){for(var i in window.Designer_Control_Company_Event_List['"+id+"']){window.Designer_Control_Company_Event_List['"+id+"'][i](rtn);}}</script>");
			//设置默认值
			if(props.containsKey("defaultValue")){
				jsp.append("<script>Com_AddEventListener(window,'load',function(){setDefaultValue(\""+id+"\",\""+name+"\",\"getDefaultCompany\")})</script>");
