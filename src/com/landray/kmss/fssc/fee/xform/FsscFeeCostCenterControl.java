...
public class FsscFeeCostCenterControl  implements ISysFormTemplateControl, FilterAction,ISysFormTemplateDetailsTableControl{
	...
	private void doParse(Node node, final StringBuilder jsp, String idPrefix)
		...
		if (node instanceof InputTag) {
			...
			jsp.append(ENTER).append(getElementTab()).append("<xformflag flagtype=\"costCenter\" flagid='"+id+"' _xform_type='costCenter' property='extendDataFormInfo.value("+id+")' id='_xform_extendDataFormInfo.value("+id+")'>");
			...
			setAttribute(jsp, "dialogJs", "Designer_DialogSelect(false,'eop_basedata_cost_center_selectCostCenter','extendDataFormInfo.value("+id+")','extendDataFormInfo.value("+name+")',null,{fdCompanyId:'"+props.getString("companyId")+"'});");
			jsp.append(">");
			...
			jsp.append("<script>").append(ENTER).append("window.Designer_Control_Cost_Center_List=window.Designer_Control_Cost_Center_List||[];").append(ENTER);
			jsp.append("window.Designer_Control_Cost_Center_List.push('"+id+"')").append(ENTER);
			jsp.append("window.Designer_Control_Company_Event_List['"+props.getString("companyId")+"'].push(Designer_Control_Cost_Center_Load);").append(ENTER).append("</script>");
			//设置默认值
			if(props.containsKey("defaultValue")){
				jsp.append("<script>Com_AddEventListener(window,'load',function(){setDefaultValue(\""+id+"\",\""+name+"\",\"getDefaultCostCenter\",{fdCompanyId:\""+props.getString("companyId")+"\"})})</script>");
