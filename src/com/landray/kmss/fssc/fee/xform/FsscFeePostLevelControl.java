...
public class FsscFeePostLevelControl  implements ISysFormTemplateControl, FilterAction,ISysFormTemplateDetailsTableControl{
	...
	private void doParse(Node node, final StringBuilder jsp, String idPrefix)
		...
		if (node instanceof InputTag) {
			...
			setAttribute(jsp, "dialogJs", "Designer_DialogSelect(false,'eop_basedata_level_fdLevel','extendDataFormInfo.value("+id+")','extendDataFormInfo.value("+name+")',null,{fdCompanyId:'"+props.getString("companyId")+"'});");
			jsp.append(">");
			...
			jsp.append("<script>").append(ENTER).append("window.Designer_Control_Post_Level_List=window.Designer_Control_Post_Level_List||[];").append(ENTER);
			jsp.append("window.Designer_Control_Post_Level_List.push('"+id+"')").append(ENTER);
			jsp.append("window.Designer_Control_Company_Event_List['"+props.getString("companyId")+"'].push(Designer_Control_Post_Level_Load);").append(ENTER).append("</script>");
		}
	}
