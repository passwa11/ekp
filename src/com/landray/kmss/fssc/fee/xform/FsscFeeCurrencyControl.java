...
public class FsscFeeCurrencyControl  implements ISysFormTemplateControl, FilterAction,ISysFormTemplateDetailsTableControl{
	...
	private void doParse(Node node, final StringBuilder jsp, String idPrefix)
		...
		if (node instanceof InputTag) {
			...
			jsp.append(ENTER).append(getElementTab()).append("<xformflag flagtype='currency' flagid='"+id+"' _xform_type='currency' property='extendDataFormInfo.value("+id+")' id='_xform_extendDataFormInfo.value("+id+")'>");
			jsp.append(ENTER).append("<xform:text showStatus=\"noShow\" property=\"extendDataFormInfo.value(").append(id).append("_cost_rate)\"/>");
			jsp.append(ENTER).append(getElementTab()).append("<xform:text showStatus=\"noShow\" property=\"extendDataFormInfo.value(").append(id).append("_budget_rate)\"/>");
			...
			setAttribute(jsp, "dialogJs", "Designer_DialogSelect(false,'eop_basedata_currency_selectCurrency','extendDataFormInfo.value("+id+")','extendDataFormInfo.value("+name+")',Designer_Control_Currency_Evnet_Changed,{fdCompanyId:'"+props.getString("companyId")+"'});");
			jsp.append(">");
			jsp.append(ENTER).append(getElementTab()).append(getElementTab());
			jsp.append(ENTER).append(getElementTab()).append("</xform:dialog>").append(ENTER);
			//引入对话框组件
			...
			jsp.append("<script>").append(ENTER).append("window.Designer_Control_Currency_List=window.Designer_Control_Currency_List||[];").append(ENTER);
			jsp.append("window.Designer_Control_Currency_List.push('"+id+"')").append(ENTER);
			jsp.append("window.Designer_Control_Company_Event_List['"+props.getString("companyId")+"'].push(Designer_Control_Currency_Load);").append(ENTER).append("</script>");
			//设置默认值
			if(props.containsKey("defaultValue")){
				jsp.append("<script>Com_AddEventListener(window,'load',function(){setDefaultValue(\""+id+"\",\""+name+"\",\"getDefaultCurrency\",{fdCompanyId:\""+props.getString("companyId")+"\"},['_cost_rate','_budget_rate'])})</script>");
				jsp.append("<script>Com_IncludeFile(\"defaultValue.js\",Com_Parameter.ContextPath+\"fssc/fee/fssc_fee_xform/extendJs/\",\"js\",true);</script>");
			}
