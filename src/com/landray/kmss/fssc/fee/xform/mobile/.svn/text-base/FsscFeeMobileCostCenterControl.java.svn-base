...
@ParseAnnotation(acceptType = "costCenter", removeDesignAttrs = false)
public class FsscFeeMobileCostCenterControl implements ParseHandler{
	...
	private void doParse(InputTag node, final JSPBuilder jsp)
		...
		if (node instanceof InputTag) {
			...
			jsp.html("</c:if>");
			jsp.html("<c:if test=\"${param.method!='view'}\"><input type='hidden' name='extendDataFormInfo.value("+name+")' value=''/></c:if>");
		}
	}
}
