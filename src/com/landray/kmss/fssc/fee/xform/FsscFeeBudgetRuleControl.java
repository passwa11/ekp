...
public class FsscFeeBudgetRuleControl  implements ISysFormTemplateControl, FilterAction,ISysFormTemplateDetailsTableControl{
	...
	private void doParse(Node node, final StringBuilder jsp, String idPrefix)
		...
		if (node instanceof InputTag) {
			...
			String fdBudgetStatus = id+"_budget_status";
			String fdBudgetInfo = id+"_budget_info";
			jsp.append(ENTER).append(getElementTab()).append("<xform:text showStatus=\"noShow\" property=\"extendDataFormInfo.value("+fdBudgetStatus+")\"/>");
			jsp.append(ENTER).append(getElementTab()).append("<xform:text showStatus=\"noShow\" property=\"extendDataFormInfo.value("+fdBudgetInfo+")\"/>");
			String showId = "budget_status";
			if("2".equals(props.get("fdMatchType"))){
				showId+="_!{index}";
			}
			if("1".equals(props.getString("fdShowType"))){
				jsp.append("<div id='"+showId+"' class='budget_container'></div>");
			}else{
				jsp.append("<div id='"+showId+"'></div>");
			}
			...
			jsp.append("<script>").append(ENTER).append("if(!window.Designer_Control_Budget_Rule_Props)window.Designer_Control_Budget_Rule_Props=JSON.parse(\"").append(input.getAttribute("props")).append("\".replace(/\'/g,'\"'))").append("</script>").append(ENTER);
