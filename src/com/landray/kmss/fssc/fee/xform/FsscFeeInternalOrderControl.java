...
public class FsscFeeInternalOrderControl implements ISysFormTemplateControl, FilterAction,ISysFormTemplateDetailsTableControl{
	...
	public boolean parse(Node node, Lexer lexer, StringBuilder jsp, List<ISysFormTemplateControl> controls)
		...
		if(node instanceof TagNode){
			...
			if(isType("internalOrder",tagNode)){
	...
	private void doParse(Node node, final StringBuilder jsp, String idPrefix)
		...
		if (node instanceof InputTag) {
			...
			tempJsp.append(ENTER).append(getElementTab()).append("<xformflag flagtype='innerOrder' flagid='"+id+"' _xform_type='internalOrder' property='extendDataFormInfo.value("+id+")' id='_xform_extendDataFormInfo.value("+id+")'>");
			...
			setAttribute(tempJsp, "dialogJs", "Designer_DialogSelect(false,'eop_basedata_inner_order_fdInnerOrder','extendDataFormInfo.value("+id+")','extendDataFormInfo.value("+name+")',null,{fdCompanyId:'"+props.getString("companyId")+"'});");
			tempJsp.append(">");
			...
			tempJsp.append("<script>").append(ENTER).append("window.Designer_Control_InternalOrder_List=window.Designer_Control_InternalOrder_List||[];").append(ENTER);
			tempJsp.append("window.Designer_Control_InternalOrder_List.push('"+id+"')").append(ENTER);
			tempJsp.append("window.Designer_Control_Company_Event_List['"+props.getString("companyId")+"'].push(Designer_Control_InternalOrder_Load);").append(ENTER).append("</script>");
			jsp.append(tempJsp).append("<script>Designer_Control_ReplaceTag('"+id+"')</script></xformflag>");
		}
	}
