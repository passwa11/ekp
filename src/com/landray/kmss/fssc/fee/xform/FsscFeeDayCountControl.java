 ...
 * 本币金额
 */
public class FsscFeeDayCountControl implements ISysFormTemplateControl, FilterAction,ISysFormTemplateDetailsTableControl{
	...
	private void doParse(Node node, final StringBuilder jsp, String idPrefix)
		...
		if (node instanceof InputTag) {
			...
			jsp.append(ENTER).append("<xformflag flagtype='dayCount' _xform_type='dayCount' flagid='"+id+"' property='extendDataFormInfo.value("+id+")' id='_xform_extendDataFormInfo.value("+id+")'>");
			...
			jsp.append("/>");
			jsp.append("<script>");
			jsp.append(ENTER).append("if(!window.Designer_Control_DayCount_Props)window.Designer_Control_DayCount_Props=JSON.parse(\"").append(input.getAttribute("props")).append("\".replace(/\'/g,'\"'));").append(ENTER);
