 ...
 *
 */
public class FsscBudgetCriteriaService implements IXMLDataBean{
	...
	public List getDataList(RequestContext requestInfo) throws Exception {
		...
		String where="";
		if("fdYear".equals(queryType)){//年份筛选条件
			hqlInfo = new HQLInfo();
			hqlInfo.setSelectBlock("distinct fsscBudgetData.fdYear");
			List<Object> years = fsscBudgetDataService.findValue(hqlInfo);
			for (int i = 0; i < years.size(); i++) {
				String fdYear = (String) years.get(i);
				if(StringUtil.isNotNull(fdYear)){
					Map<String, String> nodeMap = new HashMap<String, String>();
					nodeMap.put("value", fdYear.toString());
					nodeMap.put("text", fdYear.substring(1, 5));
					rtnList.add(nodeMap);
				}
			}
		}else if("fdCompanyGroup".equals(queryType)){
			hqlInfo = new HQLInfo();
			where="";
			...
			where=StringUtil.linkString(where, " and ", "eopBasedataProject.fdCompany.fdId=:fdCompanyId and eopBasedataProject.fdType='1' ");
			hqlInfo.setParameter("fdCompanyId", fdCompanyId);
			if(StringUtil.isNotNull(fdIsAvailable)){
