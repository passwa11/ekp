 ...
 */
public class SysAttendOutgoingResumeListener extends SysAttendListenerCommonImp
	...
	public void handleEvent(EventExecutionContext execution, String parameter) throws Exception {
		...
		if ((NodeInstanceUtils.ROUTE_TYPE_NORMAL.equals(routeType)
			...
			if (mainModel instanceof IExtendDataModel) {
				...
				// 同一流程不会重复操作
				if (list.isEmpty()) {
					List<SysAttendBusiness> busList = getBusinessList(params, mainModel);
					if (busList == null || busList.isEmpty()) {
						logger.warn("销外出流程数据配置不准确,忽略处理!parameter:" + parameter);
						throw new Exception("销外出流程数据配置不准确,忽略处理!parameter:" + parameter);
					}
					if (busList != null && !busList.isEmpty()) {
						// 1.保存流程表单数据
					...
					logger.warn("同个流程只执行一次,忽略此次操作!流程id:" + mainModel.getFdId() + ";parameter=" + parameter);
					throw new Exception("同个流程只执行一次,忽略此次操作!流程id:" + mainModel.getFdId() + ";parameter=" + parameter);
				}
			}
		}
	}
