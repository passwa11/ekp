 ...
 *
 */
public class SysAttendLeaveResumeListener extends SysAttendListenerCommonImp
		implements IEventListener, IEventMulticasterAware,
		ApplicationListener<Event_Common> {

	private static final Logger logger = org.slf4j.LoggerFactory.getLogger(SysAttendLeaveResumeListener.class);
				...
				if (list.isEmpty()) {
					...
					if (busList != null && !busList.isEmpty()) {
						...
						for (SysAttendBusiness bus : busList) {
							updateAttendBusiness(bus, execution.getMainModel());
						}
						// 2.重新统计
						restat(busList);
					}
	...
	private void updateAttendBusiness(SysAttendBusiness resume, IBaseModel model)
			throws Exception {
		...
		for (SysOrgPerson person : personList) {
			...
			for (SysAttendBusiness bus : userBusList) {
				...
				boolean isUpdate = updateUserBusiness(bus, fdBusStartTime,
						fdBusEndTime, dateMap, model);
				if (isUpdate) {
					// 保存销出差流程
					SysAttendBusiness userResume = this.cloneBusBiz(resume, model);
	 ...
	 * @throws Exception
	 */
	private boolean updateUserBusiness(SysAttendBusiness bus,
			Date startTime, Date endTime, Map<String, Object> dateMap, IBaseModel model)
			throws Exception {
		...
		if (fdBusEndTime.getTime() > endTime.getTime()
			...
			SysAttendBusiness newBus = cloneBusBiz(bus,model);
			bus.setFdBusEndTime(startTime);
			getSysAttendBusinessService().update(bus);
			endTime.setSeconds(endTime.getSeconds() + 1);
			newBus.setFdBusStartTime(endTime);
	...
	private SysAttendBusiness cloneBusBiz(SysAttendBusiness bus, IBaseModel model) {
		if (bus == null) {
			return null;
		}
		...
		sysAttendBusiness.setDocUrl(AttendUtil.getDictUrl(model,bus.getFdProcessId()));
		List<SysOrgElement> newTargets = new ArrayList<SysOrgElement>();
		...
		if(!isNeedUpdate)
		{
			...
			for(Map<String, Object> signTimeConfiguration : signTimeConfigurations){
				...
				if(isSameWorkTime && isOverTimeType(signTimeConfiguration)){
					...
					Date fdBusEndTime_ = AttendUtil.joinYMDandHMS(fdBusEndTime, sysAttendCategory.getFdStartTime());
	...
	private void restat(List<SysAttendBusiness> busList) {
		try {
			final List<String> statOrgs = getStatOrgList(busList);
			final List<Date> statDates = getStatDateList(busList);
			...
			multicaster.attatchEvent(
					...
					new IEventCallBack() {
						...
						public void execute(ApplicationEvent arg0)
								throws Throwable {
							getSysAttendStatJobService().stat(statOrgs, statDates);
						}
					});
		} catch (Exception e) {
			e.printStackTrace();
	...
	private List<String> getStatOrgList(List<SysAttendBusiness> busList)
			throws Exception {
	...
	private List<SysAttendBusiness> getBusinessList(JSONObject params,
		...
		try {
			...
			if ("Date[]".equals(startFieldType)
				...
				if ("Date".equals(endFieldType)
					...
					if ("Date".equals(endFieldType)) {
						fdBusEndTime = getNextDate(fdBusEndTime);
					}
