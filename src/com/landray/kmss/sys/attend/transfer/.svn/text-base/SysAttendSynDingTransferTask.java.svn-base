...
public class SysAttendSynDingTransferTask
		implements ISysAdminTransferChecker, ISysAdminTransferTask {
	private final Logger logger = org.slf4j.LoggerFactory.getLogger(SysAttendSynDingTransferTask.class);
	...
	public SysAdminTransferResult
		...
		try {
			...
			String sql = "update sys_attend_syn_ding set doc_creator_id=fd_person_id where doc_creator_id is null";
	...
	public SysAdminTransferCheckResult
		...
		try {
			...
			ISysAttendSynDingService sysAttendSynDingService = (ISysAttendSynDingService) SpringBeanUtil
