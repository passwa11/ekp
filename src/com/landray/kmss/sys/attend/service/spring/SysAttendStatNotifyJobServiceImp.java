...
import com.landray.kmss.sys.attend.service.ISysAttendStatNotifyJobService;
...
public class SysAttendStatNotifyJobServiceImp implements ISysAttendStatNotifyJobService {
				...
				for (int i = 0; i < groupLists.size(); i++) {
					List<String> tmpOrgList = groupLists.get(i);
					 HQLInfo hqlInfo = new HQLInfo();
				        ...
				        hqlInfo.setParameter("fdId", tmpOrgList);
				        hqlInfo.setParameter("fdStartTime", beginTime);
				        hqlInfo.setParameter("fdEndTime", endTime);
//				        List<SysAttendStat> stats = getSysAttendStatService().findList(hqlInfo);
					...
					String where = " fd_category_his_id=:fdCategoryId and fd_work_id=:fdWorkId and fd_work_type=:fdWorkType "
							...
							+ " and fd_status=2 and (fd_state is null or fd_state=0)";
					...
					for (int j = 0; j < signedList.size(); j++) {
						...
						HQLInfo hQLInfo = new HQLInfo();
						hQLInfo.setWhereBlock("docCreator=:docCreator and fdDate>=:beginTime and fdDate<:endTime");
						hQLInfo.setParameter("docCreator", main.getDocCreator());
						hQLInfo.setParameter("beginTime", beginTime);
						hQLInfo.setParameter("endTime", endTime);
						SysAttendStat sysAttendStat = (SysAttendStat) getSysAttendStatService().findFirstOne(hQLInfo);
						if(sysAttendStat.getFdMonthLateMinNum()>=category.getFdLateTotalTime()||sysAttendStat.getFdMonthLateNum()>=category.getFdLateNumTotalTime())
	...
	public void sendResultNotify(SysAttendMain main, int fdWorkType,
		...
		try {
			...
			subject = subject + ResourceUtil.getString(
					"sys-attend:sysAttendCategory.notify.remind.late.subject");
