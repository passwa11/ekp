 ...
 *
 */
public class SysAttendManagerFilter implements IAuthenticationFilter {
		...
		if (element == null) {
			...
			if (StringUtil.isNotNull(field)) {
				...
				if (StringUtil.isNotNull(cateType)
					...
					cateIds = sysAttendCategoryService.findCategorysByLeader(
							element,
					...
					cateIds = sysAttendCategoryService.findCategorysByLeader(
							element, 1);
					cateIds.addAll(
							sysAttendCategoryService.findCategorysByLeader(
									element, 2));
				}
