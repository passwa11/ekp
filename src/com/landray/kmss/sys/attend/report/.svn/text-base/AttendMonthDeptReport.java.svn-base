 ...
 */
public class AttendMonthDeptReport {
	private static final Log log = LogFactory.getLog(AttendMonthDeptReport.class);
...
//				String sql1222 = "select * from (select * from sys_attend_stat_month where fd_month>='"+eryue_1+"' and fd_month<='"+eryue_31+"') a left join (select * from sys_org_element where fd_hierarchy_id like '%"+dep_id+"%')b on a.doc_creator_id=b.fd_id ";
				...
				try {
					...
					person_list2 = HrCurrencyParams.getListBySql(sql56);
...
//				}
				...
				time1=time/(person_list2.size()+person_list3.size());
				else time1=0;
...
//				time3+=time1;
				...
				map.put("hourTxt"+mn, hourTxt);
				map.put("personCount"+mn, person_list2.size()+person_list3.size());
				map.put("avg"+mn, hourTxt1);
				}
				//人数小计
