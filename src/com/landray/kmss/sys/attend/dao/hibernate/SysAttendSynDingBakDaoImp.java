...
public class SysAttendSynDingBakDaoImp extends BaseDaoImp
		implements ISysAttendSynDingBakDao {
	@Override
	...
	private NativeQuery createSqlQuery(SQLInfo sqlInfo) {
		NativeQuery query = getHibernateSession()
				.createNativeQuery(buildQuerySQL(sqlInfo));
	...
	public SysAttendSynDingBak findByPrimaryKey(String id,
		...
		SysAttendSynDingBak rtnVal = null;
		if (StringUtil.isNotNull(id) && !params.isEmpty()) {
			...
			if (entity != null && StringUtil.isNotNull(tableName)) {
				...
				if (!list.isEmpty()) {
					rtnVal = (SysAttendSynDingBak) list.get(0);
				}
			}
		}
	...
	public boolean isExist(SQLInfo sqlInfo) throws Exception {
		TimeCounter.logCurrentTime("Dao-isExist", true, getClass());
		sqlInfo.setGettingCount(true);
		NativeQuery q = createSqlQuery(sqlInfo);
		int total = ((Number) q.uniqueResult()).intValue();
		TimeCounter.logCurrentTime("Dao-isExist", false, getClass());
		return total > 0;
	}
}
