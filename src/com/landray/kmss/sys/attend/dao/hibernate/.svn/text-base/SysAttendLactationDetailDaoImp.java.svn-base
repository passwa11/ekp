 ...
 */
public class SysAttendLactationDetailDaoImp extends BaseDaoImp
		implements ISysAttendLactationDetailDao {
	...
	@Override
	public SysAttendLactationDetail findByPrimaryKey(String id,
											 Map<String, Object> params) throws Exception {
		TimeCounter.logCurrentTime("Dao-findByPrimaryKey", true, getClass());
		SysAttendLactationDetail rtnVal = null;
		if (StringUtil.isNotNull(id) && !params.isEmpty()) {
			...
			if (entity != null && StringUtil.isNotNull(tableName)) {
				...
				if (!list.isEmpty()) {
					rtnVal = (SysAttendLactationDetail) list.get(0);
				}
			}
		}
