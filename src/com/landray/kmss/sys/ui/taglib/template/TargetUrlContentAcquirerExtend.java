...
/**
 * 扩展一个访问url载入内容辅助类
 *提供给审批意见控件使用，因为审批意见用异步加载，
 * 没办法拿到pageContext对象，所以用request，response构造
 * @author 洪健
 *
 */
public class TargetUrlContentAcquirerExtend {
    ...
    public TargetUrlContentAcquirerExtend(String targetUrl) {
    ...
    public static String coverUrl(String targetUrl, HttpServletRequest request) {
        if (!targetUrl.startsWith("/")) {
            String sp =request
                    .getServletPath();
            targetUrl = sp.substring(0, sp.lastIndexOf('/')) + '/' + targetUrl;
    ...
    public TargetUrlContentAcquirerExtend(String targetUrl, HttpServletRequest request,HttpServletResponse response,
                                          String charEncoding) {
        this.targetUrl = targetUrl;
    ...
    public String acquireString() throws JspException {
        ...
        //此response可能没有值，所以需要pageContext来获取
        //当打印页面的时候，因为审批意见走的是标签的形式，所以response需要从pageContext获取
    ...
    protected class ImportResponseWrapper extends HttpServletResponseWrapper {
        ...
        public PrintWriter getWriter() throws IOException {
            ...
            isWriterUsed = true;
            return new PrintWriterWrapper(sw, this.getResponse().getWriter());
        }
