...
public class SysOrgPersonEmailChecker implements ISysAdminTransferChecker {
	...
	public boolean isRuned() {
		...
		try {
			...
			ps = conn.prepareStatement("select count(1) from sys_org_person where fd_email like ?");
			ps.setString(1, "{密文}%");
			rs = ps.executeQuery();
			int count = 0;
			if (rs.next()) {
