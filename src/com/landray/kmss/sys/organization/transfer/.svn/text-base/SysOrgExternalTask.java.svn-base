 ...
 * 
 */
public class SysOrgExternalTask implements ISysAdminTransferTask {
			...
			if (list != null && list.size() > 0) {
				...
				if (sysAdminTransferTask.getFdStatus() != 1) {
					...
					try {
						...
						ps = conn.prepareStatement("UPDATE sys_org_element SET fd_is_external = 0 WHERE fd_is_external IS NULL");
						ps.executeUpdate();
					} catch (Exception e) {
