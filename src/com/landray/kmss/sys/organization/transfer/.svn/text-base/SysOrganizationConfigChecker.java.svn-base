 ...
 * 
 */
public class SysOrganizationConfigChecker implements ISysAdminTransferChecker {
	...
	public boolean isRuned() {
		...
		try {
			...
			ps = conn.prepareStatement("select count(1) from sys_app_config where fd_key = ? or fd_key = ?");
			ps.setString(1, "com.landray.kmss.sys.organization.model.SysOrgRelationConfig");
			ps.setString(2, "com.landray.kmss.sys.organization.model.SysOrgSearchConfig");
			rs = ps.executeQuery();
			int count = 0;
			if (rs.next()) {
                count = rs.getInt(1);
