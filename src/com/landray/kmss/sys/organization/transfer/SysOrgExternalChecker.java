 ...
 * 外部组织
 * 
 * @author 潘永辉 2020-06-16
 * 
 */
public class SysOrgExternalChecker implements ISysAdminTransferChecker {
	...
	public boolean isRuned() {
		...
		try {
			...
			ps = conn.prepareStatement("SELECT COUNT(fd_id) FROM sys_org_element WHERE fd_is_external IS NULL");
			rs = ps.executeQuery();
			if (rs.next()) {
