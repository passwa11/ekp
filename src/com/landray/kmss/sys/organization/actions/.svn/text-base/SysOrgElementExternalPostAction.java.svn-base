 ...
 */
public class SysOrgElementExternalPostAction extends ExtendAction {
	...
	@Override
	protected ISysOrgElementExternalPostService getServiceImp(HttpServletRequest request) {
		if (sysOrgElementExternalPostService == null) {
			sysOrgElementExternalPostService = (ISysOrgElementExternalPostService) getBean("sysOrgElementExternalPostService");
		}
		return sysOrgElementExternalPostService;
	}
	...
	protected void changeDeptFindPageHQLInfo(HttpServletRequest request, HQLInfo hqlInfo) throws Exception {
		...
		if (StringUtil.isNotNull(fdName)) { // 层级查询子部门，当fdName不为空时显示子部门及其下级部门
			...
			// 部门名称查询
			whereBlockDept = StringUtil.linkString(whereBlockDept, " or ", "sysOrgPost.fdName like :fdName ");
			if (StringUtil.isNotNull(currentLocaleCountry)) {
				whereBlockDept = StringUtil.linkString(whereBlockDept, " or ",
						"sysOrgPost.fdName" + currentLocaleCountry + " like :fdName ");
			}
			hqlInfo.setParameter("fdName", "%" + fdName + "%");
			// 编号查询
			whereBlockDept = StringUtil.linkString(whereBlockDept, " or ", "sysOrgPost.fdNo like :fdName ");
			hqlInfo.setParameter("fdName", "%" + fdName + "%");
			// 名称拼音查询
			whereBlockDept = StringUtil.linkString(whereBlockDept, " or ", "sysOrgPost.fdNamePinYin like :pinyin ");
		...
		whereBlock = StringUtil.linkString(whereBlock, " and ", "sysOrgPost.fdOrgType = :orgType ");
		hqlInfo.setParameter("orgType", SysOrgConstant.ORG_TYPE_POST);
	...
	protected ActionForm createNewForm(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		SysOrgPostForm postForm = (SysOrgPostForm) super.createNewForm(mapping, form, request, response);
		String parent = request.getParameter("parent");
		if (StringUtil.isNotNull(parent)) {
			SysOrgDept dept = (SysOrgDept) getSysOrgDeptService().findByPrimaryKey(parent);
			postForm.setFdParentId(parent);
			postForm.setFdParentName(dept.getFdName());
		}
		return postForm;
	}
		...
		if (messages.hasError()) {
			...
			KmssReturnPage.getInstance(request).addMessages(messages)
					.addButton("button.back",
							"/sys/organization/sys_org_element_external/sysOrgElementExternalPost.do?method=view&fdId="
	...
	protected void loadActionForm(ActionMapping mapping, ActionForm form,
		...
		SysOrgElement sysOrgElement = (SysOrgElement) getSysOrgElementService().findByPrimaryKey(postForm.getFdId());
