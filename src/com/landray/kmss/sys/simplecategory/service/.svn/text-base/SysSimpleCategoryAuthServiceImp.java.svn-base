...
/**
 * 返回类别的可维护人员和可使用人员，可供给其它业务类别的service继承，也可以直接使用
 * 
 * @author wubing 2009－07－22
 */
public class SysSimpleCategoryAuthServiceImp implements
		ISysSimpleCategoryAuthService {
	/**
	 * 获取域模型对应的服务类
	 * 
	 * @return
	...
	public List getAllEditors(String id) throws Exception {
		...
		while (categoryModel.getFdIsinheritMaintainer() != null
	...
	public List getAllReaders(String id) throws Exception {
		...
		while (categoryModel.getFdIsinheritUser() != null
				&& categoryModel.getFdIsinheritUser().booleanValue()
				&& categoryModel.getFdParent() != null) {
			categoryModel = (ISysSimpleCategoryModel) categoryModel
					.getFdParent();
			ArrayUtil.concatTwoList(categoryModel.getAuthReaders(), allReaders);
			ArrayUtil.concatTwoList(categoryModel.getAuthEditors(), allReaders);
		}
		return allReaders;
	}
	...
	public boolean isEditors(String id) throws Exception {
		...
		while (!editor && categoryModel.getFdIsinheritMaintainer() != null
				&& categoryModel.getFdIsinheritMaintainer().booleanValue()
				&& categoryModel.getFdParent() != null) {
			categoryModel = (ISysSimpleCategoryModel) categoryModel
					.getFdParent();
	...
	public boolean isReaders(String id) throws Exception {
		...
		while (categoryModel.getFdIsinheritMaintainer() != null
			...
			categoryModel = (ISysSimpleCategoryModel) categoryModel
					.getFdParent();
			reader = UserUtil.checkUserModels(categoryModel.getAuthEditors());
			if (reader) {
                break;
            }
		}
		while (categoryModel.getFdIsinheritUser() != null
			...
			categoryModel = (ISysSimpleCategoryModel) categoryModel
					.getFdParent();
			reader = UserUtil.checkUserModels(categoryModel.getAuthReaders());
			if (reader) {
                break;
            }
		}
