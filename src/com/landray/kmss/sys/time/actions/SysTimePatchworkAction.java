 ...
 * @author 易荣烽
 */
public class SysTimePatchworkAction extends ExtendAction {
	protected ISysTimePatchworkService sysTimePatchworkService;

	@Override
	protected IBaseService getServiceImp(HttpServletRequest request) {
		if (sysTimePatchworkService == null) {
			sysTimePatchworkService = (ISysTimePatchworkService) getBean("sysTimePatchworkService");
		}
		return sysTimePatchworkService;
	}
	...
	protected void loadActionForm(ActionMapping mapping, ActionForm form,
		...
		if (!StringUtil.isNull(id)) {
			IBaseModel model = getServiceImp(request).findByPrimaryKey(id,
					null, true);
			SysTimePatchwork patchWork = (SysTimePatchwork) model;
			common = patchWork.getSysTimeCommonTime();
	...
	protected void changeFindPageHQLInfo(HttpServletRequest request,
		...
		if (!StringUtil.isNull(sysTimeAreaId)) {
			whereBlock += " and sysTimePatchwork.sysTimeArea.fdId = :sysTimeAreaId";
	...
	protected ActionForm createNewForm(ActionMapping mapping, ActionForm form,
									   HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		SysTimePatchworkForm sysTimePatchworkForm = (SysTimePatchworkForm) form;
		sysTimePatchworkForm.reset(mapping, request);
		sysTimePatchworkForm
				.setSysTimeAreaId(request.getParameter("sysTimeAreaId"));
		
		sysTimePatchworkForm
				.setDocCreateTime(DateUtil.convertDateToString(new Date(),
						DateUtil.TYPE_DATETIME, request.getLocale()));
		sysTimePatchworkForm.setDocCreatorId(UserUtil.getUser().getFdId());
		sysTimePatchworkForm.setDocCreatorName(UserUtil.getUser().getFdName());
		return sysTimePatchworkForm;
	}
	@Override
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			SysTimePatchworkForm patchForm = (SysTimePatchworkForm) form;
			SysTimePatchwork patchWork = (SysTimePatchwork) getServiceImp(request)
					.convertFormToModel(patchForm, null,
							new RequestContext(request));
			ISysTimeCommonTimeService commonTimeService = (ISysTimeCommonTimeService) SpringBeanUtil
					.getBean("sysTimeCommonTimeService");
			if ("1".equals(patchForm.getTimeType())) {
				SysTimeCommonTime commonTimes = (SysTimeCommonTime) commonTimeService
						.findByPrimaryKey(patchForm.getSysTimeCommonId());
				patchWork.setFdPatchWorkColor(commonTimes.getFdWorkTimeColor());
			}
			else {
				SysTimeCommonTime common = new SysTimeCommonTime();
				common.setType("2");
				common.setFdName(patchForm.getFdName());
				common.setFdWorkTimeColor(patchForm.getFdPatchWorkColor());
				common.setFdRestStartTime(DateUtil.convertStringToDate(
						patchForm.getSysTimeCommonTimeForm()
								.getFdRestStartTime(),
						DateUtil.TYPE_TIME, null));
				common.setFdRestEndTime(DateUtil.convertStringToDate(
						patchForm.getSysTimeCommonTimeForm().getFdRestEndTime(),
						DateUtil.TYPE_TIME, null));
				common.setFdRestStartType(patchForm.getSysTimeCommonTimeForm().getFdRestStartType());
				common.setFdRestEndType(patchForm.getSysTimeCommonTimeForm().getFdRestEndType());

				common.setFdWorkHour(
						patchForm.getSysTimeCommonTimeForm().getFdWorkHour());
				List<SysTimePatchworkTimeForm> twts = patchForm
						.getSysTimePatchworkTimeFormList();
				List<SysTimeWorkDetail> detais = new ArrayList<>();
				for (SysTimePatchworkTimeForm twt : twts) {
					SysTimeWorkDetail newDetail = new SysTimeWorkDetail();
				...
				common.setFdTotalDay(patchForm.getFdTotalDay());
				common.setSysTimeWorkDetails(detais);
				commonTimeService.add(common);
				patchWork.setSysTimeCommonTime(common);
			}
			getServiceImp(request).add(patchWork);
		} catch (Exception e) {
			messages.addError(e);
		}
