 ...
 * 失败内存队列实现
 */
public class FailConvertQueueImpl implements ConvertQueue {

    private static final Logger logger = LoggerFactory.getLogger(FailConvertQueueImpl.class);
    ...
    public  void put(SysFileConvertQueue sysFileConvertQueue, String convertType) throws Exception {
        ...
        if(sysFileConvertQueue.getFdConvertNumber() > 2) {
            logger.warn("转换失败队列的转换次数大于2次，不加入队列。队列已经存在于【其它失败队列】。");
            return;
        }
        if (getUniqueQueue(convertType).contains(sysFileConvertQueue.getFdId())) {
            logger.warn("【{}】已经存在于失败内存队列，无须重复添加。", sysFileConvertQueue.getFdId());
            return;
        }
        ...
        if(queueSize >= ConvertQueueConstant.QUEUE_MAX_SIZE) {
            logger.warn("转换队列已满,当前队列数量:{}",queueSize);
            return;
        }
    ...
    public ArrayBlockingQueue<SysFileConvertQueue> getQueue(String convertType) {
        ...
        if(queue == null) {
            logger.warn("获取【{}】中队列为空，创建新队列.", convertType);
