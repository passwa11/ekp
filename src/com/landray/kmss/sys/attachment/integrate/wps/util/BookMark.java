...
public class BookMark {
	...
	public static final String STYLE_NODE_ATTR_NAME = "w:rsidRPr";
	public static final String STYLE_NODE_ATTR_NAME_1 = "w:rsidR";
	...
	/**
	 * 标签是否处于修订状态，0表示正常的，1表示删除，2表示插入
	 */
	private String _bookTrackFlag = "";
	 ...
	 * @param para
	 */
	public BookMark(CTBookmark ctBookmark, XWPFParagraph para,String trackFlag) {
		this._ctBookmark = ctBookmark;
		this._para = para;
		this._bookmarkName = ctBookmark.getName();
		this._tableCell = null;
		this._isCell = false;
		this._bookTrackFlag = trackFlag;
	}
	...
	public BookMark(CTBookmark ctBookmark, XWPFParagraph para,
			XWPFTableCell tableCell,String trackFlag) {
		this(ctBookmark, para, trackFlag);
	...
	public CTBookmark get_ctBookmark() {
		return _ctBookmark;
	}
	 ...
	 * @return
	 */
	public String get_bookTrackFlag() {
		return _bookTrackFlag;
	}
	...
	public void insertTextAtBookMark(String bookmarkValue, int where) {
		...
		if (this._isCell) {
			...
			String[] multine = bookmarkValue.split("\n");
			if (multine.length <= 1) {
				run.setText(bookmarkValue);
			} else {
				run.setText(multine[0]);
			}
			...
			if (multine.length > 1) {
				//保存书签结束符后面的所有后续节点
				Node textStyleNode = null;
				Node paraNode = this._ctBookmark.getDomNode();
				List<Node> saveNodeList = new ArrayList<Node>();
				boolean saveNode = false;
				while (true) {
					if (paraNode.getNodeName().contains(BookMark.BOOKMARK_END_TAG)) {
						saveNode = true;
					}
					paraNode = paraNode.getNextSibling();
					if (paraNode == null) {
						break;
					}
					if (saveNode) {
						saveNodeList.add(paraNode);
					} else {
						//获取书签的文本样式节点
						if (BookMark.RUN_NODE_NAME.equals(paraNode.getNodeName())
								|| "r".equals(paraNode.getNodeName())) {
							textStyleNode = this.getStyleNode(paraNode);
							paraNode = paraNode.getNextSibling();
						}
					}
				}
				
				for (int p = 1; p < multine.length; p++) {
					//存在分段则新建一个run
					XWPFRun newrun = this._para.createRun();
					//设置文本样式
					if (textStyleNode != null) {
						newrun.getCTR().getDomNode().insertBefore(
								textStyleNode.cloneNode(true),
								newrun.getCTR().getDomNode().getFirstChild());
					}
	    			//换行
	    			newrun.addBreak();
	    			newrun.setText(multine[p]);
				}
				
				//移除书签结尾后续节点
				List<Node> appendNodeList = new ArrayList<Node>();
				for (Node tempNode : saveNodeList) {
					Node newNode = tempNode.cloneNode(true);
					this._para.getCTP().getDomNode().removeChild(tempNode);
					appendNodeList.add(newNode);
				}
				
				//调整书签的结束符
				moveBookMarkEnd(appendNodeList);
			}
		}
	}
	...
	/**
	 * 移动书签的结束符
	 * @param appendNodeList
	 */
	private void moveBookMarkEnd(List<Node> appendNodeList) {
		//移动书签的结束符
		//起始节点为w:bookmarkStart
		Node nextNode = this._ctBookmark.getDomNode();
		//获取书签起始节点的w:id属性,因为文档默认包含_GoBack书签，影响书签结束标识判断
		String startBookMarkFlag = "";
		Node w_id = nextNode.getAttributes().getNamedItem(BookMark.BOOKMARK_ID_ATTR_NAME);
		if (w_id != null) {
			startBookMarkFlag = w_id.getNodeValue();
		}
		//获取下一个节点
		nextNode = nextNode.getNextSibling();
		//如果下一个节点是w:bookmarkEnd，且w:id相同，则为光标书签
		if (nextNode == null) {
			return;
		}
		if (nextNode.getNodeName().contains(BookMark.BOOKMARK_END_TAG)) {
			Node end_w_id = nextNode.getAttributes().getNamedItem(BookMark.BOOKMARK_ID_ATTR_NAME);
			if (end_w_id != null) {
				String endBookMarkFlag = end_w_id.getNodeValue();
				if (endBookMarkFlag.equals(startBookMarkFlag)) {
					return;
				}
			}
		}
		//书签结束符
		Node endBookMark = null;
		//书签结束符是否有后续文本
		...
		while (true) {
			...
			if (!nextNode.getNodeName().contains(BookMark.RUN_NODE_NAME)) {
				//非文本节点，退出
				break;
			}
		}
		
		if (hasNextRun && endBookMark != null) {
			Node newEndNode = endBookMark.cloneNode(true);
			this._para.getCTP().getDomNode().removeChild(endBookMark);
			this._para.getCTP().getDomNode().appendChild(newEndNode);
		}
		
		//增加后续节点
		for (Node addNode :appendNodeList) {
			this._para.getCTP().getDomNode().appendChild(addNode);
		}
				...
				try {
					bookmarkEndID = Integer.parseInt(
							nextNode.getAttributes().getNamedItem(
									BookMark.BOOKMARK_ID_ATTR_NAME)
									.getNodeValue());
	...
	private void replaceBookmark(XWPFRun run) {
		...
		for (int i = 0; i < nodeStack.size(); i++) {
			Node curNode = nodeStack.elementAt(i);
			if ("w:ins".equals(curNode.getNodeName())) {
				//修订模式下的书签节点
				Node childNode = curNode.getFirstChild();
				if (childNode != null && BookMark.RUN_NODE_NAME.equals(childNode.getNodeName())) {
					styleNode = this.getStyleNode(childNode);
					if (styleNode != null) {
						run.getCTR().getDomNode().insertBefore(
								styleNode.cloneNode(true),
			...
			} else if (BookMark.RUN_NODE_NAME.equals(curNode.getNodeName())) {
				styleNode = this.getStyleNode(curNode);
				if (styleNode != null) {
					run.getCTR().getDomNode().insertBefore(
							styleNode.cloneNode(true),
							run.getCTR().getDomNode().getFirstChild());
		...
		for (int i = 1; i < nodeStack.size(); i++) {
			...
			if (toDelete.getNodeName().contains(BookMark.BOOKMARK_START_TAG)) {
				bookmarkStartID = Integer.parseInt(
						toDelete.getAttributes()
								.getNamedItem(BookMark.BOOKMARK_ID_ATTR_NAME)
								.getNodeValue());
				inNestedBookmark = true;
			} else if (toDelete.getNodeName()
					.contains(BookMark.BOOKMARK_END_TAG)) {
				bookmarkEndID = Integer.parseInt(
						toDelete.getAttributes()
								.getNamedItem(BookMark.BOOKMARK_ID_ATTR_NAME)
								.getNodeValue());
	...
	private Node getStyleNode(Node parentNode) {
		...
		if (parentNode != null) {
			...
			if ((parentNode.getNodeName()
					.equalsIgnoreCase(BookMark.RUN_NODE_NAME) || "r".equals(parentNode.getNodeName()))
				...
				if ("w:rPr".equals(childNode.getNodeName())) {
					...
					while ((childNode = childNode.getNextSibling()) != null) {
						if (childNode.getNodeName()
								.equals(BookMark.STYLE_NODE_NAME)) {
							styleNode = childNode;
...
public XWPFTableCell getContainerTableCell() {
