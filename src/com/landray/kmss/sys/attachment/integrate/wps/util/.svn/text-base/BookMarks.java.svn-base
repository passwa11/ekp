...
public class BookMarks {
	 ...
	 *
	 * @param document
	 *            Word OOXML document instance.
	 */
	public BookMarks(XWPFDocument document) {

		// 初始化标签缓存
		this._bookmarks = new LinkedHashMap<String, BookMark>();
	...
	private void procParaList(XWPFTableCell cell) {
		...
		for (XWPFParagraph paragraph : paragraphList) {
			// 得到段落中的标签标记
			List<CTBookmark> bookmarkList = paragraph.getCTP()
					.getBookmarkStartList();
			for (CTBookmark bookmark : bookmarkList) {
				this._bookmarks.put(bookmark.getName(),
						new BookMark(bookmark, paragraph, cell,"0"));
			}
			// 修订模式下获取标签标记
			List<CTRunTrackChange> delList = paragraph.getCTP().getDelList();
			for (CTRunTrackChange del : delList) {
				List<CTBookmark> trackBookMarkStartList = del.getBookmarkStartList();
				for (CTBookmark bookmark : trackBookMarkStartList) {
					this._bookmarks.put(bookmark.getName(),
							new BookMark(bookmark, paragraph, cell, "1"));
				}
			}
			

			List<CTRunTrackChange> insList = paragraph.getCTP().getInsList();
			for (CTRunTrackChange ins : insList) {
				List<CTBookmark> trackBookMarkStartList = ins.getBookmarkStartList();
				for (CTBookmark bookmark : trackBookMarkStartList) {
					this._bookmarks.put(bookmark.getName(),
							new BookMark(bookmark, paragraph, cell,"2"));
				}
			}
		}
	}
			...
			for (CTBookmark bookmark : bookmarkList) {
				...
				if (attributes != null) {
					...
					if (colFirstNode != null && colLastNode != null) {
						...
						firstColIndex = Integer
								.parseInt(colFirstNode.getNodeValue());
						lastColIndex = Integer
								.parseInt(colLastNode.getNodeValue());
						...
						if (firstColIndex == lastColIndex) {
							this._bookmarks.put(bookmark.getName(),
									new BookMark(bookmark, paragraph,
											tableRow.getCell(firstColIndex),"0"));
						} else {
							System.out.println(
									"This bookmark " + bookmark.getName() +
	...
	private void procParaList(List<XWPFParagraph> paragraphList) {
		...
		for (XWPFParagraph paragraph : paragraphList) {
			...
			for (CTBookmark bookmark : bookmarkList) {
				this._bookmarks.put(bookmark.getName(),
						new BookMark(bookmark, paragraph, "0"));
			}
			// 修订模式下获取标签标记
			List<CTRunTrackChange> delList = paragraph.getCTP()
					.getDelList();
			// 循环加入标签
			for (CTRunTrackChange del : delList) {
				List<CTBookmark> trackBookMarkStartList = del.getBookmarkStartList();
				for (CTBookmark bookmark : trackBookMarkStartList) {
					this._bookmarks.put(bookmark.getName(),
							new BookMark(bookmark, paragraph,"1"));
				}
			}
			
			
			List<CTRunTrackChange> insList = paragraph.getCTP()
					.getInsList();

			// 循环加入标签
			for (CTRunTrackChange inl : insList) {
				List<CTBookmark> trackBookMarkStartList = inl.getBookmarkStartList();
				for (CTBookmark bookmark : trackBookMarkStartList) {
					this._bookmarks.put(bookmark.getName(),
							new BookMark(bookmark, paragraph,"2"));
				}
			}
