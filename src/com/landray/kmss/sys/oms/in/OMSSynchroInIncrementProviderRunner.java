...
public class OMSSynchroInIncrementProviderRunner implements
		IOMSSynchroInIncrementProviderRunner, SysOrgConstant, SysOMSConstant {
	...
	private static final Logger logger = org.slf4j.LoggerFactory.getLogger(OMSSynchroInIncrementProviderRunner.class);

	private IOMSSynchroInIncrementProvider provider = null;
	...
	private void flagDeleted() throws Exception {
		...
		try {
			...
			ps = conn
					.prepareStatement(
							"update sys_org_element set fd_flag_deleted = ? where fd_import_info like '"
							+ provider.getKey() + "%'");
	...
	@Override
	public void synchro(IOMSSynchroInIncrementProvider provider,
		...
		try {
			...
			initMappings(ekpIds, importInfos);

			importAddedRecordBaseAttributes(
					provider.getAddedRecordBaseAttributes(), ekpIds,
					importInfos);
			importDeletedRecordBaseAttributes(
					provider.getDeletedRecordBaseAttributes(), ekpIds,
					importInfos);
	...
	private void initMappings(Set<String> ekpIds,
			Map<String, String> importInfos)
			throws Exception {
		DataSource dataSource = (DataSource) SpringBeanUtil
				.getBean("dataSource");
		Connection conn = null;
		...
		try {
			...
			psselect_all = conn
					.prepareStatement(
							"select fd_id,fd_import_info,fd_org_type from sys_org_element where fd_is_external = 0");
	...
	private void importAddedRecordBaseAttributes(IOMSResultSet rs,
			Set<String> ekpIds, Map<String, String> importInfos)
			throws Exception {
		logger.info("开始新增记录");
		DataSource dataSource = (DataSource) SpringBeanUtil
				.getBean("dataSource");
			...
			while (rs.next()) {
				...
				if (baseAtts != null) {
					...
					if (StringUtil.isNotNull(fd_id)) {
						...
						ekpIds.add(fd_id);
						importInfos.put(baseAtts.getElementUUID()
								+ "---" + baseAtts.getElementLdapDN() + "#"
	...
	private void importDeletedRecordBaseAttributes(IOMSResultSet rs,
			Set<String> ekpIds, Map<String, String> importInfos)
			throws Exception {
		logger.info("开始删除的记录");
		DataSource dataSource = (DataSource) SpringBeanUtil
		...
		try {
			...
			psupdate = conn
					.prepareStatement(
							"update sys_org_element set fd_flag_deleted=? where fd_id=? and fd_is_available = ?");
			...
			while (rs.next()) {
				...
				if (baseAtts != null) {
					...
					if (StringUtil.isNotNull(fd_id)) {
						logger.debug("需删除的记录：" + baseAtts.getElementUUID()
								+ "---" + baseAtts.getElementLdapDN());
			...
			psupdate.executeBatch();
			conn.commit();
			jobContext.logMessage("需要删除的记录数:" + loop);
			jobContext.logMessage("baseAtts为空的记录数:" + count_baseAtts_null);
			logger.info("需要删除的记录数：" + loop);
			logger.info("baseAtts为空的记录数：" + count_baseAtts_null);
		} catch (Exception ex) {
	...
	private void update(SysOrgElement sysOrgElement, IOrgElement element)
		...
		sysOrgElement.setFdIsExternal(false);
	...
	private void setOrgParent(SysOrgElement sysOrgElement, IOrgElement element,
			...
			throws Exception {
		logger.debug("更新上级部门：" + sysOrgElement.getFdId() + "---"
				+ sysOrgElement.getFdName());
		if (element.getRequiredOms() == null
				|| element.getRequiredOms().contains("parent")) {
			if (StringUtil.isNull(value)) {
				logger.debug("上级部门value为空");
				sysOrgElement.setFdParent(null);
				return;
			}
			if (valueMapTo == null) {
				logger.debug("上级部门valueMapTo为空");
				sysOrgElement.setFdParent(null);
				return;
			}
			...
			if (perents != null && perents.size() > 0) {
				for (Object o : perents) {
					SysOrgElement parent = (SysOrgElement) o;
					logger.debug("上级部门列表：" + parent.getFdId() + "---"
							+ parent.getFdName());
				}
			} else {
				logger.debug("上级部门为空");
			...
			if (perents != null && perents.size() > 0) {
				...
				if (parent.getFdIsAvailable()) {
					...
					logger.debug("设置上级部门：" + parent.getFdId() + "---"
							+ parent.getFdName());
				}
			}
		}
	...
	private void buildDeptCache() throws Exception {
		HQLInfo info = new HQLInfo();
		info.setSelectBlock(
				"sysOrgElement.fdId,sysOrgElement.fdNo,sysOrgElement.fdKeyword,sysOrgElement.fdLdapDN,sysOrgElement.fdImportInfo");
		info.setWhereBlock(
				"sysOrgElement.fdIsAvailable = :fdIsAvailable and sysOrgElement.fdOrgType < 4 and sysOrgElement.fdImportInfo is not null and fd_is_external = 0");
		info.setParameter("fdIsAvailable", Boolean.TRUE);
		List<Object[]> rtnList = orgElementService.findValue(info);

		for (Object[] obj : rtnList) {
			String[] dept = new String[5];
			dept[0] = String.valueOf(obj[0]);
			dept[1] = String.valueOf(obj[1]);
			dept[2] = String.valueOf(obj[2]);
			dept[3] = String.valueOf(obj[3]);
			dept[4] = String.valueOf(obj[4]);
			if (dept[4].startsWith(provider.getKey())) {
				dept[4] = dept[4].substring(dept[4].indexOf(provider.getKey()));
			}
