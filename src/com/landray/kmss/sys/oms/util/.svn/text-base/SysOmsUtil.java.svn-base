package com.landray.kmss.sys.oms.util;
...
public class SysOmsUtil{
	...
	public Long getTimestampByStr(String date){ 
		try{
			return Long.parseLong(date) > 1585218 ? Long.parseLong(date) : null;
		}catch(Exception e){
			//logger.error(date,e);
		}
		try {
			return DateUtil.convertStringToDate(date,"yyyy/MM/dd HH:mm:ss").getTime();
		} catch (Exception e) {
			//logger.error(date,e);
		}
		try {
		...
		try {
			return DateUtil.convertStringToDate(date,"yyyy.MM.dd").getTime();
		} catch (Exception e) {
			//logger.error(date,e);
		}
		try {
		...
		try {
			return DateUtil.convertStringToDate(date,"yyyy年MM月dd日 HH:mm:ss").getTime();
		} catch (Exception e) {
			//logger.error(date,e);
		}
		return null;
