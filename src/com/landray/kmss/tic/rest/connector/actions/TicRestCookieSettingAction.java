 ...
 * REST服务配置 Action
 */
public class TicRestCookieSettingAction extends ExtendAction {

	protected ITicRestCookieSettingService ticRestCookieSettingService;

	@Override
    protected IBaseService getServiceImp(HttpServletRequest request) {
		if (ticRestCookieSettingService == null) {
			ticRestCookieSettingService = (ITicRestCookieSettingService) getBean(
					"ticRestCookieSettingService");
		}
		return ticRestCookieSettingService;
	}
	
	public ActionForward getCookie(ActionMapping mapping, ActionForm form,
			HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		TimeCounter.logCurrentTime("Action-getCookie", true, getClass());
		...
		try {
//			TicRestCookieSetting cookieSetting = (TicRestCookieSetting) getServiceImp(
//					request)
//					.findByPrimaryKey(request.getParameter("fdId"));
			TicRestCookieSetting cookieSetting = new TicRestCookieSetting();
			cookieSetting.setDocSubject("");
			cookieSetting.setFdCookieSettingClazz(request.getParameter("fdCookieSettingClazz"));
			cookieSetting.setFdCookieSettingParam(request.getParameter("fdCookieSettingParam"));
			cookieSetting.setFdUseCustCt(Boolean.valueOf(request.getParameter("fdUseCustCt")));
			...
			js.accumulate("cookies", rest.getCookiesTest(cookieSetting));
		...
		TimeCounter.logCurrentTime("Action-getCookie", false, getClass());
		return null;
	}
    ...
    protected void changeFindPageHQLInfo(HttpServletRequest request,
		...
		if (!StringUtil.isNull(fdAppType)) {
			hql = StringUtil.linkString(hql, " and ",
					"ticRestCookieSetting.fdAppType =:fdAppType ");
			hqlInfo.setParameter("fdAppType", fdAppType);
		}
		if (StringUtil.isNotNull(fdEnviromentId)) {
			hql = StringUtil.linkString(hql, " and ",
					"ticRestCookieSetting.fdEnviromentId =:fdEnviromentId ");
    ...
    public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			boolean noCookieSetting = ((ITicRestMainService) SpringBeanUtil
					.getBean("ticRestMainService"))
							.validateRestCookieSetting(id);
			if (!noCookieSetting) {
