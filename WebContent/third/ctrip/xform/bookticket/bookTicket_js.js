...
//预订机票
function third_ctrip_bookTicket_bookTicket(bookTicketData){
	third_bookticket_data = bookTicketData;
	if(bookTicketData && bookTicketData != null){
		bookTicket_relationidCollection.passengerList = bookTicketData.getAttribute("passengerlist_relationid")+".id";
		...
		if(third_ctrip_bookTicket_validateNotNull(bookTicket_DomValueCollection)){
			...
			if(bookTypeId){
				if(!$("[name*='"+bookTypeId+"']").is(":checked")){
					alert("携程控件要求 \""+$("[name*='"+bookTypeId+"']").attr('subject') + "\"不能为空!");
					return false;
				}
			}
			return true;
		...
		alert("携程控件没找到关联数据信息的字段!");
		return false;
	}
}

//获取必须的值存到集合里面
function third_ctrip_bookTicket_getNeededDom(){
	for(var relationName in bookTicket_relationidCollection){
		if(bookTicket_relationidCollection[relationName] && bookTicket_relationidCollection[relationName] != ''){
			...
			for(var len = 0 ; len < relationDom.length ; len++){
				...
				//判断是否是复选框
				if(relationDomSub.type.toLowerCase() == 'checkbox'){
					if(relationDomSub.checked){
...
//校验所有必填字段是否齐全
function third_ctrip_bookTicket_validateNotNull(collection){
	for(var fileName in collection){
		...
		if(collection[fileName] == ''){
			...
			if(bookTicket_relationidCollection[fileName] && bookTicket_relationidCollection[fileName] != ''){
				...
				alert("携程控件要求 \""+$(nullFile).attr('subject') + "\"不能为空");
				return false;
			}			
		}
	}
