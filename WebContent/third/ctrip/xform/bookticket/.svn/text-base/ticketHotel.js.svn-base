...
Designer_Config.controls['ticketHotel'] = {
		type : "ticketHotel",
		storeType : 'none',
		inherit    : 'base',
		onDraw : _Designer_Control_TicketHotel_OnDraw,
		drawXML : _Designer_Control_TicketHotel_DrawXML,
...
Designer_Config.operations['ticketHotel'] = {
		lab : "2",
		imgIndex : 38,
		title : Designer_Lang.bookTicket,
		order : 16,
		run : function (designer) {
			designer.toolBar.selectButton('ticketHotel');
		},
		type : 'cmd',
		select: true,
...
Designer_Config.buttons.tool.push("ticketHotel");

Designer_Menus.add.menu['ticketHotel'] = Designer_Config.operations['ticketHotel'];

function _Designer_Control_TicketHotel_OnDraw(parentNode,childNode){
	...
	bookTicketDataDiv.setAttribute("name","sysXformTicketHotelData_"+domElement.id);
...
function _Designer_Control_TicketHotel_DrawXML(){
	var values = this.options.values;
	var customFields = ['retinueList','passengerList','beginDate','endDate','hotelBeginDate','hotelEndDate','costDept','project3','remarks1','remarks2','costCompany','toCity','bookNodeValue'];
