...
<html:form action="/third/wxwork/menu/wxworkMenuDefine.do">
	...
	<kmss:authShow roles="ROLE_THIRDWXWORK_ADMIN">
			<input type="button" value="<bean:message key="button.add"/>"
				onclick="Com_OpenWindow('<c:url value="/third/wxwork/menu/wxworkMenuDefine.do" />?method=add');">
	...
	<table id="List_ViewTable">
		<tr>
			<sunbor:columnHead htmlTag="td">
				...
				<td>
					<bean:message bundle="third-weixin-work" key="third.wx.menu.btn.publish.title"/>
				</td>
		...
		<c:forEach items="${queryPage.list}" var="wxMenuModel" varStatus="vstatus">
			<tr
				kmss_href="<c:url value="/third/wxwork/menu/wxworkMenuDefine.do" />?method=view&fdId=${wxMenuModel.fdId}">
				<td>
