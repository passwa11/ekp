...
<br><p class="txttitle"><bean:message bundle="third-weixin-mutil" key="table.wxOmsRelation"/></p><br>
			...
			<c:if test="${param.type=='dept' }">
				...
				<tr>
					...
					<td class="td_normal_title" width=15%>
						...
						<xform:text property="fdWxKey" style="width:85%" onValueChange="checkThird" value="${param.fdWxKey }"/><span class="txtstrong">*</span>
					</td>
				</tr>
			</c:if>
			...
			<c:if test="${param.type=='dept' }">
				...
				<tr>
					<td class="td_normal_title" width=15%>
						...
						<xform:text property="fdWxKey" style="width:85%" onValueChange="checkThird"/><span class="txtstrong">*</span>
					</td>
					<td class="td_normal_title" width=15%>
					</td><td width="35%"></td>
				</tr>
...
function ekpHandle(type) {
	...
	var url = '<c:url value="/third/weixin/mutil/spi/wxwork_oms_relation/wxworkOmsRelation.do?method=checkEKP&fdEkpId=" />'+fdEkpId;
...
function checkEKP(type){
    ...
    'thirdWxworkOmsInitListService&search=!{keyword}&type='+type, false, false,'<bean:message bundle="third-weixin-mutil" key="thirdWxOmsInit.omsinit.ekp.org"/>');
}
function checkThird(){
	var fdAppPkId = $("input[name='fdAppPkId']").val();
	var fdWxKey = $("input[name='fdWxKey']").val();
	var fdId = '${wxworkOmsRelationModelForm.fdId}';
	var url = '<c:url value="/third/weixin/mutil/spi/wxwork_oms_relation/wxworkOmsRelation.do?method=checkThird&fdId=" />'+fdId+"&fdAppPkId="+fdAppPkId+"&fdWxKey="+fdWxKey;
	$.post(url,function(data){
	    	if(data.status=="0"){
	    		var info = '<bean:message bundle="third-weixin-mutil" key="wxOmsRelation.error.info"/>';
	    		$("#msg").empty();
	    		$("#msg").show();
...
function addOrUpdateCheck(method,type){
	...
	if(fdEkpId==null||fdEkpId==""){
		if('${param.type}'=='dept'){
			fdAppPkIdInfo = '<bean:message bundle="third-weixin-mutil" key="wxOmsRelation.fdEkpId.org"/>';
		}else{
			fdAppPkIdInfo = '<bean:message bundle="third-weixin-mutil" key="wxOmsRelation.fdEkpId"/>';
		}
		fdAppPkIdInfo += '<bean:message bundle="third-weixin-mutil" key="wxOmsRelation.no.empty"/>';
		$("#msg").empty();
		$("#msg").show();
	...
	if(fdAppPkId==null||fdAppPkId==""){
		if('${param.type}'=='dept'){
			fdEkpIdInfo = '<bean:message bundle="third-weixin-mutil" key="wxOmsRelation.fdAppPkId.org"/>';
		}else{
			fdEkpIdInfo = '<bean:message bundle="third-weixin-mutil" key="wxOmsRelation.fdAppPkId"/>';
		}
		fdEkpIdInfo += '<bean:message bundle="third-weixin-mutil" key="wxOmsRelation.no.empty"/>';
		$("#msg").empty();
		$("#msg").show();
		$("#msg").html(info+"<br>&nbsp;&nbsp;&nbsp;&nbsp;"+fdEkpIdInfo);
		return false;
	}
	var fdWxKeyInfo = null;
	if(fdWxKey == null || fdWxKey ==""){
		fdWxKeyInfo = '所属企业微信标识不能为空';
		$("#msg").empty();
		$("#msg").show();
		$("#msg").html(info+"<br>&nbsp;&nbsp;&nbsp;&nbsp;"+fdWxKeyInfo);
		return false;
	}
	var fdId = '${wxworkOmsRelationModelForm.fdId}';
	var url = '<c:url value="/third/weixin/mutil/spi/wxwork_oms_relation/wxworkOmsRelation.do?method=addOrUpdateCheck" />';
	$.post(url, {fdAppPkId:fdAppPkId,type:type,fdId:fdId,fdWxKey:fdWxKey,fdEkpId:fdEkpId},
	   	function(data){
	    	if(data.status=="0"){
	    		if(data.msg!=""){
	    			$("#msg").empty();
