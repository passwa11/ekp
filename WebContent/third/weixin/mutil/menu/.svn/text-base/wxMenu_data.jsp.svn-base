...
<list:data>
	<list:data-columns var="wxMenuModel" list="${queryPage.list }">
		...
		<!-- 微信应用ID -->
		<list:data-column  property="fdAgentId" title="${ lfn:message('third-weixin-mutil:third.wx.menu.agentId') }" />
		<!-- 序号 -->
		<list:data-column  property="fdAgentName" title="${ lfn:message('third-weixin-mutil:third.wx.menu.agentName') }" />
		<!-- 所属分类 -->
