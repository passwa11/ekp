...
<%
	...
	JSONObject jsonIsHot = new JSONObject();

	Page p = (Page) request.getAttribute("queryPage");
	Map map = (Map)request.getAttribute("isHot");
	List<KmsKnowledgeBaseDoc> list = p.getList();
	String fdIds = "";
	int i = 0;
	for (KmsKnowledgeBaseDoc kmsKnowledgeBaseDoc : list) {
		Boolean d = (Boolean)map.get(kmsKnowledgeBaseDoc.getFdId());
		jsonIsHot.element(kmsKnowledgeBaseDoc.getFdId(),d);
	...
	request.setAttribute("jsonIsHot", jsonIsHot);
...
<list:data>
	<list:data-columns var="item" list="${queryPage.list }">
		...
		<list:data-column
				...
				escape="false">
			<c:if test="${not empty item.fdDocAuthorList}">
				<c:forEach var="obj"  items="${item.fdDocAuthorList}" varStatus="status"><c:out value="${obj.fdSysOrgElement.fdName}"/>;</c:forEach>
			</c:if>
			<c:if test="${empty item.fdDocAuthorList  }">
		...
		<list:data-column col="icon" escape="false" title="${lfn:message('kms-knowledge:kmsKnowledge.introduced')}">
			<span>
				...
				<c:if test="${item.docStatus=='25'}">
					<span class="lui_icon_s lui_icon_s_new_know status_20_or_25" title="${ lfn:message('kms-common:kmsDocStatus.waitpublish') }">
						${ lfn:message('kms-common:kmsDocStatus.waitpublish') }
					</span>
				</c:if>
			</span>
		</list:data-column>
		...
		<list:data-column col="isHot">${jsonIsHot[item.fdId]}</list:data-column>
	</list:data-columns>
