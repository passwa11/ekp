...
var _validation = $KMSSValidation(document.forms['kmsKnowledgeBorrowAttAuthForm']);
	
var validators = {
compareNow: {
  error: '{name} ' + lang['kmsKnowledgeBorrowAttAuth.fdEffectiveTime.now.tip'],
  test: function(v) {
    if (!v) return true;
    var now = new Date();
...
window.selectDoc = function(curId,docSubject) {
  dialog.iframe(
	'/kms/knowledge/kms_knowledge_borrow/att_auth/kmsKnowledgeBorrowAttAuth_select.jsp?curId='+curId+'&docSubject='+docSubject,
    lang['kmsKnowledgeBorrowAttAuth.select.tip'],
    function() {},
		{
		  width: 900,
		  height: 620,
					...
					if (contentWindow.onSubmit()) {
					  ...
					  if (datas.length > 0) {
						  ...
						  if(method == "edit"){
							  ...
							  var reloadUrl = Com_Parameter.ContextPath + "kms/knowledge/kms_knowledge_borrow/kmsKnowledgeBorrowAttAuth.do?method=add&fdDocId="+datas[0].id;
...
window.submitForm = function(method, status) {
    var formObj = document.kmsKnowledgeBorrowAttAuthForm;
	if(status){
