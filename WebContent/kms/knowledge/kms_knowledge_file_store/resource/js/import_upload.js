...
function upload(){
	...
	if(file[0].value==null || file[0].value.length==0){
		...
		$.each(file, function(index, obj){
			var _tname = $(obj).val();
			var _fileType = _tname.substring(_tname.lastIndexOf("."));
			_fileType = _fileType.toLowerCase();
			if (_fileType != ".xls" && _fileType != ".xlsx"){
				seajs.use(['lui/dialog'],function(dialog){
					dialog.alert(upload_doing_notExcel);
				});
				_checkReturn = true;
				return;
			}
...
//excel文件上传完毕,显示操作信息
function callback(isAsync,result, msg){
	if(isAsync == "true"){
		var json = JSON.parse(result);
		window.__progressKey = json.progressKey;
		window.__progressResultId = json.fdId;
		window.__progressErrorCount = 0;
		window.__isProgressing = true;
		window.__preProgressMsg = "";
		doUploadProgressSearch();
		$("#div_uploadProgressErrorDetail").hide();
		$("#div_uploadProgressDetail").show();
	}else{
		window.__isProgressing = false;
 ...
 * 前往查看结果详情
 */
function toViewProgressDetail(){
	seajs.use(["lui/util/env"],function (env) {
		var url = env.fn.formatUrl("/kms/knowledge/kms_knowledge_fs_record/kmsKnowledgeFsRecord.do?method=view&fdId="+window.__progressResultId);
		Com_OpenWindow(url);
	})
}
...
function resetResult(){
	...
	$("#div_uploadProgressErrorDetail").show();
	$("#div_uploadProgressDetail").hide();
}
