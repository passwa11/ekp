			...
			<ui:tabpanel layout="sys.ui.tabpanel.list">
		 		<ui:content title="员工入职">
				    ...
				    <list:criteria id="criteria2">
					     ...
					     <list:cri-auto modelName="com.landray.kmss.hr.ratify.model.HrRatifyEntry" property="docNumber" />
					     <list:cri-auto modelName="com.landray.kmss.hr.ratify.model.HrRatifyEntry" property="docCreator" />
					     <list:cri-auto modelName="com.landray.kmss.hr.ratify.model.HrRatifyEntry" property="docCreateTime" />
					     <list:cri-auto modelName="com.landray.kmss.hr.ratify.model.HrRatifyEntry" property="docPublishTime" />
				  	</list:criteria>
				  	<!-- 操作 -->
				        ...
				        <div style="float:left">
				            <div style="display: inline-block;vertical-align: middle;">
				                <ui:toolbar layout="sys.ui.toolbar.sort" style="float:left">
				                    <list:sort property="hrRatifyEntry.docCreateTime" text="${lfn:message('hr-ratify:hrRatifyMain.docCreateTime')}" group="sort.list" />
				                    <list:sort property="hrRatifyEntry.docPublishTime" text="${lfn:message('hr-ratify:hrRatifyMain.docPublishTime')}" group="sort.list" />
				                </ui:toolbar>
				            </div>
				        </div>
				        <div style="float:left;">
				        ...
				        <div style="float:right">
				            <div style="display: inline-block;vertical-align: middle;">
				                <ui:toolbar count="3">
				                    ...
				                    <kmss:auth requestURL="/hr/ratify/hr_ratify_entry/hrRatifyEntry.do?method=add">
				                        <ui:button text="${lfn:message('button.add')}" onclick="addDoc();" order="2" />
				                    </kmss:auth>
				                    <kmss:auth requestURL="/hr/ratify/hr_ratify_entry/hrRatifyEntry.do?method=deleteall">
				    ...
				    <list:listview id="listview">
				        <ui:source type="AjaxJson">
				            {url:'/hr/ratify/hr_ratify_entry/hrRatifyEntry.do?method=data'}
				        </ui:source>
				        <!-- 列表视图 -->
				        <list:colTable isDefault="false" rowHref="/hr/ratify/hr_ratify_entry/hrRatifyEntry.do?method=view&fdId=!{fdId}" name="columntable">
				            <list:col-checkbox />
				            <list:col-serial/>
				    ...
				    <script>
					    seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'],function($, dialog, topic){
								...
								dialog.category('com.landray.kmss.hr.ratify.model.HrRatifyTemplate','docTemplateId','docTemplateName',false,function(rtn){
									if(rtn != false&&rtn != null){
										...
										if(tempId !=null && tempId != ''){
											var url = Com_Parameter.ContextPath+"hr/ratify/hr_ratify_entry/hrRatifyEntry.do?method=add&i.docTemplate="+tempId;
				    		...
				    		window.delDoc = function(draft){
				    			...
				    			var url = Com_Parameter.ContextPath + 'hr/ratify/hr_ratify_entry/hrRatifyEntry.do?method=deleteall';
				    			url = Com_SetUrlParameter(url, 'categoryId', cateId);
				    			url = Com_SetUrlParameter(url, 'nodeType', nodeType); 
				    			if(draft == '0'){
				    				url = Com_Parameter.ContextPath + 'hr/ratify/hr_ratify_entry/hrRatifyEntry.do?method=deleteall&status=10';
				    			}
				    			var config = {
				    				url : url, // 删除数据的URL
				    				data : $.param({"List_Selected":values},true), // 要删除的数据
				    				modelName : "com.landray.kmss.hr.ratify.model.HrRatifyEntry" // 主要是判断此文档是否有部署软删除
				    			};
				    			// 通用删除方法
				  ...
				  <ui:content title="发起员工入职">
		   			<%@ include file="/hr/ratify/hr_ratify_entry/hrRatifyEntry_index.jsp"%>
		 		  </ui:content>
			</ui:tabpanel>
		</div> 
    </template:replace>
