...
<template:replace name="content">
	...
	<c:if test="${param.approveModel ne 'right'}">
		<form name="hrRatifyLeaveForm" method="post" action ="${KMSS_Parameter_ContextPath}hr/ratify/hr_ratify_leave/hrRatifyLeave.do">
	</c:if>
    ...
    <c:choose>
		<c:when test="${param.approveModel eq 'right'}">
			<ui:tabpage collapsed="true" id="reviewTabPage">
				...
				<c:import url="/hr/ratify/hr_ratify_leave/hrRatifyLeave_editContent.jsp" charEncoding="UTF-8">
	...
	<script>
		seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'],function($, dialog, topic){
			window.addDeptPost = function(obj){
				var fdLeaveDept = Address_GetAddressObj('fdLeaveDeptName');
				var fdLeaveEnterDate = $('input[name="fdLeaveEnterDate"]')[0];
					...
					$.ajax({
						...
						success : function(data, textStatus, xhr){
							...
							if(fdLeaveDept && d.dept){
								...
								fdLeaveDept.reset(";",ORG_TYPE_ORGORDEPT,false,values);
							}
							if (fdLeaveEnterDate) {
								$('input[name="fdLeaveEnterDate"]').val(d.entryTime);
							}
						}
					});
				}else{
					if(fdLeaveDept)
						fdLeaveDept.emptyAddress();
					if(fdLeaveEnterDate)
						fdLeaveEnterDate.value = '';
				}
			};
		});
