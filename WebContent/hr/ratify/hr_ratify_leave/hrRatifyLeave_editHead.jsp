...
<template:replace name="head">
    ...
    <script type="text/javascript">
        ...
        Com_IncludeFile("form_option.js", "${LUI_ContextPath}/hr/ratify/hr_ratify_leave/", 'js', true);
        Com_IncludeFile("main_edit.js", "${LUI_ContextPath}/hr/ratify/resource/js/", 'js', true);
        Com_IncludeFile("swf_attachment.js", "${KMSS_Parameter_ContextPath}sys/attachment/js/", "js", true);
    </script>
</template:replace>
<c:if test="${hrRatifyLeaveForm.method_GET == 'edit' || (param['i.docTemplate']!=null && param['i.docTemplate']!='')}">
	...
	<c:choose>
		<c:when test="${param.approveModel eq 'right'}">
			<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
	            <c:choose>
	                <c:when test="${ hrRatifyLeaveForm.method_GET == 'edit' }">
	                    <c:if test="${ hrRatifyLeaveForm.docStatus=='10' || hrRatifyLeaveForm.docStatus=='11' }">
	                        <ui:button text="${ lfn:message('button.savedraft') }" onclick="if(validateDetail()){submitForm('${hrRatifyLeaveForm.docStatus}','update',true);}" styleClass="lui_widget_btn_primary" isForcedAddClass="true"/>
	                    </c:if>
		...
		<c:otherwise>
			<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
	            <c:choose>
	                <c:when test="${ hrRatifyLeaveForm.method_GET == 'edit' }">
	                    <c:if test="${ hrRatifyLeaveForm.docStatus=='10' || hrRatifyLeaveForm.docStatus=='11' }">
	                        <ui:button text="${ lfn:message('button.savedraft') }" onclick="if(validateDetail()){submitForm('${hrRatifyLeaveForm.docStatus}','update',true);}" />
	                    </c:if>
	                    <c:if test="${ hrRatifyLeaveForm.docStatus=='10' || hrRatifyLeaveForm.docStatus=='11' || hrRatifyLeaveForm.docStatus=='20' }">
...
<template:replace name="path">
    <ui:combin ref="menu.path.category">
		<ui:varParams 
    		moduleTitle="${ lfn:message('hr-ratify:table.hrRatifyLeave') }" 
