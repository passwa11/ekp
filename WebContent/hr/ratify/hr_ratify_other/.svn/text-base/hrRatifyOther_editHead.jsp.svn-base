...
<template:replace name="head">
    ...
    <script type="text/javascript">
        ...
        Com_IncludeFile("form_option.js", "${LUI_ContextPath}/hr/ratify/hr_ratify_other/", 'js', true);
	...
	<c:choose>
		<c:when test="${param.approveModel eq 'right'}">
			<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
	            <c:choose>
	                <c:when test="${ hrRatifyOtherForm.method_GET == 'edit' }">
	                    <c:if test="${ hrRatifyOtherForm.docStatus=='10' || hrRatifyOtherForm.docStatus=='11' }">
	                        <ui:button text="${ lfn:message('button.savedraft') }" onclick="if(validateDetail()){submitForm('${hrRatifyOtherForm.docStatus}','update',true);}" styleClass="lui_widget_btn_primary" isForcedAddClass="true"/>
	                    </c:if>
		...
		<c:otherwise>
			<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
	            <c:choose>
	                <c:when test="${ hrRatifyOtherForm.method_GET == 'edit' }">
	                    <c:if test="${ hrRatifyOtherForm.docStatus=='10' || hrRatifyOtherForm.docStatus=='11' }">
	                        <ui:button text="${ lfn:message('button.savedraft') }" onclick="if(validateDetail()){submitForm('${hrRatifyOtherForm.docStatus}','update',true);}" />
	                    </c:if>
	                    <c:if test="${ hrRatifyOtherForm.docStatus=='10' || hrRatifyOtherForm.docStatus=='11' || hrRatifyOtherForm.docStatus=='20' }">
