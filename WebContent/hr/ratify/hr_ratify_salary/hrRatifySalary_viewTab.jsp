	...
	<c:choose>
		<c:when test="${param.approveModel eq 'right'}">
			<ui:tabpage collapsed="true" id="reviewTabPage">
				...
				<c:import url="/hr/ratify/hr_ratify_salary/hrRatifySalary_viewContent.jsp" charEncoding="UTF-8">
			...
			<ui:tabpanel suckTop="true" layout="sys.ui.tabpanel.sucktop" var-supportExpand="true" var-extend="true" var-average='false' var-useMaxWidth='true'>
				<%--流程--%>
				<c:choose>
					<c:when test="${hrRatifySalaryForm.docStatus>='30' || hrRatifySalaryForm.docStatus=='00'}">
						<%-- 流程 --%>
						<c:choose>
							<c:when test="${hrRatifySalaryForm.docUseXform == 'true' || empty hrRatifySalaryForm.docUseXform}">
		...
		<template:replace name="barRight">
			<c:choose>
				<c:when test="${hrRatifySalaryForm.docStatus>='30' || hrRatifySalaryForm.docStatus=='00'}">
					<ui:accordionpanel>
						<!-- 基本信息-->
						<c:import url="/hr/ratify/hr_ratify_salary/hrRatifySalary_viewBaseInfoContent.jsp" charEncoding="UTF-8">
						</c:import>
						<!-- 关联机制(与原有机制有差异) -->
