...
<c:choose>
	<c:when test="${param.contentType eq 'xform'}">
		<ui:content title="${lfn:message('hr-ratify:py.BiaoDanNeiRong')}"
				...
				<tr>
					...
					<td style="border-left: 0px !important;">
						<div id="_xform_docSubject" _xform_type="text">
							<c:if
								test="${hrRatifyRehireForm.titleRegulation==null || hrRatifyRehireForm.titleRegulation=='' }">
			...
			<c:if
				test="${hrRatifyRehireForm.docUseXform == 'true' || empty hrRatifyRehireForm.docUseXform}">
				...
				<tr>
					...
					<td width="35%">
						...
						<div id="_xform_docCreatorId" _xform_type="address">
							<ui:person personId="${hrRatifyRehireForm.docCreatorId}"
								personName="${hrRatifyRehireForm.docCreatorName}" />
						</div>
					</td>
					<td class="td_normal_title" width="15%">
					...
					<td width="35%">
						...
						<div id="_xform_docNumber" _xform_type="text">
							<c:if test="${not empty hrRatifyRehireForm.docNumber}">
								<xform:text property="docNumber" showStatus="readOnly"
									style="width:95%;" />
							</c:if>
							<c:if test="${empty hrRatifyRehireForm.docNumber}">
	...
	<c:when test="${param.contentType eq 'other'}">
		...
		<c:import url="/sys/right/import/right_edit.jsp" charEncoding="UTF-8">
			...
			<c:param name="moduleModelName"
				value="com.landray.kmss.hr.ratify.model.HrRatifyRehire" />
		</c:import>
	</c:when>
</c:choose>