...
<div id="_fdSalaryPost" valField="fdSalaryPostId" xform_type="address">
  <xform:address 
            isLoadDataDict="false"
            showStatus="edit"
            mobile="${param.mobile eq 'true'? 'true':'false'}"
			required="<%=required %>" style="<%=parse.getStyle()%>"
			subject="${lfn:message('hr-ratify:hrRatifySalary.fdSalaryPost')}"
			propertyId="fdSalaryPostId" propertyName="fdSalaryPostName"
