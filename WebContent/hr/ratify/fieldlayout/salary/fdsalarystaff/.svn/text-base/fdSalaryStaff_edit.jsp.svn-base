...
<c:choose>
	<c:when test="${haveRight eq 'true'}">
		<div id="_fdSalaryStaff" valField="fdSalaryStaffId" xform_type="address">
			<xform:address
				...
				subject="${lfn:message('hr-ratify:hrRatifySalary.fdSalaryStaff')}"
				propertyId="fdSalaryStaffId" propertyName="fdSalaryStaffName"
	...
	<c:otherwise>
		<!--没有权限默认是自己-->
		<html:hidden property="fdSalaryStaffName" value="<%=UserUtil.getUser().getFdName()%>" />
		<html:hidden property="fdSalaryStaffId" value="<%=UserUtil.getUser().getFdId()%>" />
