...
<div id="_fdRemovePost" valField="fdRemovePostId" xform_type="address">
  <xform:address 
            isLoadDataDict="false"
            showStatus="edit"
            mobile="${param.mobile eq 'true'? 'true':'false'}"
			required="<%=required %>" style="<%=parse.getStyle()%>"
			subject="${lfn:message('hr-ratify:hrRatifyRemove.fdRemovePost')}"
			propertyId="fdRemovePostId" propertyName="fdRemovePostName"
