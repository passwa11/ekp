...
<div id="_fdLeavePost" valField="fdLeavePostId" xform_type="address">
  <xform:address 
            isLoadDataDict="false"
            showStatus="edit"
            mobile="${param.mobile eq 'true'? 'true':'false'}"
			required="<%=required %>" style="<%=parse.getStyle()%>"
			subject="${lfn:message('hr-ratify:hrRatifyLeave.fdLeavePost')}"
			propertyId="fdLeavePostId" propertyName="fdLeavePostName"
