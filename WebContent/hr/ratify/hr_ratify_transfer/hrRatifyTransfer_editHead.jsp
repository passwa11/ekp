...
<template:replace name="head">
    ...
    <script type="text/javascript">
        ...
        Com_IncludeFile("form_option.js", "${LUI_ContextPath}/hr/ratify/hr_ratify_transfer/", 'js', true);
	...
	<c:choose>
		<c:when test="${param.approveModel eq 'right'}">
			<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
	            <c:choose>
	                <c:when test="${ hrRatifyTransferForm.method_GET == 'edit' }">
	                    <c:if test="${ hrRatifyTransferForm.docStatus=='10' || hrRatifyTransferForm.docStatus=='11' }">
	                        <ui:button text="${ lfn:message('button.savedraft') }" onclick="if(validateDetail()){submitForm('${hrRatifyTransferForm.docStatus}','update',true);}" styleClass="lui_widget_btn_primary" isForcedAddClass="true"/>
	                    </c:if>
		...
		<c:otherwise>
			<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
	            <c:choose>
	                <c:when test="${ hrRatifyTransferForm.method_GET == 'edit' }">
	                    <c:if test="${ hrRatifyTransferForm.docStatus=='10' || hrRatifyTransferForm.docStatus=='11' }">
	                        <ui:button text="${ lfn:message('button.savedraft') }" onclick="if(validateDetail()){submitForm('${hrRatifyTransferForm.docStatus}','update',true);}" />
	                    </c:if>
	                    <c:if test="${ hrRatifyTransferForm.docStatus=='10' || hrRatifyTransferForm.docStatus=='11' || hrRatifyTransferForm.docStatus=='20' }">
...
<template:replace name="path">
	<ui:combin ref="menu.path.category">
		<ui:varParams 
    		moduleTitle="${ lfn:message('hr-ratify:table.hrRatifyTransfer') }" 
