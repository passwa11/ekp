...
<list:data>
    <list:data-columns var="hrRatifyTransfer" list="${queryPage.list}" varIndex="status" custom="false">
        ...
        <list:data-column col="docCreator.id" escape="false">
            <c:out value="${hrRatifyTransfer.docCreator.fdId}" />
        </list:data-column>
        <list:data-column col="docCreateTime" title="${lfn:message('hr-ratify:hrRatifyMain.docCreateTime')}">
            <kmss:showDate value="${hrRatifyTransfer.docCreateTime}" type="datetime"></kmss:showDate>
        </list:data-column>
        <list:data-column col="docPublishTime" title="${lfn:message('hr-ratify:hrRatifyMain.docPublishTime')}">
            <kmss:showDate value="${hrRatifyTransfer.docPublishTime}" type="datetime"></kmss:showDate>
        </list:data-column>
        <list:data-column col="docStatus.name" title="${lfn:message('hr-ratify:hrRatifyMain.docStatus')}">
            <sunbor:enumsShow value="${hrRatifyTransfer.docStatus}" enumsType="hr_ratify_doc_status" />
        </list:data-column>
        <list:data-column col="docStatus">
            <c:out value="${hrRatifyTransfer.docStatus}" />
        </list:data-column>
        <!-- lbpm_main -->
        <list:data-column col="lbpm_main_listcolumn_node" title="${lfn:message('hr-ratify:lbpm.currentNode') }" escape="false">
            <kmss:showWfPropertyValues var="nodevalue" idValue="${hrRatifyTransfer.fdId}" propertyName="nodeName" />
            <div title="${nodevalue}">
        ...
        <list:data-column col="lbpm_main_listcolumn_handler" title="${lfn:message('hr-ratify:lbpm.currentHandler') }" escape="false">
            <kmss:showWfPropertyValues var="handlerValue" idValue="${hrRatifyTransfer.fdId}" propertyName="handlerName" />
