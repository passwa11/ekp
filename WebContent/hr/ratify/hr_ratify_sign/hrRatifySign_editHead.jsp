...
.inputsgl[readonly], .tb_normal .inputsgl[readonly] {
	...
	<script type="text/javascript">
		...
		Com_IncludeFile("form_option.js",
				"${LUI_ContextPath}/hr/ratify/hr_ratify_sign/", 'js', true);
...
<c:if
	test="${hrRatifySignForm.method_GET == 'edit' || (param['i.docTemplate']!=null && param['i.docTemplate']!='')}">
		...
		<c:choose>
			<c:when test="${param.approveModel eq 'right'}">
				<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
					<c:choose>
						<c:when test="${ hrRatifySignForm.method_GET == 'edit' }">
							<c:if
								test="${ hrRatifySignForm.docStatus=='10' || hrRatifySignForm.docStatus=='11' }">
								<ui:button text="${ lfn:message('button.savedraft') }"
									onclick="if(validateDetail()){submitForm('${hrRatifySignForm.docStatus}','update',true);}"
									styleClass="lui_widget_btn_primary" />
							</c:if>
							<c:if
								test="${ hrRatifySignForm.docStatus=='10' || hrRatifySignForm.docStatus=='11' || hrRatifySignForm.docStatus=='20' }">
	...
	<template:replace name="path">
		<ui:combin ref="menu.path.category">
			<ui:varParams
				moduleTitle="${ lfn:message('hr-ratify:table.hrRatifySign') }"
