...
<template:replace name="content">
	...
	<c:if test="${param.approveModel ne 'right'}">
		<form name="hrRatifyFireForm" method="post" action ="${KMSS_Parameter_ContextPath}hr/ratify/hr_ratify_fire/hrRatifyFire.do">
	</c:if>
    ...
    <c:choose>
		<c:when test="${param.approveModel eq 'right'}">
			<ui:tabpage collapsed="true" id="reviewTabPage">
				...
				<c:import url="/hr/ratify/hr_ratify_fire/hrRatifyFire_editContent.jsp" charEncoding="UTF-8">
	...
	<script>
		seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'],function($, dialog, topic){
			window.addDeptPost = function(obj){
				var fdFireDept = Address_GetAddressObj('fdFireDeptName');
				var fdFireEnterDate = $('input[name="fdFireEnterDate"]')[0];
				var fdFireStaffId = $('input[name="fdFireStaffId"]').val();
				if(fdFireStaffId){
					$.ajax({
						...
						success : function(data, textStatus, xhr){
							...
							if(fdFireDept && d.dept){
								...
								fdFireDept.reset(";",ORG_TYPE_ORGORDEPT,false,values);
							}
							if (fdFireEnterDate) {
								$('input[name="fdFireEnterDate"]').val(d.entryTime);
							}
						}
					});
				}else{
					if(fdFireDept)
						fdFireDept.emptyAddress();
					if(fdFireEnterDate)
						fdFireEnterDate.value = '';
				}
			};
		});
	</script>
