	...
	<template:replace name="content" >
		...
		<div class="hr_contract_batch_sign_warp">
		            ...
		            <div class="lui_hr_criteria_wrap">
		                <em class="lui_text_primary">${ lfn:message('hr-ratify:hrRatifyPositive.selected') }（<i>${fn:length(list) }</i>人）：</em>
		            ...
		            <html:form action="/hr/staff/hr_staff_person_experience/contract/hrStaffPersonExperienceContract.do">
			            <div class="lui_hr_tb_simple_wrap">
			                <table class="tb_simple lui_hr_tb_simple">
								<tr>
									<td class="tr_normal_title">
										${ lfn:message('hr-ratify:hrRatifySign.fdSignName') }
									</td>
									<td>
										<xform:text property="fdName" required="true" showStatus="edit"  validators="maxLength(200)"/>
									</td>
								</tr>
								<tr>
									<td class="tr_normal_title">
										${ lfn:message('hr-ratify:hrRatifySign.fdSignContType') }
									</td>
									<td>
										<xform:select property="fdStaffContTypeId" required="true" showPleaseSelect="true" showStatus="edit" value="${hrStaffPersonExperienceContractForm.fdStaffContTypeId }">
							...
							<html:hidden property="fdSignType" value="1" />
			            </div>
		            </html:form>
		            <!--放弃入职表格信息Ends-->
					...
					if(validation.validate()){
						...
						$.ajax({
							...
							success: function(data) {
								if(data.result){
									__isSubmit = true;
									var url = '<c:url value="/hr/staff/hr_staff_person_experience/contract/hrStaffPersonExperienceContract.do?method=saveBatchSign"/>' + "&ids="+ids;
