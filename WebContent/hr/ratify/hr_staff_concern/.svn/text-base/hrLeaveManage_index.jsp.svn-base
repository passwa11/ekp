...
<template:include ref="default.simple" spa="true">
    <template:replace name="body">
	    ...
	    <list:criteria id="criteria2">
		    <list:cri-ref key="_fdKey" ref="criterion.sys.docSubject" title="${ lfn:message('hr-ratify:hrStaffPersonInfo.search.placehoder') }" />
		  	<list:cri-ref key="_fdDept" ref="criterion.sys.dept" title="${ lfn:message('hr-staff:hrStaffPersonInfo.fdOrgParent') }"></list:cri-ref>
		  	<list:cri-criterion title="${ lfn:message('hr-staff:hrStaffPersonInfo.fdLeaveReason') }" key="fdLeaveReason" multi="false" expand="false">
				<list:box-select>
					<list:item-select>
						<ui:source type="AjaxJson">
							{"url":"/hr/ratify/hr_ratify_leave_reason/hrRatifyLeaveReason.do?method=criteria"} 
						</ui:source>
					</list:item-select>
				</list:box-select>
			</list:cri-criterion>
	    ...
	    <div class="lui_list_operation">
               ...
               <div class="lui_list_operation_sort_btn">
				...
				<div class="lui_list_operation_sort_toolbar">
	                <ui:toolbar layout="sys.ui.toolbar.sort" style="float:left">
	                    <list:sort property="hrRatifyLeave.fdLeaveDate" text="${lfn:message('hr-ratify:hrRatifyLeave.fdLeaveDate')}" group="sort.list" />
	                </ui:toolbar>
	            </div>
	        </div>
	        ...
	        <div style="float:right">
	            <div style="display: inline-block;vertical-align: middle;">
	                <ui:toolbar count="3">
	                	<ui:button text="${lfn:message('hr-staff:hrStaffEntry.handlingResignation')}" onclick="dealLeave();" />
	                </ui:toolbar> 
	            </div> 
	        </div>
	    </div>
	    ...
	    <list:listview id="listview">
	        <ui:source type="AjaxJson">
	            {url:'/hr/ratify/hr_ratify_leave/hrRatifyLeave.do?method=leaveManageList&fdLeaveStatus=${param.fdLeaveStatus }'}
	        </ui:source>
	        <!-- 列表视图 -->
	        <list:colTable isDefault="false" name="columntable">
	            <list:col-checkbox />
	            <list:col-serial/>
	            <list:col-auto props="message;fdStaffStatus;fdLeaveStatus;fdCompanyAge;fdLeaveDate;fdLeaveRealDate;fdLeaveType;docNumber;wait-operations" />
	        </list:colTable>
	    </list:listview>
	    <!-- 翻页 -->
	    <list:paging />
	    <script>
		    ...
		    seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic', 'lui/util/env', 'lang!sys-ui'],function($, dialog, topic, env, ui_lang){
	    		...
	    		window.openPersonInfo = function(fdOrgPersonId){
	    			var url = env.fn.formatUrl("/hr/staff/hr_staff_person_info/hrStaffPersonInfo.do?method=view&fdId="+fdOrgPersonId);
	    			Com_OpenWindow(url);
	    		};
	    		window.editLeaveInfo = function(fdOrgPersonId){
	    			var url = "/hr/ratify/hr_ratify_leave_dr/hrRatifyLeaveDR.do?method=editLeave&fdUserId="+fdOrgPersonId;
    				dialog.iframe(url,'${lfn:message('hr-staff:hrStaffEntry.handlingResignation')}',function(value){
						if(value == 'success')
							topic.publish("list.refresh");
					},{
