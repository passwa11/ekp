...
<%-- 		<list:data-column property="fdId">
		...
		<list:data-column col="icon" escape="false">
			    <person:headimageUrl personId="${hrRatifyMain[4]}" size="m" />
		</list:data-column>
		  创建时间
	 	<list:data-column col="created" title="${ lfn:message('sys-news:sysNewsMain.docCreateTime') }">
	        <kmss:showDate value="${hrRatifyMain[5]}" type="date"></kmss:showDate>
      	</list:data-column>
		链接
		<list:data-column col="href" escape="false">
			<c:if test="${empty LandrayOffline }">
				/hr/ratify/hr_ratify_main/hrRatifyMain.do?method=view4Main&fdId=${hrRatifyMain[0]}
			</c:if>
			<c:if test="${not empty LandrayOffline and LandrayOffline =='true' }">
				/hr/ratify/mobile/offline/view.jsp?fdId=${hrRatifyMain[0]}
			</c:if>
		</list:data-column>
		 创建时间
	 	<list:data-column col="summary" title="${ lfn:message('hr-ratify:sysWfNode.processingNode.currentProcessor') }" escape="false">
	        <kmss:showWfPropertyValues idValue="${hrRatifyMain[0]}" propertyName="summary" />
      	</list:data-column> 
	</list:data-columns>
