...
<list:data>
	<list:data-columns var="hrRatifyPositive" list="${queryPage.list }" varIndex="status" mobile="true">
		...
		<list:data-column col="label" title="${ lfn:message('sys-news:sysNewsMain.docSubject') }" escape="false">
		         <c:out value="${hrRatifyPositive.docSubject}"/>
		</list:data-column>
		 <%-- 创建者--%>
		<list:data-column col="creator" title="${ lfn:message('sys-news:sysNewsMain.docCreatorId') }" >
		         <c:out value="${hrRatifyPositive.docCreator.fdName}"/>
		</list:data-column>
		 <%-- 创建者头像--%>
		<list:data-column col="icon" escape="false">
			    <person:headimageUrl personId="${hrRatifyPositive.docCreator.fdId}" size="m" />
		</list:data-column>
		 <%-- 创建时间--%>
	 	<list:data-column col="created" title="${ lfn:message('sys-news:sysNewsMain.docCreateTime') }">
	        <kmss:showDate value="${hrRatifyPositive.docCreateTime}" type="date"></kmss:showDate>
      	</list:data-column>
		<%--链接--%>
		<list:data-column col="href" escape="false">
			<c:if test="${empty LandrayOffline }">
				/hr/ratify/hr_ratify_positive/hrRatifyPositive.do?method=view&fdId=${hrRatifyPositive.fdId}
			</c:if>
			<c:if test="${not empty LandrayOffline and LandrayOffline =='true' }">
				/hr/ratify/mobile/offline/view.jsp?fdId=${hrRatifyPositive.fdId}
			</c:if>
		</list:data-column>
		 <%-- 创建时间--%>
	 	<list:data-column col="summary" title="${ lfn:message('hr-ratify:sysWfNode.processingNode.currentProcessor') }" escape="false">
	        <kmss:showWfPropertyValues idValue="${hrRatifyPositive.fdId}" propertyName="summary" />
      	</list:data-column>
	</list:data-columns>
