...
<list:data>
	<list:data-columns var="hrRatifyEntry" list="${queryPage.list }" varIndex="status" mobile="true">
		...
		<list:data-column col="label" title="${ lfn:message('sys-news:sysNewsMain.docSubject') }" escape="false">
		         <c:out value="${hrRatifyEntry.docSubject}"/>
		</list:data-column>
		 <%-- 创建者--%>
		<list:data-column col="creator" title="${ lfn:message('sys-news:sysNewsMain.docCreatorId') }" >
		         <c:out value="${hrRatifyEntry.docCreator.fdName}"/>
		</list:data-column>
		 <%-- 创建者头像--%>
		<list:data-column col="icon" escape="false">
			    <person:headimageUrl personId="${hrRatifyEntry.docCreator.fdId}" size="m" />
		</list:data-column>
		 <%-- 创建时间--%>
	 	<list:data-column col="created" title="${ lfn:message('sys-news:sysNewsMain.docCreateTime') }">
	        <kmss:showDate value="${hrRatifyEntry.docCreateTime}" type="date"></kmss:showDate>
      	</list:data-column>

		<%--链接--%>
		<list:data-column col="href" escape="false">
			<c:if test="${empty LandrayOffline }">
				/hr/ratify/hr_ratify_entry/hrRatifyEntry.do?method=view&fdId=${hrRatifyEntry.fdId}
			</c:if>
			<c:if test="${not empty LandrayOffline and LandrayOffline =='true' }">
				/hr/ratify/mobile/offline/view.jsp?fdId=${hrRatifyEntry.fdId}
			</c:if>
		</list:data-column>
		 <%-- 创建时间--%>
	 	<list:data-column col="summary" title="${ lfn:message('hr-ratify:sysWfNode.processingNode.currentProcessor') }" escape="false">
	        <kmss:showWfPropertyValues idValue="${hrRatifyEntry.fdId}" propertyName="summary" />
      	</list:data-column>
	</list:data-columns>
