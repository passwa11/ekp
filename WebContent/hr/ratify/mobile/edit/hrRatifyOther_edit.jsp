...
<template:include ref="mobile.edit" compatibleMode="true">
	<template:replace name="title">
		<c:choose>
	        <c:when test="${hrRatifyOtherForm.method_GET == 'add' }">
	...
	<template:replace name="content">
		<c:choose>
			...
			<c:otherwise>
				<html:form action="/hr/ratify/hr_ratify_other/hrRatifyOther.do?method=save">
					<div>
						...
						<div data-dojo-type="mui/view/DocScrollableView" 
							...
							<div class="muiFlowInfoW muiFormContent">
								...
								<%-- <html:hidden property="fdCanCircularize" value="${hrRatifyOtherForm.fdCanCircularize}"/> --%>
								<table class="muiSimple" cellpadding="0" cellspacing="0">
									<tr>
										...
										<td>
											<c:if test="${hrRatifyOtherForm.titleRegulation==null || hrRatifyOtherForm.titleRegulation=='' }">
							...
							<div class="muiFlowInfoW muiFormContent">
								<c:if test="${hrRatifyOtherForm.docUseXform == 'false'}">
									<table class="muiSimple" cellpadding="0" cellspacing="0">
										<tr>
											<td colspan="2">
												<c:if test="${empty hrRatifyOtherForm.docContent }">
													<c:set property="docContent" target="${hrRatifyOtherForm}" value=""/>
													<xform:textarea property="docContent" mobile="true"/>
												</c:if>
												<c:if test="${not empty hrRatifyOtherForm.docContent }">
							...
							<ul data-dojo-type="mui/tabbar/TabBar" fixed="bottom" <c:if test="${'false' ne true}">data-dojo-props='fill:"grid"'</c:if>>
							  	<li data-dojo-type="hr/ratify/mobile/resource/js/SaveDraftButton" 
							  		data-dojo-props='validateDomId:"scrollView",validateElementId:"docSubject",docStatus:"${hrRatifyOtherForm.docStatus }"'>
						...
						<c:import url="/sys/lbpmservice/mobile/import/edit.jsp" charEncoding="UTF-8">
							<c:param name="formName" value="hrRatifyOtherForm" />
							<c:param name="fdKey" value="attRatifyOther" />
							<c:param name="viewName" value="lbpmView" />
							<c:param name="backTo" value="scrollView" />
							<c:param name="onClickSubmitButton" value="review_submit();" />
						</c:import>
						<script type="text/javascript" src="<%=request.getContextPath()%>/hr/ratify/mobile/resource/js/edit_js.js"></script>
					</div>
				</html:form>
			</c:otherwise>
		</c:choose>
