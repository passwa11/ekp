...
<template:include ref="mobile.edit" compatibleMode="true">
	<template:replace name="title">
		<c:choose>
	        <c:when test="${hrRatifyLeaveForm.method_GET == 'add' }">
	            <c:out value="${ lfn:message('operation.create') } - ${ lfn:message('hr-ratify:table.hrRatifyLeave') }" />
	        </c:when>
	        <c:otherwise>
	...
	<template:replace name="content">
		<c:choose>
			...
			<c:otherwise>
				<html:form action="/hr/ratify/hr_ratify_leave/hrRatifyLeave.do?method=save">
					<div>
						...
						<div data-dojo-type="mui/view/DocScrollableView" 
							...
							<div class="muiFlowInfoW muiFormContent">
								...
								<%-- <html:hidden property="fdCanCircularize" value="${hrRatifyLeaveForm.fdCanCircularize}"/> --%>
								<table class="muiSimple" cellpadding="0" cellspacing="0">
									<tr>
										...
										<td>
											<c:if test="${hrRatifyLeaveForm.titleRegulation==null || hrRatifyLeaveForm.titleRegulation=='' }">
							...
							<div class="muiFlowInfoW muiFormContent">
								<c:if test="${hrRatifyLeaveForm.docUseXform == 'false'}">
									<table class="muiSimple" cellpadding="0" cellspacing="0">
										<tr>
											<td colspan="2">
												<c:if test="${empty hrRatifyLeaveForm.docContent }">
													<c:set property="docContent" target="${hrRatifyLeaveForm}" value=""/>
													<xform:textarea property="docContent" mobile="true"/>
												</c:if>
												<c:if test="${not empty hrRatifyLeaveForm.docContent }">
						...
						<c:import url="/sys/lbpmservice/mobile/import/edit.jsp" charEncoding="UTF-8">
							<c:param name="formName" value="hrRatifyLeaveForm" />
							<c:param name="fdKey" value="attRatifyLeave" />
							<c:param name="viewName" value="lbpmView" />
							<c:param name="backTo" value="scrollView" />
						...
						require(['dojo/ready','dijit/registry','dojo/query','dojo/ready','dojo/topic',"dojo/request"],function(ready,registry,query,ready,topic,request){
							ready(function(){
								var fdLeaveDept = registry.byId('fdLeaveDept');
								var fdLeaveEnterDate = registry.byId('fdLeaveEnterDate');
								var fdLeaveStaffId = $('input[name="fdLeaveStaffId"]').val();
									 ...
									 if(obj){
											$.ajax({
												...
												success : function(data, textStatus, xhr){
													var d = eval(data);
													if(fdLeaveDept&&d.dept){
														  var dept = d.dept;
														  fdLeaveDept._setCurIdsAttr(dept.id);
														  fdLeaveDept._setCurNamesAttr(dept.name);
													}
													if(fdLeaveEnterDate && d.entryTime)
														fdLeaveEnterDate._setValueAttr(d.entryTime);
											...
											if(fdLeaveDept){
												fdLeaveDept._setCurIdsAttr("");
												fdLeaveDept._setCurNamesAttr("");
											}
											if(fdLeaveEnterDate)
												fdLeaveEnterDate._setValueAttr('');
										}
									
									
								 }
								window.addDeptPost(fdLeaveStaffId);
							})
						})
