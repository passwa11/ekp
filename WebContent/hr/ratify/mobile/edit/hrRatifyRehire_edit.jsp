...
<template:include ref="mobile.edit" compatibleMode="true">
	<template:replace name="title">
		<c:choose>
			<c:when test="${hrRatifyRehireForm.method_GET == 'add' }">
				<c:out
					value="${ lfn:message('operation.create') } - ${ lfn:message('hr-ratify:table.hrRatifyRehire') }" />
			</c:when>
			<c:otherwise>
	...
	<template:replace name="content">
		<c:choose>
			...
			<c:otherwise>
				<html:form action="/hr/ratify/hr_ratify_rehire/hrRatifyRehire.do?method=save">
					<div>
						...
						<div data-dojo-type="mui/view/DocScrollableView" 
							...
							<div class="muiFlowInfoW muiFormContent">
								...
								<%-- <html:hidden property="fdCanCircularize" value="${hrRatifyRehireForm.fdCanCircularize}"/> --%>
								<table class="muiSimple" cellpadding="0" cellspacing="0">
									<tr>
										...
										<td>
											<c:if test="${hrRatifyRehireForm.titleRegulation==null || hrRatifyRehireForm.titleRegulation=='' }">
							...
							<div class="muiFlowInfoW muiFormContent">
								<c:if test="${hrRatifyRehireForm.docUseXform == 'false'}">
									<table class="muiSimple" cellpadding="0" cellspacing="0">
										<tr>
											<td colspan="2">
												<c:if test="${empty hrRatifyRehireForm.docContent }">
													<c:set property="docContent" target="${hrRatifyRehireForm}" value=""/>
													<xform:textarea property="docContent" mobile="true"/>
												</c:if>
												<c:if test="${not empty hrRatifyRehireForm.docContent }">
							...
							<ul data-dojo-type="mui/tabbar/TabBar" fixed="bottom" <c:if test="${'false' ne true}">data-dojo-props='fill:"grid"'</c:if>>
							  	<li data-dojo-type="hr/ratify/mobile/resource/js/SaveDraftButton" 
							  		data-dojo-props='validateDomId:"scrollView",validateElementId:"docSubject",docStatus:"${hrRatifyRehireForm.docStatus }"'>
						...
						<c:import url="/sys/lbpmservice/mobile/import/edit.jsp" charEncoding="UTF-8">
							<c:param name="formName" value="hrRatifyRehireForm" />
							<c:param name="fdKey" value="attRatifyRehire" />
							<c:param name="viewName" value="lbpmView" />
							<c:param name="backTo" value="scrollView" />
						...
						require(['dojo/ready','dijit/registry','dojo/query','dojo/ready','dojo/topic',"dojo/request"],function(ready,registry,query,ready,topic,request){
							ready(function(){
								var fdRehireDept = registry.byId('fdRehireDept');
								var fdRehireEnterDate = registry.byId('fdRehireEnterDate');
								var baseUrl = dojo.config.baseUrl;
								 window.addDeptPost=function(obj){
									 if(obj){
											$.ajax({
												...
												success : function(data, textStatus, xhr){
													...
													if(fdRehireDept&&d.dept){
														  var dept = d.dept;
														  fdRehireDept._setCurIdsAttr(dept.id);
														  fdRehireDept._setCurNamesAttr(dept.name);
													}
													if(fdRehireEnterDate && d.entryTime)
														fdRehireEnterDate._setValueAttr(d.entryTime);
											...
											if(fdRehireDept){
												fdRehireDept._setCurIdsAttr("");
												fdRehireDept._setCurNamesAttr("");
											}
											if(fdRehireEnterDate)
												fdRehireEnterDate._setValueAttr('');
										}
