...
<template:include ref="mobile.edit" compatibleMode="true">
	...
	<template:replace name="content">
		<c:choose>
			...
			<c:otherwise>
				<html:form action="/hr/ratify/hr_ratify_fire/hrRatifyFire.do?method=save">
					<div>
						...
						<div data-dojo-type="mui/view/DocScrollableView" 
							...
							<div class="muiFlowInfoW muiFormContent">
								...
								<%-- <html:hidden property="fdCanCircularize" value="${hrRatifyFireForm.fdCanCircularize}"/> --%>
							...
							<div class="muiFlowInfoW muiFormContent">
								<c:if test="${hrRatifyFireForm.docUseXform == 'false'}">
									<table class="muiSimple" cellpadding="0" cellspacing="0">
										<tr>
											<td colspan="2">
												<c:if test="${empty hrRatifyFireForm.docContent }">
													<c:set property="docContent" target="${hrRatifyFireForm}" value=""/>
													<xform:textarea property="docContent" mobile="true"/>
												</c:if>
												<c:if test="${not empty hrRatifyFireForm.docContent }">
							...
							<ul data-dojo-type="mui/tabbar/TabBar" fixed="bottom" <c:if test="${'false' ne true}">data-dojo-props='fill:"grid"'</c:if>>
							  	<li data-dojo-type="hr/ratify/mobile/resource/js/SaveDraftButton" 
							  		data-dojo-props='validateDomId:"scrollView",validateElementId:"docSubject",docStatus:"${hrRatifyFireForm.docStatus }"'>
						...
						<c:import url="/sys/lbpmservice/mobile/import/edit.jsp" charEncoding="UTF-8">
							<c:param name="formName" value="hrRatifyFireForm" />
							<c:param name="fdKey" value="attRatifyFire" />
							<c:param name="viewName" value="lbpmView" />
							<c:param name="backTo" value="scrollView" />
						...
						require(['dojo/ready','dijit/registry','dojo/query','dojo/ready','dojo/topic',"dojo/request"],function(ready,registry,query,ready,topic,request){
							ready(function(){
								var fdFireDept = registry.byId('fdFireDept');
								var fdFireEnterDate = registry.byId('fdFireEnterDate');
									 ...
									 if(obj){
											$.ajax({
												...
												success : function(data, textStatus, xhr){
													var d = eval(data);
													if(fdFireDept&&d.dept){
														  var dept = d.dept;
														  fdFireDept._setCurIdsAttr(dept.id);
														  fdFireDept._setCurNamesAttr(dept.name);
													}
													if(fdFireEnterDate && d.entryTime)
														fdFireEnterDate._setValueAttr(d.entryTime);
											...
											if(fdFireDept){
												fdFireDept._setCurIdsAttr("");
												fdFireDept._setCurNamesAttr("");
											}
											if(fdFireEnterDate)
												fdFireEnterDate._setValueAttr('');
										}
