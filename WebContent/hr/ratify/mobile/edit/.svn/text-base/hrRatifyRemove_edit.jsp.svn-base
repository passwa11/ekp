...
<template:include ref="mobile.edit" compatibleMode="true">
	...
	<template:replace name="content">
		<c:choose>
			...
			<c:otherwise>
				<html:form action="/hr/ratify/hr_ratify_remove/hrRatifyRemove.do?method=save">
					<div>
						...
						<div data-dojo-type="mui/view/DocScrollableView" 
							...
							<div class="muiFlowInfoW muiFormContent">
								...
								<%-- <html:hidden property="fdCanCircularize" value="${hrRatifyRemoveForm.fdCanCircularize}"/> --%>
								<table class="muiSimple" cellpadding="0" cellspacing="0">
									<tr>
										...
										<td>
											<c:if test="${hrRatifyRemoveForm.titleRegulation==null || hrRatifyRemoveForm.titleRegulation=='' }">
							...
							<div class="muiFlowInfoW muiFormContent">
								<c:if test="${hrRatifyRemoveForm.docUseXform == 'false'}">
									<table class="muiSimple" cellpadding="0" cellspacing="0">
										<tr>
											<td colspan="2">
												<c:if test="${empty hrRatifyRemoveForm.docContent }">
													<c:set property="docContent" target="${hrRatifyRemoveForm}" value=""/>
													<xform:textarea property="docContent" mobile="true"/>
												</c:if>
												<c:if test="${not empty hrRatifyRemoveForm.docContent }">
							...
							<ul data-dojo-type="mui/tabbar/TabBar" fixed="bottom" <c:if test="${'false' ne true}">data-dojo-props='fill:"grid"'</c:if>>
							  	<li data-dojo-type="hr/ratify/mobile/resource/js/SaveDraftButton" 
							  		data-dojo-props='validateDomId:"scrollView",validateElementId:"docSubject",docStatus:"${hrRatifyRemoveForm.docStatus }"'>
						...
						<c:import url="/sys/lbpmservice/mobile/import/edit.jsp" charEncoding="UTF-8">
							<c:param name="formName" value="hrRatifyRemoveForm" />
							<c:param name="fdKey" value="attRatifyRemove" />
							<c:param name="viewName" value="lbpmView" />
							<c:param name="backTo" value="scrollView" />
						...
						require(['dojo/ready','dijit/registry','dojo/query','dojo/ready','dojo/topic',"dojo/request"],function(ready,registry,query,ready,topic,request){
							ready(function(){
								var fdRemoveDept = registry.byId('fdRemoveDept');
								var baseUrl = dojo.config.baseUrl;
								 window.addDeptPost=function(obj){
									 if(obj){
											$.ajax({
												...
												success : function(data, textStatus, xhr){
													...
													if(fdRemoveDept&&d.dept){
														  var dept = d.dept;
														  fdRemoveDept._setCurIdsAttr(dept.id);
														  fdRemoveDept._setCurNamesAttr(dept.name);
													}
											...
											if(fdRemoveDept){
												fdRemoveDept._setCurIdsAttr("");
												fdRemoveDept._setCurNamesAttr("");
											}
										}
