...
<template:include ref="mobile.edit" compatibleMode="true">
	...
	<template:replace name="content">
		<c:choose>
			...
			<c:otherwise>
				<html:form action="/hr/ratify/hr_ratify_retire/hrRatifyRetire.do?method=save">
					<div>
						...
						<div data-dojo-type="mui/view/DocScrollableView" 
							...
							<div class="muiFlowInfoW muiFormContent">
								...
								<%-- <html:hidden property="fdCanCircularize" value="${hrRatifyRetireForm.fdCanCircularize}"/> --%>
								<table class="muiSimple" cellpadding="0" cellspacing="0">
									<tr>
										...
										<td>
											<c:if test="${hrRatifyRetireForm.titleRegulation==null || hrRatifyRetireForm.titleRegulation=='' }">
							...
							<div class="muiFlowInfoW muiFormContent">
								<c:if test="${hrRatifyRetireForm.docUseXform == 'false'}">
									<table class="muiSimple" cellpadding="0" cellspacing="0">
										<tr>
											<td colspan="2">
												<c:if test="${empty hrRatifyRetireForm.docContent }">
													<c:set property="docContent" target="${hrRatifyRetireForm}" value=""/>
													<xform:textarea property="docContent" mobile="true"/>
												</c:if>
												<c:if test="${not empty hrRatifyRetireForm.docContent }">
							...
							<ul data-dojo-type="mui/tabbar/TabBar" fixed="bottom" <c:if test="${'false' ne true}">data-dojo-props='fill:"grid"'</c:if>>
							  	<li data-dojo-type="hr/ratify/mobile/resource/js/SaveDraftButton" 
							  		data-dojo-props='validateDomId:"scrollView",validateElementId:"docSubject",docStatus:"${hrRatifyRetireForm.docStatus }"'>
						...
						<c:import url="/sys/lbpmservice/mobile/import/edit.jsp" charEncoding="UTF-8">
							<c:param name="formName" value="hrRatifyRetireForm" />
							<c:param name="fdKey" value="attRatifyRetire" />
							<c:param name="viewName" value="lbpmView" />
							<c:param name="backTo" value="scrollView" />
						...
						require(['dojo/ready','dijit/registry','dojo/query','dojo/ready','dojo/topic',"dojo/request"],function(ready,registry,query,ready,topic,request){
							ready(function(){
								var fdRetireDept = registry.byId('fdRetireDept');
								var fdRetireEnterDate = registry.byId('fdRetireEnterDate');
								var fdRetireStaffId = $('input[name="fdRetireStaffId"]').val();
									 ...
									 if(obj){
											$.ajax({
												...
												success : function(data, textStatus, xhr){
													var d = eval(data);
													if(fdRetireDept&&d.dept){
														  var dept = d.dept;
														  fdRetireDept._setCurIdsAttr(dept.id);
														  fdRetireDept._setCurNamesAttr(dept.name);
													}
													if(fdRetireEnterDate && d.entryTime)
														fdRetireEnterDate._setValueAttr(d.entryTime);
											...
											if(fdRetireDept){
												fdRetireDept._setCurIdsAttr("");
												fdRetireDept._setCurNamesAttr("");
											}
											if(fdRetireEnterDate)
												fdRetireEnterDate._setValueAttr('');
										}
									
									
								 }
								window.addDeptPost(fdRetireStaffId);
							})
						})
