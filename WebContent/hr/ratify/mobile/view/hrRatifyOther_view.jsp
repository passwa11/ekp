...
<template:include ref="mobile.view" compatibleMode="true" >
	...
	<template:replace name="head">
		...
		<link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/sys/mobile/css/themes/default/checkbox-tiny.css?s_cache=${MUI_Cache}"></link>		</style>
		<!--块状end--> 
	...
	<template:replace name="content">
		<html:form action="/hr/ratify/hr_ratify_other/hrRatifyOther.do">		
		<div id="scrollView"
	...
	<%-- 		<c:import url="/sys/agenda/mobile/edit_hidden.jsp" charEncoding="UTF-8">
			...
			<%-- 支持移动端查阅 --%>
			<%-- <c:if test="${'true' eq hrRatifyOtherForm.fdIsMobileView}"> --%>
				...
				<div class="muiFlowInfoW">
					...
					<%-- 		<c:choose> --%>
							<%-- 	<c:when test="${hrRatifyOtherForm.fdUseWord == 'true'}">
									<c:import url="/sys/attachment/mobile/import/viewContent.jsp" charEncoding="UTF-8">
										<c:param name="formName" value="hrRatifyOtherForm"></c:param>
					...
					<div data-dojo-type="sys/mobile/js/mui/DelayView" id="_noteView">
						<div class="muiFormContent">
							<c:import url="/sys/lbpmservice/mobile/lbpm_audit_note/import/view.jsp" charEncoding="UTF-8">
								...
								<c:param name="formBeanName" value="hrRatifyOtherForm"/>
							</c:import>
			...
			<template:include file="/sys/lbpmservice/mobile/import/tarbar.jsp" 
							editUrl="/hr/ratify/hr_ratify_other/hrRatifyOther.do?method=edit&fdId=${param.fdId }"
							formName="hrRatifyOtherForm"
							viewName="lbpmView"
							allowReview="true">
				<template:replace name="flowArea">
				<%-- 	<c:if test="${'true' eq hrRatifyOtherForm.fdIsMobileView}">
						<c:import url="/sys/bookmark/mobile/import/view.jsp" charEncoding="UTF-8">
						  <c:param name="fdModelName" value="${hrRatifyOtherForm.modelClass.name}"/>
						  <c:param name="fdModelId" value="${hrRatifyOtherForm.fdId}"/>
					...
					<%--传阅 --%>
					<%-- <c:if test="${'true' eq hrRatifyOtherForm.fdCanCircularize }"> --%>
		...
		<%-- <c:if test="${'true' eq hrRatifyOtherForm.fdIsMobileView}"> --%>
			...
			<!-- 钉钉图标 end -->
			<c:if test="${hrRatifyOtherForm.docStatus < '30' }">
				<%-- <c:choose> --%>
					<%-- <c:when test="${'false' eq hrRatifyOtherForm.fdIsMobileApprove}">
						<script type="text/javascript">
			...
			<c:if test="${hrRatifyOtherForm.docStatus eq '30'}">
				<script type="text/javascript">
			...
			<c:if test="${hrRatifyOtherForm.docStatus eq '00'}">
				<script type="text/javascript">
					require(["dojo/ready"], function(ready) {
						ready(function() {
	...
	<%-- <c:if test="${'true' eq hrRatifyOtherForm.fdIsMobileView}"> --%>
		...
		<kmss:ifModuleExist path="/third/ywork/">
			 <c:import url="/third/ywork/ywork_share/yworkDoc_mobile_share.jsp"
				charEncoding="UTF-8">
				<c:param name="modelId" value="${hrRatifyOtherForm.fdId}" />
				<c:param name="modelName" value="com.landray.kmss.hr.ratify.model.HrRatifyOther" />
				<c:param name="templateId" value="${ hrRatifyOtherForm.docTemplateId}" />
				<c:param name="allPath" value="${ hrRatifyOtherForm.docTemplateName}" />
			</c:import>
		</kmss:ifModuleExist>
	<%-- </c:if> --%>
	
		<%-- 不支持移动端查阅 --%>
		<%-- <c:if test="${'false' eq hrRatifyOtherForm.fdIsMobileView}">
			<script type="text/javascript">
				require(["mui/dialog/BarTip", "dojo/ready"], function(BarTip, ready) {
					ready(function() {
