...
<template:include ref="mobile.view" compatibleMode="true" >
	...
	<template:replace name="content">
		<html:form action="/hr/ratify/hr_ratify_remove/hrRatifyRemove.do">		
		<div id="scrollView"
			...
			<c:import url="/sys/right/mobile/edit_hidden.jsp" charEncoding="UTF-8">
				...
				<c:param name="moduleModelName" value="com.landray.kmss.hr.ratify.model.HrRatifyRemove" />
			</c:import>
	...
	<%-- 		<c:import url="/sys/agenda/mobile/edit_hidden.jsp" charEncoding="UTF-8">
			...
			<%-- 支持移动端查阅 --%>
			<%-- <c:if test="${'true' eq hrRatifyRemoveForm.fdIsMobileView}"> --%>
				...
				<div class="muiFlowInfoW">
					...
					<%-- 		<c:choose> --%>
							<%-- 	<c:when test="${hrRatifyRemoveForm.fdUseWord == 'true'}">
									<c:import url="/sys/attachment/mobile/import/viewContent.jsp" charEncoding="UTF-8">
										<c:param name="formName" value="hrRatifyRemoveForm"></c:param>
					...
					<div data-dojo-type="sys/mobile/js/mui/DelayView" id="_noteView">
						<div class="muiFormContent">
							<c:import url="/sys/lbpmservice/mobile/lbpm_audit_note/import/view.jsp" charEncoding="UTF-8">
								<c:param name="fdModelId" value="${hrRatifyRemoveForm.fdId }"/>
			...
			<template:include file="/sys/lbpmservice/mobile/import/tarbar.jsp" 
							editUrl="/hr/ratify/hr_ratify_remove/hrRatifyRemove.do?method=edit&fdId=${param.fdId }"
							formName="hrRatifyRemoveForm"
							viewName="lbpmView"
							allowReview="true">
				<template:replace name="flowArea">
				<%-- 	<c:if test="${'true' eq hrRatifyRemoveForm.fdIsMobileView}">
						<c:import url="/sys/bookmark/mobile/import/view.jsp" charEncoding="UTF-8">
						  <c:param name="fdModelName" value="${hrRatifyRemoveForm.modelClass.name}"/>
						  <c:param name="fdModelId" value="${hrRatifyRemoveForm.fdId}"/>
						  <c:param name="fdSubject" value="${hrRatifyRemoveForm.docSubject}"/>
						  <c:param name="showOption" value="label"></c:param>
						</c:import>
					</c:if> --%>
					<%--传阅 --%>
					<%-- <c:if test="${'true' eq hrRatifyRemoveForm.fdCanCircularize }"> --%>
		...
		<%-- <c:if test="${'true' eq hrRatifyRemoveForm.fdIsMobileView}"> --%>
			...
			<!-- 钉钉图标 end -->
			<c:if test="${hrRatifyRemoveForm.docStatus < '30' }">
				<%-- <c:choose> --%>
					<%-- <c:when test="${'false' eq hrRatifyRemoveForm.fdIsMobileApprove}">
						<script type="text/javascript">
					...
					<%-- <c:otherwise> --%>
						<c:import url="/sys/lbpmservice/mobile/import/view.jsp" charEncoding="UTF-8">
							...
							<c:param name="onClickSubmitButton" value="Com_Submit(document.hrRatifyRemoveForm, 'update');" />
						</c:import>
						<script type="text/javascript">
							require(["mui/form/ajax-form!hrRatifyRemoveForm"]);
						</script>
			<%-- 		</c:otherwise>
				</c:choose> --%>
			</c:if>
			<c:if test="${hrRatifyRemoveForm.docStatus eq '30'}">
				<script type="text/javascript">
			...
			<c:if test="${hrRatifyRemoveForm.docStatus eq '00'}">
				<script type="text/javascript">
					require(["dojo/ready"], function(ready) {
						ready(function() {
	...
	<%-- <c:if test="${'true' eq hrRatifyRemoveForm.fdIsMobileView}"> --%>
		...
		<kmss:ifModuleExist path="/third/ywork/">
			 <c:import url="/third/ywork/ywork_share/yworkDoc_mobile_share.jsp"
				charEncoding="UTF-8">
				<c:param name="modelId" value="${hrRatifyRemoveForm.fdId}" />
				<c:param name="modelName" value="com.landray.kmss.hr.ratify.model.HrRatifyRemove" />
				<c:param name="templateId" value="${ hrRatifyRemoveForm.docTemplateId}" />
				<c:param name="allPath" value="${ hrRatifyRemoveForm.docTemplateName}" />
			</c:import>
		</kmss:ifModuleExist>
	<%-- </c:if> --%>
	
		<%-- 不支持移动端查阅 --%>
		<%-- <c:if test="${'false' eq hrRatifyRemoveForm.fdIsMobileView}">
			<script type="text/javascript">
				require(["mui/dialog/BarTip", "dojo/ready"], function(BarTip, ready) {
					ready(function() {
