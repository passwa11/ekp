...
<template:include ref="mobile.view" compatibleMode="true" >
	...
	<template:replace name="title">
		<c:out value="${hrRatifySignForm.docSubject}"></c:out>
	</template:replace>
	...
	<template:replace name="content">
		<html:form action="/hr/ratify/hr_ratify_sign/hrRatifySign.do">		
		<div id="scrollView"
			...
			<c:import url="/sys/right/mobile/edit_hidden.jsp" charEncoding="UTF-8">
				...
				<c:param name="moduleModelName" value="com.landray.kmss.hr.ratify.model.HrRatifySign" />
			</c:import>
	...
	<%-- 		<c:import url="/sys/agenda/mobile/edit_hidden.jsp" charEncoding="UTF-8">
			...
			<%-- 支持移动端查阅 --%>
			<%-- <c:if test="${'true' eq hrRatifySignForm.fdIsMobileView}"> --%>
				...
				<div class="muiFlowInfoW">
					...
					<%-- 		<c:choose> --%>
							<%-- 	<c:when test="${hrRatifySignForm.fdUseWord == 'true'}">
					...
					<div data-dojo-type="sys/mobile/js/mui/DelayView" id="_noteView">
						<div class="muiFormContent">
							<c:import url="/sys/lbpmservice/mobile/lbpm_audit_note/import/view.jsp" charEncoding="UTF-8">
								...
								<c:param name="formBeanName" value="hrRatifySignForm"/>
							</c:import>
			...
			<template:include file="/sys/lbpmservice/mobile/import/tarbar.jsp" 
							editUrl="/hr/ratify/hr_ratify_sign/hrRatifySign.do?method=edit&fdId=${param.fdId }"
							formName="hrRatifySignForm"
							viewName="lbpmView"
							allowReview="true">
				<template:replace name="flowArea">
				<%-- 	<c:if test="${'true' eq hrRatifySignForm.fdIsMobileView}">
						<c:import url="/sys/bookmark/mobile/import/view.jsp" charEncoding="UTF-8">
						  <c:param name="fdModelName" value="${hrRatifySignForm.modelClass.name}"/>
						  <c:param name="fdModelId" value="${hrRatifySignForm.fdId}"/>
						  <c:param name="fdSubject" value="${hrRatifySignForm.docSubject}"/>
						  <c:param name="showOption" value="label"></c:param>
						</c:import>
					</c:if> --%>
					<%--传阅 --%>
					<%-- <c:if test="${'true' eq hrRatifySignForm.fdCanCircularize }"> --%>
		...
		<%-- <c:if test="${'true' eq hrRatifySignForm.fdIsMobileView}"> --%>
			...
			<!-- 钉钉图标 end -->
			<c:if test="${hrRatifySignForm.docStatus < '30' }">
				<%-- <c:choose> --%>
					<%-- <c:when test="${'false' eq hrRatifySignForm.fdIsMobileApprove}">
						<script type="text/javascript">
			...
			<c:if test="${hrRatifySignForm.docStatus eq '30'}">
				<script type="text/javascript">
			...
			<c:if test="${hrRatifySignForm.docStatus eq '00'}">
				<script type="text/javascript">
					require(["dojo/ready"], function(ready) {
						ready(function() {
	...
	<%-- <c:if test="${'true' eq hrRatifySignForm.fdIsMobileView}"> --%>
		...
		<kmss:ifModuleExist path="/third/ywork/">
			 <c:import url="/third/ywork/ywork_share/yworkDoc_mobile_share.jsp"
				charEncoding="UTF-8">
				<c:param name="modelId" value="${hrRatifySignForm.fdId}" />
				<c:param name="modelName" value="com.landray.kmss.hr.ratify.model.HrRatifySign" />
				<c:param name="templateId" value="${ hrRatifySignForm.docTemplateId}" />
				<c:param name="allPath" value="${ hrRatifySignForm.docTemplateName}" />
			</c:import>
		</kmss:ifModuleExist>
	<%-- </c:if> --%>
	
		<%-- 不支持移动端查阅 --%>
		<%-- <c:if test="${'false' eq hrRatifySignForm.fdIsMobileView}">
			<script type="text/javascript">
				require(["mui/dialog/BarTip", "dojo/ready"], function(BarTip, ready) {
					ready(function() {
