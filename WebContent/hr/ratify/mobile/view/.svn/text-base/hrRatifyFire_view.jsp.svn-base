...
<template:include ref="mobile.view" compatibleMode="true" >
	<template:replace name="loading">
		<c:import url="/hr/ratify/mobile/view/view_banner.jsp" charEncoding="UTF-8">
			<c:param name="formBeanName" value="hrRatifyFireForm"></c:param>
			<c:param name="loading" value="true"></c:param>
		</c:import>
	</template:replace>
	<template:replace name="title">
		<c:out value="${hrRatifyFireForm.docSubject}"></c:out>
	</template:replace>
	...
	<template:replace name="content">
		<html:form action="/hr/ratify/hr_ratify_fire/hrRatifyFire.do">		
		<div id="scrollView"
	...
	<%-- 		<c:import url="/sys/agenda/mobile/edit_hidden.jsp" charEncoding="UTF-8">
			...
			<%-- 支持移动端查阅 --%>
			<%-- <c:if test="${'true' eq hrRatifyFireForm.fdIsMobileView}"> --%>
				...
				<div class="muiFlowInfoW">
					...
					<%-- 		<c:choose> --%>
							<%-- 	<c:when test="${hrRatifyFireForm.fdUseWord == 'true'}">
									<c:import url="/sys/attachment/mobile/import/viewContent.jsp" charEncoding="UTF-8">
										<c:param name="formName" value="hrRatifyFireForm"></c:param>
					...
					<div data-dojo-type="sys/mobile/js/mui/DelayView" id="_noteView">
						<div class="muiFormContent">
							<c:import url="/sys/lbpmservice/mobile/lbpm_audit_note/import/view.jsp" charEncoding="UTF-8">
								<c:param name="fdModelId" value="${hrRatifyFireForm.fdId }"/>
								<c:param name="fdModelName" value="com.landray.kmss.hr.ratify.model.HrRatifyFire"/>
								<c:param name="formBeanName" value="hrRatifyFireForm"/>
							</c:import>
			...
			<template:include file="/sys/lbpmservice/mobile/import/tarbar.jsp" 
							editUrl="/hr/ratify/hr_ratify_fire/hrRatifyFire.do?method=edit&fdId=${param.fdId }"
							formName="hrRatifyFireForm"
							viewName="lbpmView"
							allowReview="true">
				<template:replace name="flowArea">
				<%-- 	<c:if test="${'true' eq hrRatifyFireForm.fdIsMobileView}">
						<c:import url="/sys/bookmark/mobile/import/view.jsp" charEncoding="UTF-8">
						  <c:param name="fdModelName" value="${hrRatifyFireForm.modelClass.name}"/>
						  <c:param name="fdModelId" value="${hrRatifyFireForm.fdId}"/>
						  <c:param name="fdSubject" value="${hrRatifyFireForm.docSubject}"/>
						  <c:param name="showOption" value="label"></c:param>
						</c:import>
					</c:if> --%>
					<%--传阅 --%>
					<%-- <c:if test="${'true' eq hrRatifyFireForm.fdCanCircularize }"> --%>
		...
		<%-- <c:if test="${'true' eq hrRatifyFireForm.fdIsMobileView}"> --%>
			...
			<!-- 钉钉图标 end -->
			<c:if test="${hrRatifyFireForm.docStatus < '30' }">
				<%-- <c:choose> --%>
					<%-- <c:when test="${'false' eq hrRatifyFireForm.fdIsMobileApprove}">
						<script type="text/javascript">
					...
					<%-- <c:otherwise> --%>
						<c:import url="/sys/lbpmservice/mobile/import/view.jsp" charEncoding="UTF-8">
							...
							<c:param name="onClickSubmitButton" value="Com_Submit(document.hrRatifyFireForm, 'update');" />
						</c:import>
						<script type="text/javascript">
							require(["mui/form/ajax-form!hrRatifyFireForm"]);
						</script>
			<%-- 		</c:otherwise>
				</c:choose> --%>
			</c:if>
			<c:if test="${hrRatifyFireForm.docStatus eq '30'}">
				<script type="text/javascript">
			...
			<c:if test="${hrRatifyFireForm.docStatus eq '00'}">
				<script type="text/javascript">
					require(["dojo/ready"], function(ready) {
						ready(function() {
	...
	<%-- <c:if test="${'true' eq hrRatifyFireForm.fdIsMobileView}"> --%>
		...
		<kmss:ifModuleExist path="/third/ywork/">
			 <c:import url="/third/ywork/ywork_share/yworkDoc_mobile_share.jsp"
				charEncoding="UTF-8">
				<c:param name="modelId" value="${hrRatifyFireForm.fdId}" />
				<c:param name="modelName" value="com.landray.kmss.hr.ratify.model.HrRatifyFire" />
				<c:param name="templateId" value="${ hrRatifyFireForm.docTemplateId}" />
				<c:param name="allPath" value="${ hrRatifyFireForm.docTemplateName}" />
			</c:import>
		</kmss:ifModuleExist>
	<%-- </c:if> --%>
	
		<%-- 不支持移动端查阅 --%>
		<%-- <c:if test="${'false' eq hrRatifyFireForm.fdIsMobileView}">
			<script type="text/javascript">
				require(["mui/dialog/BarTip", "dojo/ready"], function(BarTip, ready) {
					ready(function() {
