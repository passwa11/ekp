...
<template:include ref="mobile.view" compatibleMode="true" >
	...
	<template:replace name="title">
		<c:out value="${hrRatifySalaryForm.docSubject}"></c:out>
	</template:replace>
	...
	<template:replace name="content">
		<html:form action="/hr/ratify/hr_ratify_salary/hrRatifySalary.do">		
		<div id="scrollView"
	...
	<%-- 		<c:import url="/sys/agenda/mobile/edit_hidden.jsp" charEncoding="UTF-8">
			...
			<%-- 支持移动端查阅 --%>
			<%-- <c:if test="${'true' eq hrRatifySalaryForm.fdIsMobileView}"> --%>
				...
				<div class="muiFlowInfoW">
					...
					<%-- 		<c:choose> --%>
							<%-- 	<c:when test="${hrRatifySalaryForm.fdUseWord == 'true'}">
					...
					<div data-dojo-type="sys/mobile/js/mui/DelayView" id="_noteView">
						<div class="muiFormContent">
							<c:import url="/sys/lbpmservice/mobile/lbpm_audit_note/import/view.jsp" charEncoding="UTF-8">
								...
								<c:param name="formBeanName" value="hrRatifySalaryForm"/>
							</c:import>
			...
			<template:include file="/sys/lbpmservice/mobile/import/tarbar.jsp" 
							editUrl="/hr/ratify/hr_ratify_salary/hrRatifySalary.do?method=edit&fdId=${param.fdId }"
							formName="hrRatifySalaryForm"
							viewName="lbpmView"
							allowReview="true">
				<template:replace name="flowArea">
				<%-- 	<c:if test="${'true' eq hrRatifySalaryForm.fdIsMobileView}">
						<c:import url="/sys/bookmark/mobile/import/view.jsp" charEncoding="UTF-8">
						  <c:param name="fdModelName" value="${hrRatifySalaryForm.modelClass.name}"/>
						  <c:param name="fdModelId" value="${hrRatifySalaryForm.fdId}"/>
					...
					<%--传阅 --%>
					<%-- <c:if test="${'true' eq hrRatifySalaryForm.fdCanCircularize }"> --%>
		...
		<%-- <c:if test="${'true' eq hrRatifySalaryForm.fdIsMobileView}"> --%>
			...
			<!-- 钉钉图标 end -->
			<c:if test="${hrRatifySalaryForm.docStatus < '30' }">
				<c:import url="/sys/lbpmservice/mobile/import/view.jsp" charEncoding="UTF-8">
					...
					<c:param name="onClickSubmitButton" value="Com_Submit(document.hrRatifySalaryForm, 'update');" />
				</c:import>
				<script type="text/javascript">
					require(["mui/form/ajax-form!hrRatifySalaryForm"]);
				</script>
			</c:if>
			<c:if test="${hrRatifySalaryForm.docStatus eq '30'}">
				<script type="text/javascript">
			...
			<c:if test="${hrRatifySalaryForm.docStatus eq '00'}">
				<script type="text/javascript">
					require(["dojo/ready"], function(ready) {
						ready(function() {
	...
	<%-- <c:if test="${'true' eq hrRatifySalaryForm.fdIsMobileView}"> --%>
		...
		<kmss:ifModuleExist path="/third/ywork/">
			 <c:import url="/third/ywork/ywork_share/yworkDoc_mobile_share.jsp"
				charEncoding="UTF-8">
				<c:param name="modelId" value="${hrRatifySalaryForm.fdId}" />
				<c:param name="modelName" value="com.landray.kmss.hr.ratify.model.HrRatifySalary" />
				<c:param name="templateId" value="${ hrRatifySalaryForm.docTemplateId}" />
				<c:param name="allPath" value="${ hrRatifySalaryForm.docTemplateName}" />
			</c:import>
		</kmss:ifModuleExist>
	<%-- </c:if> --%>
