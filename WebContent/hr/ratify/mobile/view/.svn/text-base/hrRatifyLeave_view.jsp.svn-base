...
<template:include ref="mobile.view" compatibleMode="true" >
	<template:replace name="loading">
		<c:import url="/hr/ratify/mobile/view/view_banner.jsp" charEncoding="UTF-8">
			<c:param name="formBeanName" value="hrRatifyLeaveForm"></c:param>
			<c:param name="loading" value="true"></c:param>
		</c:import>
	</template:replace>
	<template:replace name="title">
		<c:out value="${hrRatifyLeaveForm.docSubject}"></c:out>
	</template:replace>
	...
	<template:replace name="content">
		<html:form action="/hr/ratify/hr_ratify_leave/hrRatifyLeave.do">		
		<div id="scrollView"
			...
			<c:import url="/sys/right/mobile/edit_hidden.jsp" charEncoding="UTF-8">
				<c:param name="formName" value="hrRatifyLeaveForm" />
				<c:param name="moduleModelName" value="com.landray.kmss.hr.ratify.model.HrRatifyLeave" />
			</c:import>
	...
	<%-- 		<c:import url="/sys/agenda/mobile/edit_hidden.jsp" charEncoding="UTF-8">
			...
			<%-- 支持移动端查阅 --%>
			<%-- <c:if test="${'true' eq hrRatifyLeaveForm.fdIsMobileView}"> --%>
				...
				<div class="muiFlowInfoW">
					...
					<div data-dojo-type="dojox/mobile/View" id="_contentView">
						<c:if test="${hrRatifyLeaveForm.docUseXform == 'false'}">
					<%-- 		<c:choose> --%>
							<%-- 	<c:when test="${hrRatifyLeaveForm.fdUseWord == 'true'}">
									<c:import url="/sys/attachment/mobile/import/viewContent.jsp" charEncoding="UTF-8">
										<c:param name="formName" value="hrRatifyLeaveForm"></c:param>
						...
						<c:if test="${hrRatifyLeaveForm.docUseXform == 'true' || empty hrRatifyLeaveForm.docUseXform}">
					...
					<div data-dojo-type="sys/mobile/js/mui/DelayView" id="_noteView">
						<div class="muiFormContent">
							<c:import url="/sys/lbpmservice/mobile/lbpm_audit_note/import/view.jsp" charEncoding="UTF-8">
								<c:param name="fdModelId" value="${hrRatifyLeaveForm.fdId }"/>
								<c:param name="fdModelName" value="com.landray.kmss.hr.ratify.model.HrRatifyLeave"/>
								<c:param name="formBeanName" value="hrRatifyLeaveForm"/>
							</c:import>
			...
			<template:include file="/sys/lbpmservice/mobile/import/tarbar.jsp" 
							editUrl="/hr/ratify/hr_ratify_leave/hrRatifyLeave.do?method=edit&fdId=${param.fdId }"
							formName="hrRatifyLeaveForm"
							viewName="lbpmView"
							allowReview="true">
				<template:replace name="flowArea">
				<%-- 	<c:if test="${'true' eq hrRatifyLeaveForm.fdIsMobileView}">
						<c:import url="/sys/bookmark/mobile/import/view.jsp" charEncoding="UTF-8">
						  <c:param name="fdModelName" value="${hrRatifyLeaveForm.modelClass.name}"/>
						  <c:param name="fdModelId" value="${hrRatifyLeaveForm.fdId}"/>
						  <c:param name="fdSubject" value="${hrRatifyLeaveForm.docSubject}"/>
						  <c:param name="showOption" value="label"></c:param>
						</c:import>
					</c:if> --%>
					<%--传阅 --%>
					<%-- <c:if test="${'true' eq hrRatifyLeaveForm.fdCanCircularize }"> --%>
		...
		<%-- <c:if test="${'true' eq hrRatifyLeaveForm.fdIsMobileView}"> --%>
			...
			<!-- 钉钉图标 end -->
			<c:if test="${hrRatifyLeaveForm.docStatus < '30' }">
						<c:import url="/sys/lbpmservice/mobile/import/view.jsp" charEncoding="UTF-8">
							<c:param name="formName" value="hrRatifyLeaveForm" />
							<c:param name="fdKey" value="${fdTempKey }" />
							<c:param name="lbpmViewName" value="lbpmView" />
							<c:param name="onClickSubmitButton" value="Com_Submit(document.hrRatifyLeaveForm, 'update');" />
						</c:import>
						<script type="text/javascript">
							require(["mui/form/ajax-form!hrRatifyLeaveForm"]);
						</script>
			</c:if>
			<c:if test="${hrRatifyLeaveForm.docStatus eq '30'}">
				<script type="text/javascript">
			...
			<c:if test="${hrRatifyLeaveForm.docStatus eq '00'}">
				<script type="text/javascript">
					require(["dojo/ready"], function(ready) {
						ready(function() {
	...
	<%-- <c:if test="${'true' eq hrRatifyLeaveForm.fdIsMobileView}"> --%>
		...
		<kmss:ifModuleExist path="/third/ywork/">
			 <c:import url="/third/ywork/ywork_share/yworkDoc_mobile_share.jsp"
				charEncoding="UTF-8">
				<c:param name="modelId" value="${hrRatifyLeaveForm.fdId}" />
				<c:param name="modelName" value="com.landray.kmss.hr.ratify.model.HrRatifyLeave" />
				<c:param name="templateId" value="${ hrRatifyLeaveForm.docTemplateId}" />
				<c:param name="allPath" value="${ hrRatifyLeaveForm.docTemplateName}" />
			</c:import>
		</kmss:ifModuleExist>
	<%-- </c:if> --%>
	
		<%-- 不支持移动端查阅 --%>
		<%-- <c:if test="${'false' eq hrRatifyLeaveForm.fdIsMobileView}">
			<script type="text/javascript">
