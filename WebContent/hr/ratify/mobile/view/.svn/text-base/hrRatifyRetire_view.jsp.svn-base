...
<template:include ref="mobile.view" compatibleMode="true" >
	<template:replace name="loading">
		<c:import url="/hr/ratify/mobile/view/view_banner.jsp" charEncoding="UTF-8">
			<c:param name="formBeanName" value="hrRatifyRetireForm"></c:param>
	...
	<template:replace name="content">
		<html:form action="/hr/ratify/hr_ratify_retire/hrRatifyRetire.do">		
		<div id="scrollView"
			...
			<c:import url="/sys/right/mobile/edit_hidden.jsp" charEncoding="UTF-8">
				...
				<c:param name="moduleModelName" value="com.landray.kmss.hr.ratify.model.HrRatifyRetire" />
			</c:import>
	...
	<%-- 		<c:import url="/sys/agenda/mobile/edit_hidden.jsp" charEncoding="UTF-8">
			...
			<%-- 支持移动端查阅 --%>
			<%-- <c:if test="${'true' eq hrRatifyRetireForm.fdIsMobileView}"> --%>
				...
				<div class="muiFlowInfoW">
					...
					<%-- 		<c:choose> --%>
							<%-- 	<c:when test="${hrRatifyRetireForm.fdUseWord == 'true'}">
									<c:import url="/sys/attachment/mobile/import/viewContent.jsp" charEncoding="UTF-8">
										<c:param name="formName" value="hrRatifyRetireForm"></c:param>
					...
					<div data-dojo-type="sys/mobile/js/mui/DelayView" id="_noteView">
						<div class="muiFormContent">
							<c:import url="/sys/lbpmservice/mobile/lbpm_audit_note/import/view.jsp" charEncoding="UTF-8">
								<c:param name="fdModelId" value="${hrRatifyRetireForm.fdId }"/>
								<c:param name="fdModelName" value="com.landray.kmss.hr.ratify.model.HrRatifyRetire"/>
								<c:param name="formBeanName" value="hrRatifyRetireForm"/>
							</c:import>
			...
			<template:include file="/sys/lbpmservice/mobile/import/tarbar.jsp" 
							editUrl="/hr/ratify/hr_ratify_retire/hrRatifyRetire.do?method=edit&fdId=${param.fdId }"
							formName="hrRatifyRetireForm"
							viewName="lbpmView"
							allowReview="true">
				<template:replace name="flowArea">
				<%-- 	<c:if test="${'true' eq hrRatifyRetireForm.fdIsMobileView}">
						<c:import url="/sys/bookmark/mobile/import/view.jsp" charEncoding="UTF-8">
						  <c:param name="fdModelName" value="${hrRatifyRetireForm.modelClass.name}"/>
						  <c:param name="fdModelId" value="${hrRatifyRetireForm.fdId}"/>
						  <c:param name="fdSubject" value="${hrRatifyRetireForm.docSubject}"/>
						  <c:param name="showOption" value="label"></c:param>
						</c:import>
					</c:if> --%>
					<%--传阅 --%>
					<%-- <c:if test="${'true' eq hrRatifyRetireForm.fdCanCircularize }"> --%>
		...
		<%-- <c:if test="${'true' eq hrRatifyRetireForm.fdIsMobileView}"> --%>
			...
			<!-- 钉钉图标 end -->
			<c:if test="${hrRatifyRetireForm.docStatus < '30' }">
				<%-- <c:choose> --%>
					<%-- <c:when test="${'false' eq hrRatifyRetireForm.fdIsMobileApprove}">
						<script type="text/javascript">
					...
					<%-- <c:otherwise> --%>
						<c:import url="/sys/lbpmservice/mobile/import/view.jsp" charEncoding="UTF-8">
							...
							<c:param name="onClickSubmitButton" value="Com_Submit(document.hrRatifyRetireForm, 'update');" />
						</c:import>
						<script type="text/javascript">
							require(["mui/form/ajax-form!hrRatifyRetireForm"]);
						</script>
			<%-- 		</c:otherwise>
				</c:choose> --%>
			</c:if>
			<c:if test="${hrRatifyRetireForm.docStatus eq '30'}">
				<script type="text/javascript">
			...
			<c:if test="${hrRatifyRetireForm.docStatus eq '00'}">
				<script type="text/javascript">
					require(["dojo/ready"], function(ready) {
						ready(function() {
	...
	<%-- <c:if test="${'true' eq hrRatifyRetireForm.fdIsMobileView}"> --%>
		...
		<kmss:ifModuleExist path="/third/ywork/">
			 <c:import url="/third/ywork/ywork_share/yworkDoc_mobile_share.jsp"
				charEncoding="UTF-8">
				<c:param name="modelId" value="${hrRatifyRetireForm.fdId}" />
				<c:param name="modelName" value="com.landray.kmss.hr.ratify.model.HrRatifyRetire" />
				<c:param name="templateId" value="${ hrRatifyRetireForm.docTemplateId}" />
				<c:param name="allPath" value="${ hrRatifyRetireForm.docTemplateName}" />
			</c:import>
		</kmss:ifModuleExist>
	<%-- </c:if> --%>
	
		<%-- 不支持移动端查阅 --%>
		<%-- <c:if test="${'false' eq hrRatifyRetireForm.fdIsMobileView}">
			<script type="text/javascript">
				require(["mui/dialog/BarTip", "dojo/ready"], function(BarTip, ready) {
					ready(function() {
