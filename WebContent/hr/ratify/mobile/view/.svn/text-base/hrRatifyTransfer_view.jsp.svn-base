...
<template:include ref="mobile.view" compatibleMode="true" >
	...
	<template:replace name="title">
		<c:out value="${hrRatifyTransferForm.docSubject}"></c:out>
	</template:replace>
	...
	<template:replace name="content">
		<html:form action="/hr/ratify/hr_ratify_transfer/hrRatifyTransfer.do">		
		<div id="scrollView"
	...
	<%-- 		<c:import url="/sys/agenda/mobile/edit_hidden.jsp" charEncoding="UTF-8">
			...
			<%-- 支持移动端查阅 --%>
			<%-- <c:if test="${'true' eq hrRatifyTransferForm.fdIsMobileView}"> --%>
				...
				<div class="muiFlowInfoW">
					...
					<%-- 		<c:choose> --%>
							<%-- 	<c:when test="${hrRatifyTransferForm.fdUseWord == 'true'}">
					...
					<div data-dojo-type="sys/mobile/js/mui/DelayView" id="_noteView">
						<div class="muiFormContent">
							<c:import url="/sys/lbpmservice/mobile/lbpm_audit_note/import/view.jsp" charEncoding="UTF-8">
								...
								<c:param name="formBeanName" value="hrRatifyTransferForm"/>
							</c:import>
			...
			<template:include file="/sys/lbpmservice/mobile/import/tarbar.jsp" 
							editUrl="/hr/ratify/hr_ratify_transfer/hrRatifyTransfer.do?method=edit&fdId=${param.fdId }"
							formName="hrRatifyTransferForm"
							viewName="lbpmView"
							allowReview="true">
				<template:replace name="flowArea">
				<%-- 	<c:if test="${'true' eq hrRatifyTransferForm.fdIsMobileView}">
						<c:import url="/sys/bookmark/mobile/import/view.jsp" charEncoding="UTF-8">
						  <c:param name="fdModelName" value="${hrRatifyTransferForm.modelClass.name}"/>
						  <c:param name="fdModelId" value="${hrRatifyTransferForm.fdId}"/>
						  <c:param name="fdSubject" value="${hrRatifyTransferForm.docSubject}"/>
						  <c:param name="showOption" value="label"></c:param>
						</c:import>
					</c:if> --%>
					<%--传阅 --%>
					<%-- <c:if test="${'true' eq hrRatifyTransferForm.fdCanCircularize }"> --%>
		...
		<%-- <c:if test="${'true' eq hrRatifyTransferForm.fdIsMobileView}"> --%>
			...
			<!-- 钉钉图标 end -->
			<c:if test="${hrRatifyTransferForm.docStatus < '30' }">
				<%-- <c:choose> --%>
					<%-- <c:when test="${'false' eq hrRatifyTransferForm.fdIsMobileApprove}">
						<script type="text/javascript">
			...
			<c:if test="${hrRatifyTransferForm.docStatus eq '30'}">
				<script type="text/javascript">
			...
			<c:if test="${hrRatifyTransferForm.docStatus eq '00'}">
				<script type="text/javascript">
					require(["dojo/ready"], function(ready) {
						ready(function() {
	...
	<%-- <c:if test="${'true' eq hrRatifyTransferForm.fdIsMobileView}"> --%>
		...
		<kmss:ifModuleExist path="/third/ywork/">
			 <c:import url="/third/ywork/ywork_share/yworkDoc_mobile_share.jsp"
				charEncoding="UTF-8">
				<c:param name="modelId" value="${hrRatifyTransferForm.fdId}" />
				<c:param name="modelName" value="com.landray.kmss.hr.ratify.model.HrRatifyTransfer" />
				<c:param name="templateId" value="${ hrRatifyTransferForm.docTemplateId}" />
				<c:param name="allPath" value="${ hrRatifyTransferForm.docTemplateName}" />
			</c:import>
		</kmss:ifModuleExist>
	<%-- </c:if> --%>
	
		<%-- 不支持移动端查阅 --%>
		<%-- <c:if test="${'false' eq hrRatifyTransferForm.fdIsMobileView}">
			<script type="text/javascript">
				require(["mui/dialog/BarTip", "dojo/ready"], function(BarTip, ready) {
					ready(function() {
