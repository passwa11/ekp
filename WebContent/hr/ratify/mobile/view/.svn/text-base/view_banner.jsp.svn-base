		...
		<tr>
			<td rowspan="2" class="muiFlowIconTd">
				 <div class="muiProcessIcon">
                    <img class="muiProcessImg" src='<person:headimageUrl contextPath="true"  personId="${__kmReviewMainForm.docCreatorId}" size="m" />'/>
                </div>
			...
			<td class="muiFlowSummary">
				<div>${lfn:message('hr-ratify:kmReviewTemplate.fdName') }： <xform:text property="docTemplateName"></xform:text> </div>
				<div>${lfn:message('hr-ratify:kmReviewMain.fdNumber') }： <xform:text property="docNumber"></xform:text> </div>
				<div>${lfn:message('hr-ratify:kmReviewMain.docCreateTime') }： <xform:text property="docCreateTime"></xform:text> </div>
			</td>
		</tr>
	</table>
</div>
