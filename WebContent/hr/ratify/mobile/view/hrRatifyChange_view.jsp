...
<template:include ref="mobile.view" compatibleMode="true" >
	...
	<template:replace name="content">
		<html:form action="/hr/ratify/hr_ratify_change/hrRatifyChange.do">		
		<div id="scrollView"
			...
			<c:import url="/sys/right/mobile/edit_hidden.jsp" charEncoding="UTF-8">
				...
				<c:param name="moduleModelName" value="com.landray.kmss.hr.ratify.model.HrRatifyChange" />
			</c:import>
	...
	<%-- 		<c:import url="/sys/agenda/mobile/edit_hidden.jsp" charEncoding="UTF-8">
			...
			<%-- 支持移动端查阅 --%>
			<%-- <c:if test="${'true' eq hrRatifyChangeForm.fdIsMobileView}"> --%>
				...
				<div class="muiFlowInfoW">
					...
					<%-- 		<c:choose> --%>
							<%-- 	<c:when test="${hrRatifyChangeForm.fdUseWord == 'true'}">
					...
					<div data-dojo-type="sys/mobile/js/mui/DelayView" id="_noteView">
						<div class="muiFormContent">
							<c:import url="/sys/lbpmservice/mobile/lbpm_audit_note/import/view.jsp" charEncoding="UTF-8">
								...
								<c:param name="formBeanName" value="hrRatifyChangeForm"/>
							</c:import>
			...
			<template:include file="/sys/lbpmservice/mobile/import/tarbar.jsp" 
							editUrl="/hr/ratify/hr_ratify_change/hrRatifyChange.do?method=edit&fdId=${param.fdId }"
							formName="hrRatifyChangeForm"
							viewName="lbpmView"
							allowReview="true">
				<template:replace name="flowArea">
				<%-- 	<c:if test="${'true' eq hrRatifyChangeForm.fdIsMobileView}">
						<c:import url="/sys/bookmark/mobile/import/view.jsp" charEncoding="UTF-8">
						  <c:param name="fdModelName" value="${hrRatifyChangeForm.modelClass.name}"/>
						  <c:param name="fdModelId" value="${hrRatifyChangeForm.fdId}"/>
						  <c:param name="fdSubject" value="${hrRatifyChangeForm.docSubject}"/>
						  <c:param name="showOption" value="label"></c:param>
						</c:import>
					</c:if> --%>
					<%--传阅 --%>
					<%-- <c:if test="${'true' eq hrRatifyChangeForm.fdCanCircularize }"> --%>
		...
		<%-- <c:if test="${'true' eq hrRatifyChangeForm.fdIsMobileView}"> --%>
			...
			<!-- 钉钉图标 end -->
			<c:if test="${hrRatifyChangeForm.docStatus < '30' }">
			...
			<c:if test="${hrRatifyChangeForm.docStatus eq '30'}">
				<script type="text/javascript">
			...
			<c:if test="${hrRatifyChangeForm.docStatus eq '00'}">
				<script type="text/javascript">
					require(["dojo/ready"], function(ready) {
						ready(function() {
	...
	<%-- <c:if test="${'true' eq hrRatifyChangeForm.fdIsMobileView}"> --%>
		...
		<kmss:ifModuleExist path="/third/ywork/">
			 <c:import url="/third/ywork/ywork_share/yworkDoc_mobile_share.jsp"
				charEncoding="UTF-8">
				<c:param name="modelId" value="${hrRatifyChangeForm.fdId}" />
				<c:param name="modelName" value="com.landray.kmss.hr.ratify.model.HrRatifyChange" />
				<c:param name="templateId" value="${ hrRatifyChangeForm.docTemplateId}" />
				<c:param name="allPath" value="${ hrRatifyChangeForm.docTemplateName}" />
			</c:import>
		</kmss:ifModuleExist>
