...
<c:choose>
	<c:when test="${param.contentType eq 'xform'}">
		<div>
			...
			<c:if
				test="${hrRatifyChangeForm.docUseXform == 'true' || empty hrRatifyChangeForm.docUseXform}">
			...
			<table class="tb_normal" width=100%>
				<tr>
					<td align="left" class="td_normal_title" width=15.5%>变更合同附件</td>
					<td><c:import
					...
					<tr>
						...
						<td width="35%">
							...
							<div id="_xform_docCreatorId" _xform_type="address">
								<ui:person personId="${hrRatifyChangeForm.docCreatorId}"
									personName="${hrRatifyChangeForm.docCreatorName}" />
							</div>
						</td>
						<td class="td_normal_title" width="15%">
	...
	<c:when test="${param.contentType eq 'other'}">
				...
				<c:when
					...
					<c:import url="/sys/lbpmservice/import/sysLbpmProcess_view.jsp"
						...
						<c:param name="onClickSubmitButton"
							value="Com_Submit(document.hrRatifyChangeForm, 'update');" />
					</c:import>
				</c:when>
				<c:otherwise>
					<c:import url="/sys/lbpmservice/import/sysLbpmProcess_view.jsp"
		...
		<c:import url="/sys/right/import/right_view.jsp" charEncoding="UTF-8">
			...
			<c:param name="moduleModelName"
				value="com.landray.kmss.hr.ratify.model.HrRatifyChange" />
		</c:import>
