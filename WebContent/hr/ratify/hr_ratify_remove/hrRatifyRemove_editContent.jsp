...
<c:choose>
	<c:when test="${param.contentType eq 'xform'}">
		<ui:content title="${lfn:message('hr-ratify:py.BiaoDanNeiRong')}"
				...
				<tr>
					...
					<td style="border-left: 0px !important;">
						<div id="_xform_docSubject" _xform_type="text">
							<c:if
								test="${hrRatifyRemoveForm.titleRegulation==null || hrRatifyRemoveForm.titleRegulation=='' }">
			...
			<c:if
				test="${hrRatifyRemoveForm.docUseXform == 'true' || empty hrRatifyRemoveForm.docUseXform}">
				...
				<tr>
					...
					<td width="35%">
						...
						<div id="_xform_docCreatorId" _xform_type="address">
							<ui:person personId="${hrRatifyRemoveForm.docCreatorId}"
								personName="${hrRatifyRemoveForm.docCreatorName}" />
						</div>
					</td>
					<td class="td_normal_title" width="15%">
					...
					<td width="35%">
						...
						<div id="_xform_docNumber" _xform_type="text">
							<c:if test="${not empty hrRatifyRemoveForm.docNumber}">
								<xform:text property="docNumber" showStatus="readOnly"
									style="width:95%;" />
							</c:if>
							<c:if test="${empty hrRatifyRemoveForm.docNumber}">
	...
	<c:when test="${param.contentType eq 'other'}">
		...
		<c:import url="/sys/right/import/right_edit.jsp" charEncoding="UTF-8">
			...
			<c:param name="moduleModelName"
				value="com.landray.kmss.hr.ratify.model.HrRatifyRemove" />
		</c:import>
	</c:when>
</c:choose>