...
<c:if test="${hrRatifyRetireForm.method_GET == 'edit' || (param['i.docTemplate']!=null && param['i.docTemplate']!='')}">
<template:replace name="content">
	<c:import url="/sys/recycle/import/redirect.jsp">
		<c:param name="formBeanName" value="hrRatifyRetireForm"></c:param>
	</c:import>
	<c:if test="${param.approveModel ne 'right'}">
	<form name="hrRatifyRetireForm" method="post" action ="${KMSS_Parameter_ContextPath}hr/ratify/hr_ratify_retire/hrRatifyRetire.do">
    ...
    <c:choose>
		<c:when test="${param.approveModel eq 'right'}">
			<ui:tabpage collapsed="true" id="reviewTabPage">
				...
				<c:import url="/hr/ratify/hr_ratify_retire/hrRatifyRetire_editContent.jsp" charEncoding="UTF-8">
	...
	seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'],function($, dialog, topic){
		window.addDeptPost = function(obj){
			var fdRetireDept = Address_GetAddressObj('fdRetireDeptName');
			var fdRetireEnterDate = $('input[name="fdRetireEnterDate"]')[0];
				...
				$.ajax({
					...
					success : function(data, textStatus, xhr){
						...
						if(fdRetireDept && d.dept){
							...
							fdRetireDept.reset(";",ORG_TYPE_ORGORDEPT,false,values);
						}
						if (fdRetireEnterDate) {
							$('input[name="fdRetireEnterDate"]').val(d.entryTime);
						}
					}
				});
			}else{
				if(fdRetireDept)
					fdRetireDept.emptyAddress();
				if(fdRetireEnterDate)
					fdRetireEnterDate.value = '';
			}
		};
	});
