			...
			seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic) {
				...
				window.clickOk = function(){
					...
					if(validation.validate()){
						__isSubmit = true;
						var ids = $("#ids").val();
						var url = '<c:url value="/hr/organization/hr_organization_post_seq/hrOrganizationPostSeq.do?method=delete"/>' + "&fdId="+ids;
						window.del_load = dialog.loading();
						$.ajax({
							url : url,
							type : 'GET',
							data : $(document.hrOrganizationPostSeqForm).serialize(),
							error : function(data) {
