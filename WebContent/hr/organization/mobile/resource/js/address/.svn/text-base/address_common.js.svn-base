define([
	...
	function tmplList(params){
		...
		mixins = mixins.join(',');
		var html = 
			'<ul data-dojo-type="hr/organization/mobile/resource/js/address/AddressList" ' + 
				'data-dojo-mixins="'+ mixins +'" ' + 
				"data-dojo-props=\"deptLimit:'{categroy.deptLimit}',beforeSelectCateHistoryId: '{categroy.beforeSelectCateHistoryId}',isMul:{categroy.isMul},key:'{categroy.key}'," + "channel:'" + params.channel +"',curIds:'{categroy.curIds}',curNames:'{categroy.curNames}',selType:{categroy.type},exceptValue:'{categroy.exceptValue}',scope:'{categroy.scope}'\">" +
			'</ul>';
		return html;
	}
	...
	function tmplLoad(params) {
		...
		html += h('div', {
			...
			'<ul data-dojo-type="hr/organization/mobile/resource/js/address/AddressList" ' +
				...
				"data-dojo-props=\"history:true,deptLimit:'{categroy.deptLimit}',beforeSelectCateHistoryId: '{categroy.beforeSelectCateHistoryId}',isMul:{categroy.isMul},key:'{categroy.key}',channel:'search',curIds:'{categroy.curIds}',curNames:'{categroy.curNames}',selType:{categroy.type},orgType:{categroy.type},exceptValue:'{categroy.exceptValue}',scope:'{categroy.scope}'\">" +
			'</ul>' 
		]);
