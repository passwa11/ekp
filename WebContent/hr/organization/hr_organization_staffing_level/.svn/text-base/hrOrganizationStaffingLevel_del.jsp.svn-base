			...
			seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic) {
				...
				window.clickOk = function(){
					...
					if(validation.validate()){
						__isSubmit = true;
						var ids = $("#ids").val();
						var url = '<c:url value="/hr/organization/hr_organization_staffing_level/hrOrganizationStaffingLevel.do?method=delete"/>' + "&fdId="+ids;
						window.del_load = dialog.loading();
						$.ajax({
							url : url,
							type : 'GET',
							data : $(document.sysOrganizationStaffingLevelForm).serialize(),
							/* dataType : 'json', */
							error : function(data) {
								console.log(data);
