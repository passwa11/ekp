...
function GetOrgChartAjax( fdId, sceneId, load_callback ){
	...
	var url = Com_Parameter.ContextPath + "hr/organization/hr_organization_chart/hrOrganizationChart.do?method=GetAllList";
	$.ajax({
    	type: "post",
        url: url,
        dataType: "json",
...
function openDept(fdId, orgType){
	Com_OpenWindow('../../../hr/organization/hr_organization_element/hrOrganizationElement.do?method=view&fdId='+fdId);
}
function openPerson(fdId){
	if (fdId == "" || fdId == undefined || fdId == null){
		// do nothing
	}else{
		Com_OpenWindow('../../../hr/staff/hr_staff_person_info/hrStaffPersonInfo.do?method=view&fdId='+fdId);
	}
}
