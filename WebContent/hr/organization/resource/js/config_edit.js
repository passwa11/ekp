...
function dialogSelect(mul, key, idField, nameField, action){
	...
	if(dialogCfg){
				...
				if(argu["value"].indexOf('*')>-1){
					...
					if(modelVal==null||modelVal==''){
						if(argu["required"]=="true"){
							var errorInfo=$lang.error2.replace("1",argu["label"]);
							alert(errorInfo);
							return;
						}
					}else{
		...
		// 有可能存在打开页面后没有任何选择的情况，比如：#168731
		var _initIdValue = null;
		var _initNameValue = null;
		dialog[propKey] = function(){
			if(idField.indexOf('*')>-1){
				var initField=idField.replace('*',rowIndex);
				_initIdValue = document.getElementsByName(initField)[0].value;
				_initNameValue = document.getElementsByName(nameField.replace('*',rowIndex))[0].value;
				return {url:source+params,init:document.getElementsByName(initField)[0].value};
			}else{
				_initIdValue = document.getElementsByName(idField)[0].value;
				_initNameValue = document.getElementsByName(nameField)[0].value;
		...
		dialog[propKey] = function(rtnInfo){
			...
			for(var i=0;i<datas.length;i++){
				...
				if (rowData[rtnInfo.idField] || rowData[rtnInfo.nameField]){
			...
			if(idField.indexOf('*')>-1){
				...
				if (ids.length==0 || names.length==0){
					//主表
					$form(idField).val(_initIdValue);
					$form(nameField).val(_initNameValue);
				}else{
					//主表
...
function validateDetail(){
	...
	if(details!=null){
		for(var i=0;i<details.length;i++){
			...
			if(trSize<=2){
				dialog.alert($lang.error.replace("1",message));
				return false;
			}else{
				var flag=true;
