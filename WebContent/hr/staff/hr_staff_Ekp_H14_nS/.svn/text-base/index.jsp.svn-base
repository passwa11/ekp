...
<template:replace name="title">
	...
	<template:replace name="path">
		<ui:menu layout="sys.ui.menu.nav"  id="categoryId"> 
			...
			<ui:menu-item text="${ lfn:message('hr-staff:table.hrStaffEkp_H14_nS')}"></ui:menu-item>
		</ui:menu>
	</template:replace>
	<template:replace name="nav">
		<ui:combin ref="menu.nav.create">
			...
			<ui:varParam name="button">
				[
					{
						"text": "${ lfn:message('hr-staff:table.hrStaffEkp_H14_nS') }",
						"href": "javascript:add();",
	...
	<script type="text/javascript">
		...
		<list:criteria id="criteria">
			<list:cri-ref key="_fdKey" ref="criterion.sys.docSubject" title="${ lfn:message('hr-staff:hrStaffEkp_H14_nS.criteria.fdKey') }" style="width:400px;">
			</list:cri-ref>
...
<%-- 						<ui:button text="${lfn:message('button.deleteall') }" onclick="_delete();" order="4"></ui:button> --%>
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
				{url:'/hr/staff/hr_staff_Ekp_H14_nS/hrStaffEkp_H14_nS.do?method=list'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false" layout="sys.ui.listview.columntable" 
				rowHref="/hr/staff/hr_staff_Ekp_H14_nS/hrStaffEkp_H14_nS.do?method=view&fdId=!{fdId}&anchor=Ekp_H14_nS" name="columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial> 
				<list:col-auto props="nameAccount;fdBeiKaoHeRenXingMing;fdYiJiBuMen;fdErJiBuMen;fdZhuYaoGongZuoYeJi;fdZiWoPingJia;fdJiXiaoDengJi;
				;fdJiXiaoPingYu;fdShangJiPingJiaDeFen;fdBeiKaoHeRenGangWei;fdJiDuJiXiaoDengJi;fdNianDuJiXiaoDengJi;
				;fdChengJiYuBuZu;fdKaoHeJieShuShiJian;fdKaoHeKaiShiShiJian;
				;fdFenQiDian;fdMianTanJieGuo;fdQiWangJiGaiShan;fdYouShiYuLieShi;fdGaiJinShiXiang;
		...
		<!-- 模板下载表单 -->
		<form id="downloadTempletForm" action="${LUI_ContextPath}/hr/staff/hr_staff_Ekp_H14_nS/hrStaffEkp_H14_nS.do?method=downloadTemplet" method="post"></form>
		...
		<script type="text/javascript">
			seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic) {
				...
				// 新建
				window.add = function() {
					Com_OpenWindow('<c:url value="/hr/staff/hr_staff_Ekp_H14_nS/hrStaffEkp_H14_nS.do" />?method=add');
				};

				// 编辑
				window.edit = function(id) {
					Com_OpenWindow('<c:url value="/hr/staff/hr_staff_Ekp_H14_nS/hrStaffEkp_H14_nS.do" />?method=edit&fdId=' + id);
				};
				function ajax(callback){
					$.ajax({
						type :'POST',
						url : '${LUI_ContextPath}/hr/staff/hr_staff_Ekp_H14_nS/hrStaffEkp_H14_nS.do?method=downloadAjax',
				...
				function downTemp(){
	                ...
	                form.attr("action","${LUI_ContextPath}/hr/staff/hr_staff_Ekp_H14_nS/hrStaffEkp_H14_nS.do?method=exportPerson");  
	                var input1=$("<input>");  
				...
				window.listExport = function (){
					ajax(downTemp);
// 					$.ajax({
// 						type :'POST',
// 						url : '${LUI_ContextPath}/hr/staff/hr_staff_Ekp_H14_nS/hrStaffEkp_H14_nS.do?method=listExport',
// 						success : function(res){
// 							if(res == 'true'){
// 								callback();
// 							}else{
...
// 					});
				...
				window._modify = function() {
					var actionUrl = "${LUI_ContextPath}/hr/staff/hr_staff_Ekp_H14_nS/hrStaffEkp_H14_nS.do?method=fileUpload";
					var downLoadUrl = "${LUI_ContextPath}/hr/staff/hr_staff_Ekp_H14_nS/hrStaffEkp_H14_nS.do?method=downloadTemplet";
					dialog.iframe('/hr/staff/upload_files/common_upload_download.jsp?uploadActionUrl=' + actionUrl + '&isRollBack=false'+'&downLoadUrl='+downLoadUrl, 
							'<bean:message key="table.hrStaffEkp_H14_nS" bundle="hr-staff"/>', function(data) {
							topic.publish('list.refresh');
					}, {
						width : 680,
						height : 380
				...
				window._delete = function(id) {
					...
					dialog.confirm('<bean:message key="page.comfirmDelete"/>', function(value) {
						if(value == true) {
							...
							$.ajax({
								url : "${LUI_ContextPath}/hr/staff/hr_staff_Ekp_H14_nS/hrStaffEkp_H14_nS.do?method=deleteall",
								type : 'POST',
