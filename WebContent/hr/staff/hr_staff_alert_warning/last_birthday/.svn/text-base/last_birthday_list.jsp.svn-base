...
<list:data>
	<list:data-columns var="hrStaffPersonInfo" list="${queryPage.list }" varIndex="status">
		...
		<!--生日-->
		<list:data-column headerClass="width80" col="fdBirthday" title="${ lfn:message('hr-staff:hrStaffLastBirthday.birthday') }" > 
		     <c:if test="${hrStaffPersonInfo.fdDateOfBirth != null}">
	    		<sunbor:date value="${hrStaffPersonInfo.fdDateOfBirth}" datePattern="MM-dd"></sunbor:date>
	    	</c:if>
		</list:data-column>
	</list:data-columns>
