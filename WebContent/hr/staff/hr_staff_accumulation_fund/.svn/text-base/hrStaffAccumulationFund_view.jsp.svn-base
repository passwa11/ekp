...
<template:include ref="default.view" sidebar="no">
	...
	<template:replace name="toolbar">
		...
		<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float">
			<%--删除--%>
			<ui:button text="${lfn:message('button.delete') }" onclick="deleteDoc('hrStaffAccumulationFund.do?method=delete&fdId=${param.fdId}');" order="4">
			</ui:button>
			<%--关闭--%>
		...
		<html:form action="/hr/staff/hr_staff_accumulation_fund/hrStaffAccumulationFund.do">		
			...
			<div class="lui_form_content_frame">
				<p class="lui_form_subject">
					<bean:message bundle="hr-staff" key="table.hrStaffAccumulationFund" />
				</p>
