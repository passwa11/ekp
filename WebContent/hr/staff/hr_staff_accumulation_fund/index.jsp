...
<template:replace name="title">
	...
	<template:replace name="path">
		<ui:menu layout="sys.ui.menu.nav"  id="categoryId"> 
			...
			<ui:menu-item text="${ lfn:message('hr-staff:table.hrStaffAccumulationFund')}"></ui:menu-item>
		</ui:menu>
	</template:replace>
	<template:replace name="nav">
		<ui:combin ref="menu.nav.create">
			...
			<ui:varParam name="button">
				[
					{
						"text": "${ lfn:message('hr-staff:table.hrStaffAccumulationFund') }",
						"href": "javascript:add();",
	...
	<script type="text/javascript">
		...
		<list:criteria id="criteria">
			<list:cri-ref key="_fdKey" ref="criterion.sys.docSubject" title="${ lfn:message('hr-staff:hrStaffAccumulationFund.criteria.fdKey') }" style="width:400px;">
			</list:cri-ref>
			...
			<list:cri-ref key="fdDeliveryDate" ref="criterion.sys.calendar" title="${ lfn:message('hr-staff:hrStaffAccumulationFund.fdDeliveryDate') }"></list:cri-ref>
		</list:criteria>
		<!-- 排序 -->
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
				{url:'/hr/staff/hr_staff_accumulation_fund/hrStaffAccumulationFund.do?method=list'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false" layout="sys.ui.listview.columntable" 
				rowHref="/hr/staff/hr_staff_accumulation_fund/hrStaffAccumulationFund.do?method=view&fdId=!{fdId}&anchor=accumulationFund" name="columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial> 
				<list:col-auto props="fdDeliveryDate;fdIndividualProvidentFundAccount;nameAccount;fdIdCard;staffId;fdProvidentFundInsuranceCompany;fdPaymentBase;fdPersonalDelivery;fdUnitDelivery;fdDeliveryAmount;fdRemark;fdPlaceOfInsurancePayment;fdAmountReceivableTotalReceivable;fdAmountReceivablePersonalTotal;fdAmountReceivableUnitTotal;
				fdSocialInsuranceCompany;fdProvidentFundInsuranceCompany;
				fdIndustrialAndCommercialInsurancePaymentBase;fdUnemploymentInsurancePaymentBase;fdUnemploymentInsuranceUnitDelivery;
				fdReproductiveMedicinePaymentBase;fdReproductiveMedicinePersonalDelivery;fdReproductiveMedicineUnitDelivery;fdStaffId;operations"></list:col-auto>
			</list:colTable>
		</list:listview> 
		<list:paging></list:paging>
		
		<!-- 模板下载表单 -->
		<form id="downloadTempletForm" action="${LUI_ContextPath}/hr/staff/hr_staff_accumulation_fund/hrStaffAccumulationFund.do?method=downloadTemplet" method="post"></form>
		...
		<script type="text/javascript">
			seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic) {
				...
				// 新建
				window.add = function() {
					Com_OpenWindow('<c:url value="/hr/staff/hr_staff_accumulation_fund/hrStaffAccumulationFund.do" />?method=add');
				};

				// 编辑
				window.edit = function(id) {
					Com_OpenWindow('<c:url value="/hr/staff/hr_staff_accumulation_fund/hrStaffAccumulationFund.do" />?method=edit&fdId=' + id);
				};
			
				// 导入
				window._modify = function() {
					var actionUrl = "${LUI_ContextPath}/hr/staff/hr_staff_accumulation_fund/hrStaffAccumulationFund.do?method=fileUpload";
					var downLoadUrl = "${LUI_ContextPath}/hr/staff/hr_staff_accumulation_fund/hrStaffAccumulationFund.do?method=downloadTemplet";
					dialog.iframe('/hr/staff/upload_files/common_upload_download.jsp?uploadActionUrl=' + actionUrl + '&isRollBack=false'+'&downLoadUrl='+downLoadUrl, 
							'<bean:message key="table.hrStaffAccumulationFund" bundle="hr-staff"/>', function(data) {
							topic.publish('list.refresh');
					}, {
						width : 680,
						height : 380
				...
				window._delete = function(id) {
					...
					dialog.confirm('<bean:message key="page.comfirmDelete"/>', function(value) {
						if(value == true) {
							...
							$.ajax({
								url : "${LUI_ContextPath}/hr/staff/hr_staff_accumulation_fund/hrStaffAccumulationFund.do?method=deleteall",
								type : 'POST',
