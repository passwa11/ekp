...
<link type="text/css" rel="stylesheet" href="${LUI_ContextPath}/hr/staff/resource/css/maincss.css?s_cache=${LUI_Cache}"/>
<link type="text/css" rel="stylesheet" href="${LUI_ContextPath}/hr/staff/resource/css/common.css?s_cache=${LUI_Cache}"/>
	<template:replace name="content">
			...
			<div class="lui_batch_process_container">
			    <div class="lui_batch_process-content">
			      ...
			      <div class="lui_batch_process-content-confirm">
			        <button id="submitBtn" onclick="_submit();">${lfn:message('hr-staff:hrStaff.import.button.submit')}</button>
			      </div>
		...
		<script type="text/javascript">
			...
			seajs.use([ 'lui/jquery','lui/dialog'],function($, dialog) {
				$(function() { 
					var uploadActionUrl ="${fn:escapeXml(JsParam.uploadActionUrl)}";   
					var downLoadUrl = "${fn:escapeXml(JsParam.downLoadUrl)}";
					console.info(uploadActionUrl)
					if(uploadActionUrl.length < 1) {
						uploadActionUrl = "${fn:escapeXml(uploadActionUrl)}";
					}
					if(downLoadUrl.length < 1) {
						downLoadUrl ="${fn:escapeXml(downLoadUrl)}";
					}
					...
					var isRollBack = "${fn:escapeXml(JsParam.isRollBack)}";
					if(isRollBack.length < 1) {
						isRollBack = "${fn:escapeXml(isRollBack)}";
					}
