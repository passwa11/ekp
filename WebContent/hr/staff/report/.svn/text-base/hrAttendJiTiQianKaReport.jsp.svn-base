...
<script language="JavaScript">
    ...
    function exportResult() {
        let url = makeSrc('exportHrAttendJiTiQianKaReport');
        //alert(url)
        window.open(url);
    }
</script>
<form action="hrAttendJiTiQianKaReport.jsp" method="post"
	...
	</select><span>月</span> <input type="submit" value="查询" style="margin-right: 20px 100px;">
...
<table class='hovertable' style="text-align: center; margin: auto;">
	...
	<%
	HrAttendJiTiQianKaReport service = new HrAttendJiTiQianKaReport();
	...
	<tr onmouseover="this.style.backgroundColor='#ffff66';"
		...
		<td>
			<!--上班卡--><%=map.get("fd_end_month_count")%></td>
		<td>
			<!--下班卡--><%=map.get("fd_new_month_count")%></td>
		<td>
			<!--签卡次数--><%=map.get("fd_leave_month_count")%></td>
		<td>
			<!--签卡原因--><%=map.get("fd_in_month_count")%></td>
		<%
			}
		%>
	</tr>
