...
<template:include ref="default.edit" sidebar="auto">
	<template:replace name="title">
		<c:choose>
			<c:when test="${ hrStaffPerformanceSearchForm.method_GET == 'add' }">
				<c:out value="${ lfn:message('button.add') } - ${ lfn:message('hr-staff:table.hrStaffPerformanceSearch') }"></c:out>	
			</c:when>
			<c:otherwise>
				${ hrStaffPerformanceSearchForm.fdPersonInfoName } - ${ lfn:message('hr-staff:table.hrStaffPerformanceSearch') }
			</c:otherwise>
		</c:choose>
	</template:replace>
	<template:replace name="toolbar">
		<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
			<c:if test="${ hrStaffPerformanceSearchForm.method_GET == 'add' }">
				<ui:button text="${lfn:message('button.submit')}" 
					onclick="Com_Submit(document.hrStaffPerformanceSearchForm, 'save');" order="1">
				</ui:button>
			</c:if>
			<c:if test="${ hrStaffPerformanceSearchForm.method_GET == 'edit' }">
				<ui:button text="${lfn:message('button.save')}" 
					onclick="Com_Submit(document.hrStaffPerformanceSearchForm, 'update');" order="1">
				</ui:button>
			</c:if>
	...
	<template:replace name="path">
		<ui:menu layout="sys.ui.menu.nav"  id="categoryId"> 
			...
			<ui:menu-item text="${ lfn:message('hr-staff:table.hrStaffPerformanceSearch') }" href="/hr/staff/hr_staff_performance_search/" target="_self"></ui:menu-item>
		</ui:menu>
	</template:replace>	
	<template:replace name="content">
		<div class="lui_form_content_frame">
		<html:form action="/hr/staff/hr_staff_performance_search/hrStaffPerformanceSearch.do" >
			...
			<table class="tb_normal" width=98%>
				<tr>
					...
					<td width="35%">
						<c:choose>
							<c:when test="${ hrStaffPerformanceSearchForm.method_GET == 'add' }">
								<xform:address propertyId="fdOrgPersonId" idValue="${ hrStaffPerformanceSearchForm.fdPersonInfoId }" 
								propertyName="fdOrgPersonName" nameValue="${ hrStaffPerformanceSearchForm.fdPersonInfoName }" 
					...
					<!-- 工资账户名 -->
					<td width="15%" class="td_normal_title">
						<bean:message bundle="hr-staff" key="hrStaffPerformanceSearch.fdFirstLevelDepartment" />
					</td>
					<td width="35%">
				...
				<tr>
					...
					<td width="15%" class="td_normal_title">
						<bean:message bundle="hr-staff" key="hrStaffPerformanceSearch.fdSecondLevelDepartment" />
					</td>
					<td width="35%">
					...
					<!-- 公积金账户 -->
					<td width="15%" class="td_normal_title">
						<bean:message bundle="hr-staff" key="hrStaffPerformanceSearch.fdThirdLevelDepartment" />
					</td>
					<td width="35%">
				...
				<tr>
					<!-- 公积金账户 -->
					<td width="15%" class="td_normal_title">
						<bean:message bundle="hr-staff" key="hrStaffPerformanceSearch.fdExpiryDate" />
					</td>
					<td width="35%">
					...
					<!-- 公积金账户 -->
					<td width="15%" class="td_normal_title">
						<bean:message bundle="hr-staff" key="hrStaffPerformanceSearch.fdEvaluationDimension" />
					</td>
					<td width="35%">
				...
				<tr>
					<!-- 公积金账户 -->
					<td width="15%" class="td_normal_title">
						<bean:message bundle="hr-staff" key="hrStaffPerformanceSearch.fdEvaluationIndex" />
					</td>
					<td width="35%">
					...
					<!-- 公积金账户 -->
					<td width="15%" class="td_normal_title">
						<bean:message bundle="hr-staff" key="hrStaffPerformanceSearch.fdTargetValue" />
					</td>
					<td width="35%">
				...
				<tr>
					<!-- 公积金账户 -->
					<td width="15%" class="td_normal_title">
						<bean:message bundle="hr-staff" key="hrStaffPerformanceSearch.fdWeight" />
					</td>
					<td width="35%">
					...
					<!-- 公积金账户 -->
					<td width="15%" class="td_normal_title">
						<bean:message bundle="hr-staff" key="hrStaffPerformanceSearch.fdBeginDate" />
					</td>
					<td width="35%">
				...
				<tr>
					<!-- 公积金账户 -->
					<td width="15%" class="td_normal_title">
						<bean:message bundle="hr-staff" key="hrStaffPerformanceSearch.fdJobNature" />
					</td>
					<td width="35%">
		...
		<script language="JavaScript">
			$KMSSValidation(document.forms['hrStaffPerformanceSearchForm']);

			// 修改员工信息回调
			function personInfoChange(value) {
				var fdPersonInfoId = value[0];
				$("input[name=fdPersonInfoId]").val(fdPersonInfoId);
				$.post("${LUI_ContextPath}/hr/staff/hr_staff_performance_search/hrStaffPerformanceSearch.do?method=checkPerson",
