define(function(require, exports, module) {
		...
		var dialogObj = dialog.build({
			config:{
				...
				content : {
					...
					scroll : false,
					type : "iframe",
					url : '/eop/basedata/resource/jsp/common-template.jsp?_key=template_select',
					params:null
				}
			}
		});
	...
	var dialogSelect = function(modelName, mulSelect, source, params, idField, nameField, winTitle, action){
		var subject=$("[name='"+nameField.replace("*",0)+"']").attr("subject");
		var nameTitle="选择"+(subject?subject:"");
		var buttons = [ {
			...
			fn : function(value, dialog) {
				...
				if(mulSelect&&window.buildMultiSelector){
					var fields = rtfield(idField, nameField);
					window.buildMultiSelector(fields.idObj.parentNode);
				}
				if(action){
					action(rtnDatas);
				}
		...
		var tempUrl = '/eop/basedata/resource/jsp/common-template.jsp?modelName=' + modelName + '&_key=dialog_' + idField+'&_name='+nameField;
