var dialog_,lang_;
seajs.use(['lui/jquery','lui/dialog','eop/basedata/resource/js/dialog_common','lui/util/str','lang!eop-basedata'], function($, dialog, dialogCommon,strutil,lang){
	dialog_=dialog;
	lang_=lang;
...
function validateDetail(){
	...
	if(details!=null){
		for(var i=0;i<details.length;i++){
			...
			if(trSize<=2){
				dialog_.alert("明细表【"+message+"】不能为空");
				return false;
			}else{
				var flag=true;
