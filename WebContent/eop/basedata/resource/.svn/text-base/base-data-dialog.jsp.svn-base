...
<%
	...
	if(dictModel!=null){
        ...
        for (SysDictCommonProperty sysDictCommonProperty : propertyList) {
            String name = sysDictCommonProperty.getName();
            if("fdName".equals(name)){
                displayProperty = "fdName";
            }
            if("docSubject".equals(name)){
        ...
        pageContext.setAttribute("nameField", displayProperty);
	}
%>
			...
			function _source() {
				...
				if(typeof(tempUrl)=="string"){
					...
					if(tempUrl.init.length>0){
						...
						// 	url : ajaxUrl,
						// 	type : 'get',
						// 	//data : {fdId:tempUrl.init,rowsize:init.length,order:'',where:''},
						// 	async : true,           
						// 	error : function(){
						// 	} ,   
