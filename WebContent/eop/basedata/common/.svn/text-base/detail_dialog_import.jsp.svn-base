	...
	<template:replace name="content">
		<div style="position:relative;padding:7px 25px 5px 0px;text-align: right">
			<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
				<ui:button text="${ lfn:message('eop-basedata:detail.transport.templateDownLoad') }" onclick="exportTemplate()" order="1">
				</ui:button>
				<ui:button text="${ lfn:message('eop-basedata:detail.transport.upLoad') }" onclick="_submit()" order="2">
				</ui:button>
		...
		<form action="" method="post" enctype="multipart/form-data">
			...
			<table class="tb_normal" style="margin: 20px 0" width=98%>
				<tr>
					...
					<td colspan="3">
						<input id="docFile" class="input_file" type="file" name="file" accept=".xls,.xlsx" enctype = 'multipart/form-data'/>
					</td>
				</tr>
				<tr>
				...
				<td>
					<input type="radio" checked="checked" name="fdImportType" value="cover"
						 title='${ lfn:message('eop-basedata:detail.transport.cover') }'/>
						${ lfn:message('eop-basedata:detail.transport.cover') }&nbsp;
					<input type="radio" name="fdImportType" value="append"
						title='${ lfn:message('eop-basedata:detail.transport.append') }'/>
						${ lfn:message('eop-basedata:detail.transport.append') }&nbsp;
				</td>
				</tr>
				
				<tr>
					<td colspan="2">
						${ lfn:message('eop-basedata:detail.transport.result') }：<span style="color:red" id="result"></span>
					</td>
				</tr>
				<tr>
				<!-- 详情 -->
...
seajs.use([ 'lui/jquery','lui/dialog'],function($, dialog) {
	...
	window._submit = function() {
		...
		form.append('fdImportType',$("input[name='fdImportType'][type='radio']:checked").val());
		var success=true;
		message = "";
		var fileInput = $('.input_file').get(0).files[0];
		if(!fileInput){
			$("#result").text("${ lfn:message('eop-basedata:detail.transport.result.failed') }");
			$("#div_errorCell").empty();
			$("#div_errorCell").append("${ lfn:message('eop-basedata:detail.transport.result.fileEmpty') }");
			return;
		}
		...
		$.ajax({
			...
			success:function(data) {
				...
				if(!success){
					...
					$("#result").text("${ lfn:message('eop-basedata:detail.transport.result.success') }");
				}
			}
		});
	}
