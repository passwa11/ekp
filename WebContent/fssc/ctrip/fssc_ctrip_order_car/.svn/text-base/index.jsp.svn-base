...
<template:include ref="default.simple4list">
    <template:replace name="title">
        <c:out value="${ lfn:message('fssc-ctrip:module.fssc.ctrip') }-${ lfn:message('fssc-ctrip:table.fsscCtripOrderCar') }" />
    </template:replace>
    <template:replace name="path">
        <ui:menu layout="sys.ui.menu.nav">
            <ui:menu-item text="${ lfn:message('home.home') }" icon="lui_icon_s_home" href="/" target="_top" />
            <ui:menu-item text="${ lfn:message('fssc-ctrip:table.fsscCtripOrderCar') }" />
        </ui:menu>
    </template:replace>
    <template:replace name="nav">
        <ui:combin ref="menu.nav.create">
            <ui:varParam name="title" value="${ lfn:message('fssc-ctrip:table.fsscCtripOrderCar') }" />
            <ui:varParam name="button">
                [ {"text": "","href": "javascript:void(0);","icon": "/fssc/ctrip/fssc_ctrip_order_car/fsscCtripOrderCar.do"} ]
            </ui:varParam>
        </ui:combin>
    ...
    <template:replace name="content">
        <div style="margin:5px 10px;">
                ...
                <div style="float:right">
                    <div style="display: inline-block;vertical-align: middle;">
                        <ui:toolbar count="3">
                            ...
                            <kmss:auth requestURL="/fssc/ctrip/fssc_ctrip_order_car/fsscCtripOrderCar.do?method=deleteall">
            ...
            <list:listview id="listview">
                <ui:source type="AjaxJson">
                    {url:appendQueryParameter('/fssc/ctrip/fssc_ctrip_order_car/fsscCtripOrderCar.do?method=data')}
                </ui:source>
                <!-- 列表视图 -->
                <list:colTable isDefault="false" rowHref="/fssc/ctrip/fssc_ctrip_order_car/fsscCtripOrderCar.do?method=view&fdId=!{fdId}" name="columntable">
                    <list:col-checkbox />
                    <list:col-serial/>
                    <list:col-auto props="fdOrderId;fdTripId;fdOrderAmount;fdDealAmount;fdServiceFee;fdOrderDetailStatus;fdPaymentStatus" url="" /></list:colTable>
            </list:listview>
            <!-- 翻页 -->
            <list:paging />
        </div>
        <script>
            var listOption = {
                ...
                jPath: 'carOrder',
                modelName: 'com.landray.kmss.fssc.ctrip.model.FsscCtripOrderCar',
                templateName: '',
