...
<template:include ref="default.simple4list">
    <template:replace name="title">
        <c:out value="${ lfn:message('fssc-ctrip:module.fssc.ctrip') }-${ lfn:message('fssc-ctrip:table.fsscCtripTrainSettleInfo') }" />
    </template:replace>
    <template:replace name="nav">
        <ui:combin ref="menu.nav.create">
            <ui:varParam name="title" value="${ lfn:message('fssc-ctrip:table.fsscCtripTrainSettleInfo') }" />
            <ui:varParam name="button">
                [ {"text": "","href": "javascript:void(0);","icon": "/fssc/ctrip/fssc_ctrip_train_settle_info/fsscCtripTrainSettleInfo.do"} ]
            </ui:varParam>
        </ui:combin>
    ...
    <template:replace name="content">
        <div style="margin:5px 10px;">
            ...
            <list:listview id="listview">
                <ui:source type="AjaxJson">
                    {url:appendQueryParameter('/fssc/ctrip/fssc_ctrip_train_settle_info/fsscCtripTrainSettleInfo.do?method=data')}
                </ui:source>
                <!-- 列表视图 -->
                <list:colTable isDefault="false" rowHref="/fssc/ctrip/fssc_ctrip_train_settle_info/fsscCtripTrainSettleInfo.do?method=view&fdId=!{fdId}" name="columntable">
                    <list:col-checkbox />
                    <list:col-serial/>
        ...
        <script>
            var listOption = {
                ...
                jPath: 'trainSettle',
                modelName: 'com.landray.kmss.fssc.ctrip.model.FsscCtripTrainSettleInfo',
                templateName: '',
