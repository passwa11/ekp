	...
	<template:replace name="content">
		<div style="position:relative;padding:7px 25px 5px 0px;text-align: right">
			<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
				<ui:button text="${ lfn:message('fssc-ctrip:detail.transport.templateDownLoad') }" onclick="exportTemplate()" order="1">
				</ui:button>
				<ui:button text="${ lfn:message('fssc-ctrip:detail.transport.upLoad') }" onclick="_submit()" order="2">
				</ui:button>
			...
			<table class="tb_normal" style="margin: 20px 0" width=98%>
				...
				<td>
					<input type="radio" checked="checked" name="fdImportType" value="cover"
						 title='${ lfn:message('fssc-ctrip:detail.transport.cover') }'/>
						${ lfn:message('fssc-ctrip:detail.transport.cover') }&nbsp;
					<input type="radio" name="fdImportType" value="append"
						title='${ lfn:message('fssc-ctrip:detail.transport.append') }'/>
						${ lfn:message('fssc-ctrip:detail.transport.append') }&nbsp;
				</td>
				</tr>
				
				<tr>
					<td colspan="2">
						${ lfn:message('fssc-ctrip:detail.transport.result') }：<span style="color:red" id="result"></span>
					</td>
				</tr>
				<tr>
				<!-- 详情 -->
...
seajs.use([ 'lui/jquery','lui/dialog'],function($, dialog) {
	...
	window._submit = function() {
		...
		if(!fileInput){
			$("#result").text("${ lfn:message('fssc-ctrip:detail.transport.result.failed') }");
			$("#div_errorCell").empty();
			$("#div_errorCell").append("${ lfn:message('fssc-ctrip:detail.transport.result.fileEmpty') }");
			return;
		}
		...
		$.ajax({
			...
			success:function(data) {
				...
				if(!success){
					...
					$("#result").text("${ lfn:message('fssc-ctrip:detail.transport.result.success') }");
				}
			}
		});
	}
