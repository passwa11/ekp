					...
					<script>
						function generateTree(){
							LKSTree = new TreeView(
								"LKSTree",
								'${lfn:message("fssc-ctrip:fssc.ctrip.select.city")}',
								document.getElementById("treeDiv")
							);
							LKSTree.ClickNode = function(id){
								node = Tree_GetNodeByID(LKSTree.treeRoot, id);
								var id = node.parameter[0];
								LUI('listview1').source.setUrl('/fssc/ctrip/fssc_ctrip_city/fsscCtripCity.do?method=listCity');
							...
							data.AddBeanData("fsscCtripCityService&fdCompanyId=${param.fdCompanyId}");
					...
					<div id="listview1_">
						<list:listview id="listview1" style="height:380px;overflow-y:scroll;">
							<ui:source type="AjaxJson" >
								{url:'/fssc/ctrip/fssc_ctrip_city/fsscCtripCity.do?method=listCity }'}
							</ui:source>
