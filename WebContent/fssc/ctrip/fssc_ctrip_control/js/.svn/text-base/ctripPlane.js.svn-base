Designer_Config.operations['ctripPlane']={
    imgIndex : 77,
    title:Designer_Lang.controlCtripPlane_attr_title,
    titleTip:Designer_Lang.controlCtripPlane_attr_title,
    run : function (designer) {
        designer.toolBar.selectButton('ctripPlane');
    },
    type : 'cmd',
    order: 1,
    select: true,
...
Designer_Config.controls.ctripPlane = {
    type : "ctripPlane",
    storeType : 'field',
    inherit    : 'base',
    onDraw : _Designer_Control_Attr_Self_CtripPlane_Draw,
    drawXML : _Designer_Control_CtripPlane_CtripPlane_DrawXML,
    ...
    attrs : {
        ...
        matchType : {
            ...
            required: false,
            type : 'self',
            draw:_Designer_Control_Attr_Plane_Self_Match_Type_Draw,
            show : true
        },
        docNumberId : {
            text :Designer_Lang.controlCtripPlane_attr_docNumber,
            value : '',
            required: true,
            type : 'self',
            ...
            checkout: function(msg, name, attr, value, values, control){
                if(!values.docNumberId){
                    msg.push(Designer_Lang.controlCtripPlane_attr_docNumber_null)
                    return false;
                }
                return true;
            }
    ...
    onAttrLoad:_Designer_Control_Attr_CtripPlane_OnAttrLoad,
    info : {
        name: Designer_Lang.controlCtripPlane_attr_title
    },
    resizeMode : 'onlyWidth'
}

function _Designer_Control_Attr_Self_CtripPlane_Draw(parentNode, childNode){
	// 必须要做ID设置
	...
	var htmlCode = _Designer_Control_CtripPlane_DrawByType(this.parent, this.attrs, this.options.values, this);
	domElement.innerHTML = htmlCode;	
}

function _Designer_Control_CtripPlane_DrawByType(parent,attrs,values,target){
    ...
    var htmlCode = '<div style="width:65px;height:28px;line-height:28px;text-align:center;color:#fff;background:#47b5ea;border-radius:5px;" props="'+JSON.stringify(props).replace(/\"/g,"'")+'"><input type="hidden" props="'+JSON.stringify(props).replace(/\"/g,"'")+'"/>'+Designer_Lang.controlCtripPlane_attr_title+'</div>';
    return htmlCode;
}

function _Designer_Control_Attr_Plane_Self_Match_Type_Draw(name, attr,value, form, attrs, values, control){
    var html=[];
...
function _Designer_Control_CtripPlane_CtripPlane_DrawXML() {
	return "";
};

function _Designer_Control_Attr_CtripPlane_OnAttrLoad(form,control){
	
}


Designer_Config.buttons.control.push("ctripPlane");
//把控件增加到右击菜单区
Designer_Menus.add.menu['ctripPlane'] = Designer_Config.operations['ctripPlane'];
