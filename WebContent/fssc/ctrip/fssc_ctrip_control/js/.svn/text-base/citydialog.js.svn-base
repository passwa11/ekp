function dialogSelect1(mul, key, idField, nameField, action){
	...
	var dialogCfg = Designer_FormOption1.dialogs[key];
	if(dialogCfg){
		var params='';
...
function validateDetail(){
	var details=Designer_FormOption1.detailTables;
	if(details!=null){
...
function getDialogInputs(idField){
	var dialogLinks=Designer_FormOption1.dialogLinks;
...
window.onload=function(){
	...
	// 函数注册
	Designer_FormOption1.fn = {
		sum : function(data, detailTable, func){
			var size = Designer_FormOption1.fn.count(data, detailTable);
			var result = 0;
		...
		avg : function(data, detailTable, func){
			...
			return Designer_FormOption1.fn.sum(data, detailTable, func)*1.0/size;
		},
	...
	function bindAttrLinks(){
		if(Designer_FormOption1.attrLinks==null || Designer_FormOption1.attrLinks.length==0){
			return;
		}
		for(var i=0;i<Designer_FormOption1.attrLinks.length;i++){
			var attrLink = Designer_FormOption1.attrLinks[i];
	...
	function bindValueLinks(){
		if(Designer_FormOption1.valueLinks==null || Designer_FormOption1.valueLinks.length==0){
			return;
		}
		for(var i=0;i<Designer_FormOption1.valueLinks.length;i++){
			var valueLink = Designer_FormOption1.valueLinks[i];
	...
	function bindOptionLinks(){
		if(Designer_FormOption1.optionLinks==null||Designer_FormOption1.optionLinks.length==0){
			return;
		}
		for(var i=0;i<Designer_FormOption1.optionLinks.length;i++){
			var optionLink = Designer_FormOption1.optionLinks[i];
	...
	function addLinkValidates(){
		var linkValidates=Designer_FormOption1.linkValidates;
...
function fireValueLink(valueLink, fieldIndex){
	...
	function execValue(fieldIndex){
		...
		if(result!=null){
			result=$form.str(result,Designer_FormOption1.dataType[valueLink.output]);
			if(fieldIndex!=null){
	...
	if(inputs){
		for(var i=0; i<inputs.length; i++){
			...
			if(data[input]==null || fieldIndex!=null && input.indexOf('*')>-1){
				...
				value = $form.format(value, Designer_FormOption1.dataType[input]);
				if($.isArray(value)){
...
var Designer_FormOption1 = {
	    dialogs: {
	    	fssc_ctrip_city_fdCityList: {
	            modelName: 'com.landray.kmss.fssc.ctrip.model.FsscCtripCity',
	            sourceUrl: '/fssc/ctrip/fssc_ctrip_city/fsscCtripCityData.do?method=fdCityList'
	        }
	    },
	    dialogLinks: [],
	    valueLinks: [],
...
function selectCtripCity(obj,id,name){
	var idInput = $(obj).find('input:eq(0)');
	var nameInput = $(obj).find('input:eq(1)');
	dialogSelect1(false,'fssc_ctrip_city_fdCityList','extendDataFormInfo.value('+id+')','extendDataFormInfo.value('+name+')',function(rtn){
		if(!rtn)return;
		idInput.val(rtn[0].fdId);
		nameInput.val(rtn[0].fdName);
	});
} 
