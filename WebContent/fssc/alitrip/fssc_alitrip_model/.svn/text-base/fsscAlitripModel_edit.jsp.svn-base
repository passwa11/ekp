...
<script type="text/javascript">
    if("${fsscAlitripModelForm.method_GET}" == 'edit') {
        window.document.title = "${ lfn:message('button.edit') } - ${ lfn:message('fssc-alitrip:table.fsscAlitripModel') }";
    }
    if("${fsscAlitripModelForm.method_GET}" == 'add') {
        window.document.title = "${ lfn:message('button.add') } - ${ lfn:message('fssc-alitrip:table.fsscAlitripModel') }";
    }
    var formInitData = {
    ...
    Com_IncludeFile("form_option.js", "${LUI_ContextPath}/fssc/alitrip/fssc_alitrip_model/", 'js', true);
...
<html:form action="/fssc/alitrip/fssc_alitrip_model/fsscAlitripModel.do">
    <div id="optBarDiv">
        <c:choose>
            <c:when test="${fsscAlitripModelForm.method_GET=='edit'}">
                <input type="button" value="${ lfn:message('button.update') }" onclick="if(validateDetail()){Com_Submit(document.fsscAlitripModelForm, 'update');}">
            </c:when>
            <c:when test="${fsscAlitripModelForm.method_GET=='add'}">
                <input type="button" value="${ lfn:message('button.save') }" onclick="if(validateDetail()){Com_Submit(document.fsscAlitripModelForm, 'save');}">
            </c:when>
        </c:choose>
