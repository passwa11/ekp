...
<body>
		...
		<div class="lui_fssc_iframe_invoice">
			<div class="lui_fssc_invoice_card_content">
				<table class="lui_fssc_invoice_table" rules="rows" frame="hsides">
					<thead id="approval_thead"></thead>
					<tbody id="approval_tbody"></tbody>
				</table>
			</div>
		</div>
	</div>
	...
	$(document).ready(function(){
		$.ajax({
			url : '${LUI_ContextPath}/fssc/common/fssc_common_portlet/fsscCommonPortlet.do?method=listApproval',
			async : false,
			success : function(rtn) {
				var tbody = $("#approval_tbody");
				var thead = $("#approval_thead");
			 	tbody.empty();
			 	thead.empty();
				if(rtn != null){
					rtn = JSON.parse(rtn);
					...
					if(str.length > 0){
						...
						var json = JSON.stringify(str);
					 	$.each($.parseJSON(json), function (i, n) {
						    ...
						    tr.append($("<td><span>"+(n.money + '').replace(/(\d{1,3})(?=(\d{3})+(?:$|\.))/g,'$1,')+"</span></td>"));
						    if(n.status == '00'){
