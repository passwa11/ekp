...
<title>${lfn:message('fssc-voucher:fssc.voucher.bookkeeping')}</title>
...
<body>
	<div class="lui_fssc_iframe_invoice">
		<div class="lui_fssc_invoice_card_content">
			<table class="lui_fssc_invoice_table" rules="rows" frame="hsides">
				<thead id="bookkeep_thead"></thead>
				<tbody id="bookkeep_tbody"></tbody>
			</table>
		</div>
	</div>
	...
	$(document).ready(function(){
		$.ajax({
			url : '${LUI_ContextPath}/fssc/voucher/fssc_voucher_portlet/fsscVoucherPortlet.do?method=bookkeeping&status=30',
			async : false,
			success : function(rtn) {
				var tbody = $("#bookkeep_tbody");
				var thead = $("#bookkeep_thead");
			 	tbody.empty();
				thead.empty();
				if(rtn != null  && rtn != ""){
					rtn = JSON.parse(rtn);
