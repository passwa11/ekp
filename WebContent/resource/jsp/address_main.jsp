...
<%
	...
	if(StringUtil.isNotNull(isExternal)) {
		if("true".equals(isExternal)) {
			%>
			<script type="text/javascript">window.Address_params_isExternal = "true";</script>
			<%
		} else if("false".equals(isExternal)) {
			%>
			<script type="text/javascript">window.Address_params_isExternal = "false";</script>
			<%
		}
	}
	String enableDing = SysFormDingUtil.getEnableDing();
	if("true".equals(enableDing)) {
		%>
		<link type="text/css" rel="stylesheet" href="<%=request.getContextPath() %>/third/ding/third_ding_xform/resource/css/address.css">
		<script type="text/javascript">window.Address_params_enableDingCss = "true";</script>
		<%
	}
	//我的部门默认展开层级
...
//	Com_AddEventListener(window, "beforeunload", beforeClose);
	...
	dialogObject.searchBeanURL = "organizationDialogSearch"+(window.Address_params_isExternal?"&isExternal="+window.Address_params_isExternal:"")+(dialogObject.addressBookParameter.startWith==null?"":"&startWith="+dialogObject.addressBookParameter.startWith);
	...
	seajs.use(['lui/address/address','lui/jquery'],function(Address,$){
		$(document).ready(function(){
			...
			if(window.Address_params_isExternal) {
				params['isExternal'] = window.Address_params_isExternal;
			}
			if (window.Address_params_enableDingCss) {
				params['enableDingCss'] = window.Address_params_enableDingCss;
			}
			if(window.console) {
				console.log("Address_params:", params);
			}
