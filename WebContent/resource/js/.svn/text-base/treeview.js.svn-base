...
//钉钉目录树图片路径
var Ding_StylePath = Com_Parameter.ContextPath + "third/ding/third_ding_xform/style/default/";
var DING_IMGPATHPREFIX = Ding_StylePath + "tree/";
var dingFlag = typeof Com_Parameter.dingXForm != "undefined" && Com_Parameter.dingXForm === "true" && Com_Parameter.isFromDingPC === "true";
if(dingFlag) {
	IMGPATHPREFIX = DING_IMGPATHPREFIX;
	Com_Parameter.StylePath = Ding_StylePath;
}
...
function TreeFunc_SetCurrentNode(node){
		...
		if(CurNode==null){
			...
			if(element!=null){
				if(Com_Parameter.dingXForm){
					element.rows[0].cells[0].className = "";
				}
	...
	if(element!=null){
		if(Com_Parameter.dingXForm){
			element.rows[0].cells[0].className = "TVN_TreeNode_Current_icon";
		}
