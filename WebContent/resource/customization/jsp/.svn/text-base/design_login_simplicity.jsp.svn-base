...
<div class="option" data-hover="isLogoActive" @mouseenter="enter" @mouseleave="leave">
	...
	<div class="content">
		...
		<form id="simplicity_logo" enctype="multipart/form-data" method="post">
			<span class="btn-file">
		        ...
		        <input type="file" name="file" accept=".png,.jpg,.jpeg,.gif,ico" onclick="this.value='';" onchange="uploadImg('simplicity_logo');"/>
		    </span>
		</form>
	</div>
	<div>
		...
		<label v-show="login_logo_position === 'logoPositionOnForm' || login_logo_position === 'logoPositionOnTopLeft' " class="lui_design_logo_hegiht_label">
			<span class="lui_design_logo_hegiht_label_title">LOGO显示高度：</span>
			<input v-model="login_login_height" type="number" value="20">
			<span class="lui_design_logo_hegiht_label_unit">px</span>
		</label>
	</div>
</div>
<!-- 背景图 -->
<div class="option">
	...
	<div class="content">
		...
		<form id="simplicity_login_bg" enctype="multipart/form-data" method="post">
			<span class="btn-file">
		        <span class="f12"><bean:message bundle="sys-profile" key="sys.profile.design.selectImg"/></span>
		        <input type="hidden" name="configKey"/>
		        <input type="hidden" name="fileName"/>
		        <input type="hidden" name="isRandom"/>
		        <input type="file" name="file" accept=".png,.jpg,.jpeg,.gif" onclick="this.value='';" onchange="uploadImg('simplicity_login_bg',true);"/>
		    </span>
		</form>
	</div>
...
<!-- 友情链接 v-bind:simplicity_head_link_href="link.simplicity_head_link_href"-->
<div class="option" data-hover="isHeadLinksActive" @mouseenter="enter" @mouseleave="leave">
	...
	<div class="inputGroup"
		...
		<div class="inputSgl">
			<div class="input">
				<input placeholder="<%=entry.getValue() %>" title="<%=entry.getValue() %>" type="text" v-model="link.simplicity_head_link_<%=entry.getKey() %>"/>
			</div>
		</div>
		<%} %>
		<div class="inputSgl">
			<div class="input">
				<input placeholder="http://product.landray.com.cn" type="text" v-model="link.simplicity_head_link_href" />
			</div>
		</div>
	</div>
</div>
...
<div class="option" data-hover="isCopyrightActive" @mouseenter="enter" @mouseleave="leave">
	...
	<c:forEach items="${langs }" var="lang">
		<div class="content">
			<div class="input">
				<input placeholder="${lang.value }" title="${lang.value }" type="text" v-model="simplicity_footerInfo_${lang.key }"/>
			</div>
		</div>
	</c:forEach>
</div>
