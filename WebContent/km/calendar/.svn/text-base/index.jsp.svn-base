		...
		<script type="text/javascript">
			...
			seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/toolbar','lui/util/env','lui/dateUtil'], function($,dialog , topic ,toolbar,env,dateutil) {
				...
				window.clickLabel=function(_this,id){
					...
					//重新请求刷新右侧日程管理
					var newUrl = Com_SetUrlParameter(url,"exceptLabelIds",exceptLabelIds);

					//记录标签id（包括固定的通用标签）
					var labelFdId = $(_this).closest("li").attr("labelFdId");
					labelFdId = labelFdId || "";
					//通用标签标识
					var commanLabelFlag = id;
					//通过ajax异步处理标签的选中存储 labelFdId + commanLabelFlag + isSelected
					updateLabel2Selected(_this,labelFdId,commanLabelFlag,isSelected);

					LUI("calendar").source.source.setUrl(newUrl);//修改请求地址
				...
				topic.subscribe('calendar.thing.change',function(arg){
					...
					$("#calendar_add,#calendar_view,#note_view").hide();//隐藏所有弹出框
					var ownerId=arg.schedule.ownerId; //获取当前日程的拥有者
					var currentId = "${userPersonId}"; //当前登录人
					if(ownerId!=currentId) {
						loading.hide();
						dialog.failure('仅日程的拥有者可以修改日程时间！');
						arg.moveFun();
						return;
					}
				...
				topic.subscribe('calendar.thing.click',function(arg){
					$("#ul_label_edit").show();//切换标签隐藏
					$("#button_save_event").show();//确认按钮隐藏
					$("#button_delete_event").show();//删除按钮隐藏
					$("#button_view_detail").show(); //查看详情按钮隐藏
					var ownerId = arg.schedule.ownerId;
					var currentId = "${userPersonId}";
					//如果当前登录人是日程的相关人不能查看详情以及修改日程标签
					if(ownerId!=currentId) {
						$("#ul_label_edit").hide(); //切换标签隐藏
						$("#button_save_event").hide();  //确认按钮隐藏
						$("#button_view_detail").hide(); //查看详情按钮隐藏
					}
					//题头:群组日程、我的日程
				...
				//更新后台标签的选中取消
				window.updateLabel2Selected = function (ob,labelFdId,commanLabelFlag,isSelected){
					var _url = "${LUI_ContextPath}/km/calendar/km_calendar_label/kmCalendarLabel.do?method=updateLabel2Selected";
					var inData = {
						"labelFdId":labelFdId,
						"commanLabelFlag":commanLabelFlag,
						"isSelected": (isSelected?"1":"0")
					}
					$.ajax({
						url: _url,
						type: 'POST',
						data: $.param(inData, true),
						dataType: 'json',
						success: function (data) {
							console.log("更新标签完成："+ JSON.stringify(data) );
							//rsData
							if(typeof data.rsData != "undefined" && !!data.rsData){
								$(ob).closest("li").attr("labelFdId",data.rsData);
							}
						},
						error: function (XMLHttpRequest, textStatus, errorThrown) {
							//这里是详细的信息
							console.log("网络i/o交互异常："+ XMLHttpRequest.responseText );
						}
					});
				}
