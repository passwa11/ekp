					...
					<table class="muiSimple" cellpadding="0" cellspacing="0">
						...
						<tr>
							<td>
								...
								<span class="auth_type_desc" style="display: none;padding-top: 0px;color: red;">
									<bean:message bundle="km-calendar" key="kmCalendarMain.fdAuthorityType.private.desc"/>
								</span>
							</td>
						</tr>
						<tr>
...
require(['dojo/query','mui/util','dojo/topic','dijit/registry','dojo/ready','dojo/dom-style','dojo/dom-geometry','mui/dialog/Tip','mui/calendar/CalendarUtil','dojo/date/locale','mui/device/adapter','dojo/request','dojo/dom-attr',"mui/form/ajax-form"],
	...
	window.authorityTypeSelect=function(value){
	  ...
	  //隐藏公开属性
	    ...
	    if(publicNode && publicNode.length>0){
			...
			window.showAuthTypeDesc=function(value){
				var docOwner = $("input[name='docOwnerId']").val(),
						typeDesc = $(".auth_type_desc");
				if (value == 'PRIVATE' && (docOwner == 'multiCreate' || docOwner != '${pageScope.currentUserId}')) {
					typeDesc.show();
				} else {
					typeDesc.hide();
				}
	...
	topic.subscribe('mui/form/select/callback',function(widget){
		...
		if(valueField == 'docOwnerId'){
			...
			var fdAuthorityType = query('[name="fdAuthorityType"]')[0].value;
			showAuthTypeDesc(fdAuthorityType);
			return;
		}
	...
	//提交后返回查看页面
	/* Com_Submit.ajaxAfterSubmit=function(){  #137839 屏蔽此段代码，新建日程编辑后再删除会提示文件不存在，手动跳转页面会导致回退页错乱
		if('${ kmCalendarMainForm.method_GET}'=='edit'){
			setTimeout(function(){
