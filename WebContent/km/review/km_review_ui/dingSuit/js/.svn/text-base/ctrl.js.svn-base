seajs.use(['lui/jquery'],function($){
	...
	function Frame_OpenWindow(frameWin, url, target, winStyle){
		...
		if(target==null || target=="" || typeof(target) == "undefined") {
			//在Mac端的钉钉内部浏览器中，frameWin.name={"hostOrigin":"app:\/\/desktop.dingtalk.com","hostVersion":"5.1.16","containerId":1,"language":""}，故此处需要判断srcWin == null
			target = (srcWin == null || srcWin == 1) ? 3 : srcWin + 1;
		}else{
		...
		var iframeObj = document[frameMap[target]].frameElement;
		if(iframeObj){
			return iframeObj.src=url;
		}
		...
		function setLeftCSS(_ctrlName){
			...
			downChildFrame.each(function(_index,elem){
				...
				if(_index == index -1){
					...
					var toLeftShow = toLeft.css('display');
					var toRightShow = toRight.css('display');
...
//					}
	...
	function Frame_SetLV2FrameSize(size, remainCtrl){
		...
		if(!remainCtrl){
			...
			var toRight = ctrl2Frame.find(".toRight");
			var toRightShow = toRight.css('display');
			if(toRightShow=='block'&& rightFrameLeft>360){
				rightFrame.css('left',rightFrameLeft + _width-180 + 'px' );
			}else{
