...
%>
						<lable class="ding-lable-checkbox ding-checkbox-space"><sunbor:multiSelectCheckbox 
							beanLabelProperty="label"
							beanValueProperty="value"
							avalables="${avalables}" 
							assignProperty="true" 
							propertyValue="${v0}" 
							property="v0_${entryIndex}" /></lable>
					</c:when>
							...
							<tr>
								...
								<td>
							        ...
							        <c:if test="${conditionEntry.treeModel}">
										<br><lable class="ding-lable-checkbox"><input name="t1_${entryIndex}" type="checkbox" ${"true" == parameters[propertyName].t1_ ? "checked" : ""} value="true">
										<bean:message bundle="sys-search" key="search.includeChild"/></lable>
									</c:if>
<%	} %>
								</td>
								...
								<td width="140px">
								<div class="ding-search-div">
									<lable class="ding-lable-checkbox"><input name="v2_${entryIndex}" type="checkbox" onclick="refreshMatchDisplay(this);" ${empty parameters[propertyName].v1_ ? "" : "checked"} >
									<%--模糊搜索chickbox --%>
									<bean:message bundle="sys-search" key="search.blursearch"/></lable>
					...
					<c:when test="${conditionEntry.type=='enum'}">
					 <% SearchConditionUtil.setEnumPropertyValue(pageContext);  %>
						<lable class="ding-lable-checkbox ding-checkbox-space"><sunbor:multiSelectCheckbox enumsType="${conditionEntry.enumsType}" assignProperty="true" propertyValue="${v0}" property="v0_${entryIndex}" /></lable>
					</c:when>
				</c:choose>
			</td>
...
function refreshMatchDisplay(obj){
	...
	/* #127690 数据导出页面，部门和申请人选项中勾选了模糊搜索，部门和申请人输入框已置灰，但还可以输入，并能按输入条件查询（此时模糊搜索无效）点击模糊搜索禁用掉输入框 */
	tbObj.rows[0].cells[1].getElementsByTagName("input")[0].setAttribute("disabled",false);
}
