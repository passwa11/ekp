	...
	<template:replace name="content"> 
		...
		<list:listview>
			<ui:source type="AjaxJson">
					{url:'/km/review/km_review_main/kmReviewMain.do?method=manageListData&categoryId=${param.categoryId}&nodeType=${param.nodeType}'}
			</ui:source>
...
//         	  setTimeout('initMenuNav()', 300);
			...
			window.refreshBtn=function (){
				seajs.use(['lui/jquery', 'lui/util/str', 'lui/dialog','lui/topic','lui/toolbar'], function($, strutil, dialog , topic,toolbar) {
					//国际化多语言伪元素
					var refer = $("div[class='lui_paging_t_refresh_c']");
					var lang = refer.context.cookie;
					if(lang.includes('US')){
						$("div[class='lui_paging_t_refresh_c']").find("a").each(function(){
							//更改伪元素样式
							$(this).removeClass("lui_icon_s lui_paging_t_refresh");
							$(this).addClass("lui_icon_s lui_paging_t_refresh_US");
						});
					}
				});
			}
