...
<template:include ref="default.list" spa="true">
					...
					<ui:combin ref="menu.nav.simple">
						<ui:varParam name="source">
							<ui:source type="Static">
								[
								{
								"text" : "${ lfn:message('km-review:kmReview.nav.home') }",
								"href" :  "/homepage",
								"router" : true,
								...
								<c:if test="${enableModule.enableDbcenterEcharts ne 'false'}">
								...
								<kmss:ifModuleExist path="/sys/datamng/">
									<kmss:authShow roles="ROLE_KMREVIEW_DATAMNG">
										,{
				...
				<ui:content title="${ lfn:message('list.otherOpt') }" expand="false" >
					<ui:combin ref="menu.nav.simple">
						<ui:varParam name="source">
							<ui:source type="Static">
								[
								<c:if test="${enableModule.enableKmArchives ne 'false'}">
									{
	...
	<template:replace name="content">
		<ui:tabpanel id="kmReviewPanel" layout="sys.ui.tabpanel.list"  cfg-router="true">
			<ui:content id="homepageContent" title="${ lfn:message('km-review:kmReview.nav.home') }" cfg-route="{path:'/homepage'}">
				<ui:iframe id="homeInfoIfram"  src="${LUI_ContextPath }/km/review/km_review_homepage/km_review_homepage.jsp"></ui:iframe>
			</ui:content>
			<ui:content id="kmReviewContent" title="${ lfn:message('km-review:kmReviewMain.portlet.myFlow.all') }" cfg-route="{path:'/listCreate'}">
				<list:criteria id="criteria1">
					...
					<%-- 增加待我审的送审时间筛选项 --%>
					<list:cri-ref ref="criterion.sys.calendar" key="arrivalTime" cfg-if="(param['mydoc'] == 'approval')" multi="false"
								  title="${lfn:message('km-review:sysWfNode.processingNode.currentarrTime') }" />
					<%-- 增加我已审的处理时间筛选项 --%>
					<list:cri-ref ref="criterion.sys.calendar" key="resolveTime" cfg-if="(param['mydoc'] == 'approved')" multi="false"
								  title="${lfn:message('km-review:sysWfNode.processingNode.resolveTime') }" />
					...
					<div class="lui_list_operation_sort_btn">
						...
						<div class="lui_list_operation_sort_toolbar">
							<ui:toolbar layout="sys.ui.toolbar.sort" style="float:left" >
								<list:sortgroup>
									<list:sort id="createTime" property="docCreateTime" text="${lfn:message('km-review:kmReviewMain.docCreateTime') }" group="sort.list" value="down"></list:sort>
									<list:sort id="publishTime" property="docPublishTime" text="${lfn:message('km-review:kmReviewMain.docPublishTime') }" group="sort.list"></list:sort>
									<list:sort id="arrTime" property="arrivalTime" text="${lfn:message('km-review:sysWfNode.processingNode.currentarrTime') }" group="sort.list"></list:sort>
									<list:sort id="resTime" property="resolveTime" text="${lfn:message('km-review:sysWfNode.processingNode.resolveTime') }" group="sort.list"></list:sort>
								</list:sortgroup>
							</ui:toolbar>
						</div>
					</div>
			...
			<ui:content id="datamngContent" title="${ lfn:message('sys-datamng:module.sys.datamng') }" cfg-route="{path:'/datamng'}">
				<ui:iframe id="datamng"  src="${LUI_ContextPath }/sys/datamng/sys_datamng_main/sysDatamngData.do?method=modulePolicyList&modulePrefix=/km/review/"></ui:iframe>
			</ui:content>
		</ui:tabpanel>
	...
	<template:replace name="script">
		...
		<%--<script type="text/javascript">
			window.onload = function (){
				$("#homepageContent").find("lui_widget_iframe").css("height","auto");
				alert($("#homepageContent").find("lui_widget_iframe"));
			}
		</script>--%>
		<%--列表页面筛选项页面缓存--%>
