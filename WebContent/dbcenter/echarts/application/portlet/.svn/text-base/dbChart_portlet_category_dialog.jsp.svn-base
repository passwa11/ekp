...
<template:include ref="default.simple">
	<template:replace name="title">${lfn:message('dbcenter-echarts:portlet.dbchart')}</template:replace>
	<template:replace name="body">
		...
		function submitSelected(fdId,docSubject) {
			...
			<tr>
				<td valign="top"><list:listview id="listview">
					<ui:source type="AjaxJson">
						{"url":"/dbcenter/echarts/application/dbEchartsApplication.do?method=list&modelName=${param.modelName }"}
					</ui:source>
					<list:colTable sort="false"
						onRowClick="submitSelected('!{fdId}','!{docSubject}')">
						<list:col-serial style="width:25%;"></list:col-serial>
						<list:col-html
							title="${lfn:message('dbcenter-echarts:portlet.dbchart')}"
							style="width:75%;">
							{$<div style="overflow: hidden; text-overflow: ellipsis;">{%row['docSubject']%}</div>$}
						</list:col-html>
					</list:colTable>
