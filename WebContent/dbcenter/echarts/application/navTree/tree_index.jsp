	...
	<template:replace name="content">
		...
		<list:listview>
			<ui:source type="AjaxJson">
				{url:'/dbcenter/echarts/application/dbEchartsNavTree.do?method=list&mainModelName=${param.mainModelName }&fdKey=${param.fdKey }&modelName=${param.modelName }'}
			</ui:source>
			<list:colTable var-className="lui-ding-list-table" isDefault="true" layout="sys.ui.listview.columntable" 
			     rowHref="${actionUrl}method=view&fdId=!{fdId}">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial>
				<list:col-auto props="fdNavTitleTxt,fdEchartsCategory.fdName,docCreateTime,docCreator.fdName,operations"></list:col-auto>
			</list:colTable>
		</list:listview>
		<br>
		<!-- 分页 -->
	 	...
	 	<script type="text/javascript">
		 	seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'], function($, dialog, topic) {
		 		window.add = function(){
		 			Com_OpenWindow("<c:url value='${actionUrl}method=add&modelName=${param.mainModelName }&fdKey=${param.fdKey }'/>");	
		 		};
		 		window.edit = function(id){
					if(id){
						Com_OpenWindow("<c:url value='${actionUrl}method=edit&fdId=' />" + id);
					}
			 	};
		 		...
		 		window.deleteAll = function(id){
					...
					dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(value){
						if(value==true){
							...
							$.ajax({
								url:'<c:url value="${actionUrl}method=deleteall"/>',
										type:"POST",
