	...
	<template:replace name="content">
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
				{url:'/dbcenter/echarts/db_echarts_chart_set/dbEchartsChartSet.do?method=list&fdKey=${JsParam.fdKey}&fdModelName=${JsParam.fdModelName}&orderby=docCreateTime&ordertype=down'}
			</ui:source>
			<%--列表视图--%>
		 	...
		 	seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'], function($, dialog, topic) {
				...
				topic.subscribe('successReloadPage', function() {
		 		...
		 		window.addChartSet = function(){
		 			...
		 			if(cateId==""){
			 			var addUrl = "dbEchartsChartSet.do?method=add&fdKey=${JsParam.fdKey}&fdModelName=${JsParam.fdModelName}";
			 			Com_OpenWindow(addUrl);	
		 			}else{
			 			dialog.simpleCategoryForNewFile(
								'com.landray.kmss.dbcenter.echarts.model.DbEchartsTemplate',
								'/dbcenter/echarts/db_echarts_chart_set/dbEchartsChartSet.do?method=add&fdKey=${JsParam.fdKey}&fdModelName=${JsParam.fdModelName}&fdTemplateId=!{id}',false,null,null,cateId);
		 			}
		 		...
		 		window.edit = function(id){
					if(id){
						Com_OpenWindow('dbEchartsChartSet.do?method=edit&fdId='+id);
					}
			 	};
			 	...
			 	window.deleteDoc = function(id){
					dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(isOk){
						if(isOk){
							...
							var delUrl = '<c:url value="/dbcenter/echarts/db_echarts_chart_set/dbEchartsChartSet.do?method=delete&fdId="/>'+id;
							if("${JsParam.fdModelName}" === "com.landray.kmss.sys.modeling.base.model.ModelingAppModel") {
								delUrl = '<c:url value="/sys/modeling/base/dbEchartsChartSet.do?method=delete&fdId="/>'+id;
							}
							$.ajax({
								url: delUrl,
								type: "GET",
		 		...
		 		window.deleteAll = function(){
					...
					dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(value){
						if(value==true){
							window.del_load = dialog.loading();
							let delUrl = '<c:url value="/dbcenter/echarts/db_echarts_chart_set/dbEchartsChartSet.do?method=deleteall"/>';
							if("${JsParam.fdModelName}" === "com.landray.kmss.sys.modeling.base.model.ModelingAppModel") {
								delUrl = '<c:url value="/sys/modeling/base/dbEchartsChartSet.do?method=deleteall"/>';
							}
							$.ajax({
								url: delUrl,
								type:"POST",
