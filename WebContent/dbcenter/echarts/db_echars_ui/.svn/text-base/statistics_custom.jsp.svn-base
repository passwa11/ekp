...
<template:include ref="default.simple" spa="true">
	...
	<template:replace name="body">
		...
		<ui:tabpanel id="echartsPanel" layout="sys.ui.tabpanel.list" >
			<ui:content id="customContent" title="${ lfn:message('dbcenter-echarts:table.dbEchartsCustom') }">
		...
		<list:listview>
			<ui:source type="AjaxJson">
				{url:'/dbcenter/echarts/db_echarts_custom/dbEchartsCustom.do?method=list&orderby=docCreateTime&ordertype=down'}
			</ui:source>
...
<script type="text/javascript">
	seajs.use(['lui/jquery','lui/dialog','lui/topic'],function($,dialog,topic){
		...
		window.addChart = function(){
 			...
 			if(cateId==""){
	 			var addUrl = "${LUI_ContextPath}/dbcenter/echarts/db_echarts_custom/dbEchartsCustom.do?method=add";
	 			Com_OpenWindow(addUrl);	
 			}else{
	 			dialog.simpleCategoryForNewFile(
						'com.landray.kmss.dbcenter.echarts.model.DbEchartsTemplate',
						'/dbcenter/echarts/db_echarts_custom/dbEchartsCustom.do?method=add&fdTemplateId=!{id}',false,null,null,cateId);
 			}
 		};
 		...
 		window.edit = function(id){
			if(id){
				Com_OpenWindow('${LUI_ContextPath}/dbcenter/echarts/db_echarts_custom/dbEchartsCustom.do?method=edit&fdId='+id);
			}
	 	};
