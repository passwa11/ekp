...
<c:if test="${JsParam.fdModelName eq 'com.landray.kmss.sys.modeling.base.model.ModelingAppModel'}">
	<%@ include file="/sys/modeling/base/modeling_head.jsp" %>
</c:if>
<template:include ref="default.list">
	<template:replace name="title">
		<c:out value="${ lfn:message('dbcenter-echarts:module.dbcenter.stat') }"></c:out>
	</template:replace>
	<template:replace name="path">			
		<ui:menu layout="sys.ui.menu.nav"> 
			<ui:menu-item text="${ lfn:message('home.home') }" icon="lui_icon_s_home">
			</ui:menu-item>	
			<ui:menu-item text="${ lfn:message('dbcenter-echarts:module.dbcenter.stat') }">
			</ui:menu-item>
		</ui:menu>
	</template:replace>
	<template:replace name="nav">
		<script>
			function addDoc(){
				Com_OpenWindow("${LUI_ContextPath}/dbcenter/echarts/db_echarts_table/dbEchartsTable.do?method=add&fdKey=${JsParam.fdKey}&fdModelName=${JsParam.fdModelName}");
			}
			function delDoc(){
				var values = [];
				...
				seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic){
					dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(value){
						if(value==true){
							...
							$.post('${LUI_ContextPath}/dbcenter/echarts/db_echarts_table/dbEchartsTable.do?method=deleteall&categoryId=${JsParam.categoryId}',$.param({"List_Selected":values},true),function(data){
								if(del_load!=null){
									del_load.hide();
		...
		<ui:combin ref="menu.nav.create">
			<ui:varParam name="title" value="${ lfn:message('dbcenter-echarts:module.dbcenter.stat') }" />
			<ui:varParam name="button">
				[
					<kmss:auth requestURL="/dbcenter/echarts/db_echarts_table/dbEchartsTable.do?method=add">
					{
						"text": "${ lfn:message('dbcenter-echarts:module.dbcenter.stat') }",
		...
		<div class="lui_list_nav_frame">
			 <ui:accordionpanel>				 
				...
				<ui:content title="${ lfn:message('list.otherOpt') }">
					<ul class='lui_list_nav_list'>
						<li><a href="${LUI_ContextPath }/sys/?module=dbcenter/echarts" target="_blank">${ lfn:message('list.manager') }</a></li>
					</ul>
				</ui:content>
			</ui:accordionpanel>
		</div>
	...
	<template:replace name="content">
		...
		<list:criteria id="criteria1">
			...
			<list:cri-auto modelName="com.landray.kmss.dbcenter.echarts.model.DbEchartsTable" 
				property="fdConfig;docCreateTime" />
		</list:criteria>
					...
					<td align="right">
						<ui:toolbar count="3">
							...
							<c:import url="/sys/introduce/import/sysIntroduceMain_cancelbtn.jsp" charEncoding="UTF-8">
								<c:param name="fdModelName" value="com.landray.kmss.dbcenter.echarts.model.DbEchartsTable" />
							</c:import>
							--%>
							<%-- 修改权限 
							<c:import url="/sys/right/import/doc_right_change_button.jsp" charEncoding="UTF-8">
								<c:param name="modelName" value="com.landray.kmss.dbcenter.echarts.model.DbEchartsTable" />
							</c:import>
							--%>				 
						</ui:toolbar>
					</td>
		...
		<ui:fixed elem=".lui_list_operation"></ui:fixed>
	 	...
	 	<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/dbcenter/echarts/db_echarts_table/dbEchartsTable.do?method=data&categoryId=${JsParam.categoryId}&fdKey=${JsParam.fdKey}&fdModelName=${JsParam.fdModelName}'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false"
				rowHref="/dbcenter/echarts/db_echarts_table/dbEchartsTable.do?method=view&fdId=!{fdId}&chartDefault=1"  name="columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial>
				 
				<list:col-auto props="docSubject;fdConfig;docCreateTime;docCreator.fdName"></list:col-auto>
			</list:colTable>
			<!-- 摘要视图 -->	
			<list:rowTable isDefault="false"
				rowHref="/dbcenter/echarts/db_echarts_table/dbEchartsTable.do?method=view&fdId=!{fdId}&chartDefault=1" name="rowtable" >
				...
				docSubject;icon;fdDescription;docCategory.fdName;docAuthor.fdName;docDept.fdName;docPublishTime;tag
				 --%>
			   	{"showOtherProps":"fdConfig;docCreateTime;docCreator.fdName"}
				</list:row-template>
			</list:rowTable>
		</list:listview> 
