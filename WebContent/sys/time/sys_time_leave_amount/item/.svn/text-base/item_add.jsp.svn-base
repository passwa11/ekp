...
<table class="tb_normal" width=100%>
	<tr>
		...
		<td width="35%">
			<xform:address onValueChange="onChangePerson" propertyId="fdPersonId" propertyName="fdPersonName" orgType="ORG_TYPE_PERSON" required="true" subject="${ lfn:message('sys-time:sysTimeLeaveAmount.fdPerson') }" validators="" isExternal="false" style="width: 95%">
			</xform:address>
		</td>
		<%-- 年份 --%>
		<td width="15%" class="td_normal_title">
	...
	<c:forEach items="${sysTimeLeaveAmountForm.fdAmountItems }" var="amountItem" varStatus="vstatus">
		...
		<tr>
			...
			<td width="35%">
				...
				<c:if test="${amountItem.fdIsAuto eq null || !amountItem.fdIsAuto}">
					<xform:datetime property="fdAmountItems[${vstatus.index }].fdValidDate" dateTimeType="date" style="width: 95%" required="true" subject="${ lfn:message('sys-time:sysTimeLeaveAmount.fdValidDate') }" validators="validYearDate validDateRange" onValueChange="onChangeVDate"></xform:datetime>
				</c:if>
			</td>
