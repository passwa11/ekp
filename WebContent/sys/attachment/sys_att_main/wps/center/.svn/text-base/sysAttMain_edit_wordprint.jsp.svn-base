...
<script>
	...
	$(document).ready(function(){		
		   //请求在线编辑附件的id
		   var url="${KMSS_Parameter_ContextPath}sys/print/data/sysPrintWordData.do?method=wpssetRet&isCenter=true";
		   $.ajax({     
		 	     type:"post",     
		 	     url:url,     
		 	    	...
		 	    	if(results['flag']){
		 	    		seajs.use(['lui/jquery','lui/dialog'], function($, dialog){
	  						...
	  						function wpsSetRedResult(){
	  							var ajaxUrl = Com_Parameter.ContextPath+"sys/print/data/sysPrintWordData.do?method=getCovertDownload&fdModelId=${fdModelId}&fdModelName=${fdModelName}&type=3&isCenter=true";
	  							$.ajax({
	  								url : ajaxUrl,
	  								type : 'post',
	  								...
	  								success : function(data) {
	  									...
	  									if(data.flag){
	  										...
	  										wps_center_${param.fdKey} = new WPSCenterOffice_AttachmentObject(fdAttMainId,"${param.fdKey}","${fdModelId}","${fdModelName}","${isWrite}");
	...
	function wpsCenterClearRevisions(){
		seajs.use(['lui/jquery','lui/dialog'], function($, dialog){
			var loading=dialog.loading("正在清稿,请稍候...");
			wps_center_${param.fdKey}.accent();
			loading.hide();
		});
	}
</script>