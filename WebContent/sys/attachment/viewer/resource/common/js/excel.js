...
var excel = {
	...
	loadSheet : function(sheetNum){
		...
		seajs.use('lui/jquery',function($){
			...
			var src = dataSrc+"?method=view&viewer=htmlviewer&fdId="+fdId+"&filekey="+commonFuncs.getFileName(sheetNum);
			// 兼容数据权限管理
			if (window.appendDatamngToken) {
				src = window.appendDatamngToken(src);
			}
			$(sheetContentFrame).removeAttr("src").attr("src", src);
		});
		if(commonFuncs.contains(viewerStyle,"aspose",true)){
