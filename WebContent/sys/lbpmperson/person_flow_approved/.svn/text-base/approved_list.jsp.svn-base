...
<list:data>
	<list:data-columns var="lbpmProcess" list="${queryPage.list }" varIndex="status">
	    ...
	    <!--创建人-->
		<c:if test="${fn:containsIgnoreCase(lbpmProcess, 'fdCreator')}">
			<list:data-column headerStyle="width:8%" col="fdCreator" title="${ lfn:message('sys-lbpmperson:lbpmperson.person.creator') }">
				${lbpmProcess.fdCreator.fdName}
			</list:data-column>
		</c:if>
		<c:if test="${!fn:containsIgnoreCase(lbpmProcess, 'creator')}">
		...
		<list:data-column headerStyle="width:6%" col="fdStatus" title="${ lfn:message('sys-lbpmperson:lbpmperson.flow.docStatus') }" escape="false">
					...
					<c:if test="${docStatusMap[lbpmProcess.fdId]=='31'}">
						${ lfn:message('sys-lbpmperson:lbpmperson.status.feedback') }
					</c:if>
		</list:data-column>
		<!--当前处理（节点）-->
		...
		<!-- 处理时间-->
		<list:data-column headerStyle="width:10%" col="resolveTime" title="${ lfn:message('sys-lbpmperson:lbpmperson.person.resolveTime') }" escape="false">
			<kmss:showWfPropertyValues idValue="${lbpmProcess.fdId}" propertyName="resolveTime" />
		</list:data-column>
	</list:data-columns>
