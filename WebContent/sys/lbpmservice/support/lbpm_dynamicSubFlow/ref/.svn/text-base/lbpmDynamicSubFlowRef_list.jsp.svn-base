	...
	<template:replace name="content">
		...
		<list:listview>
			<ui:source type="AjaxJson">
				{url:'/sys/lbpmservice/support/lbpmDynamicSubFlow.do?method=listRef&fdDynamicId=${JsParam.fdDynamicId}'}
			</ui:source>
			<list:colTable isDefault="true" layout="sys.ui.listview.columntable" 
				rowHref="/sys/lbpmservice/support/lbpmDynamicSubFlow.do?method=viewTemp&fdProcessTemplateId=!{fdProcessTemplateId}&fdProcessDefinitionId=!{fdProcessDefinitionId}&fdType=!{fdType}">
		...
		<script type="text/javascript">
			seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'], function($, dialog, topic) {
				window.editParam = function(fdProcessTemplateId,fdNodeId,nodeParams) {
					var content = $("textarea[name='fdParamSetContent']",parent.document).val();
					var url='/sys/lbpmservice/support/lbpm_dynamicSubFlow/lbpmDynamicSubFlow_param.jsp';
					var param = {
					...
					dialog.iframe(url,"${ lfn:message('sys-lbpmservice-support:lbpmDynamicSubFlow.paramSet')}",function(rtn){
						if(rtn){
