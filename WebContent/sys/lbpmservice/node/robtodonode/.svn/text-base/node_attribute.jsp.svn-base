		...
		<td>
			<table width="100%" class="tb_normal">
				...
				<tr style="display: none">
					...
					<td>
						<label>
							<input name="wf_processType" type="radio" value="1" checked>
							<kmss:message key="FlowChartObject.Lang.Node.processType_1" bundle="sys-lbpmservice" />
						</label>
					</td>
				</tr>
				<tr>
					<td width="100px"><bean:message bundle="sys-lbpmservice" key="lbpmNode.currentNode.distributeType" /></td>
					<td>
						<label>
							<input name="wf_distributeType" type="radio" value="0" checked>
							<bean:message bundle="sys-lbpmservice" key="lbpmNode.currentNode.distributeType.robtodo" />
						</label>
						<%--<label>
				...
				<tr>
					...
					<td id="NODE_TD_notifyType">
						<label>
							<input name="wf_notifyMode" type="radio" value="0" checked>
							<bean:message bundle="sys-lbpmservice" key="lbpmNode.currentNode.robtodo.notify.before" />
						</label>
						<label>
							<input name="wf_notifyMode" type="radio" value="1">
							<bean:message bundle="sys-lbpmservice" key="lbpmNode.currentNode.robtodo.notify.after" />
						</label><br>
	...
	<tr LKS_LabelName="<kmss:message key="FlowChartObject.Lang.Label_Operation" bundle="sys-lbpm-engine" />">
		<td>
			<c:import url="/sys/lbpmservice/node/common/node_operation_attribute.jsp" charEncoding="UTF-8">
				<c:param name="nodeType" value="reviewNode" />
				<c:param name="modelName" value="${param.modelName}" />
			...
			<table class="tb_normal" width="100%">
					...
					<td>
						...
						<div class="passRuleLimitDiv" style="margin-top: 8px">
							<label>
								...
								<kmss:message key="FlowChartObject.Lang.Node.Limitedtime.rule.day" bundle="sys-lbpmservice" />
							</label>
							<label>
								<input name="wf_ruleTimeType" type="radio" value="1">
								<kmss:message key="FlowChartObject.Lang.Node.Limitedtime.rule.formula" bundle="sys-lbpmservice" />
							</label>
						</div>
					</td>
				</tr>
				...
				<%--<tr>
					...
					<td colspan="3">
						...
						<input name="wf_canFastReject" type="checkbox" value="true"><kmss:message key="FlowChartObject.Lang.Node.fastReject" bundle="sys-lbpmservice"/></input>
						<input name="wf_canAssignPass" type="checkbox" value="true"><kmss:message key="FlowChartObject.Lang.Node.fastAssignPass" bundle="sys-lbpmservice"/></input>
						<input name="wf_canAssignBack" type="checkbox" value="true"><kmss:message key="FlowChartObject.Lang.Node.fastAssignBack" bundle="sys-lbpmservice"/></input><br/>
				...
				<c:if test="${isOpinionTypeEnabled eq 'true'}">
					<tr>
						<td width="100px">意见配置参数</td>
						<td colspan="3">
							<label><input name="ext_opinionConfig" type="checkbox" value="true">意见是否可配置</input></label>
						</td>
					</tr>
					<tr>
						<td width="100px">默认意见类型</td>
						<td colspan="3">
							<label>
				...
				<!-- 驳回的节点通过后选择方式配置开始 -->
				<tr style="display: none">
					<c:import url="/sys/lbpmservice/node/common/node_chooseRefuseNode_to_config.jsp" charEncoding="UTF-8">
						<c:param name="nodeType" value="${param.nodeType}" />
					</c:import>
				</tr>
				<!-- 驳回的节点通过后选择方式配置结束 -->
...
AttributeObject.CheckDataFuns.push(function(data) {
	...
	if(data.extAttributes){
		var containsRefuseTypes = false;
		for(var i=0;i<data.extAttributes.length;i++){
			if(data.extAttributes[i].name=="refuse_types"){
				containsRefuseTypes = true;
			}
		}
		if(!containsRefuseTypes){
			alert('<kmss:message key="lbpmNode.processingNode.operationsTDTitle.handlerOperationTypeRefuse.selectTypesNotnull" bundle="sys-lbpmservice" />');
			return false;
		}
	}
	return true;
