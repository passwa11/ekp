define([ "dojo/_base/declare",
	...
	var button = declare("sys.circulation.CirculationOperationButton", [TabBarButton], {
		...
		_onClick: function(){
			if(this.fdType=="cancel"){
				topic.publish("/sys/circulation/cancel",this);
			}else if(this.fdType=="ok"){
				var validorObj = registry.byId('circulationScrollView');
				...
				if(Com_Submit(document.sysCirculationMainForm)){
					topic.publish("/sys/circulation/cancel",this);
				}else{
				...
				request(url, {handleAs:'json',sync:true}).then(lang.hitch(this, function(results) {
					if(results && results.datas){
						...
						for(var i =0;i<results.datas.length;i++){
							...
							if(getValByName(obj,"fdBelongPersonDept")=='' || getValByName(obj,"fdBelongPersonDept")==null) {
								store.push({
									text: getValByName(obj, "fdBelongPerson"),
									value: getValByName(obj, "fdId")
								});
							}else {
								store.push({
									text: getValByName(obj, "fdBelongPersonDept") + "-" + getValByName(obj, "fdBelongPerson"),
									value: getValByName(obj, "fdId")
								});
							}
						}
				...
				if(document.getElementsByName("docContent")[0].value == ""){
					document.getElementsByName("docContent")[0].value = "已阅";
				}
			...
			if(this.dialog){
				...
				if(group){
					...
					if(value){
						...
						request(url, {
								method : 'post',
								data : {"List_Selected":value.split(";"),"fdMainId": this.fdMainId}
						}).then(lang.hitch(this, function(results) {
							self.refush();
							self._destroyDialog();
							Tip.success({
