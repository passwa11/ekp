			...
			var selection = declare("mui.address.AddressSelection",[ CategorySelection ],{
				...
				buildIcon : function(selDom, item) {
					...
					if(isPngIcon){
						...
						iconUtils.setIcon(util.formatUrl(item.icon), null, this._headerIcon, null,
								iconNode);
					}else if(dojoConfig.dingXForm && dojoConfig.dingXForm === "true" && item.type == window.ORG_TYPE_PERSON){
						iconNode.innerHTML = iconUtils.createDingIcon(item.label,"div");
					}
				},
