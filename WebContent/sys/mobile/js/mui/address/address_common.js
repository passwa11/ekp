define([
	...
	function tmplList(params){
		...
		var html = 
			'<ul data-dojo-type="mui/address/AddressList" ' + 
				...
				"data-dojo-props=\"isExternal:'{categroy.isExternal}',isEco:'{categroy.isEco}',exceptValue:'{categroy.exceptValue}',cateId:'{categroy.cateId}',deptLimit:'{categroy.deptLimit}',beforeSelectCateHistoryId: '{categroy.beforeSelectCateHistoryId}',isMul:{categroy.isMul},key:'{categroy.key}'," + "channel:'" + params.channel +"',curIds:'{categroy.curIds}',curNames:'${lfn:escapeJs({categroy.curNames})}',selType:{categroy.type},exceptValue:'{categroy.exceptValue}',scope:'{categroy.scope}'\">" +
	...
	function tmplLoad(params) {
		...
		var allView = h('div', {
			...
			tmplPath({ isExt:'{categroy.isExt}',key:'{categroy.key}', channel: 'all', height: '3rem', titleNode : Msg['mui.mobile.address.allCate'] ,visible: true, mixins: 'mui/address/AddressOrgCategoryPathMixin'  }),
			allScrollView
		]);
		...
		html += h('div', {
			...
			'<ul data-dojo-type="mui/address/AddressList" ' +
				...
				"data-dojo-props=\"isExt:'{categroy.isExt}',exceptValue:'{categroy.exceptValue}',cateId:'{categroy.cateId}',history:true,deptLimit:'{categroy.deptLimit}',beforeSelectCateHistoryId: '{categroy.beforeSelectCateHistoryId}',isMul:{categroy.isMul},key:'{categroy.key}',channel:'search',curIds:'{categroy.curIds}',curNames:'${lfn:escapeJs({categroy.curNames})}',selType:{categroy.type},orgType:{categroy.type},exceptValue:'{categroy.exceptValue}',scope:'{categroy.scope}'\">" +
			'</ul>' 
		]);
