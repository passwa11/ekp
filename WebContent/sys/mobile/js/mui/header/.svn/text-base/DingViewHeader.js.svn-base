 ...
 */
define("mui/header/DingViewHeader", [ 
       "dojo/_base/declare", 
       "dijit/_WidgetBase",
	   "dojo/dom-class",
	   "dojo/dom-style",
	...
	var header = declare("mui.header.DingViewHeader", [ WidgetBase ], {
		...
		buildRendering : function() {
			...
			//钉钉渠道过来的
			if(this.serviceSource && "ding" == this.serviceSource){
				//1、状态为待审时则拼接
				if(this.docStatus && "20" == this.docStatus){
					var spanHtml = "等待" + this.handlerValue+"处理" ;
					domConstruct.create("div", {className :"nodeNamesShow",innerHTML:spanHtml}, baseNode);
				}
				//展示所属分类、审批编号、创建时间
