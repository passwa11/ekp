define(
		...
		function(declare, domConstruct, emoji, domStyle, lang, domAttr,
			...
			return declare(
					"mui.form.editor.plugins.dnyling.Plugin",
					[ EditorPluginBaseMixin ],
					{

						type : 'dnyling',

						icon : 'mui-editor-dnyling',

						iconImg:  util.formatUrl('/resource/mobile/dny-f.png'),
						...
						_linglingSaveHandler: function(html,editorObj){
							...
							var imgs = query("img[data-type='dnyling']", textareaNode[0]);
							imgs.forEach(function(item){
								var imgSrc = item.src;
								var outerHTML = item.outerHTML;
								var imgId = getImgageId(outerHTML);
								if(imgId != -1){
									var result = '[dnyling'+imgId+']'; 
							...
							this.connect(this.linglingNode,'click',lang.hitch(this,function(evt) {
										...
										while (target) {
											if (target.tagName == 'IMG') {
												...
												var imgNode = domConstruct.create('img',{width:width,src : src,className : 'muiEditorDnyLingImg','data-type' : self.type});
