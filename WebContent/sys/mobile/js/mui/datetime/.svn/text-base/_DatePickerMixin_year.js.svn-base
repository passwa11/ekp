define([
	...
	return declare("mui/datetime/_DatePickerMixin_year", null, {
		// summary:
		...
		_disableEndDaysOfMonth: function(){
			...
			var pat = this.slots[0].pattern + "/" + this.slots[1].pattern,
				v = this.get("values"),
				date = datelocale.parse(v[0] + "/" + "01", {datePattern:pat, selector:"date",locale: this.lang}),
		...
		_getDateAttr: function(){
			...
			var v = this.get("values"), // [year, month, day]
				s = this.slots,
				pat = s[0].pattern ;
			return datelocale.parse(v[0] , {datePattern:pat, selector:"date", locale: this.lang});
		},
