...
CKEDITOR.dialog.add( 'smiley', function( editor ) {
	...
	for ( i = 0; i < size; i++ ) {
		...
		var imgStyle = '<img class="cke_hand" title="';
		var finalPath = config.smiley_path;
		if(i<33){ //凌凌表情强制设置成23:23
			imgStyle = '<img style="width:23px;height:23px" class="cke_hand" title="';
			finalPath = config.lingling_path;
		}
		...
		html.push(
			...
			imgStyle, config.smiley_descriptions[ i ], '"' +
			' cke_src="', CKEDITOR.tools.htmlEncode( finalPath + images[ i ] ), '" alt="', config.smiley_descriptions[ i ], '"',
			' src="', CKEDITOR.tools.htmlEncode( finalPath + images[ i ] ), '"',
