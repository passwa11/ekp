	...
	<template:replace name="content">
		...
		<list:listview cfg-needMinHeight="false">
			<ui:source type="AjaxJson">
				{url:'/sys/organization/sys_org_person/sysOrgPerson.do?method=list&fdIsExternal=false&available=${JsParam.available}&hidetoplist=${JsParam.hidetoplist}&locked=${JsParam.locked}&all=${JsParam.all}&parent=${JsParam.parent}&fdFlagDeleted=${JsParam.fdFlagDeleted}&fdImportInfo=${JsParam.fdImportInfo}'}
			</ui:source>
			<list:colTable isDefault="true" layout="sys.ui.listview.columntable" 
				rowHref="/sys/organization/sys_org_person/sysOrgPerson.do?method=view&fdId=!{fdId}">
				<list:col-checkbox></list:col-checkbox>
				<c:choose>
					<c:when test="${JsParam.available == '0'}">
						<list:col-auto props="fdOrder,fdPreDept,fdNo,fdName,fdLoginName,fdCanLogin,fdPrePosts,fdEmail,fdMobileNo"></list:col-auto>
					</c:when>
					<c:otherwise>
						<list:col-auto props="fdOrder,fdParent,fdNo,fdName,fdLoginName,fdCanLogin,fdPosts,fdEmail,fdMobileNo"></list:col-auto>
					</c:otherwise>
				</c:choose>
				<c:choose>
