	...
	<template:replace name="content">
			...
			<div style="float:right">
				<div style="display: inline-block;vertical-align: middle;">
					<ui:toolbar id="Btntoolbar" count="5">
						<kmss:auth requestURL="/sys/organization/sys_org_element_external/sysOrgElementExternalPost.do?method=add&parent=${param.parent}">
							<!-- 增加 -->
							<ui:button text="${lfn:message('sys-organization:sysOrgElementExternal.addPost')}" onclick="add()" order="1" ></ui:button>
						</kmss:auth>
						<kmss:auth requestURL="/sys/organization/sys_org_element_external/sysOrgElementExternalPost.do?method=invalidatedAll&cateId=${param.parent}" requestMethod="POST">
							<!-- 所有置为无效 -->
		...
		<list:listview cfg-needMinHeight="false">
			<ui:source type="AjaxJson">
				{url:'/sys/organization/sys_org_element_external/sysOrgElementExternalPost.do?method=list&parent=${JsParam.parent}'}
			</ui:source>
			<list:colTable isDefault="true" layout="sys.ui.listview.columntable"
				rowHref="/sys/organization/sys_org_element_external/sysOrgElementExternalPost.do?method=view&fdId=!{fdId}">
		...
		<!-- 分页 -->
	 	...
	 	<script type="text/javascript">
			...
			window.addAction = function(parentId){
				Com_OpenWindow('<c:url value="/sys/organization/sys_org_element_external/sysOrgElementExternalPost.do" />?method=add&parent='+parentId);
			}
		 	seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'], function($, dialog, topic) {
				...
				window.add = function() {
					...
					if(parent && '2'=='${JsParam.nodeType}'){
						Com_OpenWindow('<c:url value="/sys/organization/sys_org_element_external/sysOrgElementExternalPost.do" />?method=add&parent=${JsParam.parent}');
					}else{
						// 选择外部组织
		 		...
		 		window.edit = function(id) {
			 		if(id)
		 				Com_OpenWindow('<c:url value="/sys/organization/sys_org_element_external/sysOrgElementExternalPost.do" />?method=edit&fdId=' + id);
		 		};
		 		// 所有置为无效
		 		window.invalidatedAll = function() {
					...
					dialog.confirm('<bean:message key="organization.invalidatedAll.comfirm" bundle="sys-organization"/>', function(value) {
						if(value == true) {
							...
							$.ajax({
								url : '<c:url value="/sys/organization/sys_org_element_external/sysOrgElementExternalPost.do?method=invalidatedAll"/>&cateId=${JsParam.parent}',
								type : 'POST',
		 		...
		 		window.invalidated = function(id) {
					...
					dialog.confirm('<bean:message key="organization.invalidatedAll.comfirm" bundle="sys-organization"/>', function(value) {
						if(value == true) {
							window.del_load = dialog.loading();
							$.ajax({
								url : '<c:url value="/sys/organization/sys_org_element_external/sysOrgElementExternalPost.do?method=invalidated"/>',
								type : 'GET',
