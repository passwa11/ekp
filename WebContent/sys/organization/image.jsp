		...
		if(StringUtil.isNotNull(orgId)&&!orgId.equals("formula")&&!orgId.equals("matrix")&&!orgId.equals("rule")){
			...
			if(elem.getFdOrgType() == 8){
				...
				if (BooleanUtils.isTrue(elem.getFdIsExternal())) {
					src = "/sys/organization/resource/image/post_external.png";
				}
				request.getRequestDispatcher(src).forward(
						request, response);
				...
				if (BooleanUtils.isTrue(elem.getFdIsExternal())) {
					src = "/sys/organization/resource/image/dept_external.png";
				}
				request.getRequestDispatcher(src).forward(
						request, response);
				...
				if (BooleanUtils.isTrue(elem.getFdIsExternal())) {
					src = "/sys/organization/resource/image/org_external.png";
				}
				request.getRequestDispatcher(src).forward(
						request, response);
			}
