			...
			<tr>
				...
				<td width=85% colspan="3">
					<table class="tb_simple" width="100%" >
						<tr>
							<td>
								<ui:switch property="fdHideRange.fdIsOpenLimit" enabledText="${ lfn:message('sys-organization:sysOrgEco.hide') }${ lfn:message('sys-organization:sysOrgEco.hide.type.in2') }" disabledText="${ lfn:message('sys-organization:sysOrgEco.hide.not') }${ lfn:message('sys-organization:sysOrgEco.hide.type.in2') }" onValueChange="changeReviewHideRange()"></ui:switch>
							</td>
						</tr>
...
<script language="JavaScript">
	...
	function changeSubType(value) {
		...
		if (fdViewSubType.indexOf('2') != -1) {
			...
			// 解决地址本不能校验输入值的问题
			var textNames = $(".paddingLeft .inputselectmul textarea[name='fdRange.fdOtherNames']");
			textNames.hide();
			textNames.parent().find("div.mf_container").css("display", "inline-block");
		} else {
			hideAndDisabled('otherReview');
