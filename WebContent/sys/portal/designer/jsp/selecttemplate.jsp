...
<template:include ref="default.simple">
	<template:replace name="title">选择页面模板</template:replace>
	...
	<template:replace name="body">
		...
		<script>
			...
			function doSearch(){
				...
				var url = "/sys/portal/sys_portal_portlet/sysPortalPortlet.do?method=selectTemplate&scene=${ param['scene'] }&keywords=" + keywords;
				var source = LUI('listview_template_list').source;
				source.url = url;
				source.get();
			}
			var curTemplateId=Com_GetUrlParameter(window.location.href,'curTemplateId') || '';
		</script>
		<div class="lui_portal_imgTxt_frame lui_templateList_frame">
			...
			<list:listview id="listview_template_list" style="margin-top:40px;">
				...
				<list:gridTable name="gridtable_render" columnNum="3"  gridHref="" >
					<list:row-template>
						{$
						<div class="lui_portal_imgTxt_item {%curTemplateId == grid['fdId'] ? 'lui_portal_imgTxt_item_current' : ''%}" onclick="selectPortletRender('{%grid['fdId']%}','{%grid['fdName'] %}')" >$}
							...
							{$
							<span class="lui_portal_imgTxt_item_flag">当前模板</span>
