				...
				seajs.use(['lui/dialog'],function(dialog){
					dialog.build({
						config : {
								...
								content : {  
									type : "iframe",
									url : "/sys/portal/designer/jsp/selecttheme.jsp?curThemeId="+$("[name='fdTheme']").val()
								}
						},
							...
							<td colspan="3">
								...
								<% if (com.landray.kmss.sys.organization.util.SysOrgEcoUtil.IS_ENABLED_ECO) { %>
								    <% if(com.landray.kmss.sys.organization.util.SysOrgEcoUtil.isExternal()) { %>
								        <!-- （为空则本组织人员可使用） -->
								        <bean:message  bundle="sys-portal" key="sysPortalPage.msg.nreader.outter" />
								    <% } else { %>
								        <!-- （为空则所有内部人员可使用） -->
								        <bean:message  bundle="sys-portal" key="sysPortalPage.msg.nreader.inner" />
								    <% } %>
								<% } else { %>
								    <!-- （为空则所有人可使用） -->
								    <bean:message  bundle="sys-portal" key="sysPortalPage.msg.nreader" />
								<% } %>
								</span>
							</td>
						</tr>
