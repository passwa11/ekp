define([
	...
	], function(declare,topic,query,EdgeToEdgeStoreList,_StoreListMixin,ListNoDataMixin,dateClz,util,lang,
			request,domStyle,domGeometry,LocationListMixin) {
	...
	return declare("sys.attend.LocationPoiList", [EdgeToEdgeStoreList,_StoreListMixin,ListNoDataMixin,LocationListMixin], {
		...
		_formatDatas :function(widget,args){
			...
			var tmpTitle = location.name.split(' ');
			location.name = tmpTitle.length>0 ?tmpTitle[0]:location.name;
			if(pois.length>0){
				var result = [];
				for(var i = 0;i< pois.length;i++){
					var loc = pois[i];
					loc.showStatus = showStatus;
					if(loc.name==location.name && loc.address==location.address){
						continue;
					}
					result.push(loc);
				}
...
//				var winH = domStyle.get(query('.muiLocationDialog')[0],'height');
...
//				var bottomH = domStyle.get(query('.muiLocationToolbar')[0],'height');
//				bottomH = widget.showStatus=='edit' || widget.canSearch ? 2 * bottomH : bottomH;
				domStyle.set(query('.muiLocationListDom .mblScrollableView')[0],'height',contentH+'px');
				//domStyle.set(query('.muiLocationListDom .mblScrollableView')[0],'top',(winH-contentH-bottomH)+'px ');
				//列表滚动位置
