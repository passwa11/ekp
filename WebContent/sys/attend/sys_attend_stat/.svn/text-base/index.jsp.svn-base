...
<template:include ref="default.list">
	...
	<template:replace name="nav">
		<script>
			function addDoc(){
				Com_OpenWindow("${LUI_ContextPath}/sys/attend/sys_attend_stat/sysAttendStat.do?method=add");
			}
			function delDoc(){
				var values = [];
				...
				seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic){
					dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(value){
						if(value==true){
							...
							$.post('${LUI_ContextPath}/sys/attend/sys_attend_stat/sysAttendStat.do?method=deleteall&categoryId=${param.categoryId}',$.param({"List_Selected":values},true),function(data){
								if(del_load!=null){
									del_load.hide();
		...
		<ui:combin ref="menu.nav.create">
			...
			<ui:varParam name="button">
				[
					<kmss:auth requestURL="/sys/attend/sys_attend_stat/sysAttendStat.do?method=add">
					{
	...
	<template:replace name="content">
		...
		<list:criteria id="criteria1">
			...
			<list:cri-auto modelName="com.landray.kmss.sys.attend.model.SysAttendStat" 
				property="fdDate;fdCategoryId;fdTotalTime;docCreateTime;fdLateTime;fdLeftTime;fdStatus;fdOutside;fdCategoryName" />
		</list:criteria>
				...
				<tr>
					...
					<td align="right">
						<ui:toolbar count="3">
							...
							<kmss:auth requestURL="/sys/attend/sys_attend_stat/sysAttendStat.do?method=deleteall">					 								
							...
							<c:import url="/sys/introduce/import/sysIntroduceMain_cancelbtn.jsp" charEncoding="UTF-8">
								<c:param name="fdModelName" value="com.landray.kmss.sys.attend.model.SysAttendStat" />
							</c:import>
							--%>
							<%-- 修改权限 
							<c:import url="/sys/right/import/doc_right_change_button.jsp" charEncoding="UTF-8">
								<c:param name="modelName" value="com.landray.kmss.sys.attend.model.SysAttendStat" />
							</c:import>
							--%>				 
						</ui:toolbar>
					</td>
		...
		<ui:fixed elem=".lui_list_operation"></ui:fixed>
	 	...
	 	<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/sys/attend/sys_attend_stat/sysAttendStat.do?method=data&categoryId=${param.categoryId}'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false"
				rowHref="/sys/attend/sys_attend_stat/sysAttendStat.do?method=view&fdId=!{fdId}"  name="columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial>
				 
				<list:col-auto props="fdDate;fdCategoryId;fdTotalTime;docCreateTime;fdLateTime;fdLeftTime;fdStatus;fdOutside;fdCategoryName;docCreator.fdName"></list:col-auto>
			</list:colTable>
			<!-- 摘要视图 -->	
			<list:rowTable isDefault="false"
				rowHref="/sys/attend/sys_attend_stat/sysAttendStat.do?method=view&fdId=!{fdId}" name="rowtable" >
				...
				docSubject;icon;fdDescription;docCategory.fdName;docAuthor.fdName;docDept.fdName;docPublishTime;tag
				 --%>
			   	{"showOtherProps":"fdDate;fdCategoryId;fdTotalTime;docCreateTime;fdLateTime;fdLeftTime;fdStatus;fdOutside;fdCategoryName;docCreator.fdName"}
				</list:row-template>
			</list:rowTable>
		</list:listview> 
