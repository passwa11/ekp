...
function checkValue() {
	...
	fdCardDate = form['fdCardDate'],
	fdCardTime = form['fdCardTime'],
...
var isDateList = fdCardDate['model'].indexOf('[]') >= 0;
var isCardTimeList = fdCardTime['model'].indexOf('[]') >= 0;

if(isDateList || isCardTimeList) {
	if(!(isDateList && isCardTimeList)) {
		alert('“签卡日期”，“签卡时间”，必须在同一个明细表中');
		return false;
	} else {
		var listName = fdCardDate['value'].split('.')[0];
		if(listName != fdCardTime['value'].split('.')[0] 
			|| listName != fdCardDate['value'].split('.')[0]) {
			alert('“签卡日期”，“签卡时间”，必须在同一个明细表中');
			return false;
		}
	}
...
$(function(){
	...
	createSelectHTML('#dateTd', 'fdCardDate', 'Date;Date[]');
	createSelectHTML('#timeTd', 'fdCardTime', 'Time;Time[]');
});
...
<table id="List_ViewTable" class="tb_normal" width="98%">
	...
	<tr height="21">
		<td class="td_normal_title" width="20%">
			<bean:message bundle="sys-attend" key="sysAttend.event.card.date" />
		</td>
		<td id='dateTd'>
			
		<span class="txtstrong">*</span><span style="margin-left: 5px;color: #999"><bean:message bundle="sys-attend" key="sysAttend.event.card.date" /></span>
		</td>
	</tr>
	<tr height="21">
		<td class="td_normal_title" width="20%">
			<bean:message bundle="sys-attend" key="sysAttend.event.card.time" />
		</td>
		<td id='timeTd'>
			
		<span class="txtstrong">*</span><span style="margin-left: 5px;color: #999"><bean:message bundle="sys-attend" key="sysAttend.event.card.time" /></span>
		</td>
	</tr>
</table>
</form>
