...
%>
<form action="sysAttendMain_MonthDeptReport.jsp" method="post"
...
%>
		<td>
			<!--人数小计--><%=map.get("hourTxt"+j)%></td>
		<td>
			<!--人数小计--><%=map.get("personCount"+j)%></td>
		<td>
			<!--人数小计--><%=map.get("avg"+j)%></td>
				<%
			}
		%>
<!-- 		<td> -->
...
<script language="JavaScript">
    ...
    function exportResult() {
        let url = makeSrc('exportLeaveMonth2Change');
        window.open(url);
    }
