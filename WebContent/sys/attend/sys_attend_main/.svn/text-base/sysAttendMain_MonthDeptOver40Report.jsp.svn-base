...
%>
<form action="sysAttendMain_MonthDeptOver40Report.jsp" method="post"
...
<thead>
	<tr>
		<th nowrap>序号</th>
		<th nowrap id="aaa" style="margin:0; padding:0;">
		一级部门
		</th>	
		...
		<th nowrap >总人数</th>
		<th nowrap >加班人数</th>
		<th nowrap >超过40小时人数</th>
		<th nowrap >超过40小时占加班人数比</th>
		<th nowrap >法定加班(h)</th>
		<th nowrap >周末加班(h)</th>
		<th nowrap >工作日加班(h)</th>
		<th nowrap >>40小时汇总</th>
		<th nowrap ><%=m %>月总加班小时</th>
	</tr>
        </thead>
        <tbody >
     <%
   if (list.size() > 0) {
	...
	<tr onmouseover="this.style.backgroundColor='#ffff66';"
		...
		<td>
			<!--人数小计--><%=map.get("personCount")%></td>
		<td>
			<!--人数小计--><%=map.get("jiabanCount")%></td>
		<td>
			<!--人数小计--><%=map.get("over40")%></td>
		<td>
			<!--人数小计--><%=map.get("jiabanlv")%></td>
			<td>
			<!--人数小计--><%=map.get("holiday")%></td>
		<td>
			<!--人数小计--><%=map.get("off")%></td>
		<td>
			<!--人数小计--><%=map.get("work")%></td>
		<td>
			<!--人数小计--><%=map.get("countover40")%></td>
		<td>
			<!--人数小计--><%=map.get("monthcount")%></td>
<!-- 		<td> -->
...
<script language="JavaScript">
    ...
    function exportResult() {
        let url = makeSrc('exportLeaveMonth3Change');
        window.open(url);
    }
