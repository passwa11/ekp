...
function initValue(config, eventType, nodeObject, context){
	...
	for(key in listenerConfig) {
		...
		// 兼容代码
		if(key == 'fdResumeTargets') {
			selectObj = $('select[name="fdResumeTargets"]');
		...
		} else if(key == 'fdResumeType') {
			selectObj = $('select[name="fdResumeType"]');
		}
...
// 校验
function checkValue() {
	var form = formatForm($("#resumeForm")),
		fdResumeTargets = form['fdResumeTargets'],
		fdResumeType = form['fdResumeType'],
	...
	if(!fdResumeTargets['value']){
		alert('销假人不能为空');
		return false;
	}
	if(!fdResumeType['value']){
		alert('销假类型不能为空');
		return false;
	}
	...
	if(dayListName || halfListName || hourListName){
		var isSameList = false;
		if(fdResumeType['value'].indexOf('.') > -1){
			if(fdResumeType['value'].split('.')[0] == dayListName
					|| fdResumeType['value'].split('.')[0] == halfListName
					|| fdResumeType['value'].split('.')[0] == hourListName){
				isSameList = true;
			}
		}
		if(!isSameList) {
			alert('“销假类型”必须在其中一个明细表中');
			return false;
		}
	}
...
function returnValue() {
	return JSON.stringify(formatForm($("#resumeForm")));
};
/****流程模块业务方法 end*****/
...
$(function(){
	createSelectHTML('#targetsTd', 'fdResumeTargets', 'com.landray.kmss.sys.organization.model.SysOrgPerson');
	
	createSelectHTML('#resumeTypeTd', 'fdResumeType', 'String;String[];Double;Double[]');
...
<center>
<form id="resumeForm" name="resumeForm" action="">
<table id="List_ViewTable" class="tb_normal" width="98%">
	<tr height="21">
		<td class="td_normal_title" width="20%">
			<bean:message bundle="sys-attend" key="sysAttend.event.terminateLeave.persons" />
		</td>
		<td id='targetsTd'>
	...
	<tr height="21">
		<td class="td_normal_title" width="20%">
			<bean:message bundle="sys-attend" key="sysAttend.event.terminateLeave.type" />
		</td>
		<td id='resumeTypeTd'>
