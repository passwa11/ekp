...
function generateTree(){
	...
	LKSTree = new TreeView("LKSTree", "<bean:message bundle="sys-unit" key="kmImissiveUnit.cate.exchange" />", document.getElementById("treeDiv"));
	LKSTree.ClickNode = Dialog_ClickNode;
	var n1 = LKSTree.treeRoot;
	var param = "sysUnitDecTreeService"
	n1.isExpanded = true;
	var getUnitType = Com_GetUrlParameter(cateUnitParam, "type");
	if ('${param.allUnitByDec}' == 'true') {
		getUnitType = "allUnitByDec";
	} else if (!getUnitType) {
		getUnitType = "allUnit";
	}
	var treeBean  = "sysUnitDecTreeService", unitDataBean = "sysUnitListWithAuthByDecService&parentId=!{value}&type=" + getUnitType;

	var fdUnitId = Com_GetUrlParameter(cateUnitParam, "fdUnitId");
	if(fdUnitId){
		unitDataBean += "&fdUnitId="+fdUnitId;
	}

	n1.AppendBeanData(treeBean, unitDataBean, null, false, null);
...
function Dialog_ClickNode(node){
	...
	window.top.sysUnitSelectType = 2;
