...
define(function(require, exports, module) {
	...
	var OrgTreeViewComponent = AbstractTreeViewComponent.extend({
		...
		generateTree : function($super){
			...
			if(params.isExternal){
				para += "&fdIsExternal=" + params.isExternal
			}
			...
			n1.AppendOrgData(treeOrgType, para, null, params.startWith, null, deptLimit, params.isExternal);
			n1.isExpanded = true;
		...
		clickNode : function(node){
			...
			if(node.parameter.indexOf('organizationDialogRoleList')==-1){
				...
				if(params.isExternal){
					beanData += "&fdIsExternal=" + params.isExternal
				}
	...
	var GroupTreeViewComponent = AbstractTreeViewComponent.extend({
		...
		generateTree : function($super){
			...
			// 开启生态组织，并且当前用户是外部人员时，不显示公共群组
			if(!(Com_Parameter && Com_Parameter.isEnabledEco == "true" && Com_Parameter.isExternal == "true")) {
			...
			var external = "";
			if(params.isExternal) {
				external = "&isExternal=" + params.isExternal;
			}
			n3.AppendBeanData("organizationDialogMyAddress","organizationDialogMyAddress&id=!{value}&orgType="+selectType+external,null,false);
			n3.isExpanded = true;
