...
<tr class="lui_varkind_string_wrap_tr">
	<td ><span>${ luivar['require'] ? "<i class='lui_varkind_require'>*</i>" : "" }${ lfn:msg(luivar['name']) }</span></td>
	<td><input style="width:95%" class="inputsgl" id="${ luivarid }" name="${ luivar['key'] }" type="text" value="${luivar['default']}" />
		<p class="com_help">${ lfn:msg(luivarparam['help']) }</p></td>
</tr>