...
<html class="lui_portal_default">
	<head>
		...
		<!-- 是否开启合并加载js模式  -->
		<c:choose>
			<c:when test="${compressSwitch eq 'true' && lfn:jsCompressEnabled('sysUiCompressExecutor', 'default_list_all_combined')}">
				<script src="<%= PcJsOptimizeUtil.getScriptSrcByExtension("sysUiCompressExecutor","default_list_all_combined") %>?s_cache=${ LUI_Cache }">
				</script>
			</c:when>
		</c:choose>
