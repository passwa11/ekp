...
var _css = '<style>  ' +
	'#' + render.dataview.cid + '{\t  ' + _style + '  }  ' +
	'#' + render.dataview.cid + ' .lui-classic-list-6 .lui-classic-list-item{\tmargin-bottom:' + _marginB + ';  }}' +
	'  </style>';
...
for (var i = 0; i < data.length; i++) {
	...
	//回复数
	var _otherinfo = render.env.fn.formatText(item.otherinfo) || '';
	// 回复数html
	var _otherinfo_html = _otherinfo ? '<div class="lui-classic-replies">' + _otherinfo + '</div>' : '';
	...
	var _html = '<span class="lui-classic-num ' + _numClass + '">' + (i + 1) + '</span><a href="' + getUrl(_href) + '"  target="_blank" class="lui-classic-title" title="' + _text + '">'
		+ _text + '</a>  ' + _otherinfo_html + '  <div class="lui-classic-date">' + _created + '</div>';
	$li.html(_html);
