...
var cols = parseInt(data.length, 10);
...
var tr = $('<table />')
    .attr('class', 'lui_dataview_treemenu_fall lui_dataview_treemenu_rt_1')
    .appendTo(element)
    .get(0)
    .insertRow(-1);
...
function insertCell(first) {
    ...
    var _icon = (i % 8) + 1;
    td.className = 'lui_dataview_treemenu_item lui_treemenu' + _icon;
    td.style.width = width;
    if (first) {
        td.className = 'lui_dataview_treemenu_item lui_dataview_treemenu_fall_firstCol';
    }
}

function insertData(oneData, parent, lv) {
    var div = $('<div/>')
        .attr('class', 'lui_dataview_treemenu_fall_lv_' + lv)
        .appendTo(parent);
    var txt = $('<a />')
        .attr({ class: 'textEllipsis lui_dataview_treemenu_fall_lv_' + lv + '_txt', title: oneData.text })
        .text(oneData.text)
        .appendTo(div);
    ...
    if (oneData.children != null && oneData.children.length > 0) {
        var child = $('<div/>')
            .attr('class', 'lui_dataview_treemenu_fall_lv_' + lv + '_child')
            .appendTo(div);
