...
var container = $('<div class="lui_dataview_image lui-image-list-2' + extend + ' clearfloat"/>');
container.attr('id', render.cid + '____image____');
var content = $('<div class="lui_dataview_image' + extend + '_content"/>');
var table = $('<table class="lui_image_list_table" data-lui-mark="dataview.image.table"/>');
var w2h = size.split('*'),
    w = w2h[0],
    h = w2h[1];
...
function createImage(grid) {
    ...
    ___a.attr('href', env.fn.formatUrl(grid['href']));
...
function createText(text, href) {
    var __text = $('<a />');
    __text.attr('href', env.fn.formatUrl(href));
    __text.attr('title', text);
    __text.addClass('lui_com_subject');
...
var __tr = '<tr>',
    tr__ = '</tr>';
var __td = '<td width="' + ____width + '"' + (!isSlide() ? 'style="padding-left:0;padding-right:0"' : '') + '>',
    td__ = '</td>';
...
function createTd(grid) {
    ...
    var div = $('<div/>');
    div.append(grid_img);
    var imagelistitem = $('<div/>').attr('class', 'lui_imagelist_item').appendTo(div); //add weige
    imagelistitem.css('width', w);

    if (showTitle && showTitle == '0') {
        //div.append(grid_text);
        grid_text.appendTo(imagelistitem);
    }
    if (showCreated) {
        $('<p/>').attr('class', 'lui_created').text(grid['created']).appendTo(imagelistitem);
    }
...
function ___nomalBuild(html) {
    for (var i = 0; i < data.length; i++) {
        var grid = data[i];
        if (i % columnNum === 0) html += __tr;
        html += createTd(grid);
        if (i % columnNum == columnNum - 1) html += tr__;
    }
    return html;
}
...
if (isHorizontSlide()) {
    ...
    container.append(content);
    container.append($('<div class="lui_dataview_image' + extend + '_right"/>')); // 向右
...
if (isVerticalSlide()) {
    ...
    container.append($('<div class="lui_dataview_image' + extend + '_bottom"/>')); // 向下
...
function horizontSlide() {
    ...
    var ____table____ = ____image____.find('[data-lui-mark="dataview.image.table"]');
    ...
    var w = tdw * num;
    var ____left = ____image____.find('.lui_dataview_image' + extend + '_left'),
        ____right = ____image____.find('.lui_dataview_image' + extend + '_right');
    refreshStatus(____left, ____right, 0, tw, cw);
    lazyload(c);
    ____image____.bind('click', function (evt) {
        var $parent = $(evt.target);
        while ($parent.length > 0) {
            if ($parent.hasClass('lui_dataview_image' + extend + '_left')) {
            ...
            if ($parent.hasClass('lui_dataview_image' + extend + '_right')) {
                // 图片so little
                if (tw <= cw) return;
...
function verticalSlide() {
    ...
    var ____top = ____image____.find('.lui_dataview_image' + extend + '_top'),
        ____bottom = ____image____.find('.lui_dataview_image' + extend + '_bottom');
    refreshStatus(____top, ____bottom, 0, th, ch);
    lazyload(c);
    ____image____.bind('click', function (evt) {
        var $parent = $(evt.target);
        while ($parent.length > 0) {
            if ($parent.hasClass('lui_dataview_image' + extend + '_top')) {
            ...
            if ($parent.hasClass('lui_dataview_image' + extend + '_bottom')) {
                // 图片so little
                if (th <= ch) return;
