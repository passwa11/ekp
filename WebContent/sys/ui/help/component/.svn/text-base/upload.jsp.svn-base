    ...
    <script>
		...
		*/
	    function replaceComponent(){
			var replaceUrl = '<c:url value="/sys/ui/sys_ui_component/sysUiComponent.do?method=replaceExtend"/>';
			var extendId = "${extendId}";     // 部件包ID
			var folderName = "${folderName}"; // 主题包临时存放目录文件夹名称
		...
		*/	
	    function continueUpload(){
	        window.location.href= Com_Parameter.ContextPath+"sys/ui/help/component/upload.jsp";
	    }
	</script>
</template:replace>
...
<!-- 上传部件包-->
<html:form action="/sys/ui/sys_ui_component/sysUiComponent.do?method=upload" enctype="multipart/form-data">
    <c:if test="${empty componentIsExits}" >  
    <div class="ld-upload-login-tel-content">
         <div class="ld-upload-login-tel-content-file clearfix">
             <span>${lfn:message('sys-ui:mall.component.add') }</span>
         ...
         <div class="lui_portal_footer_btnGroup" align="center">
			<ui:button styleClass="lui_toolbar_btn_def" style="border-radius: 4px;" text="${lfn:message('button.close') }" onclick="delFileBeforeClose();Com_CloseWindow();"></ui:button>
    ...
    <c:if test="${not empty componentIsExits && componentIsExits==true && not empty errorMessage}" >
	...
	<div style="text-align:center;margin-top:40px;">
	   <div id="replaceButtonContent">
	      <!-- 确定按钮 -->
         <div class="confirm_replace_button" onclick="replaceComponent()">${lfn:message('button.ok')}</div>
         <!-- 取消按钮 -->
        ...
        <!-- 上传成功，提示用户 -->
        <c:if test="${not empty componentIsExits && componentIsExits==false && not empty successMessage}" >
...
<script>
var _validate = $KMSSValidation(document.sysUiComponentForm);
seajs.use(['lui/jquery','lui/dialog'],function($,dialog) {
	window.uploadTemplate = function() {
		var file = document.getElementsByName("file");
		if(file[0].value==null || file[0].value.length==0){
			dialog.alert("${lfn:message('sys-ui:sys.component.upload.error') }");
			return false;
		}
		Com_Submit(sysUiComponentForm,'upload');
	};
...
function delFileBeforeClose(){
	...
	if(path){
		$.ajax({
	    	url: Com_Parameter.ContextPath + "sys/ui/sys_ui_component/sysUiComponent.do?method=delPreviewFile",
	    	dataType : 'json',
				type : 'post',
				data: {
					"directoryPath":path
...
function afterUploadFile(){
	seajs.use(['lui/jquery','lui/dialog'],function($,dialog) {
	  ...
	  $.ajax({
      	url: Com_Parameter.ContextPath + "sys/ui/sys_ui_component/sysUiComponent.do?method=getThemeInfo",
      	dataType : 'json',
			type : 'post',
			data: formData,
			cache: false,  
			...
			success : function(rtn){
				if(rtn.status=="1"){
					...
					seajs.use(['lui/jquery','lui/dialog'],function($,dialog) {
						dialog.alert("${lfn:message('sys-ui:sys.ui.upload.component.error') }");
