...
<script type="text/javascript">
	...
	Com_IncludeFile("right.css", Com_Parameter.ContextPath + "sys/right/dingSuit/css/","css",true);
</script>
...
window.onload=function(){
	showDocZone();
	var fdIds = window.parent.$dialogParameter.fdIds || '';
	...
	<tr id="authTemplateEditorZone">
		...
		<td width=90%>
			...
			<div id="authTemplateEditorCheckInput" style="display:none">
				<xform:address showStatus="edit" textarea="true" mulSelect="true" isExternal="false" propertyId="authTemplateEditorIds" propertyName="authTemplateEditorNames" style="width:97%;height:90px;" ></xform:address>
			</div>
		</td>
	</tr>
	<%}%>
	...
	<tr id="authTemplateReaderZone">
		<td width=90%>
			...
			<div id="authTemplateReaderCheckInput" style="display:none">
				<xform:address showStatus="edit" textarea="true" mulSelect="true" isExternal="false" propertyId="authTemplateReaderIds" propertyName="authTemplateReaderNames" style="width:97%;height:90px;" ></xform:address>
			</div>
		</td>
	</tr>
	<%}%>
		...
		<td width=90%>
			...
			<% if (com.landray.kmss.sys.organization.util.SysOrgEcoUtil.IS_ENABLED_ECO) { %>
			    <% if(com.landray.kmss.sys.organization.util.SysOrgEcoUtil.isExternal()) { %>
			        <!-- （为空则本组织人员可拷贝） -->
			        <bean:message  bundle="sys-right" key="right.category.att.authAttCopys.orgnazation" />
			    <% } else { %>
			        <!-- （为空则所有内部人员可拷贝） -->
			        <bean:message  bundle="sys-right" key="right.category.att.authAttCopys.note" />
			    <% } %>
			<% } else { %>
			    <!-- （为空则所有人可拷贝） -->
...
<!-- 			<bean:message key="right.category.att.authAttDownloads.note" bundle="sys-right"/> -->
			<% if (com.landray.kmss.sys.organization.util.SysOrgEcoUtil.IS_ENABLED_ECO) { %>
			    <% if(com.landray.kmss.sys.organization.util.SysOrgEcoUtil.isExternal()) { %>
			        <!-- （为空则本组织人员可下载） -->
			        <bean:message  bundle="sys-right" key="right.category.att.authAttDownloads.orgnization" />
			    <% } else { %>
			        <!-- （为空则所有内部人员可下载） -->
			        <bean:message  bundle="sys-right" key="right.category.att.authAttDownloads.note" />
			    <% } %>
			<% } else { %>
			    <!-- （为空则所有人可下载） -->
	...
	<tr id="authTmpAttPrintZone">
		<td width=90%>
			...
			<% if (com.landray.kmss.sys.organization.util.SysOrgEcoUtil.IS_ENABLED_ECO) { %>
			    <% if(com.landray.kmss.sys.organization.util.SysOrgEcoUtil.isExternal()) { %>
			        <!-- （为空则本组织人员可打印） -->
			        <bean:message  bundle="sys-right" key="right.category.att.authAttPrints.orgnization" />
			    <% } else { %>
			        <!-- （为空则所有内部人员可打印） -->
			        <bean:message  bundle="sys-right" key="right.category.att.authAttPrints.inner" />
			    <% } %>
			<% } else { %>
			    <!-- （为空则所有人可打印） -->
...
<!-- 			<bean:message key="right.category.att.authAttPrints.note" bundle="sys-right"/> -->
				    ...
				    <% if(com.landray.kmss.sys.organization.util.SysOrgEcoUtil.isExternal()) { %>
				        <!-- （为空则本组织人员可使用） -->
				        <bean:message  bundle="sys-right" key="right.category.att.authAttPrints.orgnization" arg0="${ecoName}" />
				    <% } else { %>
				        <!-- （为空则所有内部人员可使用） -->
				        <bean:message  bundle="sys-right" key="right.category.att.authAttPrints.inner" />
				    <% } %>
...
<div class="optBarDiv">
	<div class="lui_btn lui_btn_close">
		<input type="button" value="<bean:message key="button.close"/>" onclick="Com_CloseWindow();">
	</div>
	<div class="lui_btn lui_btn_save">
