...
function getVarIdByName(varName, isFunc){
	if(isFunc){
		...
		var otherVarInfo = dialogObject.formulaParameter.otherVarInfo;
		if(otherVarInfo && otherVarInfo.length>0){
			for(var i=0; i<otherVarInfo.length; i++) {
				var varInfoData = otherVarInfo[i].data;
				if(varInfoData){
					for (var j = 0; j < varInfoData.length; j++) {
						var label = varInfoData[j].useLabel || varInfoData[j].fullLabel || varInfoData[j].label;
						if(label==varName){
							return varInfoData[j].name;
						}
					}
				}
			}
	...
	if(isFunc){
		...
		if(otherVarInfo && otherVarInfo.length>0){
			for(var i=0; i<otherVarInfo.length; i++) {
				...
				if(varInfoData){
					for (var j = 0; j < varInfoData.length; j++) {
						var name = varInfoData[j].name;
						if(name==varName){
							return varInfoData[j].useLabel || varInfoData[j].fullLabel || varInfoData[j].label;
						}
					}
				}
			}
		...
		for(var i=0; i<otherVarInfo.length; i++) {
			...
			if(varInfoData){
				for (var j = 0; j < varInfoData.length; j++) {
					var label = varInfoData[j].useLabel || varInfoData[j].fullLabel || varInfoData[j].label;
					if(label==varName){
						return varInfoData[j].controlType;
					}
				}
			}
		}
...
function validateFormula(action){
	...
	if(otherVarInfo && otherVarInfo.length>0){
		for(var i=0; i<otherVarInfo.length; i++) {
			...
			if(varInfoData){
				for (var j = 0; j < varInfoData.length; j++) {
					//兼容金额字段（BigDecimal_Money），金额字段实质上就是BigDecimal型
					otherVarType = varInfoData[j].type;
					if(otherVarType && otherVarType.indexOf('BigDecimal_') > -1){
						otherVarType = 'BigDecimal';
					}
					info[varInfoData[j].name+".type"] = otherVarType;
				}
			}
		}
	}
