			...
			<div class="conf_show_more_w">
				<div class="conf_btn_edit">
					...
					<kmss:auth requestURL="/sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=deleteall" requestMethod="POST">
						<!-- 删除 -->
						<a class="btn_txt" href="javascript:__del('deleteall','2','${sysNotifySystemTodo.fdId}')">${lfn:message('button.delete')}</a>
					</kmss:auth>
					</c:if>
					...
					<kmss:auth requestURL="/sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=mngDelete" requestMethod="POST">
						<!-- 删除 -->
						<a class="btn_txt" href="javascript:__del('mngDelete','2','${sysNotifySystemTodo.fdId}')">${lfn:message('button.delete')}</a>
					</kmss:auth>
					</c:if>
					<kmss:auth requestURL="/sys/notify/sys_notify_lang/sysNotifyLang.do?method=selfTitleLang" requestMethod="POST">
						<a class="btn_txt" href="javascript:setNotifySelfTitle('2','${sysNotifySystemTodo.fdId}')"> 自定义消息</a>
					</kmss:auth>
				</div>
			</div>
			<!--操作按钮 结束-->
