...
<div class="lui_dataview_classic_heading">
	...
	<span class="lui_notify_count_tip">
		...
		<c:if test="${not empty infoTip}">
			<a class="lui_notify_link_txt" href="${LUI_ContextPath }/sys/notify/index.jsp${pageScope.itemPagePath}${lfn:escapeHtml(_fdAppName)}${crqPrefix}" target="_blank">
	...
	<c:if test="${fastTodo && isCanFastTodo}">
		  <span class="lui_fastApproveIcon" title='<bean:message bundle="sys-notify" key="sysNotifyTodo.tab.title5" />'>
			  <a href="${LUI_ContextPath }/sys/notify/index.jsp#j_path=%2Ffastview${lfn:escapeHtml(_fdAppName)}${crqPrefix}" target="_blank">
