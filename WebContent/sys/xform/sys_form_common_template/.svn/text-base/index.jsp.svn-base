	...
	<template:replace name="content">
		...
		<list:listview>
			...
			<list:colTable isDefault="true" layout="sys.ui.listview.columntable" onRowClick="openDetail('!{fdId}')">
				...
				<list:col-html>
					{$ <input type="hidden" value="{%row['fdIsInside']%}" name="{%row['fdIsInside']%}"></input> $}
				</list:col-html>
				<list:col-auto props="fdName,fdDefaultFlag,fdCreator.fdName,fdCreateTime,operations"></list:col-auto>
			</list:colTable>
		...
		<!-- 分页 -->
	 	...
	 	<script type="text/javascript">
		 	...
		 	// 编辑
				...
				/**
				 * 初始化复选框、内置通用表单不允许操作、去除复选框
				 */
				window.initCheckbox = function() {
					var table = $("table.lui_listview_columntable_table");
					// 获取表格中所有的行
					var trs = table.find("tbody>tr");
					if(trs.length > 0) {
						$.each(trs, function(i, tr) {
							var fdIsInside = $(tr).find("input:eq(1)").val();
							if(fdIsInside=='1'){
								$(tr).find("input[type='checkbox']:eq(0)").addClass("isInside");
							}
						});
					}
				};
