	...
	seajs.use(['lui/dialog'], function(dialog){
		...
		dialog.iframe(url,'<bean:message bundle="sys-search" key="table.sysSearchCate"/>',
			function(value) {
				if (value) {
					value = value[0] || "";
					$("[name='fdCategoryId']").val(value.id);
					$("[name='fdCategoryName']").val(value.name);
				}
			}, 
    ...
    $.ajax({     
	     ...
	     success:function(data){
		    ...
		    if(results['tempName']!=null){
	    		...
	    		seajs.use(['lui/dialog'], function(dialog){
	    			var action = "sysSearchTemplateTree&modelName=" + modelName.value + "&key=" + key.value;
	    			var treeTitle = '<bean:message bundle="sys-search" key="table.sysSearchMain"/>';
	    			var params = {'action':action,'title':treeTitle,"authNodeValue":categoryIncludeValue};
	    			var url = "/sys/search/dingSuit/sysSearchCate_dialog.jsp";
	    			dialog.iframe(url,treeTitle,
	    				function(value) {
	    					console.log(value);
	    					if (value) {
	    						value = value[0] || "";
	    						$("[name='fdTemplateId']").val(value.id);
	    						$("[name='fdTemplateName']").val(value.name);
	    					}
	    					if(value == null) {
	    	    				return;
	    	    			}
