...
<c:set var="modelName" value="${viewConditionInfo.modelName }"></c:set>
<c:if test="${fn:length(viewConditionInfo.entries) > 0 }">
	<list:criteria id="criteria1">
	<c:forEach items="${viewConditionInfo.entries}" var="conditionEntry" varStatus="vStatus">
		...
		<c:choose>
			...
			<c:when test="${conditionEntry.type=='person'}">
				<list:cri-ref key="${propertyName}.fdName" ref="criterion.sys.string" title="${titleText }" expand="${expend }" multi="${muti }" defaultValue="${defaultValue}" conditionType="${conditionEntry.type}"/>
			</c:when>
