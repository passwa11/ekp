	...
	<template:replace name="body">
		...
		<script>
			...
			function ok(){
				...
				var url =  "sys/modeling/main/modelingAppModelMain.do?method=add&fdAppModelId=${JsParam.fdAppModelId }&fdAppFlowId=" + flowId;
				//新建视图相关入参透传
				var fdOprId = "${JsParam.fdOprId }";
				if(fdOprId){
					url += "&fdOprId=" + fdOprId;
				}
				var incFdId = "${JsParam.incFdId }";
				if(incFdId){
					url += "&incFdId=" + incFdId;
				}
				var data ={
				...
				$dialog.hide(JSON.stringify(data));
				// Com_CloseWindow();
			}
