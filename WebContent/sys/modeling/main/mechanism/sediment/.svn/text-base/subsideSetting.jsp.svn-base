			...
			<tr>
				...
				<td>
					<select ftype="String" name="docSubjectMapping" class="inputsgl" style="width:80%;">
						...
						<option value="DocSubject">${lfn:message('sys-modeling-main:modeling.automatic.generation.rules') }</option>
					</select>
				</td>
			</tr>
			<!-- 作者 -->
			<tr>
				...
				<td >
					<select ftype="com.landray.kmss.sys.organization.model.SysOrgPerson" name="docCreatorMapping" class="inputsgl" style="width:80%;">
						<option value="">${lfn:message('sys-modeling-main:modeling.please.choose') }</option>
						<option value="currentUser">${lfn:message('sys-modeling-main:modeling.current.user') }</option>
						<option value="docCreator">${lfn:message('sys-modeling-main:modeling.entrant') }</option>
					</select>
				</td>
			</tr>
			</table></td>
...
Com_AddEventListener(window,"load",function(){
	var url = "${LUI_ContextPath}/sys/modeling/main/modelingAppView.do?method=getSubjectOption&fdModelId=${HtmlParam.fdmodelId }";
	$.ajax({
		type:"POST",
		url:url,
		...
		error:function(){
			alert("${lfn:message('sys-modeling-main:modeling.get.title.data.error') }");
		}
	});
})
			...
			seajs.use(['lui/dialog'],function(dialog){
				...
				dialog.confirm('<bean:message key="kmsMultidocSubside.isTurnTo" bundle="kms-multidoc"/>',function(value){
					if(result.id){
						...
						alert("${lfn:message('sys-modeling-main:modeling.precipitation.failure') }");
					}
