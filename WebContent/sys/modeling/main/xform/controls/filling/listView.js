define(function (require, exports, module) {
    ...
    var listView = base.DataView.extend({
        ...
        fetchUrl: "/sys/modeling/main/modelingAppXFormMain.do?method=executeQueryFillingByRelationId&pageno=!{currentPage}&rowsize=!{rowsize}",
        ...
        initSouce: function () {
            this.source.url = this.source._url = this.fetchUrl + "&fdAppModelId=" + this.fdAppModelId + "&widgetId=" + this.widgetId+"&relationId="+this.relationId;
        },
        ...
        doRender: function ($super, cfg) {
            ...
            if(this.element.find("[name='List_Selected']").length == selectedCount){
                self.element.find("[name='List_Select_All']").prop("checked","checked");
            }
            /**************** 添加事件 start *****************/
            // var multi = self.config.multi === "true" ? true : false;
            var multi = this.cfgInfo.showType === "1" ? true : false;
            if (!multi) {
                self.element.find(".lui-listview-table-tr").on("click", function (evt) {
                    ...
                    $(this).find("[name='selected']").prop("checked","checked")
                    self.fillingSelectedMap[self.relationId] = keyId;
                    fillingSelect[self.relationId] = self.getRowInfo(rowIndex);
                });
            } else {
                // 全选
                self.element.find("[name='List_Select_All']").on("change", function () {
                    ...
                    fillingSelect[self.relationId] = self.getRowsInfo();
                });
                self.element.find(".lui-listview-table-tr").find('td:gt(0)').on("click", function (evt) {
                    ...
                    var keyId = $(this).closest('tr').attr("key-id");
                    self.changeMultiKey(keyId);
                    self.changeSelected(rowIndex);
                    fillingSelect[self.relationId] = self.getRowsInfo();
                });
            }
            // 穿透跳转
        ...
        changeMultiKey : function(keyId){
            if(keyId == ""){
                return;
            }
            var self = this;
            if(!self.fillingSelectedMap[self.relationId]){
                self.fillingSelectedMap[self.relationId] = [];
            }
            if(self.fillingSelectedMap[self.relationId].indexOf(keyId) > -1){
                self.fillingSelectedMap[self.relationId].splice(self.fillingSelectedMap[self.relationId].indexOf(keyId),1)
            }else{
                self.fillingSelectedMap[self.relationId].push(keyId);
            }
        },
        getFillingSelectedMap : function(){
          return this.fillingSelectedMap[this.relationId];
        },
        ...
        changeSelected: function (rowIdx) {
            ...
            if (row) {
                ...
                if(rowKey != ""){
                    //业务关联用明细表或者fdId作为key,但填充中弹框里面多个关系，
                    // 一个关系里面的明细表id和文档的fdId都是一样的，填充中加上关系的前缀id
                    rowKey = this.relationId +"_"+ rowKey;
                    if (this.selectedRowsMap[rowKey]) {
                        delete this.selectedRowsMap[rowKey];
                    } else {
                        // this.selectedRowsMap[rowKey] = row;
                       this.selectedRowsMap[rowKey] = {
            ...
            this.element.find("[name='List_Selected']").each(function(){
                ...
                if (row) {
                    ...
                    if(rowKey != ""){
                        //业务关联用明细表或者fdId作为key,但填充中弹框里面多个关系，
                        //一个关系里面的明细表id和文档的fdId都是一样的，填充中加上关系的前缀id
                        rowKey = self.relationId +"_"+ rowKey;
                        if (!checked) {
        ...
        getRowsInfo: function () {
            ...
            for (var k in this.selectedRowsMap) {
                var relationId = this.selectedRowsMap[k].relationId;
                if(this.relationId == relationId){
                    rowInfos.push(this.selectedRowsMap[k].row);
                }
            }
            return rowInfos;
        },
        ...
        getRowInfo: function (rowIndex) {
            ...
            var rowInfo = {};
            if(showDetail){
                rowInfo = this.getDetailRow(rowIndex,columns);
            }else{
        ...
        getDetailRow:function (rowIndex,columns){
            // 行信息
            var rowInfo = {};
            //明细表数据
            var detailData = [];
        ...
        // 是否列表穿透
        isMulti: function () {
            return this.cfgInfo.showType === "1" ? true : false;
        },
