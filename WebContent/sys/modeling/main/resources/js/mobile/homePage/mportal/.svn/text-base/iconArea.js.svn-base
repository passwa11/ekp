...
define(['dojo/_base/declare',"dijit/_WidgetBase", "dojo/dom-class", "dojo/dom-construct", "mui/openProxyMixin", "mui/util", 
	...
	return declare('sys.modeling.main.resources.js.mobile.homePage.mportal.iconArea', [WidgetBase, openProxyMixin, _IndexMixin] , {
		
		url : "",
		
		listViewUrl : "/sys/modeling/main/mobile/modelingAppMainMobileListView.do?method=listViewIndex&listViewId=!{listViewId}&nodeType=!{nodeType}&isDirectlyThrouth=true#path=!{tabIndex}",
		...
		buildRendering : function() {
			this.inherited(arguments);
			domClass.add(this.domNode, "mportal-iconArea");
		},
		
		onComplete : function(data){
			domConstruct.empty(this.domNode)

			var values = data.iconArea.attr.listViews.value;
			// 发送请求获取总数
		...
		formatItems : function(items){
			...
			for(var i = 0;i < items.length;i++){
				...
				items[i].url = util.urlResolver(this.listViewUrl, {listViewId: items[i].listView, tabIndex:tabIndex,nodeType:items[i].nodeType});
				rs.push(items[i]);
			}
			return rs;
		},
