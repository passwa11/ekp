...
define(['dojo/_base/declare',"dijit/_WidgetBase","dojo/dom-style", "dojo/dom-class", "dojo/dom-construct", "mui/openProxyMixin", "mui/util",
	...
	return declare('sys.modeling.main.resources.js.mobile.homePage.default.notify', [WidgetBase, openProxyMixin, _IndexMixin], {
		...
		url : "/sys/modeling/main/mobile/modelingAppMainMobileListView.do?method=listViewIndex&listViewId=!{listViewId}&isDirectlyThrouth=true#path=!{tabIndex}",
		...
		buildRendering : function() {
			this.inherited(arguments);
			domClass.add(this.domNode, "mui_pph_toast");
		},
		
		onComplete : function(data){
			domConstruct.empty(this.domNode)
			var info = data.notify.attr.listView.value;
			if(info.auth === "true"){
				info.count = info.count || 0;
				var html = "";
				html += "<div class='mui_pph_toast_content'>";
				...
				if(info.sourceType === "system"){
					...
					this.proxyClick(this.domNode, util.urlResolver(this.url, {listViewId: info.listView,tabIndex: tabIndex}), '_self');				
				}
			}else{
