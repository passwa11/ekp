...
<table class="tb_normal mobile_form_tb" width=100% align="center" frame=void>
    ...
    <div id="xform_${HtmlParam.fdKey}_design_title" width="100%">
        <div class="xform_client_type">
            <div class="xform_client_btn_wrap" name="sysFormTemplateForms.${HtmlParam.fdKey}.type">
				<span class="xform_client_btn is-active" val="pc"
                      onclick="MobileDesigner.instance.bindEvent(event,'formTypeClick');">
					<i class="cap-icon-pc"></i>
						${lfn:message('sys-modeling-base:sysform.PC')}
				 </span>
                <span class="xform_client_btn" val="mobile"
                      onclick="MobileDesigner.instance.bindEvent(event,'formTypeClick');">
					<i class="cap-icon-mobile"></i>
 						 ${lfn:message('sys-modeling-base:sysform.mobile')}
				 </span>
				 <div class="layoutMobileWrap" id="FormTemplate_reviewMainDoc_Layout" style="display: inline-block;">
						<div class="layoutMobileSwitch">
							<label style="margin-right: 18px;display: inline-block;height:20px;line-height:20px;">
								<input style="display: inline-block;vertical-align: middle;" type="checkbox" name="_sysFormTemplateForms.modelingApp.fdUseDefaultLayout" onclick="__cbClick(this.name.substring(1),'null',false,null);" checked="" value="true">
								<span>${lfn:message('sys-modeling-base:sysform.default.layout.mobile')}</span>
							</label>
							<div id="div_sysFormTemplateForms.modelingApp.fdUseDefaultLayout" style="display:none">
								<input name="sysFormTemplateForms.modelingApp.fdUseDefaultLayout" type="hidden" value="true">
							</div>
						</div>
						<div class="lui_prompt_tooltip">
					    	...
					    	<div class="lui_dropdown_tooltip_menu" name="useDefaultLayoutTip" style="display: none;">${lfn:message('sys-modeling-base:sysform.mobile.design.tips')}</div>
					    </div>
				 </div>
            </div>
            <!-- <div style="float:right;margin-right:5px;" class="xform_client_btn_wrap" name="sysFormTemplateForms.modelingApp.type"> -->
            <div class="xform_preview_btn" id="previewDiv"
                 onclick="MobileDesigner.instance.bindEvent(event,'modeling_preview');">
                <ui:button text="${lfn:message('sys-modeling-base:sysform.preview')}" order="1" styleClass="lui_toolbar_btn_gray"
                           style="height:24px;width:70px;"></ui:button>
            </div>
            <!-- </div> -->
        </div>
    </div>
    ...
    <tr id="xform_${HtmlParam.fdKey}_design" style="display:none;">
        <td id="td_xform_${HtmlParam.fdKey}_left" class="xform_left">
            <div id="xform_${HtmlParam.fdKey}_left" class="xform_form_left">
                        ...
                        <tr>
                            <td class="xform_form_title_left">
                                <div id="xform_${HtmlParam.fdKey}_title" class="xform_form_title">
                                    ${lfn:message('sys-modeling-base:sysform.form.configuration')}
                                </div>
                            </td>
                            <td class="xform_form_title_right">
                                <div id="xform_${HtmlParam.fdKey}_addMobileForm"
                                     class="xform_icon_add xform_add_mobile_form"
                                     onclick="MobileDesigner.instance.bindEvent(event,'addMobileForm');"
                                     title="${lfn:message('sys-modeling-base:sysform.add.mobileForm')}">
                                    <i></i>
                                </div>
                            </td>
                        </tr>
                ...
                <div id="xform_${HtmlParam.fdKey}_form" class="xform_form_wrap">
                    <table id="table_docList_${HtmlParam.fdKey}_form" class="tb_normal xform_form_tb">
                        ...
                        <tr KMSS_IsReferRow="1" style="display: none">
                            <td>
                                <a name="mobileFormText" class="xform_name_text"
                                   onclick="MobileDesigner.instance.bindEvent(event,'formClick');"
                                   ondblclick="editName(this);"></a>
                                <input type="hidden" name="${sysFormTemplateFormPrefix}fdMobileForms[!{index}].fdId"/>
                                <xform:text property="${sysFormTemplateFormPrefix}fdMobileForms[!{index}].fdName"
                                            style="width:90%" validators="maxLength(200)"
                                            htmlElementProperties="onblur='textBlur(this);'"/>
                                <input type="hidden"
                                       name="${sysFormTemplateFormPrefix}fdMobileForms[!{index}].fdDesignerHtml"/>
                                <input type="hidden"
                                       name="${sysFormTemplateFormPrefix}fdMobileForms[!{index}].fdMetadataXml"/>
                                <input type="hidden" name="${sysFormTemplateFormPrefix}fdMobileForms[!{index}].fdCss"/>
                                <input type="hidden"
                                       name="${sysFormTemplateFormPrefix}fdMobileForms[!{index}].fdCssDesigner"/>
                                <input type="hidden"
                                       name="${sysFormTemplateFormPrefix}fdMobileForms[!{index}].fdIsDefWebForm"/>
                                <input type="hidden"
                                       name="${sysFormTemplateFormPrefix}fdMobileForms[!{index}].fdPcFormId"/>
                            </td>
                            <!-- 编辑、复制、删除按钮 -->
                            <td align="right">
                                <div class="xform_operation">
                                    <div class="xform_icon_copy"
                                         onclick="MobileDesigner.instance.bindEvent(event,'copyMobileForm');"
                                         title="${lfn:message('sys-modeling-base:sysform.copy')}">
                                        <i></i>
                                    </div>
                                    <div class="xform_icon_delete"
                                         onclick="MobileDesigner.instance.bindEvent(event,'deleteMobileForm');"
                                         title="${lfn:message('sys-modeling-base:sysform.delete')}">
                                        <i></i>
                                    </div>
                                </div>
                            </td>
                        ...
                        <c:forEach items="${xFormTemplateForm.fdSubForms}" var="item" varStatus="vstatus">
                            <c:if test="${item.fdIsDefWebForm eq 'true'}">
                                <tr KMSS_IsContentRow="1" ischecked="false" id="${item.fdId}"
                                    defaultWebForm="<c:out value="${item.fdIsDefWebForm}" />">
                                    <td>
                                        <input type="hidden"
                                               name="${sysFormTemplateFormPrefix}fdMobileForms[${index}].fdId"
                                               value="${item.fdId}"/>
                                            <%-- <input type="hidden" name="${sysFormTemplateFormPrefix}fdSubForms[${vstatus.index}].fdParentFormId" value="${xFormTemplateForm.fdId}" /> --%>
                                        <xform:text
                                                property="${sysFormTemplateFormPrefix}fdMobileForms[${index}].fdName"
                                                value="${item.fdName}" style="width:90%;display:none"
                                                validators="maxLength(200)"
                                                htmlElementProperties="onblur='textBlur(this);'"/>
                                        <a name="mobileFormText" class="xform_name_text"
                                           onclick="MobileDesigner.instance.bindEvent(event,'formClick');"
                                           title="<c:out value='${item.fdName}'/>" ondblclick="editName(this);"><c:out
                                                value='${item.fdName}'/></a>
                                        <input type="hidden"
                                               name="${sysFormTemplateFormPrefix}fdMobileForms[${index}].fdDesignerHtml"
                                               value="<c:out value='${item.fdDesignerHtml}'/>"/>
                                        <input type="hidden"
                                               name="${sysFormTemplateFormPrefix}fdMobileForms[${index}].fdMetadataXml"
                                               value="<c:out value='${item.fdMetadataXml}'/>"/>
                                        <input type="hidden"
                                               name="${sysFormTemplateFormPrefix}fdMobileForms[${index}].fdCss"
                                               value="<c:out value='${item.fdCss}'/>"/>
                                        <input type="hidden"
                                               name="${sysFormTemplateFormPrefix}fdMobileForms[${index}].fdCssDesigner"
                                               value="<c:out value='${item.fdCssDesigner}'/>"/>
                                        <input type="hidden"
                                               name="${sysFormTemplateFormPrefix}fdMobileForms[${index}].fdIsDefWebForm"
                                               value="<c:out value='${item.fdIsDefWebForm}'/>"/>
                                        <input type="hidden"
                                               name="${sysFormTemplateFormPrefix}fdMobileForms[${index}].fdPcFormId"
                                               value="<c:out value='${item.fdPcFormId}'/>"/>
                                    </td>
                ...
                <div id="xform_${HtmlParam.fdKey}_control_title" class="xform_control_title">
                    ${lfn:message('sys-modeling-base:operation.new.operation')}
                </div>
        ...
        <td class="td_normal_title xform_mobile_draw_panel"
            id="TD_MobileFormTemplate_${HtmlParam.fdKey}"
            ${sysFormTemplateFormResizePrefix}onresize="LoadMobileXForm('TD_MobileFormTemplate_${JsParam.fdKey}');">
            ...
            <div>
                <iframe id="IFrame_MobileFormTemplate_${HtmlParam.fdKey}"
                        style="min-height:812px;max-width: 450px;min-width:375px;margin-left:3px;" width="40%"
                        height="100%" scrolling="yes" FRAMEBORDER=0 mobile="true"></iframe>
            </div>
            <!-- 	</div>
            </div> -->
...
<script>
    ...
    function LoadMobileXForm(dom) {
        XForm_Loading_Show();
        Doc_LoadFrame(dom, '<c:url value="/sys/xform/designer/designPanel.jsp?fdKey=" />${JsParam.fdKey}&fdModelName=${sysFormTemplateForm.modelClass.name}&sysFormTemplateFormPrefix=${sysFormTemplateFormPrefix}&encrypt=false&mobile=true');
    ...
    $(function () {
        ...
        function _dialogPreview(designerHtml){
            ...
            if (typeof(seajs) != 'undefined' && typeof( top['seajs'] ) != 'undefined' ) {
                ...
                seajs.use( [ 'lui/jquery', 'lui/dialog' ], function($, dialog) {
                    ...
                    dialog.iframe(url, "${lfn:message('sys-modeling-base:sysform.preview')}", function (rt) {
        ...
        // #120212 重写preview
        MobileDesigner.prototype.modeling_preview = function () {
            ...
            if (clientType == "pc") {
                if (!fdModelId || (fdDesignerHtmlObj && !fdDesignerHtmlObj.value)) {
                    console.log("从url中获取不到模板id！");
                    if (hasSeaJs) {
                        seajs.use(['lui/jquery', 'lui/dialog'], function ($, dialog) {
                            dialog.alert("${lfn:message('sys-modeling-base:sysform.save.template.first')}");
                        });
                    } else {
                        alert("${lfn:message('sys-modeling-base:sysform.save.template.first')}");
                    }
                    return;
                }
                if (typeof XForm_GetFlowModels != "undefined" && "com.landray.kmss.sys.modeling.main.model.ModelingAppSimpleMain"!="${fdMainModelName}") {
                    var flowModels = XForm_GetFlowModels();
                    if (flowModels.length == 0) {
                        if (hasSeaJs) {
                            seajs.use(['lui/jquery', 'lui/dialog'], function ($, dialog) {
                                dialog.alert("${lfn:message('sys-modeling-base:sysform.design.process.first')}");
                            });
                        } else {
                            alert("${lfn:message('sys-modeling-base:sysform.design.process.first')}");
                        }
                        return;
                    }
                }
            ...
            if (clientType == "pc") {
                ...
                if (typeof (seajs) != 'undefined' && typeof (top['seajs']) != 'undefined') {
                    seajs.use(['lui/jquery', 'lui/dialog'], function ($, dialog) {
                        dialog.iframe(url, "${lfn:message('sys-modeling-base:sysform.preview')}", null, {width: width, height: height});
                    });
                }
