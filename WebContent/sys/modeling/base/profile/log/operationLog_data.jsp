...
<list:data>
    <list:data-columns var="modelingOperLog" list="${queryPage.list}" varIndex="status" custom="false">
        ...
        <list:data-column property="fdAppName" title="${lfn:message('sys-modeling-base:modelingOperLog.fdAppName')}" />
        <list:data-column col="fdOperItemName" title="${lfn:message('sys-modeling-base:modelingOperLog.fdOperItemName')}" escape="false">
            <c:set var="fdOperItemName" value="${modelingOperLog.fdOperItemName}" />
        ...
        <list:data-column col="oper" title="${lfn:message('title.option')}" escape="false">
                <div class="monitorLogView">
                    <a onclick="Com_OpenNewWindow(this)" data-href="${LUI_ContextPath}/sys/modeling/base/modelingOperLog.do?method=view&fdId=${modelingOperLog.fdId }"  target="_blank">${lfn:message("button.view")}</a>
                </div>
        </list:data-column>
    </list:data-columns>
    <%-- 分页信息生成 --%>
