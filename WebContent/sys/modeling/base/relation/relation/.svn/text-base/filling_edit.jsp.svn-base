    ...
    <template:replace name="content">
        <html:form action="/sys/modeling/base/sysModelingRelation.do">
            <div class="model-mask-panel medium" style="padding-bottom: 72px">
                <div>
                    <div class="model-mask-panel-table modeling-filling-table">
                            ...
                            <li class="model-table-item" mdlng-rltn-mrk="region" mdlng-rltn-data="basic">
                                ...
                                <div class="model-table-right">
                                    <table class="tb_simple modeling_form_table " mdlng-rltn-mrk="regionTable">
                                        ...
                                        <tr>
                                            <td class="td_normal_title" width="112px">
                                                    ${lfn:message('sys-modeling-base:sysModelingRelation.fdSort')}
                                            </td>
                                            <td width="620px">
                                                <div _xform_type="text" class="relation-form-input-div"
                                                     style="border: none">
                                                    <xform:text property="fdSort" showStatus="edit" validators="number min(0) digits"/>
                                                </div>
                                            </td>
                                        </tr>
                                            ...
                                            <td width="620px">
                                                ...
                                                <div id="_xform_fdShowType" _xform_type="radio">
                                                    <xform:radio property="fdShowType"
                                                                 ...
                                                                 validators="required" required="true"
                                                                 showStatus="edit">
                                                        <xform:simpleDataSource value="0">${lfn:message("sys-modeling-base:enums.relation.show.0")}</xform:simpleDataSource>
                                                        <xform:simpleDataSource value="1">${lfn:message("sys-modeling-base:enums.relation.show.1")}</xform:simpleDataSource>
                                                    </xform:radio>
                                                </div>
                                            </td>
                                        </tr>
                                            ...
                                            <td width="620px">
                                                ...
                                                <div class="hiddenField">
                                                    <input type="hidden" name="fdWidgetName"
                                                           value="<c:out value='${sysModelingRelationForm.fdWidgetName}' />">${widgets.fillings}
                                                </div>
                                                <div id="_xform_fdWidgetId" _xform_type="select"
                                                     class="modeling-form-left">
                                                    <select name="fdWidgetId" subject="关联控件Id"
                                                            class="inputsgl">
                                                        <c:forEach var="moduleMap" varStatus="vstatus"
                                                                   items="${widgets.fillings}">
                                                            <c:if test="${moduleMap.value.relation != null }">
                                                                <c:choose>
                                                                    <c:when test="${fn:contains(moduleMap.value.relation,sysModelingRelationForm.fdId)}">
                                                                        <option selected
                                                                                title="${moduleMap.value.fullLabel==null?moduleMap.value.label:moduleMap.value.fullLabel}"
                                                                                value="${moduleMap.key}">${moduleMap.value.label}</option>
                                                                    </c:when>
                                                                    <c:when test="${ moduleMap.value.releationSubCount != null && moduleMap.value.releationSubCount > 0 }">
                                        ...
                                        <tr mdlng-rltn-property="fdOutParam">
                                            ...
                                            <td width="620px">
                                                ...
                                                <div mdlng-rltn-prprty-value="fdOutParam"
                                                     mdlng-rltn-prprty-type="table" class="model-mask-panel-outparam">
                ...
                <div id="relationHideProperty" style="display: none">
                    ...
                    <html:hidden property="fdType" value="${empty sysModelingRelationForm.fdType?2:sysModelingRelationForm.fdType}"></html:hidden>
                </div>
            ...
            seajs.use(["sys/modeling/base/relation/res/js/relation", "lui/dialog", "lui/jquery",
                ...
                , function (relation, dialog, $, relationFMMark) {
                    ...
                    window.dosubmit = function (type) {
                        ...
                        if(!fdWidgetName){
                            $("[name='fdWidgetId']").focus();
                            var $tipContainer = $("#_xform_fdWidgetId");
                            $tipContainer.find(".widgetTip").remove();
                            $tipContainer.append("<div style='color:red;margin-top:8px;' class='widgetTip'>${lfn:message('sys-modeling-base:relation.configure.associated.field')}</div>");
                            return;
                        }else{
                            var $tipContainer = $("#_xform_fdWidgetId");
                            $tipContainer.find(".widgetTip").remove();
                        ...
                        if (!$("[name='fdOutParam']").val() || $("[name='fdOutParam']").val() == "[]") {
                            var $tipContainer = $("[mdlng-rltn-prprty-value='fdOutParam']");
                            $tipContainer.find(".outParamTip").remove();
                            $tipContainer.append("<div style='color:red;margin-top:8px;margin-left: 16px;' class='outParamTip'>${lfn:message('sys-modeling-base:relation.configure.outgoing.parameters')}</div>");
                            return;
                        }else{
                            var $tipContainer = $("[mdlng-rltn-prprty-value='fdOutParam']");
                            $tipContainer.find(".outParamTip").remove();
