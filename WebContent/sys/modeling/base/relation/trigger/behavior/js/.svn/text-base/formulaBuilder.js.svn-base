...
define(function (require, exports, module) {
    ...
    var FormulaBuilder = base.Component.extend({
        ...
        initFieldList: function (xformId,isfilterDetail) {
            var fieldArr = [];
           var modelfieldList = this.XForm_getXFormDesignerObj(xformId);
            if(isfilterDetail){
                for(var i =0; i < modelfieldList.length;i++){
                    if(modelfieldList[i].businessType == "detailsTable" ||
                        (modelfieldList[i].name && modelfieldList[i].name.indexOf(".") ) > -1){
                        continue;
                    }
                    fieldArr.push(modelfieldList[i]);
                }
                fieldList = fieldArr;
            }else{
                fieldList = modelfieldList;
            }
        },
        ...
        initOtherFieldList: function (fields,formName,type,prefix) {
            var varsInfo = {};
            varsInfo.data = fields;
            varsInfo.varName = formName;
            varsInfo.type = type;
            varsInfo.prefix = prefix;
            otherFieldList.push(varsInfo);
        },
        getOtherFieldList:function(){
            return otherFieldList;
        },

        removeOtherFieldList : function(formName,type,prefix) {
            for (var i = otherFieldList.length-1; i >= 0; i--) {
                if(otherFieldList[i].varName === formName &&otherFieldList[i].type === type && otherFieldList[i].prefix === prefix){
       ...
       get_style2: function (name, type, formulaParameter, formualUrl, filterField) {
            ...
            var html_str = "<div class=\"inputselectsgl modeling_formula\" >\n" +
                ...
                "        <div class=\"input\">\n" +
                "        <input placeholder=\""+modelingLang['modeling.page.choose']+"\"  value=\"\" type=\"text\" width='' readonly>\n" +
                "        </div><div class=\"selectitem\"></div>\n" +
                "        </div>";
            var $html = $(html_str);
        ...
        get: function (name, type, formulaParameter, formualUrl, filterField) {
            ...
            $formula.on("click", function (e) {
                if(formulaParameter && formulaParameter.supportType) {
                    //支持前置表单
                    self.mergeFieldListForParameter(formulaParameter);
                }
                self.removeFieldListForOtherFieldAtt(formulaParameter, fieldList_list);
                if (type.indexOf("Attachment") > -1) {
                    if(formulaParameter && formulaParameter.supportType && formulaParameter.supportDetailAtt) {
                        //支持前置表单附件
                        self.mergeFieldListForOtherFieldAtt(formulaParameter, fieldList_list);
                    }
                    self.Formula_Attachment_Dialog(name, name + "_name", fieldList_list || "");
                } else {
                    Formula_Dialog(name, name + "_name", fieldList_list || "", type || "Object", null, null, _ModelingAppModelName, formulaParameter, formualUrl);
        ...
        mergeFieldListForParameter: function (formulaParameter) {
            if (otherFieldList) {
                var otherVarInfo = [];
                for (var i = 0; i < otherFieldList.length; i++) {
                    if (formulaParameter.supportType.indexOf(otherFieldList[i].type)<0) {
                        continue;
                    }
                    if (formulaParameter.prefix && formulaParameter.prefix !== otherFieldList[i].prefix){
                        continue;
                    }
                    otherVarInfo.push(otherFieldList[i]);
                }
                formulaParameter.otherVarInfo = otherVarInfo;
            }
        },
        mergeFieldListForOtherFieldAtt: function (formulaParameter, fieldList_list) {
            if (otherFieldList && fieldList_list) {
                for (var i = 0; i < otherFieldList.length; i++) {
                    if (formulaParameter.supportType.indexOf(otherFieldList[i].type)<0) {
                        continue;
                    }
                    ...
                    var varsInfo= otherFieldList[i].data;
                    for (var key in varsInfo) {
                        if (varsInfo[key].type == "Attachment") {
                            var name = "";
                            for(var field in fieldList_list){
                                if (fieldList_list[field].type != "Attachment" ){
                                    continue;
                                }
                                if(fieldList_list[field].name == varsInfo[key].name){
                                    name = fieldList_list[field].name;
                                    break;
                                }
                            }
                            if(name){
                            ...
                            var item = [];
                            item["name"] = varsInfo[key].name;
                            item["label"] = varsInfo[key].useLabel || varsInfo[key].fullLabel || varsInfo[key].label;
                            item["businessType"] = varsInfo[key].businessType;
                            item["type"] = varsInfo[key].type;
                            fieldList_list.push(item);
                        }
                    }
                }
        ...
        removeFieldListForOtherFieldAtt: function (formulaParameter, fieldList_list) {
            if (otherFieldList && fieldList_list) {
                for (var i = 0; i < otherFieldList.length; i++) {
                    var varsInfo= otherFieldList[i].data;
                    for (var key in varsInfo) {
                        if (varsInfo[key].type == "Attachment") {
                            for(var i=0;i< fieldList_list.length;i++){
                                if (fieldList_list[i].type != "Attachment" ){
                                    continue;
                                }
                                if(fieldList_list[i].name == varsInfo[key].name){
                                    fieldList_list.splice(i, 1);
                                    break;
                                }
                            }
                        }
        ...
        getOrgAddress: function (isMulti, name, type) {
            ...
            html.append("<input type='hidden' name='" + name + "' />");
            html.append("<input type='text' name='" + name + "_name' class='inputsgl' style='width:150px' readonly/>");
            var $address = $("<span class='highLight'><a href='javascrip:void(0);'>"+modelingLang['button.select']+"</a></span>");
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if(!data){
                            ...
                            fdHandlerObj.label=modelingLang['modeling.lbpm.expecterLog_fdHandler'];
                            ...
                            fdNodeObj.label=modelingLang['modeling.lbpm.expecterLog.fdNode'];
