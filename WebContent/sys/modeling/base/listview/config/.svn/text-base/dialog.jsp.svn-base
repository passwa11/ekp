...
<head>
    ...
    <style>
        .s_select_txt:empty:before, .m_select_txt:empty:before {
            content: '${lfn:message('sys-modeling-base:modeling.page.choose') }';
            color: #aaa;
            font-size: 12px;
        }
    </style>
...
<script type="text/javascript">
	...
	var type = '${param.type}';
    var mobileDialogFlag = '${param.mobileDialogFlag}';
	...
	function ok(){
		...
		for(var i = 0;i < selectedItems.length;i++){
			...
			if(type === "display"){
				...
				if(item.businessType === "inputCheckbox" || item.businessType === "select" || item.businessType === "inputRadio" || item.field === "docStatus"){
			...
			if(type === "condition"){
				...
				//地址本特殊处理
				if(/^com\.landray\.kmss\.sys\.organization/.test(item.type)){
				...
				//多选、单选、下拉框、文档状态
				else if(item.businessType === "inputCheckbox" || item.businessType === "select" || item.businessType === "inputRadio"){
				    if(isMuti){
                        var defaultValueArr = [];
	...
	function unselectOption(dom){
        ...
        //移除节点时，先把标志移除，然后重新计算是否需要显示tip
        if(type === "normal" && mobileDialogFlag === "fdDisplay"){
            if(_modeling_selected_tip === "true"){
                _modeling_selected_tip = "false";
                $("#selectedlist .modeling-selected-tip").remove();
            }
            addSelectedTip();
        }
	}

    function addSelectedTip(){
        var length = $("#selectedlist").find("[name*='defaultValueItem']").length;
        var tempHtml = "<div class='modeling-selected-tip'>"+
            "<div style='line-height:40px;padding-left:20px;text-align: center;color: #999999;'>${lfn:message('sys-modeling-base:listview.summary.display.maximum.6')}</div></div>";
        if(length > 6 && _modeling_selected_tip === "false"){
            _modeling_selected_tip = "true";
            $("#selectedlist").find("[name*='defaultValueItem']").eq(5).after(tempHtml);
        }
    }
	...
	function drawTableHead(){
		...
		html += '<div style="display:inline-block;width:20%;" class="filterItem">${lfn:message('sys-modeling-base:modelingAppListview.fdCondition')}</div>';
		html += '<div style="display:inline-block;width:15%;" class="expandItem">${lfn:message('sys-modeling-base:modelingAppViewtab.fdIsOpen')}</div>';
		html += '<div style="display:inline-block;width:15%;" class="muiItem">${lfn:message('sys-modeling-base:listview.allow.multiple.selection')}</div>';
		html += '<div style="display:inline-block;width:43%;" class="defaultValeItem">${lfn:message('sys-modeling-base:listview.Defaults')}</div>';
		html += '</div>';
		return html;
	}
	...
	function buildSelectedOption(item){
		var html = "";
        html += '<div data-value="'+ item.field +'" class="lui_custom_list_box_content_col_content_line"  id="defaultValueItem_'+item.field+'" name= "defaultValueItem_'+item.field+'" >';
        ...
        if(type === "display") {
	        ...
	        html += '<input type="text" placeholder="${lfn:message('sys-modeling-base:listview.width')}" class="selectedWidth" value="'+ (item.width || '') +'" />';
	      	html += '</span>';
	        ...
	        html += '</div>';
	        var ismulti = false;
	        if(item.businessType === "inputCheckbox" || item.businessType === "select" || item.businessType === "inputRadio"){
	        	ismulti = true;
	        }
	        if(/^com\.landray\.kmss\.sys\.organization/.test(item.type) || ismulti){
	        	 //允许多选(地址本、多选框等)
		        html += '<div style="display:inline-block;width:15%;" class="muiItem">';
		        html += '<label alt="${lfn:message('sys-modeling-base:listview.only.valid.someTypes')}"><input type="checkbox" class="isMuti" '+ (item.muti === 'false' ? '' : 'checked') +' /></label>';
		        html += '</div>';
	        }else{
	        	html += '<div style="display:inline-block;width:15%;" class="muiItem">';
		        html += '</div>';
	        ...
	        //默认值回显
	        var tmpHtml = DefaultValueHtml(item,item.muti);
	        var $tmphtml = $(tmpHtml);
	        if(item.field == "docStatus"){
				...
				for(var j = 0;j < tempdataArr.length;j++){
				    if(typeof (item.muti) == "string"){
				        item.muti = JSON.parse(item.muti);
                    }
				    if(item.muti){
	...
	function DefaultValueHtml(item,ismuti){
        ...
        if(typeof (ismuti) == "string"){
           ismuti = JSON.parse(ismuti);
        }
		if(typeof(item.defaultValue) == "undefined"){
			item.defaultValue = "";
		}
		html += '<div class="DefaultValueHtml" style="display:initial;">';
		if(valueBsType === "inputCheckbox" || valueBsType === "select" || valueBsType === "inputRadio"){
		    if(ismuti){
                var dictEnums = selectPanel.getEnumsById(item.field);
                //多选框或下拉列表
                ...
                var dictEnums = selectPanel.getEnumsById(item.field);
                html += '<div style="display:inline-block;width:41.3%;" class="defaultValueItem">'
                html += '<div class="s_select_box defaultValue" placeholder="${lfn:message('sys-modeling-base:modeling.page.choose')}">';
            ...
            html += '    <div class="s_select_txt"></div>';
            html += '       <ul class="s_option_list">';
            html += '			<li value="1">${lfn:message('sys-modeling-base:listview.nearly.a.week')}</li>';
            html += '			<li value="2">${lfn:message('sys-modeling-base:listview.nearly.a.month')}</li>';
            html += '			<li value="3">${lfn:message('sys-modeling-base:listview.nearly.3.month')}</li>';
            html += '			<li value="4">${lfn:message('sys-modeling-base:listview.nearly.half.a.year')}</li>';
            html += '			<li value="5">${lfn:message('sys-modeling-base:listview.nearly.a.year')}</li>';
            html += '		</ul>';
        	html += '	</div>';
			html += '</div>';
			html += '</div>';
	        ...
	        html += '<input type="text" modeling-validation="defaultValue1_'+valueName+'; ${lfn:message('sys-modeling-base:listview.enter.valid.time')};time;name:defaultValue1_'+valueName+';id:defaultValueItem_'+item.field+'" class="defaultValue1" id="defaultValue1_'+valueName+'" name="defaultValue1_'+valueName+'" onclick="selectTimeDefaultValue(event,this)" style="width:43%;" value="'+value1+'" placeholder="${lfn:message('sys-modeling-base:modeling.page.choose')}"/>';
	        html += '-';
	        html += '<input type="text" modeling-validation="defaultValue2_'+valueName+'; ${lfn:message('sys-modeling-base:listview.enter.valid.time')};time;name:defaultValue2_'+valueName+';id:defaultValueItem_'+item.field+'" class="defaultValue2" id="defaultValue2_'+valueName+'"  name="defaultValue2_'+valueName+'" onclick="selectTimeDefaultValue(event,this)" style="width:43%;" value="'+value2+'" placeholder="${lfn:message('sys-modeling-base:modeling.page.choose')}"/>';
	        html += '</div>';
			...
			//文本输入类(数字、金额)
	        html += '<div style="display:inline-block;width:43%;" class="defaultValueItem">';
	        html += '<input type="number" class="defaultValue1" name="defaultValue1"  style="width:43%;" value="'+value1+'" placeholder="${lfn:message('sys-modeling-base:modeling.please.enter')}"/>';
	        html += '-';
	        html += '<input type="number" class="defaultValue2" name="defaultValue2" style="width:43%;" value="'+value2+'" placeholder="${lfn:message('sys-modeling-base:modeling.please.enter')}"/>';
	        html += '</div>';
		}else if(/^com\.landray\.kmss\.sys\.organization/.test(valueType)){
			//地址本
			var textStr = "";
			var valueStr = "";
			...
			}else if(/SysOrgPerson$/.test(valueType)){
	        ...
	        html += '<input type="text" class="defaultValueText"  name="'+name+'_name" value="'+textStr+'" placeholder="${lfn:message('sys-modeling-base:modeling.page.choose')}">';
            ...
            html += '    <div class="s_select_txt"></div>';
            html += '       <ul class="s_option_list">';
            html += '			<li value="10">${lfn:message('sys-modeling-base:status.draft')}</li>';
            html += '			<li value="20">${lfn:message('sys-modeling-base:status.append')}</li>';
            html += '			<li value="11">${lfn:message('sys-modeling-base:status.refuse')}</li>';
            html += '			<li value="00">${lfn:message('sys-modeling-base:status.discard')}</li>';
            html += '			<li value="30">${lfn:message('sys-modeling-base:status.publish')}</li>';
            html += '			<li value="31">${lfn:message('sys-modeling-base:status.feedback')}</li>';
            html += '		</ul>';
        	html += '	</div>';
			html += '</div>';
			html += '</div>';
			...
			//文本输入类
	        html += '<div style="display:inline-block;width:41.3%;" class="defaultValueItem">';
	        html += '<input type="text" class="defaultValue" name="defaultValue" value="'+item.defaultValue+'" placeholder="${lfn:message('sys-modeling-base:modeling.please.enter')}">';
	        html += '</div>';
		}
		html += "</div>";
       return html;
	...
	function init(){
        ...
        if(type === "normal" && mobileDialogFlag === "fdCondition" ){
        	all = preSelected(all);
        }
		var allOptions = all;
		var items = {};
		...
		setTimeout(function(){
			...
			Sortable.create(list,{
			    handle:".drag_item",
                draggable: ".lui_custom_list_box_content_col_content_line",
                filter:".modeling-selected-tip",
                onEnd: function (evt) {
                    if(type === "normal" && mobileDialogFlag === "fdDisplay"){
                        if(_modeling_selected_tip === "true"){
                            _modeling_selected_tip = "false";
	 ...
	 */
    var mValue = {};
    var _modeling_selected_tip = "false";
	Com_AddEventListener(window,"load",function(){
        ...
        //摘要项超过6个出现分割线
        $(document).on("DOMNodeInserted","#selectedlist",function(){
            if(type === "normal" && mobileDialogFlag === "fdDisplay"){
                //插入节点时，如果摘要项大于6个出现提示
                addSelectedTip();
            }
	...
	function preSelected(colCfgs){
		...
		for(var i = 0;i < colCfgs.length;i++){
             var col = colCfgs[i];
             if(col["type"] == "BigDecimal" || col["type"] == "Double" || col["businessType"] === "textarea"){
            	 continue;
             }else{
