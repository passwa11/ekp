...
seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'], function($, dialog, topic) {
	...
	//列表数据加载完成进行显示项样式设置
	topic.subscribe('list.changed',function(datas){
	...
	topic.subscribe('list.loaded',function(datas){
		for(var key in rowCss){
			$("#listview .lui_listview_columntable_table tbody tr").eq(key).css(rowCss[key]);
		}
    })
    ...
    function displayCssSet(datas){
    	...
    	if(fdDisplayCssObj){
     		for(var key in fdDisplayCssObj){
     			if(dataObj[key]){
     				//key:字段ID，dataObj:预处理后的数据，fdDisplayCssObj:样式条件，datas:原data赋值style给样式
							...
							if(type != "3" && value == ""){
								...
								if ("enum" === whereArr[m].name.type){
									...
									if(matchWhere(keyValue,match,value)){
										cssSet(fieldObj,dataObj[key][i],datas.datas,fieldItemObj);
										break;//or查询满足一个直接cssSet不需要重复匹配后面的逻辑
									}
								}
							}
						}
    ...
    function cssSet(fieldObj,colDataObj,datas,fieldItemObj){
    	...
    	var col = colDataObj.y;
		    ...
		    var tabObj = fieldObj.tab;
		    if(datas[row][col].style != ""){
		    	datas[row][col].style +=";";
		    }
		   	datas[row][col].style +="color:"+fontcolor;
		    datas[row][col].style += ";background-color:"+bgcolor; 
		    var value = colDataObj.value;
		    AddTab(tabObj,datas,value,row,col);
		    
		    var itemColor = fieldItemObj.ItemColor;
