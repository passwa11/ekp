...
seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic', 'lui/dialog_common'], function($, dialog, topic, dialogCommon) {
    ...
    //在弹出框中打开查看页面
    function openPageInDialog(url, title, params){
    	var iframeParams = { width : 900, height : 600 };
    	if(params){
    		iframeParams = params;
    	}
	...
	window.del = function(id) {
		...
		var url  = listOption.contextPath + listOption.basePath + '?method=deleteall';
		dialog.confirm(listOption.lang.comfirmDelete, function(value) {
			if(value == true) {
				...
				$.ajax({
					...
					success: function(data) { delCallback(data); }
			   });
			}
		});
	};
	...
	var relatedHandler = function(datas){
		...
		dialog.iframe(url, listOption.lang.relatedDialogTitle, function(data){
		},{
			width : 600,
			height : 400,
			params : { datas : datas }
    ...
    window.openPageInDialog = openPageInDialog;
