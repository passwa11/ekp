		...
		<script>
			...
			function refreshNotify(){
				try{
					...
					} else if(window.frameElement && window.frameElement.tagName=="IFRAME"){
						if(window.$dialog && window.$dialog.config && window.$dialog.config.opener 
								&& window.$dialog.config.opener.LUI){
							window.$dialog.config.opener.LUI.fire({ type: "topic", name: "successReloadPage" });
						}else if (window.parent && window.parent.LUI) {
...
<script>
	Com_AddEventListener(window,"load",function(){
		// $dialog的赋值在onload事件中，这里先阻塞
		setTimeout(refreshNotify,10);
	});
