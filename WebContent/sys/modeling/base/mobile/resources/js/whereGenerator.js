...
define(function(require, exports, module) {
	...
	__tmpHtml += "<div class='model-edit-view-oper-head-item'><div class='del' onclick='updateRowAttr(0,\"no-position\",this);delTr(this,\"where\");'><i></i></div>" +
			"<div class='down' onclick='moveTr(1,this);updateRowAttr(1,\"no-position\",this)'><i></i></div>" +
			"<div class='up' onclick='moveTr(-1,this);updateRowAttr(-1,\"no-position\",this)'><i></i></div></div>";
	__tmpHtml += "</div>";
	//内容
	...
	//运算符
	__tmpHtml += "<li class='model-edit-view-oper-content-item' data-bind-type-value='0'><div class='item-title'>"+modelingLang['modelingAppViewopers.fdOperator']+"</div>";
	...
	//值
	__tmpHtml += "<li class='model-edit-view-oper-content-item last-item' data-bind-type-value='0'><div class='item-title'>"+modelingLang['modelingAppViewopers.fdValue']+"</div>";
	__tmpHtml += "<div class='item-content'>";
	...
	var WhereGenerator = base.Component.extend({
		...
		draw : function($super, cfg){
			...
			// 内置查询值下拉框值改变事件
			$tr.find("select[name='"+ self.randomName +"_fdPredefinedWhereType']").on("change",function(){
				// 发布事件，更新头部标题
				topic.channel("modeling").publish("whereType.change",{
					value : "1",
					dom : this,
			...
			if(argu.type === "where"){
				if(argu.dom.closest("tr")[0] === this.element[0]){
					...
					if(val && val.indexOf("|fdId") > -1){
						html += "<option value='!{equal}'>"+modelingLang['modelingAppListview.enum.equal']+"</option>";
						...
						if(isDept){
							$operator.hide();
							var tmpHtml = "<label><input type='checkbox' value='true' name='"+ this.randomName +"_isIncludeSub'/>是否包含子部门</label>";
		...
		fixDraw : function(operator){
			...
			if(val && val.indexOf("|fdId") > -1){
				...
				if(fieldType === "String"){
					...
					var info = this.fieldWgt.getCurSelectInfo();
					html += this.getEnumHtml(info.enumValues, this.randomName + "_fieldInputValue",operator);
		...
		getEnumHtml : function(options, name, operator){
			var html = "";
			var type = "radio";
			if(operator == '!{contain}'){
				type = "checkbox";
