    ...
    <template:replace name="content">
        <div style="margin:5px 10px;">
                    ...
                    <div style="display: inline-block;vertical-align: middle;">
                        <ui:toolbar count="3">
                            ...
                            <kmss:auth requestURL="/sys/modeling/base/modelingTreeView.do?method=deleteall&fdModelId=${param.fdModelId}">
                                <!-- 删除 -->
                                <ui:button text="${lfn:message('button.deleteall')}" onclick="del()" order="3"   id="btnDelete"></ui:button>
                            </kmss:auth>
                        </ui:toolbar>
                    </div>
                </div>
            ...
            <list:listview id="listview">
                <ui:source type="AjaxJson">
                    {url:appendQueryParameter('/sys/modeling/base/modelingTreeView.do?method=data&fdModelId=${param.fdModelId}')}
                </ui:source>
                <!-- 列表视图 -->
        ...
        <script>
            var listOption = {
                ...
                jPath: 'resPanel',
                modelName: 'com.landray.kmss.sys.modeling.base.treeview.model.ModelingTreeView',
                templateName: '',
            ...
            seajs.use(["lui/jquery", "sys/ui/js/dialog", "lui/topic"], function ($, dialog, topic) {
                ...
                window.addDocById = function () {
                    var url = '${LUI_ContextPath}/sys/modeling/base/modelingTreeView.do?method=add&fdModelId=${param.fdModelId}';
                ...
                window.toEdit = function (fdId) {
                    var url = '${LUI_ContextPath}/sys/modeling/base/modelingTreeView.do?method=edit&fdId=' + fdId ;
                    var iframe = window.parent.document.getElementById("trigger_iframe");
                    $(iframe).attr("src", url);
                    //修改样式
                    $(iframe).parents(".lui_modeling_main.aside_main").eq(0).css("padding-top", "0px");
