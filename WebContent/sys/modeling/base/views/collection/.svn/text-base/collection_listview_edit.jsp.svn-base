...
<template:include ref="config.edit" sidebar="no">
    ...
    <template:replace name="content">
        ...
        <div class="modeling-pam-content">
                            ...
                            <div class="model-edit-left">
                                <div data-lui-type="sys/modeling/base/resources/js/preview/preview!Preview" id="view_preview" style="display: none">
                                    ...
                                    <div data-lui-type="lui/view/render!Template" style="display:none;">
                                        ...
                                        {
                                            src : '/sys/modeling/base/views/collection/preview/listview_pc.html#'
                                        }
                                         </script>
                                    </div>
                                </div>
                        ...
                        <div class="model-edit-left">
                            <div data-lui-type="sys/modeling/base/resources/js/preview/preview!Preview" id="view_preview_mobile"
                                ...
                                <div data-lui-type="lui/view/render!Template" style="display:none;">
                                    <script type="text/config">
                                   {
                                         src : '/sys/modeling/base/views/collection/preview/listview_mobile.html#'
                                   }
                                </script>
                                </div>
                            </div>
            ...
            <!--基础设置的模板-->
            <div class="model-edit-view-content-common" id="editContent_common" style="display: none">
                ...
                <div class="model-edit-view-content-common-type">
                    ...
                    <div class="type-selectBoxs" style="display: none;">
                        <ul>
                            <li value="0">${lfn:message('sys-modeling-base:listview.normal.list')}</li>
                        </ul>
                    </div>
                </div>
                <!--可访问者-->
                ...
                <div class="common-auth-filter">
                    <div class="common_title" style="display: inline-block">
                        ...
                        <div class="modeling-viewcover-tip">
                            <span>${lfn:message('sys-modeling-base:modeling.form.ListView')}${lfn:message('sys-modeling-base:modelingAppListview.fdAuthEnabled.hit')}</span>
                        </div>
                    </div>
        ...
        <script type="text/javascript">
            ...
            seajs.use([ 'lui/topic', 'lui/jquery','lui/dialog',
                    'sys/modeling/base/resources/js/views/collection/listViewEditCommon',
                    'sys/modeling/base/formlog/res/mark/collectionListViewFMMark']
                ,function(topic,$,dialog,listViewEditCommon,collectionListViewFMMark){
                    ...
                    // 点击其他地方关闭下拉列表
                    $(document).not('#ui-datepicker-div').on('click',function(e) {
                    ...
                    window.changeContentView = function (type) {
                        ...
                        //#147429 链接直接打开这个页面获取不到父页面高度而被挤压，
                        if (bodyHeight<0){
                    ...
                    window.changeRightContentView = function(type){
                        ...
                        if(window.listViewEditCommon){
                            modeligWhereBlock = window.listViewEditCommon.getKeyData();
                        }
                        if(type === "common"){
                    ...
                    window.removeActionFdCondition = function (obj){
                        $("[data-lui-position='fdCondition']").removeClass("active");
                    }
                    window.switchSelectPosition = function (obj, direct) {
                        ...
                        $(".dots", context).removeClass("active");
                        ...
                        if (direct == 'left') {//左边
                            ...
                            } else if (position.indexOf("fdFootStatistics") != -1) {
                                changeRightContentView("design");
                                $("[data-lui-position='fdFootStatistics']", context).addClass("active");
                            }
                        } else {//右边
                    ...
                    window.getValidateHtml = function (subject, type) {
                        var html = '<div class="validation-advice" id="advice-_validate_' + type + '" _reminder="true">'
                            ...
                            + '<td class="validation-advice-msg"><span class="validation-advice-title">' + subject + '</span> 不能为空</td>'
                            + '</tr></tbody></table></div>';
                        return html;
                    }
                    ...
                    window.modelingCollection_beforeSubmitValidate = function(){
                        ...
                        var Subject = $("input[name='fd_mobile_fdSubject']",document)[0];
                        var result = $KMSSValidation.validateElement(fdAduitNote);
                        var SubjectResult = $KMSSValidation.validateElement(Subject);
                        ...
                        if (!result){
                            if(maxLength.test($("input[name='fdName']").val(),fdAduitNote) == false){
                                ...
                                $(".collection-view-name").append($("<div class='validation-container'><div class=\"validation-advice\" _reminder=\"true\">" +
                                    ...
                                    "<td class=\"validation-advice-msg\"><span class=\"validation-advice-title\">${lfn:message('sys-modeling-base:modelingAppCollecctionView.fdName')}</span>" +
                        ...
                        //校验PC端统计项
                        var pcStatisticsResult = true;
                        $("#editContent_pc").find(".name_input").each(function() {
                            if(8 < $(this).find("input").val().length){
                                pcStatisticsResult = false;
                                return false;
                            }
                        });
                        if(pcStatisticsResult == false){
                            dialog.alert('${lfn:message('sys-modeling-base:listview.pc.statistical.item.verification.fail')}');
                            return false;
                        }
                        //校验移动端统计项
                        var mobileStatisticsResult = true;
                        $("#editContent_mobile").find(".name_input").each(function() {
                            if(8 < $(this).find("input").val().length){
                                mobileStatisticsResult = false;
                                return false;
                            }
                        });
                        if(mobileStatisticsResult == false){
                            dialog.alert('${lfn:message('sys-modeling-base:listview.mobile.statistical.verification.fail')}');
                            return false;
                        }
                    ...
                    //表单映射，执行判断是否字段变更
                    var collectionListViewFMMark_pc = new collectionListViewFMMark.CollectionListViewFMMark({fdId:"${modelingAppCollectionViewForm.fdId}"});
                    collectionListViewFMMark_pc.startup();
                  ...
                  /*  window.onload=function(){
                    ...
                    //摘要项标题字段切换
                    window.changeViewFlag = function (obj, name, value, idName) {
