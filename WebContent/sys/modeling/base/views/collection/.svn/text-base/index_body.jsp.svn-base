...
<template:include ref="config.profile.list">
	...
	<template:replace name="content">
		...
		<div class="listview_no_content" style="display: none">
			...
			<div class="litview_new_tips">${lfn:message('sys-modeling-base:listview.welcome.use.new.listView') }</div>
			<div class="litview_old_tips">${lfn:message('sys-modeling-base:listview.return.old.version.maintenance') }</div>
		...
		<div class="new_listview_tips" style="display: none">
			<div class="new_listview_text">
				...
				<div class="new_listview_tips_content">
					${lfn:message('sys-modeling-base:listview.return.old.version.maintenance') }
				</div>
			</div>
		...
		<div style="float:right">
			<div style="display: inline-block;vertical-align: middle;">
				<ui:toolbar id="Btntoolbar">
					...
					<!-- 复制视图 -->
					<ui:button text="${lfn:message('sys-modeling-base:modelingCollectionView.button.copyViews')}" onclick="copyViews()" order="2" ></ui:button>
					<!-- 删除 -->
					<ui:button text="${lfn:message('button.deleteall')}" onclick="del()" order="3" ></ui:button>
					<!--返回旧版-->
					<%--<ui:button text="返回旧版" onclick="reBackOldVersion()" order="2" ></ui:button>--%>
				</ui:toolbar>
			</div>
		</div>
	</div>
	...
	<list:listview>
		<ui:source type="AjaxJson">
			{url:'/sys/modeling/base/modelingAppCollectionView.do?method=list&fdModelId=${param.fdModelId}'}
		</ui:source>
	...
	<!-- 分页 -->
 	...
 	<script type="text/javascript">
    	var listOption = {
            ...
            basePath: '/sys/modeling/base/modelingAppCollectionView.do',
            templateName: '',
            mode: '',
            templateService: '',
            ...
            lang: {
				...
				comfirmCopyViews:'${lfn:message("sys-modeling-base:modelingCollectionView.copyViewsTitle")}'
            }
        };
    	Com_IncludeFile("list.js", "${LUI_ContextPath}/sys/modeling/base/resources/js/", 'js', true);
	 	seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'], function($, dialog, topic) {
			...
			window.copyViews = function() {
				...
				dialog.confirm(listOption.lang.comfirmCopyViews,function(ok){
					if(ok==true){
						var del_load = dialog.loading();
						var param = {"List_Selected":selected};
						$.ajax({
							url:listOption.contextPath + listOption.basePath+'?method=copyViews',
			...
			// 增加
	 		window.addListview = function() {
		 		var url = '${LUI_ContextPath}/sys/modeling/base/modelingAppCollectionView.do?method=add&fdModelId=' + listOption.param.fdAppModelId;
	 			var dialogUrl ="/sys/modeling/base/views/collection/dialog_list_type.jsp";
				dialog.iframe(dialogUrl,"${lfn:message('sys-modeling-base:listview.new.list') }",function(data){
	 		...
	 		window.edit = function(id) {
		 		if(id){
		 			var url = '${LUI_ContextPath}/sys/modeling/base/modelingAppCollectionView.do?method=edit&fdModelId=' + listOption.param.fdAppModelId + '&fdId=' + id;
			...
			window.reBackOldVersion = function() {
				var  url ='${LUI_ContextPath}/sys/modeling/base/pcAndMobile/list/index_body.jsp?fdModelId='+ listOption.param.fdAppModelId +'&method=edit';
				var iframe = window.parent.document.getElementById("trigger_iframe");
				$(iframe).attr("src",url);
		...
		function checkNewListView() {
			var url = Com_Parameter.ContextPath + "sys/modeling/base/modelingAppCollectionView.do?method=checkNewListView&fdModelId="+listOption.param.fdAppModelId;
			$.ajax({
				url: url,
				type: "post",
				async : false,
