        ...
        <div class="modeling-pam-content">
                            ...
                            <div class="model-edit-left">
                                <div data-lui-type="sys/modeling/base/resources/js/preview/preview!Preview" id="view_preview" style="display: none">
                                    ...
                                    <div data-lui-type="lui/view/render!Template" style="display:none;">
                                        ...
                                        {
                                            src : '/sys/modeling/base/views/collection/board/preview/listview_pc.html#'
                                        }
                                         </script>
                                    </div>
                                </div>
            ...
            <div id="modeling-pam-content-mobile">
                ...
                <div class="model-body-content" id="editContent_mobile">
                    <!-- 这里放入你的组件 starts -->
                    <div class="model-edit">
                        <div class="model-edit-left model-board-phone-preview">
                            <div data-lui-type="sys/modeling/base/resources/js/preview/preview!Preview" id="view_preview_mobile"
                                ...
                                <div data-lui-type="lui/view/render!Template" style="display:none;">
                                    <script type="text/config">
                                   {
                                         src : '/sys/modeling/base/views/collection/board/preview/listview_mobile.html#'
                                   }
                                </script>
                                </div>
                            </div>
            ...
            <div class="model-edit-view-content-common board-view" id="editContent_common" style="display: none">
                ...
                <div class="model-edit-view-content-common-type">
                    ...
                    <div class="type-selectBoxs" style="display: none;">
                        <ul>
                            <li value="2">${lfn:message('sys-modeling-base:listview.boardView')}</li>
                        </ul>
                    </div>
                </div>
                <!--可访问者-->
                ...
                <div class="common-auth-filter">
                    <div class="common_title" style="display: inline-block">
                        ...
                        <div class="modeling-viewcover-tip">
                            <span>${lfn:message('sys-modeling-base:listview.boardView')}${lfn:message('sys-modeling-base:modelingAppListview.fdAuthEnabled.hit')}</span>
                        </div>
                    </div>
            ...
            seajs.use([ 'lui/topic', 'lui/jquery','lui/dialog','sys/modeling/base/resources/js/views/collection/listViewEditCommon']
                ,function(topic,$,dialog,listViewEditCommon){
                    ...
                    window.changeContentView = function (type) {
                        ...
                        $("#view_preview_mobile").css("height", bodyHeight + 25);
                    ...
                    window.switchSelectPosition = function (obj, direct) {
                        ...
                        if (direct == 'left') {//左边
                            ...
                            }else if (position.indexOf("fdGroup") != -1) {
                                changeRightContentView("design");
                                $("[data-lui-position='fdGroup']", context).addClass("active");
                            }
                        } else {//右边
                    ...
                    window.getValidateHtml = function (subject, type,tip) {
                        if(!tip){
                            tip = "${lfn:message('sys-modeling-base:kmReviewMain.notNull')}";
                        }
                        var html = '<div class="validation-advice" id="advice-_validate_' + type + '" _reminder="true">'
                            ...
                            + '<td class="validation-advice-msg"><span class="validation-advice-title">' + subject + '</span> ' + tip + '</td>'
                            + '</tr></tbody></table></div>';
                        return html;
                    }
                    ...
                    window.modelingCollection_beforeSubmitValidate = function(){
                        ...
                        var mobile_Subject = $("input[name='fd_mobile_fdSubject']",document)[0];
                        var mobile_fdGroup = $("input[name='fd_mobile_fdGroup']",document)[0];
                        var pc_fdSubject = $("input[name='fd_pc_fdSubject']",document)[0];
                        var pc_fdGroup = $("input[name='fd_pc_fdGroup']",document)[0];
                        var result = $KMSSValidation.validateElement(fdAduitNote);
                        var mobile_SubjectResult = $KMSSValidation.validateElement(mobile_Subject);
                        var mobile_fdGroupResult = $KMSSValidation.validateElement(mobile_fdGroup);
                        var pc_fdSubjectResult = $KMSSValidation.validateElement(pc_fdSubject);
                        var pc_fdGroupResult = $KMSSValidation.validateElement(pc_fdGroup);
                        ...
                        if (!result){
                            ...
                            var pcGroupType = LUI("pcViewContainer").views[0].getKeyData().fdGroup.groupType;
                            var mobileGroupType =LUI("mobileViewContainer").views[0].getKeyData().fdGroup.groupType;
                            if(pcGroupType == "1"){
                                var pcGroupCustom = validateGroupCustom(".modeling-pam-content-pc-right");
                                var pcGroupCustomRule = validateGroupCustomRule(".modeling-pam-content-pc-right");
                            }
                            if(mobileGroupType == "1"){
                                var mobileGroupCustom = validateGroupCustom(".modeling-pam-content-mobile-right");
                                var mobileGroupCustomRule = validateGroupCustomRule(".modeling-pam-content-mobile-right");
                            }
                            if(pc_fdSubjectResult == false || pc_fdGroupResult == false || pcGroupCustom == false || pcGroupCustomRule == false || !pcValidateResult){
                                dialog.alert('${lfn:message('sys-modeling-base:listview.pc.verification.fail')}');
                                return false;
                            }else if(mobile_SubjectResult == false || mobile_fdGroupResult == false || mobileGroupCustom == false || mobileGroupCustomRule == false){
                    ...
                    window.validateGroupCustom = function (className){
                        var result = true;
                        if(!className){
                            return result;
                        }
                        $(className).find("input.groupCustomItemName").each(function () {
                            var ruleName = $(this).val();
                            $(this).focus();
                            $(this).blur();
                            if(ruleName){
                                $(className).find("input.groupCustomItemName").not(this).each(function () {
                                    var otherRuleName = $(this).val();
                    ...
                    window.validateGroupCustomRule = function (className){
                        var result = true;
                        if(!className){
                            return result;
                        }
                        $(className).find("input.groupCustomRuleItemName").each(function () {
                            var ruleName = $(this).val();
                            $(this).focus();
                            $(this).blur();
                            if(ruleName){
                                $(".modeling-pam-content-pc").find("input.groupCustomRuleItemName").not(this).each(function () {
                                    var otherRuleName = $(this).val();
                                    if(ruleName == otherRuleName){
                                        result = false;
                                        return result;
                    ...
                    window.moreSummary= function (){
                        $(".viewMobileConfigCardContentMore").hide();
                    }
                });
