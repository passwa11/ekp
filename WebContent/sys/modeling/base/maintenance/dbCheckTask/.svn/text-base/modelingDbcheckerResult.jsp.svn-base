...
<template:include ref="default.simple">
	<template:replace name="title">
		<bean:message bundle="sys-modeling-base" key="modelingDbchecker.checkResult" />
	</template:replace>
	<template:replace name="head">
		...
		Lang = {
				plus: '<bean:message bundle="sys-modeling-base" key ="modelingDbchecker.plus" />',
				minus: '<bean:message bundle="sys-modeling-base" key ="modelingDbchecker.minus" />',
				noSelect: '<bean:message bundle="sys-modeling-base" key ="modelingDbchecker.noSelect" />',
				noSelectRepair: '<bean:message bundle="sys-modeling-base" key ="modelingDbchecker.noSelect.repair" />',
				comfirmRepair: '<bean:message bundle="sys-modeling-base" key ="modelingDbchecker.comfirmRepair" />',
				waitting: '<bean:message bundle="sys-modeling-base" key ="modelingDbchecker.waitting" />',
				running: '<bean:message bundle="sys-modeling-base" key ="modelingDbchecker.running" />',
				start: '<bean:message bundle="sys-modeling-base" key="modelingDbchecker.start" />',
				stop: '<bean:message bundle="sys-modeling-base" key="modelingDbchecker.stop" />',
				success: '<bean:message bundle="sys-modeling-base" key="modelingDbchecker.success" />',
				failure: '<bean:message bundle="sys-modeling-base" key="modelingDbchecker.failure" />',
				close: '<bean:message key="button.close" />',
				repairProgress: '<bean:message bundle="sys-modeling-base" key="modelingDbchecker.repairProgress" />',
				ignoredManualFix:	'<bean:message bundle="sys-modeling-base" key="modelingDbchecker.ignoredManualFixResult" />',
				ignoredManualFixNum:	'<bean:message bundle="sys-modeling-base" key="modelingDbchecker.ignoredManualFixResultNum" />'
			};
		    ...
		    */
			...
			function onCbClick(cb, level, moduleName, fdId, normalCheckFlag) {
				...
				var urlStr = "modelingDbCheckAction.do?method=updateCheckedResultItem&checked=" + checkedFlag + "&level=" + level;
				if (moduleName != '') {
			...
			function repair(cbname) { // 修复
				...
				if(ignoredItemStr != '') {
					...
					if(arr.length == 0) {
						seajs.use( [ 'sys/ui/js/dialog' ],function(dialog) {
							dialog.alert(Lang.noSelectRepair,function() {
							});
				...
				var component = ajaxSyncComponent(Com_Parameter.ContextPath+"sys/admin/resource/jsp/jsonp.jsp?s_name=com.landray.kmss.sys.modeling.base.maintenance.service.spring.ModelingDbRepairService");
	        ...
	        function reCheck(){
	      		window.open('<c:url value="/sys/modeling/base/modelingDbCheckAction.do" />?method=select&reCheck=true', '_self');
	        }
	        ...
	        function downloadFile(info) {
	        	if ($(":checkbox[name=agree]").is(':checked')) {
	        		window.open('<c:url value="/sys/modeling/base/modelingDbCheckAction.do?method=downloadSql&fdCheckType=3" />' + info, '_blank');
	        	} else {
		        	seajs.use( [ 'sys/ui/js/dialog' ], function(dialog) {
						dialog.alert('<bean:message bundle="sys-modeling-base" key="sys.admin.commontools.clearLog.download.agree"/>');
					});
	        	}
	        }
	...
	<ui:toolbar>
			<ui:button text="${lfn:message('sys-modeling-base:modelingDbchecker.reCheck')}" onclick="reCheck()"/>
	</ui:toolbar>
	</div>
			</div>
			<p class="txttitle"><bean:message bundle="sys-modeling-base" key="modelingDbchecker.checkResult" /></p>
		<br />
		<html:form action="/sys/modeling/base/modelingDbCheckAction.do">
...
<table width="95%" border="0" align="center">
	<tr>
		...
		<td><bean:message bundle="sys-modeling-base" key="modelingDbchecker.error" /></td>
		<td></td>
	</tr>
	<tr>
		<td width="15" bgcolor="#FFFF00">&nbsp;</td>
		<td><bean:message bundle="sys-modeling-base" key="modelingDbchecker.warn" /></td>
		<td></td>
	</tr>
	<tr>   
		<td width="15" bgcolor="#D1D1D1">&nbsp;</td>
		<td><bean:message bundle="sys-modeling-base" key="modelingDbchecker.suggest" /></td>
		<td></td>
	</tr>
	<tr>
		<td width="15" bgcolor="#00FF00">&nbsp;</td>
		<td><bean:message bundle="sys-modeling-base" key="modelingDbchecker.info" /></td>
		<td></td>
	</tr>
</table>
...
<table class="tb_normal" width=98% id="table_result">
	<tr>
		<td width="35%" class="td_normal_title" style="word-break: keep-all;"><bean:message bundle="sys-modeling-base" key="modelingDbchecker.tableName" /></td>
		<td width="45%" class="td_normal_title"><bean:message bundle="sys-modeling-base" key="modelingDbchecker.result" /></td>
		<td width="10%" class="td_normal_title"><bean:message bundle="sys-modeling-base" key="modelingDbchecker.repairSuggest" /></td>
		<td width="10%" class="td_normal_title"><bean:message bundle="sys-modeling-base" key="modelingDbchecker.repairResult" /></td>
	</tr>
	...
	<c:choose>
		...
		<td>
			<img id="img_${checkResultDetailMap.key.type}" alt="<bean:message bundle="sys-modeling-base" key ="modelingDbchecker.minus" />" border="0" 
		...
		<c:if test="${empty checkType and !(checkResultDetailMap.key.type eq 'INFO') and !(checkResultDetailMap.key.type eq 'SUGGEST')}">
			<ui:toolbar>
				<ui:button text="${lfn:message('sys-modeling-base:modelingDbchecker.start')}" onclick="repair('cb_${statusLevel.index}');"/>
			</ui:toolbar>
		</c:if>
		</td>
		<td align="left">
		<c:if test="${empty checkType and !(checkResultDetailMap.key.type eq 'INFO') }">
			<a id="downloadCheckedSql_${statusLevel.index}" onclick="checkDownloadSql(event, 'cb_${statusLevel.index}');" href="modelingDbCheckAction.do?method=downloadCheckedSql&level=<c:out value="${checkResultDetailMap.key.type}"/>">
			<bean:message bundle="sys-modeling-base" key ="modelingDbchecker.downloadSql"/>
			</a>
		</c:if>
		</td>
	</tr>
	...
	<tr module="tr_${checkResultDetailMap.key.type}">
		<td>
			...
			<img id="img_${checkResultDetailMap.key.type}_${checkResultModuleMap.key}" 
				alt='<bean:message bundle="sys-modeling-base" key ="modelingDbchecker.minus" />' border="0" 
	...
	<c:if test="${'true' == isCheckOk}">
		<tr>
			<td colspan="4"><bean:message bundle="sys-modeling-base" key="modelingDbchecker.checkResult.ok" /></td>
		</tr>
	</c:if>
</table>
<kmss:auth requestURL="/sys/admin/resource/jsp/jsonp.jsp">
