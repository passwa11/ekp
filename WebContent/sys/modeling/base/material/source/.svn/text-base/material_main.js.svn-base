...
seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic', 'lui/dialog_common'], function ($, dialog, topic, dialogCommon) {
    ...
    function onPreview(imgUrl, name) {
        ...
        dialog.iframe('/sys/modeling/base/material/import/preview_dialog.jsp?imgUrl=' + a[1],
    ...
    function onEdit(fdId) {
        dialog.iframe('/sys/modeling/base/modelingMaterialMain.do?method=toEdit&fdId='+fdId,
    ...
    function doEditSave(fdId,attId){
    	...
    	var url = Com_Parameter.ContextPath + "sys/modeling/base/modelingMaterialMain.do?method=updateMaterial";
        $.ajax({
            url: url,
            data: {"fdId":fdId,"attId":attId,"fdName":fdName,"fdSize":fdSize,"fdLength":fdLength,"fdWidth":fdWidth },
            dataType: 'json',
            type: 'POST',
    ...
    function onDeleteItem(fdId) {
        ...
        dialog.confirm(option.lang.comfirmDelete, function (ok) {
            if (ok == true) {
                var del_load = dialog.loading();
                var url = Com_Parameter.ContextPath + "sys/modeling/base/modelingMaterialMain.do?method=delete";
                url = url + "&fdId=" + fdId;
                $.ajax({
                    url: url,
                    // data: {"fdId":fdId},
    ...
    window.uploadDoc = function () {
        dialog.iframe('/sys/modeling/base/modelingMaterialMain.do?method=uploadAdd',
