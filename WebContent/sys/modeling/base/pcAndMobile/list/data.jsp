...
<list:data>
	<!-- 表格数据 -->
	<list:data-columns var="modelingPcAndMobileListView" list="${queryPage.list }">
		...
		<list:data-column col="docCreator" title="${ lfn:message('sys-modeling-base:modelingAppListview.docCreator') }" escape="false">
		 	<ui:person personId="${modelingPcAndMobileListView.docCreator.fdId}" personName="${modelingPcAndMobileListView.docCreator.fdName}"></ui:person>
		</list:data-column>
		<!-- 创建时间 -->
		<list:data-column col="docCreateTime" title="${ lfn:message('sys-modeling-base:modelingAppListview.docCreateTime') }">
		    <kmss:showDate value="${modelingPcAndMobileListView.docCreateTime}" type="date"/>
		</list:data-column>
	</list:data-columns>
