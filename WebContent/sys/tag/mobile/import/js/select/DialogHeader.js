...
], function (declare, WidgetBase, domConstruct, domStyle, request, topic, util, Msg) {
    var header = declare("mui.category.CategoryHeader", [WidgetBase], {
        ...
        buildRendering: function () {
            ...
            if (this.scope == "22" || this.scope == "33" || this.scope == "44") {
                this.titleDom = domConstruct.create('div', {
                    "className": "muiCateHeaderTitle",
                ...
                this.titleDom = domConstruct.create('div', {
                    "className": "muiCateHeaderTitle",
                    innerHTML: this.curName
                }, this.contentNode);
            }
            this.cancelNode = domConstruct.create('div', {
                "className": "muiCateHeaderCancel",
                innerHTML: Msg['mui.category.button.cancel']
            }, this.contentNode);
        },
            ...
            if (this.scope == "22" || this.scope == "33" || this.scope == "44") {
                ...
                if (this.curId) {
                    if (!this.returnIcon) {
                        ...
                        this.returnText = domConstruct.create('span', {
                            "className": "muiCateHeaderReturnTxt",
                            innerHTML: ' ' + Msg['mui.category.button.up']
                        }, this.returnDom);
                    }
                } else {
        ...
        _gotoParent: function () {
            var args = {};
            var gv = window.TAG_CATEGORY_VALUE;
            var gl = window.TAG_CATEGORY_LEVEL;

            if(gl == 0 || !gl) return;

            gl = --window.TAG_CATEGORY_LEVEL;
            if (gv && gv[gl] && gv[gl].fdId) {
                args.fdId = gv[gl].fdId;
                args.label = gv[gl].label;
            }

            topic.publish("/mui/category/changed", this, args);
        },
        ...
        resolveCateData: function (data) {
            ...
            for(var i=0; i < data.length; i++) {
                result.fdId[i] = data[i].fdId
                result.label[i] = data[i].label
                if(data[i].parentId) {
                    result.parentId[i] = data[i].parentId
                }
            }
            result.fdId = result.fdId.join(";");
            result.label = result.label.join(";");
            result.parentId = result.parentId.join(";");
            return result;
        },
            ...
            if (srcObj.key == this.key) {
                if (evt) {
                    if (evt.fdId && this.titleDom) {
                        ...
                        //     if (items.length > 0) {
                        //         var cateData = _self.resolveCateData(items);
